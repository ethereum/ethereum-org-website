...
`int<M>: enc(X)` è la codifica in complemento a due big-endian di X, riempita sul lato di ordine superiore (sinistro) con 0xff per X negativo e con byte zero per X positivo, in modo che la lunghezza sia un multiplo di 32 byte.
