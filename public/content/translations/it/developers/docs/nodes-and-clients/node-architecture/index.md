...
Quando Ethereum utilizzava la [Prova di lavoro (PoW)](/developers/docs/consensus-mechanisms/pow/), un client di esecuzione era sufficiente per eseguire un nodo completo di Ethereum. Tuttavia, dall'implementazione della [Proof-of-Stake (PoS)](/developers/docs/consensus-mechanisms/pos/), il client di esecuzione deve essere utilizzato insieme a un altro software chiamato [client di consenso](/developers/docs/nodes-and-clients/#consensus-clients).

Il diagramma sottostante mostra la relazione tra i due client di Ethereum. I due client si connettono alle rispettive reti peer-to-peer (P2P). Sono necessarie reti P2P separate poiché i client di esecuzione diffondono le transazioni sulla loro rete P2P, consentendo loro di gestire la propria pool di transazioni locale, mentre i client di consenso diffondono i blocchi sulla loro rete P2P, consentendo il consenso e la crescita della catena.
...
| Esegue/riesegue le transazioni                     | Esegue l'algoritmo di scelta del fork                                                                                                                     | Accumula ricompense/penalità |
| Verifica le modifiche di stato in entrata          | Tiene traccia della testa della catena                                                                                                                    | Effettua attestazioni        |
| Gestisce i trie di stato e delle ricevute          | Gestisce lo stato della Beacon (contiene informazioni sul consenso e sull'esecuzione)                                                                     | Richiede 32 ETH messi in staking |
| Crea il payload di esecuzione                      | Tiene traccia della casualità accumulata in RANDAO (un algoritmo che fornisce casualità verificabile per la selezione dei validatori e altre operazioni di consenso) | Può subire lo slashing       |
...
- [Proposta di blocchi](/developers/docs/consensus-mechanisms/pos/block-proposal)
