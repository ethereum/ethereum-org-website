...
Prima di approfondire ed eseguire la tua istanza di un client di Ethereum, dovresti comprendere il concetto di rete peer-to-peer e le [basi dell'EVM](/developers/docs/evm/). Dai un'occhiata alla nostra [introduzione a Ethereum](/developers/docs/intro-to-ethereum/).

Se non hai familiarità con l'argomento dei nodi, ti consigliamo di consultare prima la nostra introduzione di facile utilizzo su [come eseguire un nodo di Ethereum](/run-a-node).
...
I client lavorano assieme per tenere traccia della testa della blockchain Ethereum e permettere agli utenti di interagire con la rete Ethereum. Il design modulare con più componenti software che lavorano insieme è chiamato [complessità incapsulata](https://vitalik.eth.limo/general/2022/02/28/complexity.html). Questo approccio ha reso più semplice eseguire [La Fusione](/roadmap/merge) senza problemi, rende il software del client più facile da mantenere e sviluppare e consente il riutilizzo di singoli client, ad esempio, nell'[ecosistema di livello 2](/layer-2/).

![Client di esecuzione e di consenso accoppiati](./eth1eth2client.png)
Diagramma semplificato di un client di esecuzione e di consenso accoppiato.
...
- In origine, l'[Yellow Paper di Ethereum](https://ethereum.github.io/yellowpaper/paper.pdf)
- [Specifiche di esecuzione](https://github.com/ethereum/execution-specs/)
- [Specifiche del consenso](https://github.com/ethereum/consensus-specs)
- [EIP](https://eips.ethereum.org/) implementati in vari [aggiornamenti della rete](/ethereum-forks/)
...
- [Report settimanali sullo stato della rete](https://probelab.io) - di ProbeLab, utilizzando il [crawler Nebula](https://github.com/dennis-tra/nebula) e altri strumenti

## Tipi di nodo {#node-types}

Se vuoi [eseguire il tuo nodo](/developers/docs/nodes-and-clients/run-a-node/), dovresti capire che esistono diversi tipi di nodi che consumano dati in modo diverso. I client possono eseguire tre diversi tipi di nodi: leggero, completo e archivio. Esistono anche diverse strategie di sincronizzazione per velocizzare i tempi. La sincronizzazione è la velocità con cui si ottengono le informazioni più aggiornate sullo stato di Ethereum.

### Nodo completo {#full-node}

I nodi completi fanno una validazione blocco per blocco della blockchain, incluso il download e la verifica del corpo del blocco e i dati di stato per ogni blocco. Ci sono classi differenti di nodo completo - alcune iniziano dal blocco genesi e verificano ogni singolo blocco nell'intera cronologia della blockchain. Altri iniziano la loro verifica da un blocco più recente che ritengono valido (ad es. la 'sincronizzazione snap' di Geth). Indipendentemente da dove parta la verifica, i nodi completi tengono solo una copia locale di dati relativamente recenti (tipicamente i 128 blocchi più recenti), permettendo di eliminare i dati più vecchi per risparmiare spazio sul disco. I dati più vecchi possono essere rigenerati quando è necessario.
...
- Memorizza tutto ciò che è presente nel nodo completo e crea un archivio degli stati storici. È necessario se desideri interrogare qualcosa come il saldo di un conto al blocco n. 4.000.000 o semplicemente testare in modo affidabile le tue transazioni senza convalidarle utilizzando il tracciamento.
...
Invece di scaricare ogni blocco, i nodi leggeri scaricano solo le intestazioni dei blocchi. Queste contengono le informazioni sommarie sui contenuti dei blocchi. Ogni altra informazione di cui i nodi leggeri hanno bisogno viene richiesta a un nodo completo. Il nodo leggero può quindi verificare in modo indipendente i dati che riceve rispetto alle radici di stato nelle intestazioni dei blocchi. I nodi leggeri consentono agli utenti di partecipare alla rete Ethereum senza l'hardware potente o l'elevata larghezza di banda necessari per eseguire i nodi completi. Infine, i nodi leggeri potrebbero funzionare su telefoni cellulari o dispositivi embedded. I nodi leggeri non partecipano al consenso (ossia, non possono essere validatori), ma possono accedere alla blockchain di Ethereum con le stesse funzionalità e garanzie di sicurezza di un nodo completo.

I client leggeri sono un'area di sviluppo attivo per Ethereum, e ci aspettiamo presto di vedere nuovi client leggeri per i livelli di consenso e i livelli di esecuzione.
Ci sono anche percorsi potenziali per fornire dati di client leggeri sulla [rete di gossip](https://www.ethportal.net/). Questi sono vantaggiosi perché la rete di gossip potrebbe supportare una rete di nodi leggeri senza richiedere ai nodi completi di rispondere alle richieste.
...
- Puoi connetterti al tuo nodo usando le **Comunicazioni interprecessuali (IPC)** o riscrivere il nodo per caricare il tuo programma come plugin. Ciò conferisce una bassa latenza, che aiuta molto, ad esempio, quando si elaborano molti dati usando le librerie web3 o quando ti serve sostituire le tue transazioni il più velocemente possibile (frontrunning).
- Puoi mettere ETH direttamente in staking per proteggere la rete e guadagnare ricompense. Vedi lo [staking in solitaria](/staking/solo/) per iniziare.

![Come accedere a Ethereum tramite l'applicazione e i nodi](./nodes.png)
...
- In caso di un attacco che superi le difese critto-economiche del [proof-of-stake](/developers/docs/consensus-mechanisms/pos/#what-is-pos), un ripristino sociale può essere eseguito da nodi completi che scelgono di seguire la catena onesta.
...
- I nodi completi forniscono accesso ai dati della blockchain per i client leggeri che dipendono da essa. I nodi leggeri non memorizzano l'intera blockchain, ma verificano i dati tramite le [radici di stato nelle intestazioni dei blocchi](/developers/docs/blocks/#block-anatomy). Se ne hanno bisogno, possono richiedere ulteriori informazioni ai nodi completi.
...
Configurare un nodo può richiedere tempo e risorse e non sempre è necessario eseguire un'istanza propria. In questo caso, puoi usare un provider di API terzo. Per una panoramica sull'utilizzo di questi servizi, consulta [nodi come servizio](/developers/docs/nodes-and-clients/nodes-as-a-service/).
...
La community di Ethereum mantiene numerosi client di esecuzione open source (precedentemente noti come 'client di Eth1' o semplicemente 'client di Ethereum'), sviluppati da diversi team usando diversi linguaggi di programmazione. Ciò rende la rete più forte e più [diversificata](/developers/docs/nodes-and-clients/client-diversity/). L'obiettivo ideale è raggiungere la diversità senza che nessun client prevalga, per ridurre eventuali punti di errore singoli.

Questa tabella riepiloga i diversi client. Tutti superano i [test dei client](https://github.com/ethereum/tests) e sono mantenuti attivamente per rimanere aggiornati con gli aggiornamenti della rete.

| Client                                                                                      | Lingua                   | Sistemi operativi     | Reti                            | Strategie di sincronizzazione                                                         | Cancellazione dello stato |
| ------------------------------------------------------------------------------------------- | ------------------------ | --------------------- | ------------------------------- | ------------------------------------------------------------------------------------- | ------------------------- |
| [Geth](https://geth.ethereum.org/)                                                          | Go                       | Linux, Windows, macOS | Rete Principale, Sepolia, Hoodi | [Snap](#snap-sync), [Completa](#full-sync)                                            | Archiviata, Tagliata      |
| [Nethermind](https://www.nethermind.io/)                                                    | C#, .NET | Linux, Windows, macOS | Rete Principale, Sepolia, Hoodi | [Snap](#snap-sync) (senza servire), Veloce, [Completa](#full-sync) | Archiviata, Tagliata      |
| [Besu](https://besu.hyperledger.org/en/stable/)                                             | Java                     | Linux, Windows, macOS | Rete Principale, Sepolia, Hoodi | [Snap](#snap-sync), [Veloce](#fast-sync), [Completa](#full-sync)                      | Archiviata, Tagliata      |
| [Erigon](https://github.com/ledgerwatch/erigon)                                             | Go                       | Linux, Windows, macOS | Rete Principale, Sepolia, Hoodi | [Completa](#full-sync)                                                                | Archiviata, Tagliata      |
| [Reth](https://reth.rs/)                                                                    | Rust                     | Linux, Windows, macOS | Rete Principale, Sepolia, Hoodi | [Completa](#full-sync)                                                                | Archiviata, Tagliata      |
| [EthereumJS](https://github.com/ethereumjs/ethereumjs-monorepo) _(beta)_ | TypeScript               | Linux, Windows, macOS | Sepolia, Hoodi                  | [Completa](#full-sync)                                                                | Tagliata                  |

Per maggiori informazioni sulle reti supportate, leggi la documentazione sulle [reti di Ethereum](/developers/docs/networks/).
...
L'ampia [documentazione](https://besu.hyperledger.org/en/stable/) di Besu ti guiderà attraverso tutti i dettagli delle sue funzionalità e configurazioni.
...
Nethermind ha anche una [documentazione dettagliata](https://docs.nethermind.io), un forte supporto per gli sviluppatori, una community online e un supporto 24/7 disponibile per gli utenti premium.
...
Scopri di più consultando il [Reth Book](https://reth.rs/) o la [repo GitHub di Reth](https://github.com/paradigmxyz/reth?tab=readme-ov-file#reth).
...
Ci sono più client di consenso (precedentemente noti come client 'Eth2') per supportare gli [aggiornamenti del consenso](/roadmap/beacon-chain/). Sono responsabili di tutta la logica relativa al consenso, incluso l'algoritmo di scelta della biforcazione, l'elaborazione delle attestazioni e la gestione di ricompense e penalità del [proof-of-stake](/developers/docs/consensus-mechanisms/pos).

| Client                                                        | Lingua     | Sistemi operativi     | Reti                                                         |
| ------------------------------------------------------------- | ---------- | --------------------- | ------------------------------------------------------------ |
| [Lighthouse](https://lighthouse.sigmaprime.io/)               | Rust       | Linux, Windows, macOS | Beacon Chain, Hoodi, Pyrmont, Sepolia e altro ancora         |
| [Lodestar](https://lodestar.chainsafe.io/)                    | TypeScript | Linux, Windows, macOS | Beacon Chain, Hoodi, Sepolia e altro ancora                  |
| [Nimbus](https://nimbus.team/)                                | Nim        | Linux, Windows, macOS | Beacon Chain, Hoodi, Sepolia e altro ancora                  |
| [Prysm](https://prysm.offchainlabs.com/docs/)                 | Go         | Linux, Windows, macOS | Beacon Chain, Gnosis, Hoodi, Pyrmont, Sepolia e altro ancora |
| [Teku](https://consensys.net/knowledge-base/ethereum-2/teku/) | Java       | Linux, Windows, macOS | Beacon Chain, Gnosis, Hoodi, Sepolia e altro ancora          |
| [Grandine](https://docs.grandine.io/)                         | Rust       | Linux, Windows, macOS | Beacon Chain, Hoodi, Sepolia e altro ancora                  |
...
La documentazione si trova nel [Lighthouse Book](https://lighthouse-book.sigmaprime.io/)
...
Maggiori informazioni sono disponibili sul [sito web di Lodestar](https://lodestar.chainsafe.io/)
...
La documentazione si trova nel [Grandine Book](https://docs.grandine.io/)
...
**N.B.** La sincronizzazione leggera non funziona ancora con Ethereum in proof-of-stake - nuove versioni della sincronizzazione leggera dovrebbero essere rilasciate presto!
...
Una sincronizzazione dal punto di controllo, anche nota come sincronizzazione a soggettività debole, crea un'esperienza utente superiore per la sincronizzazione di un Nodo Beacon. Si basa su presupposti di [soggettività debole](/developers/docs/consensus-mechanisms/pos/weak-subjectivity/) che permette di sincronizzare la Beacon Chain da un recente checkpoint di soggettività debole invece che dalla genesi. Le sincronizzazioni da checkpoint rendono il tempo di sincronizzazione iniziale significativamente più veloce con presupposti di fiducia simili alla sincronizzazione dalla [genesi](/glossary/#genesis-block).
...
- [Ethereum 101 - Parte 2 - Capire i Nodi](https://kauri.io/ethereum-101-part-2-understanding-nodes/48d5098292fd4f11b251d1b1814f0bba/a) _– Wil Barnes, 13 febbraio 2019_
...
- [Trasforma il tuo Raspberry Pi 4 in un nodo validatore semplicemente flashando la scheda MicroSD – Guida all'installazione](/developers/tutorials/run-node-raspberry-pi/) _– Fai il flash del tuo Raspberry Pi 4, collega un cavo ethernet, connetti il disco SSD e accendi il dispositivo per trasformare il Raspberry Pi 4 in un nodo Ethereum completo che esegue il livello di esecuzione (Rete Principale) e/o il livello di consenso (Beacon Chain / validatore)._
