...
Validium è una [soluzione di scalabilità](/developers/docs/scaling/) che impone l'integrità delle transazioni usando prove di validità come i [rollup ZK](/developers/docs/scaling/zk-rollups/), ma non memorizza i dati delle transazioni sulla Rete Principale di Ethereum. Sebbene la disponibilità di dati off-chain introduca dei compromessi, può portare a enormi miglioramenti nella scalabilità (i validium possono elaborare [~9.000 transazioni, o più, al secondo](https://blog.matter-labs.io/zkrollup-vs-validium-starkex-5614e38bc263)).
...
## Come interagiscono i validium con Ethereum? Come interagiscono i validium con Ethereum? {#how-do-validiums-interact-with-ethereum}
...
2. **Contratto principale**: Il contratto principale memorizza gli impegni di stato (radici di Merkle) inviati dai produttori di blocchi e aggiorna lo stato del validium una volta verificata la prova di validità on-chain. Questo contratto elabora inoltre i depositi alla e i prelievi dalla catena validium.

I validium si affidano anche alla catena principale di Ethereum per quanto segue:

### Finalizzazione {#settlement}

Le transazioni eseguite su un validium non possono essere completamente confermate finché la catena genitore ne verifica la validità. Tutte le attività svolte su un validium devono essere infine regolate sulla Rete principale. La blockchain di Ethereum fornisce anche "garanzie di finalizzazione" per gli utenti di validium, il che significa che le transazioni off-chain non possono essere annullate o alterate una volta registrate on-chain.

### Sicurezza {#security}

Ethereum, agendo da livello d'accordo, garantisce anche la validità delle transizioni di stato su validium. Le transazioni off-chain eseguite sulla catena validium sono verificate tramite un contratto intelligente sul livello di base di Ethereum.
...
Gli utenti inviano le transazioni all'operatore, un nodo responsabile di eseguire le transazioni sulla catena validium. Alcuni validium possono usare un unico operatore per eseguire la catena o affidarsi a un meccanismo di [proof-of-stake (PoS)](/developers/docs/consensus-mechanisms/pos/) per la rotazione degli operatori.
...
### Invio di batch {#batch-submission}
...
A differenza di un rollup ZK, i produttori di blocchi su un validium non devono pubblicare i dati della transazione per i batch di transazioni (solo le intestazioni dei blocchi). Questo rende validium un protocollo di scalabilità puramente off-chain, a differenza dei protocolli di scalabilità "ibridi" (ovvero [livello 2](/layer-2/)) che pubblicano i dati di stato sulla catena principale di Ethereum utilizzando dati blob, `calldata` o una combinazione di entrambi.

### Disponibilità dei dati {#data-availability}

Come accennato, i validium utilizzano un modello di disponibilità dei dati off-chain, in cui gli operatori memorizzano tutti i dati delle transazioni al di fuori della Rete Principale di Ethereum. La bassa impronta di dati on-chain di Validium migliora la scalabilità (il volume non è limitato dalla capacità di elaborazione dati di Ethereum) e riduce le commissioni per l'utente (il costo di pubblicazione dei dati on-chain è inferiore).
...
Varie soluzioni di validium tentano di risolvere questo problema decentralizzando l'archiviazione dei dati di stato. Ciò comporta il forzare i produttori di blocchi a inviare i dati sottostanti a "gestori della disponibilità dei dati", responsabili dell'archiviazione dei dati off-chain e di renderli disponibili agli utenti su richiesta.

In validium i gestori della disponibilità dei dati attestano la disponibilità dei dati per le transazioni off-chain firmando ogni batch del validium. Queste firme costituiscono una forma di "prova di disponibilità", che il contratto di verifica on-chain controlla prima di approvare gli aggiornamenti di stato.
...
Per garantire la disponibilità di dati off-chain, alcune soluzioni validium nominano un gruppo di entità fidate, noto collettivamente come comitato di disponibilità dei dati (DAC), per archiviare copie dello stato e fornire la prova della disponibilità dei dati. I DAC sono più facili da implementare e richiedono un coordinamento minore, vista la bassa adesione.

Tuttavia, gli utenti devono fidarsi del fatto che i DAC rendono disponibili i dati quando serve (ad es. per generare le prove di Merkle). C'è la possibilità che i membri dei comitati per la disponibilità dei dati [vengano compromessi da un attore malevolo](https://notes.ethereum.org/DD7GyItYQ02d0ax_X-UbWg?view) che può quindi trattenere i dati off-chain.
...
In uno schema di disponibilità dei dati vincolata, chiunque può essere incaricato di conservare i dati off-chain una volta fornito lo stake richiesto. Questo espande il pool di gestori di disponibilità dei dati idonei, riducendo la centralizzazione che influenza i comitati di disponibilità dei dati (DAC). Ancora più importante, questo approccio si affida a incentivi criptoeconomici per prevenire l'attività malevola, il che è considerevolmente più sicuro che nominare parti fidate per proteggere i dati offline nel validium.
...
## Validium e compatibilità con la EVM {#validiums-and-evm-compatibility}

Come i rollup ZK, i validium sono per lo più adatti ad applicazioni semplici, come gli scambi di token e i pagamenti. Il supporto al calcolo generico e all'esecuzione di contratti intelligenti tra i validium è difficile da implementare, dato il notevole overhead per comprovare le istruzioni [EVM](/developers/docs/evm/) in un circuito di prova a conoscenza-zero.
...
Alcuni team, tuttavia, stanno cercando di ottimizzare gli opcode EVM esistenti per i circuiti di prova ZK. Questo risulterà nello sviluppo di una Macchina virtuale di Ethereum a conoscenza zero (zkEVM), una VM compatibile all'EVM che produce prove per verificare la correttezza dell'esecuzione del programma. Con una zkEVM, le catene validium possono eseguire contratti intelligenti off-chain e inviare prove di validità per verificare una computazione off-chain (senza doverla rieseguire) su Ethereum.

[Maggiori informazioni sulle zkEVM](https://www.alchemy.com/overviews/zkevm).

## Come fanno i validium a ridimensionare Ethereum? Scalare Ethereum con i validium {#scaling-ethereum-with-validiums}

### 1. Archiviazione dei dati off-chain {#offchain-data-storage}

I progetti di scalabilità di livello 2, come i rollup ottimistici e i rollup ZK, barattano la scalabilità infinita dei protocolli di scalabilità puramente off-chain (ad es. [Plasma](/developers/docs/scaling/plasma/)) con la sicurezza, pubblicando alcuni dati delle transazioni su L1. Questo però significa che le proprietà di scalabilità dei rollup sono limitate dalla larghezza di banda dei dati sulla Rete Principale di Ethereum (il [data sharding](/roadmap/danksharding/) si propone per questo di migliorare la capacità di archiviazione dati di Ethereum).

I validium ottengono la scalabilità mantenendo tutti i dati delle transazioni off-chain e pubblicando solo impegni di stato (e prove di validità) quando inoltrano gli aggiornamenti di stato alla catena principale di Ethereum. L'esistenza delle prove di validità, tuttavia, conferisce ai validium garanzie di sicurezza superiori rispetto ad altre soluzioni di scalabilità puramente off-chain, tra cui Plasma e le [catene laterali](/developers/docs/scaling/sidechains/). Riducendo la quantità di dati che Ethereum deve elaborare prima di convalidare le transazioni off-chain, i design dei validium estendono notevolmente il volume sulla Rete Principale.
...
**StarkWare StarkEx** - _StarkEx è una soluzione di scalabilità di Livello 2 (L2) di Ethereum basata su prove di validità._ Può operare in modalità di disponibilità dei dati rollup ZK o validium._
...
**Matter Labs zkPorter**- _zkPorter è un protocollo di scalabilità di Livello 2 che affronta la disponibilità dei dati con un approccio ibrido che combina le idee di zkRollup e sharding._ Può supportare arbitrariamente molti shard, ognuno con la propria politica di disponibilità dei dati._
