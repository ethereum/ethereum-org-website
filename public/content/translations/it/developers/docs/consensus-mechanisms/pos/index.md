...
La quantità di ETH sottoposta a slashing dipende da quanti validatori subiscono a loro volta uno slashing all'incirca nello stesso momento. Questa è nota come ["penalità di correlazione"](https://eth2book.info/latest/part2/incentives/slashing#the-correlation-penalty) e può essere di lieve entità (meno dello 0,1% dello stake per un singolo validatore che subisce uno slashing isolato) o può comportare la distruzione del 100% dello stake del validatore (evento di slashing di massa). Viene imposta a metà di un periodo di uscita forzata che inizia con una penalità immediata (1/4096 del saldo effettivo del validatore, fino a 0,5 ETH) il Giorno 1, la penalità di correlazione il Giorno 18 e, infine, l'espulsione dalla rete il Giorno 36. Ricevono penalità di attestazione di lieve entità ogni giorno perché sono presenti sulla rete ma non inviano voti. Tutto ciò significa che un attacco coordinato sarebbe molto costoso per l'aggressore.
