...
Una delle sfide più grandi per gli sviluppatori provenienti da un background Web2 è capire come collegare il proprio smart contract a un progetto frontend e interagirvi.

Costruendo un minter di NFT — una semplice interfaccia utente in cui puoi inserire un link al tuo asset digitale, un titolo e una descrizione — imparerai a:
...
### Distribuire uno smart contract per NFT sulla blockchain di Ethereum {#publish-nft}
...
- `minter-starter-files` contiene i file di partenza (essenzialmente l'interfaccia utente React) per questo progetto. In questo tutorial, **lavoreremo in questa directory**, mentre impari a dare vita a questa interfaccia utente collegandola al tuo portafoglio Ethereum e a uno smart contract per NFT.
- `nft-minter` contiene l'intero tutorial completato ed è lì per te come **riferimento** **se rimani bloccato.**

Successivamente, apri la tua copia di `minter-starter-files` nel tuo editor di codice, quindi naviga nella tua cartella `src`.
...
## Passaggio 2: Esaminare i nostri file di partenza {#step-2-check-out-our-starter-files}

Prima di iniziare a programmare, è importante esaminare ciò che ci è già stato fornito nei file di partenza.
...
In questo modo dovrebbe aprirsi http://localhost:3000/ nel tuo browser, dove vedrai il frontend per il nostro progetto. Dovrebbe consistere in 3 campi: un posto dove inserire un link all'asset del tuo NFT, inserire il nome del tuo NFT e fornire una descrizione.

Se provi a fare clic sui pulsanti "Connect Wallet" o "Mint NFT", noterai che non funzionano: questo perché dobbiamo ancora programmare la loro funzionalità! :\)
...
Verso la fine di questo file, abbiamo l'interfaccia utente del nostro componente. Se esamini attentamente questo codice, noterai che aggiorniamo le nostre variabili di stato `url`, `name` e `description` quando cambia l'input nei campi di testo corrispondenti.
...
Se vai al file `App.js`, che è il componente principale in React che funge da contenitore per tutti gli altri componenti, vedrai che il nostro componente Minter viene iniettato alla riga 7.

**In questo tutorial, modificheremo solo il `Minter.js file` e aggiungeremo file nella nostra cartella `src`.**
...
Per farlo, creiamo una nuova cartella chiamata `utils` nella tua directory `src` e aggiungiamo un file chiamato `interact.js` al suo interno, che conterrà tutte le nostre funzioni di interazione con il portafoglio e lo smart contract.
...
Se `window.ethereum` _non è_ presente, significa che MetaMask non è installato. Ciò si traduce nella restituzione di un oggetto JSON, in cui `address` restituito è una stringa vuota e l'oggetto JSX `status` comunica che l'utente deve installare MetaMask.

**La maggior parte delle funzioni che scriveremo restituirà oggetti JSON che possiamo utilizzare per aggiornare le nostre variabili di stato e l'interfaccia utente.**
...
Innanzitutto, dovremo importare la nostra funzione nel nostro file `Minter.js` aggiungendo `import { connectWallet } from "./utils/interact.js";` all'inizio del file `Minter.js`. Le tue prime 11 righe di `Minter.js` dovrebbero ora apparire così:
...
Noti come la maggior parte delle nostre funzionalità sia astratta dal nostro componente `Minter.js` dal file `interact.js`? Questo per rispettare il paradigma M-V-C!
...
Nota, utilizziamo la risposta della nostra chiamata a `getCurrentWalletConnected` per aggiornare le nostre variabili di stato `walletAddress` e `status`.

Dopo aver aggiunto questo codice, prova ad aggiornare la finestra del nostro browser. Il pulsante dovrebbe dire che sei connesso e mostrare un'anteprima dell'indirizzo del tuo portafoglio connesso, anche dopo l'aggiornamento!
...
- Innanzitutto, la nostra funzione controlla se `window.ethereum` è abilitato \(ovvero, MetaMask è installato\).
...
Quindi ricorda i metadati dell'NFT di cui abbiamo appena parlato nel Passaggio 0 di questo tutorial: danno vita a un NFT, consentendogli di avere proprietà, come un asset digitale, un nome, una descrizione e altri attributi.

Dovremo configurare questi metadati come un oggetto JSON e memorizzarli, in modo da poterli passare come parametro `tokenURI` quando chiamiamo la funzione `mintNFT` del nostro smart contract.
...
- Potremmo utilizzare IPFS, un protocollo decentralizzato e una rete peer-to-peer per l'archiviazione e la condivisione di dati in un file system distribuito. Poiché questo protocollo è decentralizzato e gratuito, è la nostra migliore opzione!

Per memorizzare i nostri metadati su IPFS, utilizzeremo [Pinata](https://pinata.cloud/), una comoda API e toolkit IPFS. Nel passaggio successivo, spiegheremo esattamente come farlo!

## Utilizzare Pinata per fissare i tuoi metadati su IPFS {#use-pinata-to-pin-your-metadata-to-ipfs}
...
Ora che la nostra chiave è configurata, aggiungiamola al nostro progetto in modo da poterla utilizzare.

### Creare un file .env {#create-a-env}

Possiamo memorizzare in modo sicuro la nostra chiave e il nostro segreto Pinata in un file di ambiente. Installiamo il [pacchetto dotenv](https://www.npmjs.com/package/dotenv) nella directory del tuo progetto.

Apri una nuova scheda nel tuo terminale \(separata da quella che esegue l'host locale\) e assicurati di essere nella cartella `minter-starter-files`, quindi esegui il seguente comando nel tuo terminale:

```text
npm install dotenv --save
```

Successivamente, crea un file `.env` nella directory principale del tuo `minter-starter-files` inserendo quanto segue nella riga di comando:

```javascript
vim.env
```

Questo aprirà il tuo file `.env` in vim \(un editor di testo\). Per salvarlo premi "esc" + ":" + "q" sulla tastiera in quest'ordine.
...
Fortunatamente per noi, Pinata ha un'[API specifica per il caricamento di dati JSON su IPFS](https://docs.pinata.cloud/api-reference/endpoint/ipfs/pin-json-to-ipfs#pin-json) e un comodo esempio JavaScript con axios che possiamo utilizzare, con alcune lievi modifiche.

Nella tua cartella `utils`, creiamo un altro file chiamato `pinata.js` e poi importiamo il nostro segreto e la nostra chiave Pinata dal file .env in questo modo:
...
export const pinJSONToIPFS = async (JSONBody) => {
  ...
  //effettuando la richiesta POST axios a Pinata ⬇️
  return axios
    .post(url, JSONBody, {
      headers: {
        pinata_api_key: key,
...
Innanzitutto, importa [axios](https://www.npmjs.com/package/axios), un client HTTP basato su promise per il browser e Node.js, che utilizzeremo per effettuare una richiesta a Pinata.

Poi abbiamo la nostra funzione asincrona `pinJSONToIPFS`, che accetta un `JSONBody` come input e la chiave e il segreto API Pinata nella sua intestazione, il tutto per effettuare una richiesta POST alla loro API `pinJSONToIPFS`.

- Se questa richiesta POST ha esito positivo, la nostra funzione restituisce un oggetto JSON con il booleano `success` come true e il `pinataUrl` in cui sono stati fissati i nostri metadati. Utilizzeremo questo `pinataUrl` restituito come input `tokenURI` per la funzione di conio del nostro smart contract.
...
Come per i tipi di ritorno della nostra funzione `connectWallet`, stiamo restituendo oggetti JSON in modo da poter utilizzare i loro parametri per aggiornare le nostre variabili di stato e l'interfaccia utente.

## Caricare il tuo smart contract {#load-your-smart-contract}

Ora che abbiamo un modo per caricare i metadati del nostro NFT su IPFS tramite la nostra funzione `pinJSONToIPFS`, avremo bisogno di un modo per caricare un'istanza del nostro smart contract in modo da poter chiamare la sua funzione `mintNFT`.

Come accennato in precedenza, in questo tutorial utilizzeremo [questo smart contract per NFT esistente](https://ropsten.etherscan.io/address/0x4C4a07F737Bf57F6632B6CAB089B78f62385aCaE); tuttavia, se desideri imparare come lo abbiamo realizzato, o crearne uno tu stesso, ti consigliamo vivamente di dare un'occhiata al nostro altro tutorial, ["Come creare un NFT"](https://www.alchemy.com/docs/how-to-create-an-nft).

### L'ABI del contratto {#contract-abi}

Se hai esaminato attentamente i nostri file, avrai notato che nella nostra directory `src` c'è un file `contract-abi.json`. Un'ABI è necessaria per specificare quale funzione invocherà un contratto, oltre a garantire che la funzione restituisca i dati nel formato previsto.
...
Una volta che li abbiamo entrambi, siamo pronti per iniziare a programmare la nostra funzione di conio!
...
Poiché effettueremo numerose chiamate asincrone \(a Pinata per fissare i nostri metadati su IPFS, ad Alchemy Web3 per caricare il nostro smart contract e a MetaMask per firmare le nostre transazioni\), anche la nostra funzione sarà asincrona.

I tre input per la nostra funzione saranno l'`url` del nostro asset digitale, `name` e `description`. Aggiungi la seguente firma della funzione sotto la funzione `connectWallet`:
...
Essenzialmente, se uno qualsiasi dei parametri di input è una stringa vuota, restituiamo un oggetto JSON in cui il booleano `success` è false e la stringa `status` comunica che tutti i campi nella nostra interfaccia utente devono essere completi.
...
Se c'è un errore, restituiamo un oggetto JSON in cui il booleano `success` è false e la nostra stringa `status` comunica che la nostra chiamata è fallita. Altrimenti, estraiamo il `pinataURL` da `pinataResponse` e lo memorizziamo come nostra variabile `tokenURI`.

Ora è il momento di caricare il nostro smart contract utilizzando l'API Alchemy Web3 che abbiamo inizializzato all'inizio del nostro file. Aggiungi la seguente riga di codice in fondo alla funzione `mintNFT` per impostare il contratto nella variabile globale `window.contract`:
...
Se hai già familiarità con le transazioni Ethereum, noterai che la struttura è abbastanza simile a quella che hai già visto.

- Innanzitutto, impostiamo i parametri delle nostre transazioni.
  ...
  - `data` contiene la chiamata al metodo `mintNFT` del nostro smart contract, che riceve il nostro `tokenURI` e l'indirizzo del portafoglio dell'utente, `window.ethereum.selectedAddress`, come input
- Quindi, effettuiamo una chiamata await, `window.ethereum.request,` in cui chiediamo a MetaMask di firmare la transazione. Nota, in questa richiesta, stiamo specificando il nostro metodo eth \(eth_SentTransaction\) e passando i nostri `transactionParameters`. A questo punto, MetaMask si aprirà nel browser e chiederà all'utente di firmare o rifiutare la transazione.
  - Se la transazione ha esito positivo, la funzione restituirà un oggetto JSON in cui il booleano `success` è impostato su true e la stringa `status` invita l'utente a controllare Etherscan per maggiori informazioni sulla sua transazione.
...
È una funzione gigantesca! Ora, dobbiamo solo collegare la nostra funzione `mintNFT` al nostro componente `Minter.js`...
...
Per ricapitolare, costruendo un minter di NFT, hai imparato con successo a:
