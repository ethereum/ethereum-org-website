...
description: "In questo tutorial, costruiamo una banca semi-segreta per i depositi. La banca è un componente centralizzato; conosce il saldo di ogni utente. Tuttavia, queste informazioni non sono memorizzate onchain. Invece, la banca pubblica un hash dello stato. Ogni volta che si verifica una transazione, la banca pubblica il nuovo hash, insieme a una prova a conoscenza zero di avere una transazione firmata che cambia lo stato dell'hash in quello nuovo. Dopo aver letto questo tutorial, capirai non solo come usare le prove a conoscenza zero, ma anche perché usarle e come farlo in modo sicuro."
author: Ori Pomerantz
tags: ["a conoscenza zero", "server", "offchain", "privacy"]
skill: advanced
...
A differenza dei [rollup](/developers/docs/scaling/zk-rollups/), i [Plasma](/developers/docs/scaling/plasma) utilizzano la Mainnet di Ethereum per l'integrità, ma non per la disponibilità. In questo articolo, scriviamo un'applicazione che si comporta come un Plasma, con Ethereum che garantisce l'integrità (nessuna modifica non autorizzata) ma non la disponibilità (un componente centralizzato può bloccarsi e disabilitare l'intero sistema).

L'applicazione che scriviamo qui è una banca che preserva la privacy. Diversi indirizzi hanno account con saldi e possono inviare denaro (ETH) ad altri account. La banca pubblica gli hash dello stato (gli account e i loro saldi) e delle transazioni, ma mantiene i saldi effettivi offchain dove possono rimanere privati.
...
A livello fondamentale, una prova a conoscenza zero dimostra che il prover conosce alcuni dati, _Data<sub>private</sub>_ tali che esista una relazione _Relationship_ tra alcuni dati pubblici, _Data<sub>public</sub>_, e _Data<sub>private</sub>_. Il verificatore conosce _Relationship_ e _Data<sub>public</sub>_.

Per preservare la privacy, abbiamo bisogno che gli stati e le transazioni siano privati. Ma per garantire l'integrità, abbiamo bisogno che l'[hash crittografico](https://en.wikipedia.org/wiki/Cryptographic_hash_function) degli stati sia pubblico. Per dimostrare alle persone che inviano transazioni che quelle transazioni sono realmente avvenute, dobbiamo anche pubblicare gli hash della transazione.

Nella maggior parte dei casi, _Data<sub>private</sub>_ è l'input del programma di prova a conoscenza zero e _Data<sub>public</sub>_ è l'output.

Questi campi in _Data<sub>private</sub>_:

- _State<sub>n</sub>_, il vecchio stato
- _State<sub>n+1</sub>_, il nuovo stato
- _Transaction_, una transazione che passa dal vecchio stato al nuovo. Questa transazione deve includere questi campi:
...
Questi sono i campi in _Data<sub>public</sub>_:

- _Hash(State<sub>n</sub>)_ l'hash del vecchio stato
- _Hash(State<sub>n+1</sub>)_ l'hash del nuovo stato
- _Hash(Transaction)_ l'hash della transazione che cambia lo stato da _State<sub>n</sub>_ a _State<sub>n+1</sub>_.
...
A causa delle proprietà delle funzioni di hash crittografiche, dimostrare queste condizioni è sufficiente per garantire l'integrità.

### Strutture dati {#data-structures}

La struttura dati principale è lo stato mantenuto dal server. Per ogni account, il server tiene traccia del saldo dell'account e di un [nonce](https://en.wikipedia.org/wiki/Cryptographic_nonce), utilizzato per prevenire gli [attacchi replay](https://en.wikipedia.org/wiki/Replay_attack).
...
- Uno _smart contract_ che memorizza gli hash e verifica le prove a conoscenza zero per garantire che le transizioni di stato siano legittime.

### Flusso di dati e di controllo {#flows}

Questi sono i modi in cui i vari componenti comunicano per effettuare un trasferimento da un account a un altro.
...
6. Lo smart contract verifica la prova a conoscenza zero.

7. Se la prova a conoscenza zero risulta corretta, lo smart contract esegue queste azioni:
   - Aggiorna l'hash dello stato corrente al nuovo hash dello stato
   - Emette una voce di log con il nuovo hash dello stato e l'hash della transazione

### Strumenti {#tools}

Per il codice lato client, utilizzeremo [Vite](https://vite.dev/), [React](https://react.dev/), [Viem](https://viem.sh/) e [Wagmi](https://wagmi.sh/). Questi sono strumenti standard del settore; se non hai familiarità con essi, puoi usare [questo tutorial](/developers/tutorials/creating-a-wagmi-ui-for-your-contract/).
...
La blockchain che utilizziamo è `anvil`, una blockchain di test locale che fa parte di [Foundry](https://getfoundry.sh/introduction/installation).

## Implementazione {#implementation}

Poiché si tratta di un sistema complesso, lo implementeremo in più fasi.

### Fase 1 - A conoscenza zero manuale {#stage-1}
...
2. Scarica il codice della fase 1 e avvia il server web per servire il codice client.
   ...
   Il motivo per cui qui è necessario un server web è che, per prevenire determinati tipi di frode, molti portafogli (come MetaMask) non accettano file serviti direttamente dal disco

3. Apri un browser con un portafoglio.

4. Nel portafoglio, inserisci una nuova passphrase. Nota che questo eliminerà la tua passphrase esistente, quindi _assicurati di avere un backup_.

   La passphrase è `test test test test test test test test test test test junk`, la passphrase di test predefinita per anvil.
...
10. Confronta gli ultimi due valori con l'hash che vedi sul browser web per verificare se l'hash del messaggio è stato calcolato correttamente.
...
Il messaggio è in formato testo, il che lo rende facile da comprendere per l'utente (necessario durante la firma) e da analizzare per il codice Noir. L'importo è quotato in finney per consentire trasferimenti frazionari da un lato, ed essere facilmente leggibile dall'altro. L'ultimo numero è il [nonce](https://en.wikipedia.org/wiki/Cryptographic_nonce).

La stringa è lunga 100 caratteri. Le prove a conoscenza zero non gestiscono bene i dati di dimensione variabile, quindi è spesso necessario riempire (pad) i dati.
...
Questi sono gli indirizzi degli account, gli indirizzi creati dalla passphrase `test ... test junk`. Se desideri utilizzare i tuoi indirizzi, modifica semplicemente questa definizione.
...
Questa funzione viene chiamata quando l'utente fa clic sul pulsante **Sign** (Firma). Il messaggio viene aggiornato automaticamente, ma la firma richiede l'approvazione dell'utente nel portafoglio e non vogliamo richiederla a meno che non sia necessario.
...
Imposta le variabili di stato. Facendo questo si ridisegna il componente (dopo l'uscita della funzione `sign`) e si mostrano all'utente i valori aggiornati.
...
Viem ci fornisce la chiave pubblica come stringa esadecimale di 65 byte. Il primo byte è `0x04`, un marcatore di versione. Questo è seguito da 32 byte per la `x` della chiave pubblica e poi da 32 byte per la `y` della chiave pubblica.
...
Nota che questa è una buona pratica in generale per la conoscenza zero. Il codice all'interno di una prova a conoscenza zero è costoso, quindi qualsiasi elaborazione che può essere eseguita all'esterno della prova a conoscenza zero _dovrebbe_ essere eseguita all'esterno della prova a conoscenza zero.

```tsx
signature=${hexToArray(signature.slice(2,-2))}
```

Anche la firma viene fornita come stringa esadecimale di 65 byte. Tuttavia, l'ultimo byte è necessario solo per recuperare la chiave pubblica. Poiché la chiave pubblica sarà già fornita al codice Noir, non ne abbiamo bisogno per verificare la firma e il codice Noir non lo richiede.
...
[Questo file](https://github.com/qbzzt/250911-zk-bank/blob/01-manual-zk/server/noir/src/main.nr) è il vero e proprio codice a conoscenza zero.

```
use std::hash::pedersen_hash;
```

L'[hash di Pedersen](https://rya-sge.github.io/access-denied/2024/05/07/pedersen-hash-function/) è fornito con la [libreria standard di Noir](https://noir-lang.org/docs/noir/standard_library/cryptographic_primitives/hashes#pedersen_hash). Le prove a conoscenza zero utilizzano comunemente questa funzione di hash. È molto più facile da calcolare all'interno dei [circuiti aritmetici](https://rareskills.io/post/arithmetic-circuit) rispetto alle funzioni di hash standard.
...
I tipi di dati denominati `u<number>` rappresentano quel numero di bit, senza segno. Gli unici tipi supportati sono `u8`, `u16`, `u32`, `u64` e `u128`.

```
global FLAT_ACCOUNT_FIELDS : u32 = 2;
```

Questa variabile viene utilizzata per l'hash di Pedersen degli account, come spiegato di seguito.

```
global MESSAGE_LENGTH : u32 = 100;
```

Come spiegato sopra, la lunghezza del messaggio è fissa. Viene specificata qui.
...
Le informazioni che memorizziamo su un account. [`Field`](https://noir-lang.org/docs/noir/concepts/data_types/fields) è un numero, in genere fino a 253 bit, che può essere utilizzato direttamente nel [circuito aritmetico](https://rareskills.io/post/arithmetic-circuit) che implementa la prova a conoscenza zero. Qui utilizziamo il `Field` per memorizzare un indirizzo Ethereum a 160 bit.
...
Questa funzione trasforma l'array degli account in un array `Field`, che può essere utilizzato come input per un hash di Petersen.

```
let mut flat: [Field; FLAT_ACCOUNT_FIELDS*ACCOUNT_NUMBER] = [0; FLAT_ACCOUNT_FIELDS*ACCOUNT_NUMBER];
```

Questo è il modo in cui si specifica una variabile mutabile, ovvero _non_ una costante. Le variabili in Noir devono sempre avere un valore, quindi inizializziamo questa variabile con tutti zeri.

```
for i in 0..ACCOUNT_NUMBER {
```

Questo è un ciclo `for`. Nota che i limiti sono costanti. I cicli in Noir devono avere i loro limiti noti in fase di compilazione. Il motivo è che i circuiti aritmetici non supportano il controllo del flusso. Durante l'elaborazione di un ciclo `for`, il compilatore inserisce semplicemente il codice al suo interno più volte, una per ogni iterazione.
...
Infine, siamo arrivati alla funzione che calcola l'hash dell'array degli account.
...
Tuttavia, nelle prove a conoscenza zero, non c'è controllo del flusso. Se abbiamo mai bisogno di verificare una condizione, dobbiamo verificarla ogni volta.

Una cosa simile accade con le istruzioni `if`. L'istruzione `if` nel ciclo precedente viene tradotta in queste istruzioni matematiche.

_condition<sub>result</sub> = accounts[i].address == address_ // uno se sono uguali, zero altrimenti
...
La funzione [`assert`](https://noir-lang.org/docs/dev/noir/concepts/assert) causa l'arresto anomalo della prova a conoscenza zero se l'asserzione è falsa. In questo caso, se non riusciamo a trovare un account con l'indirizzo pertinente. Per segnalare l'indirizzo, utilizziamo una [stringa di formato](https://noir-lang.org/docs/noir/concepts/data_types/strings#format-strings).

```rust
fn apply_transfer_txn(accounts: [Account; ACCOUNT_NUMBER], txn: TransferTxn) -> [Account; ACCOUNT_NUMBER] {
```

Questa funzione applica una transazione di trasferimento e restituisce il nuovo array degli account.
...
Non possiamo accedere agli elementi della struttura all'interno di una stringa di formato in Noir, quindi ne creiamo una copia utilizzabile.
...
Crea il nuovo array degli account e poi restituiscilo.
...
L'indirizzo è sempre lungo 20 byte (ovvero 40 cifre esadecimali) e inizia dal carattere n. 7.
...
Nel messaggio, il primo numero dopo l'indirizzo è l'importo di finney (ovvero un millesimo di ETH) da trasferire. Il secondo numero è il nonce. Qualsiasi testo tra di essi viene ignorato.
...
Questa funzione converte il messaggio in byte, quindi converte gli importi in un `TransferTxn`.
...
Per evitare casi in cui un'applicazione chieda all'utente di firmare un messaggio che può essere utilizzato come transazione o per qualche altro scopo, l'EIP-191 specifica che tutti i messaggi firmati iniziano con il carattere 0x19 (non un carattere ASCII valido) seguito da `Ethereum Signed Message:` e da un ritorno a capo.
...
Gestisci lunghezze dei messaggi fino a 999 e fallisci se è maggiore. Ho aggiunto questo codice, anche se la lunghezza del messaggio è una costante, perché rende più facile modificarla. In un sistema di produzione, probabilmente si presumerebbe semplicemente che `MESSAGE_LENGTH` non cambi per ottenere prestazioni migliori.
...
Questa funzione verifica la firma, che richiede l'hash del messaggio. Ci fornisce quindi l'indirizzo che lo ha firmato e l'hash del messaggio. L'hash del messaggio viene fornito in due valori `Field` perché sono più facili da usare nel resto del programma rispetto a un array di byte.

Dobbiamo usare due valori `Field` perché i calcoli dei campi vengono eseguiti [modulo](https://en.wikipedia.org/wiki/Modulo) un numero grande, ma quel numero è in genere inferiore a 256 bit (altrimenti sarebbe difficile eseguire quei calcoli nell'EVM).
...
Specifica `hash1` e `hash2` come variabili mutabili e scrivi l'hash al loro interno byte per byte.
...
- Sebbene la chiave pubblica possa essere recuperata dalla firma e dall'hash, si tratta di un'elaborazione che può essere eseguita esternamente e, pertanto, non vale la pena eseguire all'interno della prova a conoscenza zero. Se qualcuno cerca di imbrogliarci qui, la verifica della firma fallirà.
...
fn main(
        ...
        Field,  // Hash del vecchio array di account
        Field,  // Hash del nuovo array di account
...
5. Vai al codice client all'indirizzo [http://localhost:5173](http://localhost:5173)
...
8. Per vedere il trasferimento, fai clic su **Update account data** (Aggiorna dati account) o guarda nella finestra in cui esegui il server. Il server registra lo stato ogni volta che cambia.
...
Per fornire le informazioni sull'account, abbiamo solo bisogno della firma. Il motivo è che sappiamo già quale sarà il messaggio e quindi l'hash del messaggio.
...
Se c'è un errore, intercettalo e poi trasmetti una versione semplificata al client.
...
### Fase 3 - Smart contract di Ethereum {#stage-3}

1. Interrompi i processi del server e del client.

2. Scarica il branch con gli smart contract e assicurati di avere tutti i moduli necessari.
...
5. Vai agli smart contract e imposta le variabili d'ambiente per utilizzare la blockchain `anvil`.
...
11. Per verificare che lo stato sia cambiato onchain, riavvia il processo del server. Nota che `ZkBank` non accetta più transazioni, perché il valore hash originale nelle transazioni differisce dal valore hash memorizzato onchain.
...
Le modifiche in questo file riguardano principalmente la creazione della prova effettiva e il suo invio onchain.
...
Dobbiamo utilizzare [il pacchetto Barretenberg](https://github.com/AztecProtocol/aztec-packages/tree/next/barretenberg) per creare la prova effettiva da inviare onchain. Possiamo utilizzare questo pacchetto eseguendo l'interfaccia a riga di comando (`bb`) o utilizzando la [libreria JavaScript, `bb.js`](https://www.npmjs.com/package/@aztec/bb.js). La libreria JavaScript è molto più lenta rispetto all'esecuzione nativa del codice, quindi qui utilizziamo [`exec`](https://nodejs.org/api/child_process.html#child_processexeccommand-options-callback) per utilizzare la riga di comando.

Nota che se decidi di utilizzare `bb.js`, devi utilizzare una versione compatibile con la versione di Noir che stai utilizzando. Al momento della stesura, la versione attuale di Noir (1.0.0-beta.11) utilizza la versione 0.87 di `bb.js`.
...
Questa chiave privata è uno degli account prefinanziati predefiniti in `anvil`. 

```js
const generateProof = async (witness, fileID) => {
```

Genera una prova utilizzando l'eseguibile `bb`.
...
Scrivi il testimone in un file.
...
I campi pubblici devono essere un array di valori a 32 byte. Tuttavia, poiché dovevamo dividere l'hash della transazione tra due valori `Field`, appare come un valore a 16 byte. Qui aggiungiamo degli zeri in modo che Viem capisca che in realtà si tratta di 32 byte.

```js
    const proof = await generateProof(noirResult.witness, `${fromAddress}-${nonce}`)
```

Ogni indirizzo utilizza ogni nonce solo una volta in modo da poter utilizzare una combinazione di `fromAddress` e `nonce` come identificatore univoco per il file del testimone e la directory di output.
...
Questo è il codice onchain che riceve la transazione.
...
Il codice onchain deve tenere traccia di due variabili: il verificatore (un contratto separato creato da `nargo`) e l'hash dello stato corrente.
...
Questa funzione elabora le transazioni. Ottiene la prova (come `bytes`) e gli input pubblici (come array `bytes32`), nel formato richiesto dal verificatore (per ridurre al minimo l'elaborazione onchain e quindi i costi del gas).
...
Chiama il contratto del verificatore per verificare la prova a conoscenza zero. Questo passaggio annulla la transazione se la prova a conoscenza zero è errata.
...
- _Disponibilità_, gli utenti autorizzati possono utilizzare il sistema.
...
Per risolvere questo problema, possiamo scrivere un secondo programma Noir che riceve gli account come input privato e l'indirizzo per cui vengono richieste le informazioni come input pubblico. L'output è il saldo e il nonce di quell'indirizzo, e l'hash degli account.

Naturalmente, questa prova non può essere verificata onchain, perché non vogliamo pubblicare nonce e saldi onchain. Tuttavia, può essere verificata dal codice client in esecuzione nel browser.

### Transazioni forzate {#forced-txns}

Il meccanismo abituale per garantire la disponibilità e prevenire la censura sui L2 sono le [transazioni forzate](https://docs.optimism.io/stack/transactions/forced-transaction). Ma le transazioni forzate non si combinano con le prove a conoscenza zero. Il server è l'unica entità in grado di verificare le transazioni.

Possiamo modificare `smart-contracts/src/ZkBank.sol` per accettare transazioni forzate e impedire al server di modificare lo stato finché non vengono elaborate. Tuttavia, questo ci espone a un semplice attacco denial-of-service. Cosa succede se una transazione forzata non è valida ed è quindi impossibile da elaborare?
...
#### Cauzioni di disponibilità {#avail-bonds}

In un'implementazione reale, ci sarebbe probabilmente una sorta di motivo di profitto per mantenere il server in funzione. Possiamo rafforzare questo incentivo facendo in modo che il server depositi una cauzione di disponibilità che chiunque può bruciare se una transazione forzata non viene elaborata entro un certo periodo.

### Codice Noir errato {#bad-noir-code}

Normalmente, per far sì che le persone si fidino di uno smart contract, carichiamo il codice sorgente su un [block explorer](https://eth.blockscout.com/address/0x7D16d2c4e96BCFC8f815E15b771aC847EcbDB48b?tab=contract). Tuttavia, nel caso delle prove a conoscenza zero, ciò è insufficiente.

`Verifier.sol` contiene la chiave di verifica, che è una funzione del programma Noir. Tuttavia, quella chiave non ci dice quale fosse il programma Noir. Per avere effettivamente una soluzione fidata, è necessario caricare il programma Noir (e la versione che lo ha creato). Altrimenti, le prove a conoscenza zero potrebbero riflettere un programma diverso, uno con una backdoor.

Finché i block explorer non inizieranno a permetterci di caricare e verificare i programmi Noir, dovresti farlo tu stesso (preferibilmente su [IPFS](/developers/tutorials/ipfs-decentralized-ui/)). In questo modo, gli utenti esperti potranno scaricare il codice sorgente, compilarlo da soli, creare `Verifier.sol` e verificare che sia identico a quello onchain.

## Conclusione {#conclusion}

Le applicazioni di tipo Plasma richiedono un componente centralizzato per l'archiviazione delle informazioni. Questo apre a potenziali vulnerabilità ma, in cambio, ci consente di preservare la privacy in modi non disponibili sulla blockchain stessa. Con le prove a conoscenza zero possiamo garantire l'integrità e possibilmente rendere economicamente vantaggioso per chiunque gestisca il componente centralizzato mantenerne la disponibilità.

[Vedi qui per altri miei lavori](https://cryptodocguy.pro/).

## Ringraziamenti {#acknowledgements}

- Josh Crites ha letto una bozza di questo articolo e mi ha aiutato con uno spinoso problema relativo a Noir.
