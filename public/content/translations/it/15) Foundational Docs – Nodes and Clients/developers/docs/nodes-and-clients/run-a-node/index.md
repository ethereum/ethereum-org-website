...
Questo esempio avvia Besu sulla rete principale, memorizza i dati della blockchain nel formato predefinito a `/data/ethereum`, abilita JSON-RPC ed Engine RPC per la connessione del client del consenso. L'API Engine è autenticata con il token `jwtsecret` e solo le chiamate da `localhost` sono consentite.
...
Questo esempio avvia Erigon sulla rete principale, memorizza i dati della blockchain a `/data/ethereum`, abilita JSON-RPC, definisce quali spazi del nome sono consentiti e consente l'autenticazione per la connessione del client di consenso definito dal percorso `jwtsecret`.
...
Questo esempio avvia Geth sulla rete principale, memorizza i dati della blockchain a `/data/ethereum`, abilita JSON-RPC e definisce quali spazi del nome sono consentiti. Inoltre, consente l'autenticazione per connettere il client di consenso, che richiede il percorso a `jwtsecret` e, inoltre, l'opzione che definisce quali connessioni sono consentite, nel nostro esempio solo da `localhost`.
