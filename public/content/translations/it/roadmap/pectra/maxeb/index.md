...
Chiunque gestisca un validatore: probabilmente si tratta di qualcuno che conosce l'indice (es. [Validatore #12345](https://beaconcha.in/validator/12345)) di un validatore che controlla. Se utilizzi un protocollo per gestire un validatore (es. Lido CSM o Rocket Pool), dovrai verificare con loro per capire se e quando supporteranno MaxEB.

Se fai staking utilizzando un token di liquid staking (LST) (es. rETH o stETH), non è richiesta né raccomandata alcuna azione.
...
Quando gestisci un validatore, hai un set di credenziali di prelievo. Queste possono essere trovate nel json dei dati di deposito oppure puoi visualizzarle nella [scheda dei depositi](https://beaconcha.in/validator/12345#deposits) del tuo validatore su beaconcha.in.

1. Credenziali di prelievo di **Tipo 0**: se le credenziali di prelievo del tuo validatore iniziano con `0x00...`, hai depositato prima dell'hard fork Shapella e non hai ancora impostato un indirizzo di prelievo.

![Type 0 withdrawal credential](./0x00-wd.png)

2. Credenziali di prelievo di **Tipo 1**: se le credenziali di prelievo del tuo validatore iniziano con `0x01...`, hai depositato dopo l'hard fork Shapella o hai già convertito le tue credenziali di **Tipo 0** in credenziali di **Tipo 1**.

 ![Type 1 withdrawal credential](./0x01-wd.png)

3. Credenziali di prelievo di **Tipo 2**: questo nuovo tipo di credenziale di prelievo inizierà con `0x02...` e sarà abilitato dopo Pectra. I validatori con credenziali di prelievo di **Tipo 2** sono talvolta chiamati "**validatori a capitalizzazione**" (compounding validators).
...
Ci saranno due utilizzi dell'operazione `ConsolidationRequest`:
...
In una conversione da un validatore di **Tipo 1** a uno di **Tipo 2**, sia l'*origine* (source) che la *destinazione* (target) saranno il validatore che stai convertendo. L'operazione costerà gas e verrà messa in coda dietro ad altre richieste di consolidamento. Questa coda è **separata** dalla coda dei depositi, non è influenzata dai depositi di nuovi validatori e può essere visualizzata su [pectrified.com](https://pectrified.com/).
...
- non essere in uno stato di uscita (o aver subito slashing)
...
Questa è la *stessa operazione* della conversione, ma avviene quando il *validatore di origine* è diverso dal *validatore di destinazione*. L'indice del validatore di destinazione viene preservato e accetta il saldo dal validatore di origine. L'indice del validatore di origine viene messo in uno stato `EXITED`.
...
1. Indirizzo del validatore di origine (es. `0x15F4B914A0cCd14333D850ff311d6DafbFbAa32b`)
...
Per inviare un `ConsolidationRequest`, l'**indirizzo di prelievo del validatore di origine** deve firmare la richiesta. Questo dimostra il controllo sui fondi del validatore.
...
Nota: il processo di firma viene eseguito dall'indirizzo di prelievo, non dalla chiave del validatore.
...
| --- | --- | --- | --- | --- | --- | --- |
| Pectra Staking Manager | pectrastaking.com | Sì, Apache-2.0 | [Pier Two](https://piertwo.com/) | No | Interfaccia web | WalletConnect, funziona con SAFE |
...
| Siren | [GitHub](https://github.com/sigp/siren) | Sì, Apache-2.0 | [Sigma Prime](https://sigmaprime.io/) | No | In parte riga di comando, ma principalmente interfaccia web | Funziona solo se stai utilizzando il client di consenso Lighthouse |
...
No. Puoi convertirlo sul posto senza eseguire l'uscita.

### Quanto tempo ci vorrà per convertire / consolidare? {#how-long}

Un minimo di 27,3 ore, ma i consolidamenti sono soggetti anche a una coda. Questa coda è indipendente dalle code di deposito e prelievo e non ne è influenzata.
