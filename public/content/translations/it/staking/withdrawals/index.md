...
<AlertContent>
<AlertDescription>
**A ogni account del validatore può essere assegnato un solo indirizzo di prelievo, una volta sola.** Una volta scelto e inviato un indirizzo al livello di consenso, questo non può essere annullato o modificato di nuovo. Ricontrolla la proprietà e l'accuratezza dell'indirizzo fornito prima di inviarlo.
</AlertDescription>
</AlertContent>
</Alert>
...
Fornire un indirizzo di prelievo è necessario prima che _qualsiasi_ fondo possa essere trasferito fuori dal saldo di un account del validatore.
...
Quando un validatore è programmato per proporre il blocco successivo, è tenuto a costruire una coda di prelievo, fino a un massimo di 16 prelievi idonei. Questo viene fatto partendo originariamente dall'indice del validatore 0, determinando se c'è un prelievo idoneo per questo account secondo le regole del protocollo, e aggiungendolo alla coda se presente. Il validatore impostato per proporre il blocco successivo riprenderà da dove si era interrotto l'ultimo, procedendo in ordine indefinitamente.

<Alert variant="update">
<AlertIcon size="xl"><Clock /></AlertIcon>
<AlertContent>
<AlertDescription>
Pensa a un orologio analogico. La lancetta dell'orologio indica l'ora, avanza in una direzione, non salta nessuna ora e alla fine ricomincia dall'inizio dopo aver raggiunto l'ultimo numero.
...
La lancetta dell'orologio indica il validatore successivo che deve essere controllato per i prelievi idonei. Inizia da 0 e fa tutto il giro senza saltare alcun account. Quando viene raggiunto l'ultimo validatore, il ciclo ricomincia dall'inizio.
</AlertDescription>
</AlertContent>
</Alert>
...
<ExpandableCard
title="Come prelevo dallo staking se metto in staking tramite un provider, una pool di staking o partecipo con token di liquid staking?"
eventCategory="FAQ"
...
<ExpandableCard
title="Il riscatto delle ricompense della rete (prelievi parziali) avviene automaticamente?"
eventCategory="FAQ"
...
<ExpandableCard
title="Come passo a un validatore compounding?"
eventCategory="FAQ"
...
<ExpandableCard
title="Quando sono stati abilitati i prelievi di staking?"
eventCategory="FAQ"
