...
[ERC-20](/developers/docs/standards/tokens/erc-20/) ٹوکنز ایک کموڈٹی کے طور پر برتاؤ کرتے ہیں، کیونکہ انفرادی ٹوکنز کے درمیان کوئی فرق نہیں ہوتا۔
اس کے برعکس، ERC-721 ٹوکنز ایسے اثاثوں کے لیے بنائے گئے ہیں جو ملتے جلتے تو ہیں لیکن بالکل ایک جیسے نہیں، جیسے کہ مختلف [کیٹ کارٹونز](https://www.cryptokitties.co/) یا رئیل اسٹیٹ کے مختلف حصوں کے ٹائٹلز۔

اس مضمون میں ہم [ریویا ناکامورا کے ERC-721 کنٹریکٹ](https://github.com/vyperlang/vyper/blob/master/examples/tokens/ERC721.vy) کا تجزیہ کریں گے۔
یہ کنٹریکٹ [Vyper](https://vyper.readthedocs.io/en/latest/index.html) میں لکھا گیا ہے، جو کہ Python جیسی ایک کنٹریکٹ لینگویج ہے جسے اس لیے ڈیزائن کیا گیا ہے تاکہ غیر محفوظ کوڈ لکھنا Solidity کی نسبت زیادہ مشکل ہو۔
...
Vyper میں تبصرے (comments)، بالکل Python کی طرح، ہیش (`#`) سے شروع ہوتے ہیں اور لائن کے آخر تک جاری رہتے ہیں۔ وہ تبصرے جن میں `@<keyword>` شامل ہوتا ہے، انہیں [NatSpec](https://vyper.readthedocs.io/en/latest/natspec.html) انسانوں کے پڑھنے کے قابل دستاویزات (documentation) تیار کرنے کے لیے استعمال کرتا ہے۔
...
ERC-721 انٹرفیس Vyper لینگویج میں بلٹ ان (built-in) ہے۔
...
پہلی لائن انٹرفیس کو امپورٹ کرتی ہے، اور دوسری یہ بتاتی ہے کہ ہم اسے یہاں لاگو (implement) کر رہے ہیں۔
...
ERC-721 دو قسم کی ٹرانسفر کو سپورٹ کرتا ہے:

- `transferFrom`، جو بھیجنے والے کو کسی بھی منزل کا ایڈریس (destination address) بتانے کی اجازت دیتا ہے اور ٹرانسفر کی ذمہ داری بھیجنے والے پر ڈالتا ہے۔ اس کا مطلب ہے کہ آپ کسی غلط ایڈریس پر بھی ٹرانسفر کر سکتے ہیں، جس صورت میں NFT ہمیشہ کے لیے ضائع ہو جاتا ہے۔
...
`safeTransferFrom` کی درخواستوں کا جواب دینے کے لیے وصول کرنے والے کنٹریکٹ کو `ERC721Receiver` لاگو کرنا پڑتا ہے۔
...
`_from` ایڈریس ٹوکن کا موجودہ مالک ہے۔ `_operator` ایڈریس وہ ہے جس نے ٹرانسفر کی درخواست کی ہے (الاؤنسز کی وجہ سے یہ دونوں ایک جیسے نہیں بھی ہو سکتے)۔

```python
            _tokenId: uint256,
```

ERC-721 ٹوکن IDs 256 بٹس کے ہوتے ہیں۔ عام طور پر یہ ٹوکن کی نمائندگی کرنے والی کسی بھی چیز کی تفصیل کو ہیش (hash) کر کے بنائے جاتے ہیں۔

```python
            _data: Bytes[1024]
```

درخواست میں 1024 بائٹس تک کا یوزر ڈیٹا ہو سکتا ہے۔

```python
        ) -> bytes32: view
```

ایسے معاملات کو روکنے کے لیے جن میں کوئی کنٹریکٹ غلطی سے ٹرانسفر قبول کر لے، ریٹرن ویلیو (return value) بولین (boolean) نہیں ہوتی، بلکہ ایک مخصوص ویلیو کے ساتھ 256 بٹس ہوتی ہے۔

یہ فنکشن ایک `view` ہے، جس کا مطلب ہے کہ یہ بلاک چین کی اسٹیٹ کو پڑھ سکتا ہے، لیکن اسے تبدیل نہیں کر سکتا۔

### ایونٹس {#events}

[ایونٹس](https://media.consensys.net/technical-introduction-to-events-and-logs-in-ethereum-a074d65dd61e) کو بلاک چین سے باہر موجود صارفین اور سرورز کو واقعات سے آگاہ کرنے کے لیے خارج (emit) کیا جاتا ہے۔ یاد رکھیں کہ ایونٹس کا مواد بلاک چین پر موجود کنٹریکٹس کے لیے دستیاب نہیں ہوتا۔
...
یہ ERC-20 ٹرانسفر ایونٹ سے ملتا جلتا ہے، سوائے اس کے کہ ہم رقم کے بجائے `tokenId` رپورٹ کرتے ہیں۔ ایڈریس صفر (address zero) کا کوئی مالک نہیں ہوتا، اس لیے روایت کے مطابق ہم اسے ٹوکنز کی تخلیق اور تباہی کی رپورٹ دینے کے لیے استعمال کرتے ہیں۔
...
ایک ERC-721 اپروول (approval) ERC-20 الاؤنس کی طرح ہے۔ ایک مخصوص ایڈریس کو ایک مخصوص ٹوکن ٹرانسفر کرنے کی اجازت دی جاتی ہے۔ یہ کنٹریکٹس کو ٹوکن قبول کرنے پر ردعمل ظاہر کرنے کا طریقہ کار فراہم کرتا ہے۔ کنٹریکٹس ایونٹس کو نہیں سن سکتے، اس لیے اگر آپ صرف ٹوکن انہیں ٹرانسفر کر دیں تو انہیں اس کے بارے میں "پتہ" نہیں چلتا۔ اس طرح مالک پہلے ایک اپروول جمع کراتا ہے اور پھر کنٹریکٹ کو درخواست بھیجتا ہے: "میں نے آپ کو ٹوکن X ٹرانسفر کرنے کی منظوری دے دی ہے، براہ کرم کریں..."۔

یہ ایک ڈیزائن کا انتخاب ہے تاکہ ERC-721 اسٹینڈرڈ کو ERC-20 اسٹینڈرڈ کے مشابہ بنایا جا سکے۔ چونکہ ERC-721 ٹوکنز fungible نہیں ہوتے، اس لیے ایک کنٹریکٹ ٹوکن کی ملکیت کو دیکھ کر بھی یہ پہچان سکتا ہے کہ اسے ایک مخصوص ٹوکن ملا ہے۔
...
بعض اوقات ایک ایسے _آپریٹر_ کا ہونا مفید ہوتا ہے جو کسی اکاؤنٹ کے ایک مخصوص قسم کے تمام ٹوکنز (وہ جو کسی مخصوص کنٹریکٹ کے زیر انتظام ہوں) کا انتظام کر سکے، بالکل پاور آف اٹارنی کی طرح۔ مثال کے طور پر، میں شاید ایسی طاقت کسی ایسے کنٹریکٹ کو دینا چاہوں جو یہ چیک کرے کہ آیا میں نے چھ ماہ سے اس سے رابطہ نہیں کیا، اور اگر ایسا ہے تو میرے اثاثے میرے ورثاء میں تقسیم کر دے (اگر ان میں سے کوئی اس کا مطالبہ کرے، کنٹریکٹس ٹرانزیکشن کے ذریعے کال کیے بغیر کچھ نہیں کر سکتے)۔ ERC-20 میں ہم صرف ایک وراثتی کنٹریکٹ کو زیادہ الاؤنس دے سکتے ہیں، لیکن یہ ERC-721 کے لیے کام نہیں کرتا کیونکہ ٹوکنز fungible نہیں ہوتے۔ یہ اسی کا متبادل ہے۔

`approved` ویلیو ہمیں بتاتی ہے کہ آیا ایونٹ کسی اپروول کے لیے ہے، یا اپروول واپس لینے کے لیے۔
...
ایتھیریم میں صارف اور کنٹریکٹ کی شناخت 160-بٹ ایڈریسز سے ظاہر کی جاتی ہے۔ یہ دو ویری ایبلز ٹوکن IDs سے ان کے مالکان اور انہیں ٹرانسفر کرنے کے لیے منظور شدہ افراد (ہر ایک کے لیے زیادہ سے زیادہ ایک) تک میپ کرتے ہیں۔ ایتھیریم میں، غیر شروع شدہ (uninitialized) ڈیٹا ہمیشہ صفر ہوتا ہے، لہذا اگر کوئی مالک یا منظور شدہ ٹرانسفر کرنے والا نہیں ہے تو اس ٹوکن کی ویلیو صفر ہوتی ہے۔
...
یہ وہ فنکشنز ہیں جو دراصل ERC-721 کو لاگو کرتے ہیں۔
...
یہ وہ فنکشنز ہیں جو بلاک چین کی اسٹیٹ کو تبدیل نہیں کرتے، اور اس لیے اگر انہیں بیرونی طور پر کال کیا جائے تو انہیں مفت میں چلایا جا سکتا ہے۔ اگر ویو فنکشنز کو کسی کنٹریکٹ کے ذریعے کال کیا جائے تو انہیں پھر بھی ہر نوڈ پر چلانا پڑتا ہے اور اس لیے گیس کی لاگت آتی ہے۔
...
فنکشن کی تعریف سے پہلے یہ کلیدی الفاظ (keywords) جو ایٹ سائن (`@`) سے شروع ہوتے ہیں، انہیں _decorations_ کہا جاتا ہے۔ یہ ان حالات کی وضاحت کرتے ہیں جن میں کسی فنکشن کو کال کیا جا سکتا ہے۔
...
Python کے برعکس، Vyper ایک [static typed language](https://wikipedia.org/wiki/Type_system#Static_type_checking) ہے۔ آپ [ڈیٹا ٹائپ](https://vyper.readthedocs.io/en/latest/types.html) کی شناخت کیے بغیر کسی ویری ایبل، یا فنکشن پیرامیٹر کا اعلان نہیں کر سکتے۔ اس صورت میں ان پٹ پیرامیٹر `bytes32` ہے، جو کہ 256-بٹ کی ویلیو ہے (256 بٹس [Ethereum Virtual Machine](/developers/docs/evm/) کا مقامی ورڈ سائز ہے)۔ آؤٹ پٹ ایک بولین ویلیو ہے۔ روایت کے مطابق، فنکشن پیرامیٹرز کے نام انڈرسکور (`_`) سے شروع ہوتے ہیں۔
...
`self.supportedInterfaces` HashMap سے ویلیو واپس کریں، جو کنسٹرکٹر (`__init__`) میں سیٹ کی گئی ہے۔
...
یہ لائن [asserts](https://vyper.readthedocs.io/en/latest/statements.html#assert) کرتی ہے کہ `_owner` صفر نہیں ہے۔ اگر ایسا ہے، تو ایک خرابی (error) ہے اور آپریشن کو ریورٹ (revert) کر دیا جاتا ہے۔
...
Ethereum Virtual Machine (EVM) میں کوئی بھی اسٹوریج جس میں کوئی ویلیو محفوظ نہ ہو، وہ صفر ہوتی ہے۔ اگر `_tokenId` پر کوئی ٹوکن نہیں ہے تو `self.idToOwner[_tokenId]` کی ویلیو صفر ہوتی ہے۔ اس صورت میں فنکشن ریورٹ ہو جاتا ہے۔
...
یاد رکھیں کہ `getApproved` صفر واپس _کر سکتا ہے_۔ اگر ٹوکن درست ہے تو یہ `self.idToApprovals[_tokenId]` واپس کرتا ہے۔ اگر کوئی منظور کنندہ (approver) نہیں ہے تو وہ ویلیو صفر ہوتی ہے۔
...
یہ فنکشنز ان آپریشنز کو لاگو کرتے ہیں جو ٹوکنز کی منتقلی یا انتظام کا حصہ ہیں۔
...
تین طریقے ہیں جن سے کسی ایڈریس کو ٹوکن ٹرانسفر کرنے کی اجازت دی جا سکتی ہے:

1. ایڈریس ٹوکن کا مالک ہو
2. ایڈریس کو وہ ٹوکن خرچ کرنے کی منظوری دی گئی ہو
3. ایڈریس ٹوکن کے مالک کا آپریٹر ہو
...
جب ٹرانسفر میں کوئی مسئلہ ہوتا ہے تو ہم کال کو ریورٹ کر دیتے ہیں۔
...
صرف ضرورت پڑنے پر ویلیو تبدیل کریں۔ اسٹیٹ ویری ایبلز اسٹوریج میں رہتے ہیں۔ اسٹوریج میں لکھنا EVM (Ethereum Virtual Machine) کے سب سے مہنگے آپریشنز میں سے ایک ہے ([گیس](/developers/docs/gas/) کے لحاظ سے)۔ اس لیے، اسے کم سے کم کرنا ایک اچھا خیال ہے، یہاں تک کہ موجودہ ویلیو کو لکھنے کی بھی زیادہ لاگت آتی ہے۔
...
ہمارے پاس یہ اندرونی فنکشن اس لیے ہے کیونکہ ٹوکنز ٹرانسفر کرنے کے دو طریقے ہیں (باقاعدہ اور محفوظ)، لیکن ہم کوڈ میں صرف ایک ہی جگہ چاہتے ہیں جہاں ہم یہ کریں تاکہ آڈیٹنگ آسان ہو سکے۔
...
یہ فنکشن آپ کو کسی بھی من مانے ایڈریس پر ٹرانسفر کرنے دیتا ہے۔ جب تک کہ ایڈریس کوئی صارف نہ ہو، یا کوئی ایسا کنٹریکٹ نہ ہو جو ٹوکنز ٹرانسفر کرنا جانتا ہو، آپ کا ٹرانسفر کردہ کوئی بھی ٹوکن اس ایڈریس میں پھنس جائے گا اور بیکار ہو جائے گا۔
...
پہلے ٹرانسفر کرنا ٹھیک ہے کیونکہ اگر کوئی مسئلہ ہوا تو ہم ویسے بھی ریورٹ کرنے والے ہیں، اس لیے کال میں کیا گیا ہر کام منسوخ ہو جائے گا۔

```python
    if _to.is_contract: # چیک کریں کہ کیا `_to` ایک کانٹریکٹ ایڈریس ہے
```

پہلے یہ چیک کریں کہ آیا ایڈریس کوئی کنٹریکٹ ہے (اگر اس میں کوڈ ہے)۔ اگر نہیں، تو فرض کریں کہ یہ صارف کا ایڈریس ہے اور صارف ٹوکن استعمال کرنے یا اسے ٹرانسفر کرنے کے قابل ہو گا۔ لیکن اسے آپ کو تحفظ کے جھوٹے احساس میں مبتلا نہ ہونے دیں۔ آپ ٹوکنز کھو سکتے ہیں، یہاں تک کہ `safeTransferFrom` کے ساتھ بھی، اگر آپ انہیں کسی ایسے ایڈریس پر ٹرانسفر کرتے ہیں جس کی پرائیویٹ کی (private key) کوئی نہیں جانتا۔
...
اگر منزل کوئی کنٹریکٹ ہے، لیکن ایسا جو ERC-721 ٹوکنز قبول نہیں کرتا (یا جس نے اس مخصوص ٹرانسفر کو قبول نہ کرنے کا فیصلہ کیا ہے)، تو ریورٹ کریں۔
...
اپروول سیٹ کرنے کے لیے آپ یا تو مالک ہو سکتے ہیں، یا مالک کی طرف سے مجاز آپریٹر۔
...
وہ اکاؤنٹ جس نے کنٹریکٹ بنایا ہے وہ `minter` ہے، وہ سپر یوزر جو نئے NFTs منٹ (mint) کرنے کا مجاز ہے۔ تاہم، اسے بھی موجودہ ٹوکنز کو برن (burn) کرنے کی اجازت نہیں ہے۔ صرف مالک، یا مالک کی طرف سے مجاز کوئی ہستی ہی ایسا کر سکتی ہے۔
...
صرف منٹر (وہ اکاؤنٹ جس نے ERC-721 کنٹریکٹ بنایا) ہی نئے ٹوکنز منٹ کر سکتا ہے۔ مستقبل میں یہ ایک مسئلہ ہو سکتا ہے اگر ہم منٹر کی شناخت تبدیل کرنا چاہیں۔ پروڈکشن کنٹریکٹ میں آپ شاید ایک ایسا فنکشن چاہیں گے جو منٹر کو منٹر کے حقوق کسی اور کو منتقل کرنے کی اجازت دے۔
...
روایت کے مطابق، نئے ٹوکنز کی منٹنگ کو ایڈریس صفر سے ٹرانسفر کے طور پر شمار کیا جاتا ہے۔
...
کوئی بھی شخص جسے ٹوکن ٹرانسفر کرنے کی اجازت ہے اسے برن کرنے کی بھی اجازت ہے۔ اگرچہ برن کرنا صفر ایڈریس پر ٹرانسفر کرنے کے مترادف لگتا ہے، لیکن صفر ایڈریس دراصل ٹوکن وصول نہیں کرتا۔ یہ ہمیں اس تمام اسٹوریج کو خالی کرنے کی اجازت دیتا ہے جو ٹوکن کے لیے استعمال کی گئی تھی، جس سے ٹرانزیکشن کی گیس کی لاگت کم ہو سکتی ہے۔

## اس کنٹریکٹ کا استعمال {#using-contract}

Solidity کے برعکس، Vyper میں وراثت (inheritance) نہیں ہے۔ یہ ایک دانستہ ڈیزائن کا انتخاب ہے تاکہ کوڈ کو واضح اور اس لیے محفوظ بنانا آسان ہو۔ لہذا اپنا Vyper ERC-721 کنٹریکٹ بنانے کے لیے آپ [یہ کنٹریکٹ](https://github.com/vyperlang/vyper/blob/master/examples/tokens/ERC721.vy) لیتے ہیں اور اپنی مطلوبہ بزنس لاجک کو لاگو کرنے کے لیے اس میں ترمیم کرتے ہیں۔

## نتیجہ {#conclusion}

جائزے کے لیے، اس کنٹریکٹ میں کچھ اہم ترین خیالات یہ ہیں:

- محفوظ ٹرانسفر کے ساتھ ERC-721 ٹوکنز وصول کرنے کے لیے، کنٹریکٹس کو `ERC721Receiver` انٹرفیس لاگو کرنا پڑتا ہے۔
- یہاں تک کہ اگر آپ محفوظ ٹرانسفر استعمال کرتے ہیں، تب بھی ٹوکنز پھنس سکتے ہیں اگر آپ انہیں کسی ایسے ایڈریس پر بھیجتے ہیں جس کی پرائیویٹ کی نامعلوم ہو۔
- جب کسی آپریشن میں کوئی مسئلہ ہو تو کال کو `revert` کرنا ایک اچھا خیال ہے، بجائے اس کے کہ صرف ناکامی کی ویلیو واپس کی جائے۔
- ERC-721 ٹوکنز اس وقت موجود ہوتے ہیں جب ان کا کوئی مالک ہو۔
- NFT ٹرانسفر کرنے کا مجاز ہونے کے تین طریقے ہیں۔ آپ مالک ہو سکتے ہیں، کسی مخصوص ٹوکن کے لیے منظور شدہ ہو سکتے ہیں، یا مالک کے تمام ٹوکنز کے لیے آپریٹر ہو سکتے ہیں۔
- ماضی کے ایونٹس صرف بلاک چین کے باہر نظر آتے ہیں۔ بلاک چین کے اندر چلنے والا کوڈ انہیں نہیں دیکھ سکتا۔

اب جائیں اور محفوظ Vyper کنٹریکٹس لاگو کریں۔
