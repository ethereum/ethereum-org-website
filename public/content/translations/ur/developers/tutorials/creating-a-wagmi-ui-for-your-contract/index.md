...
description: TypeScript، React، Vite، اور Wagmi جیسے جدید اجزاء کا استعمال کرتے ہوئے، ہم ایک جدید، لیکن سادہ یوزر انٹرفیس کا جائزہ لیں گے اور سیکھیں گے کہ والیٹ کو یوزر انٹرفیس سے کیسے منسلک کیا جائے، معلومات پڑھنے کے لیے اسمارٹ کنٹریکٹ کو کیسے کال کیا جائے، اسمارٹ کنٹریکٹ پر ٹرانزیکشن کیسے بھیجی جائے، اور تبدیلیوں کی شناخت کے لیے اسمارٹ کنٹریکٹ سے ایونٹس کی نگرانی کیسے کی جائے۔
...
یہ مضمون آپ کے لیے ہے۔ میں فرض کرتا ہوں کہ آپ پروگرامنگ جانتے ہیں، اور شاید تھوڑی بہت JavaScript اور HTML بھی، لیکن آپ کی یوزر انٹرفیس کی مہارتیں پرانی اور زنگ آلود ہو چکی ہیں۔ ہم مل کر ایک سادہ جدید ایپلیکیشن کا جائزہ لیں گے تاکہ آپ دیکھ سکیں کہ آج کل یہ کیسے کیا جاتا ہے۔

## یہ کیوں اہم ہے {#why-important}

نظریاتی طور پر، آپ لوگوں کو اپنے کنٹریکٹس کے ساتھ تعامل کرنے کے لیے صرف [Etherscan](https://sepolia.etherscan.io/address/0xC87506C66c7896366b9E988FE0aA5B6dDE77CFfA#readContract) یا [Blockscout](https://eth-sepolia.blockscout.com/address/0xC87506C66c7896366b9E988FE0aA5B6dDE77CFfA?tab=read_write_contract) استعمال کرنے کا کہہ سکتے ہیں۔ یہ تجربہ کار ایتھیریم صارفین کے لیے تو بہت اچھا ہے۔ لیکن ہم [مزید ایک ارب لوگوں](https://blog.ethereum.org/2021/05/07/ethereum-for-the-next-billion) کی خدمت کرنے کی کوشش کر رہے ہیں۔ یہ ایک بہترین یوزر ایکسپیرینس کے بغیر ممکن نہیں ہوگا، اور ایک دوستانہ یوزر انٹرفیس اس کا ایک بڑا حصہ ہے۔

## Greeter ایپلیکیشن {#greeter-app}

جدید UI کیسے کام کرتا ہے اس کے پیچھے بہت سی تھیوری ہے، اور [بہت سی اچھی سائٹس](https://react.dev/learn/thinking-in-react) ہیں [جو اس کی وضاحت کرتی ہیں](https://wagmi.sh/core/getting-started)۔ ان سائٹس کے کیے گئے بہترین کام کو دہرانے کے بجائے، میں یہ فرض کروں گا کہ آپ عملی طور پر سیکھنے کو ترجیح دیتے ہیں اور ایک ایسی ایپلیکیشن سے شروعات کریں گے جس کے ساتھ آپ کھیل سکیں۔ آپ کو کام مکمل کرنے کے لیے اب بھی تھیوری کی ضرورت ہے، اور ہم اس تک پہنچیں گے - ہم بس سورس فائل در سورس فائل آگے بڑھیں گے، اور جیسے جیسے چیزیں سامنے آئیں گی ان پر تبادلہ خیال کریں گے۔
...
2. GitHub ریپوزٹری کو کلون کریں اور ضروری پیکجز انسٹال کریں۔
   ...
   ```sh
   git clone https://github.com/qbzzt/wagmi-ui-tutorial.git
   cd wagmi-ui-tutorial
   npm install
   ```

3. یہ ایپلیکیشن مفت ایکسیس پوائنٹس استعمال کرتی ہے، جن کی کارکردگی کی حدود ہوتی ہیں۔ اگر آپ [نوڈ بطور سروس](/developers/docs/nodes-and-clients/nodes-as-a-service/) پرووائیڈر استعمال کرنا چاہتے ہیں، تو [`src/wagmi.ts`](#wagmi-ts) میں URLs کو تبدیل کریں۔
...
وہ React component امپورٹ کریں جو ایپلیکیشن کو نافذ کرتا ہے (نیچے دیکھیں)۔
...
ایپلیکیشن [ایک `React.StrictMode` component](https://react.dev/reference/react/StrictMode) کے اندر جا رہی ہے۔ یہ component React لائبریری کو اضافی ڈیبگنگ چیکس شامل کرنے کا کہتا ہے، جو ڈیولپمنٹ کے دوران مفید ہوتا ہے۔
...
اب ہم ایپلیکیشن کے لیے component رکھ سکتے ہیں، جو دراصل UI کو نافذ کرتا ہے۔ component کے آخر میں `/>` React کو بتاتا ہے کہ XML معیار کے مطابق، اس component کے اندر کوئی تعریفیں نہیں ہیں۔
...
ہمیں درکار لائبریریاں امپورٹ کریں، اور ساتھ ہی [`Greeter` component](#greeter-tsx) بھی۔
...
روایت کے مطابق، React میں `use...` کہلانے والے فنکشنز [hooks](https://www.w3schools.com/react/react_hooks.asp) ہوتے ہیں۔ یہ فنکشنز نہ صرف component کو ڈیٹا واپس کرتے ہیں؛ بلکہ وہ یہ بھی یقینی بناتے ہیں کہ جب وہ ڈیٹا تبدیل ہو تو اسے دوبارہ رینڈر کیا جائے (component فنکشن دوبارہ چلایا جاتا ہے، اور اس کا آؤٹ پٹ HTML میں پچھلے والے کی جگہ لے لیتا ہے)۔
...
React ہک [`useEffect`](https://react.dev/reference/react/useEffect) آپ کو کسی بیرونی سسٹم کو ہم آہنگ کرنے کے لیے جب بھی کسی متغیر (variable) کی قدر تبدیل ہو تو ایک فنکشن چلانے کی سہولت دیتا ہے۔
...
اگر ہم منسلک ہیں، لیکن Sepolia بلاک چین سے نہیں، تو Sepolia پر سوئچ کریں۔

```tsx
  }, [isConnected, chainId])
```

ہر بار جب کنکشن کا اسٹیٹس یا کنکشن کی chainId تبدیل ہو تو فنکشن کو دوبارہ چلائیں۔
...
موجودہ کنکشن کے بارے میں معلومات فراہم کریں۔ JSX کے اندر، `{<expression>}` کا مطلب ہے کہ ایکسپریشن کو JavaScript کے طور پر جانچا جائے۔
...
یہ JSX کے اندر if اسٹیٹمنٹس رکھنے کا معیاری طریقہ ہے۔
...
اگر ہم منسلک _نہیں_ ہیں، تو والیٹ سے منسلک ہونے کے لیے ضروری آپشنز دکھائیں۔
...
JSX میں "sibling" ٹیگز (وہ ٹیگز جو ایک ہی پیرنٹ سے نکلتے ہیں) کے لیے مختلف شناخت کنندگان (identifiers) کا ہونا ضروری ہے۔
...
اضافی معلومات فراہم کریں۔ ایکسپریشن سنٹیکس `<variable>?.<field>` JavaScript کو بتاتا ہے کہ اگر متغیر کی وضاحت کی گئی ہے، تو اس فیلڈ کا جائزہ لیں۔ اگر متغیر کی وضاحت نہیں کی گئی ہے، تو یہ ایکسپریشن `undefined` کا جائزہ لیتا ہے۔
...
اگر آپ کنٹریکٹس اور UI ایک ہی وقت میں تیار کر رہے ہیں، تو آپ عام طور پر انہیں ایک ہی ریپوزٹری میں رکھیں گے اور Solidity کمپائلر کے ذریعے تیار کردہ ABI کو اپنی ایپلیکیشن میں ایک فائل کے طور پر استعمال کریں گے۔ تاہم، یہاں اس کی ضرورت نہیں ہے کیونکہ کنٹریکٹ پہلے ہی تیار ہو چکا ہے اور تبدیل نہیں ہوگا۔
...
```tsx
const greeterAddress: Record<number, AddressType> = {
```

TypeScript سختی سے ٹائپ شدہ (strongly typed) ہے۔ ہم اس تعریف کا استعمال اس ایڈریس کی وضاحت کرنے کے لیے کرتے ہیں جہاں `Greeter` کنٹریکٹ مختلف چینز پر تعینات کیا گیا ہے۔ کلید ایک نمبر (chainId) ہے، اور قدر ایک `AddressType` (ایک ایڈریس) ہے۔
...
`Timer` component کسی دیے گئے وقت سے لے کر اب تک کے سیکنڈز کی تعداد دکھاتا ہے۔ یہ استعمال کی سہولت کے مقاصد کے لیے اہم ہے۔ جب صارفین کچھ کرتے ہیں، تو وہ فوری ردعمل کی توقع کرتے ہیں۔ بلاک چینز میں، یہ اکثر ناممکن ہوتا ہے کیونکہ جب تک کوئی ٹرانزیکشن کسی بلاک میں نہیں رکھی جاتی تب تک کچھ نہیں ہوتا۔ اس کا ایک حل یہ دکھانا ہے کہ صارف کو کارروائی کیے کتنا وقت گزر چکا ہے، تاکہ صارف فیصلہ کر سکے کہ آیا درکار وقت معقول ہے یا نہیں۔

```tsx
function Timer({ lastUpdate }: { lastUpdate: number }) {
```
...
ہمیں اسٹیٹ (component سے منسلک ایک متغیر) رکھنے اور component کے درست طریقے سے کام کرنے کے لیے اسے اپ ڈیٹ کرنے کی ضرورت ہے۔ لیکن ہمیں اسے کبھی پڑھنے کی ضرورت نہیں ہوتی، اس لیے متغیر بنانے کی زحمت نہ کریں۔
...
[`setInterval`](https://www.w3schools.com/jsref/met_win_setinterval.asp) فنکشن ہمیں وقتاً فوقتاً چلنے کے لیے ایک فنکشن شیڈول کرنے کی سہولت دیتا ہے۔ اس صورت میں، ہر سیکنڈ۔ فنکشن اسٹیٹ کو اپ ڈیٹ کرنے کے لیے `setNow` کو کال کرتا ہے، تاکہ `Timer` component دوبارہ رینڈر ہو جائے۔ ہم اسے ایک خالی انحصاری فہرست (dependency list) کے ساتھ [`useEffect`](https://react.dev/reference/react/useEffect) کے اندر لپیٹتے ہیں تاکہ یہ ہر بار component کے رینڈر ہونے کے بجائے صرف ایک بار ہو۔

```tsx
  return <span>{Math.round((Date.now() - lastUpdate) / 1000)}</span>
}
```
...
اس چین اور اکاؤنٹ کے بارے میں معلومات جو ہم استعمال کر رہے ہیں، [wagmi](https://wagmi.sh/) کے بشکریہ۔ چونکہ یہ ایک ہک (`use...`) ہے، اس لیے جب بھی یہ معلومات تبدیل ہوتی ہیں تو component دوبارہ رینڈر ہوتا ہے۔

```tsx
  const greeterAddr = chain ? greeterAddress[chain.id] : undefined
```

Greeter کنٹریکٹ کا ایڈریس، جو `undefined` ہوتا ہے اگر ہمارے پاس چین کی معلومات نہ ہوں، یا ہم اس کنٹریکٹ کے بغیر کسی چین پر ہوں۔
...
2. ضرورت پڑنے پر اسٹیٹ متغیر میں ترمیم کرنے کا ایک فنکشن۔ چونکہ یہ ایک ہک ہے، ہر بار جب اسے کال کیا جاتا ہے تو component دوبارہ رینڈر ہوتا ہے۔
...
اگر ایک سے زیادہ صارفین ایک ہی وقت میں ایک ہی کنٹریکٹ استعمال کر رہے ہیں، تو وہ ایک دوسرے کی گریٹنگز کو اوور رائٹ کر سکتے ہیں۔ یہ صارفین کو ایسا لگے گا جیسے ایپلیکیشن خراب ہو رہی ہے۔ اگر ایپلیکیشن یہ دکھاتی ہے کہ آخری بار گریٹنگ کس نے سیٹ کی تھی، تو صارف کو معلوم ہو جائے گا کہ یہ کوئی اور تھا اور ایپلیکیشن درست طریقے سے کام کر رہی ہے۔
...
صارفین یہ دیکھنا پسند کرتے ہیں کہ ان کے اعمال کا فوری اثر ہو۔ تاہم، بلاک چین پر، ایسا نہیں ہوتا ہے۔ یہ اسٹیٹ متغیرات ہمیں کم از کم صارفین کو کچھ دکھانے کی سہولت دیتے ہیں تاکہ انہیں معلوم ہو کہ ان کی کارروائی جاری ہے۔

```tsx
  const [currentGreeting, setCurrentGreeting] = useState("")

  if (readResults.data && readResults.data !== currentGreeting) {
    setCurrentGreeting(readResults.data)
    setStatus("Greeting updated")
...
`!!<value>` کا مطلب ہے کہ اگر قدر `false` ہے، یا کوئی ایسی قدر ہے جو غلط کے طور پر جانچی جاتی ہے، جیسے کہ `undefined`، `0`، یا ایک خالی اسٹرنگ، تو مجموعی طور پر ایکسپریشن `false` ہے۔ کسی بھی دوسری قدر کے لیے، یہ `true` ہے۔ یہ اقدار کو بولینز (booleans) میں تبدیل کرنے کا ایک طریقہ ہے، کیونکہ اگر کوئی `greeterAddr` نہیں ہے، تو ہم ایونٹس کو سننا نہیں چاہتے۔

```tsx
    onLogs(logs) {
      setCurrentGreeting(logs[0].args.newGreeting!)
      setLastSetterAddress(logs[0].args.setter!)
      setStatus("Greeting updated")
      setStatusTime(Date.now())
    },
  })
...
```tsx
  const greetingChange = (evt: any) => {
    setNewGreeting(evt.target.value)
  }
```

یہ نئی گریٹنگ ان پٹ فیلڈ میں تبدیلیوں کے لیے ایونٹ ہینڈلر ہے۔ ہم `evt` پیرامیٹر کی ٹائپ بتا سکتے تھے، لیکن TypeScript ایک ٹائپ آپشنل زبان ہے۔ چونکہ یہ فنکشن صرف ایک بار کال کیا جاتا ہے، ایک HTML ایونٹ ہینڈلر میں، مجھے نہیں لگتا کہ یہ ضروری ہے۔
...
3. جب جواب موصول ہو جائے، تو صارف سے والیٹ کے ذریعے ٹرانزیکشن پر دستخط کرنے کو کہیں۔ یہ قدم نوڈ کا جواب موصول ہونے کے بعد _لازمی_ ہونا چاہیے کیونکہ صارف کو دستخط کرنے سے پہلے ٹرانزیکشن کی گیس لاگت دکھائی جاتی ہے۔
...
مرحلہ 2 میں ممکنہ طور پر قابل ذکر وقت لگ سکتا ہے، جس کے دوران صارفین سوچ سکتے ہیں کہ آیا ان کی کمانڈ یوزر انٹرفیس کو موصول ہوئی ہے یا نہیں اور ان سے ابھی تک ٹرانزیکشن پر دستخط کرنے کے لیے کیوں نہیں کہا جا رہا ہے۔ اس سے ایک خراب یوزر ایکسپیرینس (UX) پیدا ہوتا ہے۔
...
```tsx
      ...
      <p>
        <b>Current greeting:</b> {currentGreeting}
      </p>
```
...
```tsx
      {lastSetterAddress && (
        <p>
          <b>Last set by:</b> {lastSetterAddress}
        </p>
      )}
```
...
```tsx
      <input value={newGreeting} onChange={greetingChange} />
```

یہ ان پٹ ٹیکسٹ فیلڈ ہے جہاں صارف ایک نئی گریٹنگ سیٹ کر سکتا ہے۔ ہر بار جب صارف کوئی کلید (key) دباتا ہے، ہم `greetingChange` کو کال کرتے ہیں، جو `setNewGreeting` کو کال کرتا ہے۔ چونکہ `setNewGreeting`، `useState` سے آتا ہے، اس لیے یہ `Greeter` component کو دوبارہ رینڈر کرنے کا سبب بنتا ہے۔ اس کا مطلب ہے کہ:
...
- جب بھی `newGreeting` تبدیل ہوتی ہے تو `simulation` بھی اپ ڈیٹ ہو جاتی ہے، جس کا مطلب ہے کہ ہمیں درست گریٹنگ کے ساتھ ایک سیمولیشن ملے گی۔ یہ متعلقہ ہو سکتا ہے کیونکہ گیس کی لاگت کال ڈیٹا کے سائز پر منحصر ہوتی ہے، جو اسٹرنگ کی لمبائی پر منحصر ہوتا ہے۔
...
```tsx
          writeContractAsync(simulation.data!.request)
            .then(() => updateStatus("Transaction sent, waiting for inclusion"))
            .catch((err) => updateStatus(`Transaction failed: ${err}`))
        }}
      >
        Update greeting
      </button>
...
`writeContractAsync` صرف اس وقت واپس آتا ہے جب ٹرانزیکشن دراصل بھیج دی جاتی ہے۔ اس سے ہم صارف کو یہ دکھا سکتے ہیں کہ ٹرانزیکشن کو بلاک چین میں شامل ہونے کے لیے کتنا انتظار کرنا پڑا ہے۔

```tsx
      {status && (
        <p>
          <b>Status:</b> {status} (<Timer lastUpdate={statusTime} /> seconds
          ago)
        </p>
      )}
    </>
...
[یہ کنیکٹر](https://wagmi.sh/core/api/connectors/injected) ہمیں براؤزر میں انسٹال شدہ والیٹ سے بات کرنے کی سہولت دیتا ہے۔
...
3.  یہ یقینی بنانے کے لیے `src/Greeter.tsx` میں ترمیم کریں کہ ایپلیکیشن نئے نیٹ ورک پر آپ کے کنٹریکٹس کا ایڈریس جانتی ہے۔
    ...
    const greeterAddress: Record<number, AddressType> = {
      11155111: "0xC87506C66c7896366b9E988FE0aA5B6dDE77CFfA",
      11155420: "0x112141b612111111111111111111111111111111",
    }
    ```
...
4.  اپنے براؤزر میں۔
    ...
    B. ایپلیکیشن میں، بلاک چین کو تبدیل کرنے کے لیے **Disconnect** کریں اور پھر دوبارہ منسلک ہوں۔ اسے سنبھالنے کے اور بھی بہتر طریقے ہیں، لیکن ان کے لیے ایپلیکیشن میں تبدیلیوں کی ضرورت ہوگی۔
...
1. ایک wagmi ایپلیکیشن بنانے کی وضاحت کریں۔
...
اب جائیں اور اپنے کنٹریکٹس کو وسیع دنیا کے لیے قابل استعمال بنائیں۔
