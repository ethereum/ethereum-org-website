...
- پاتھ 1: <span dir="ltr">`a == 65`</span>
- پاتھ 2: <span dir="ltr">`Not (a == 65)`</span>

ہر path predicate ایک ریاضیاتی فارمولہ ہے جو ایک نام نہاد [SMT solver](https://wikipedia.org/wiki/Satisfiability_modulo_theories) کو دیا جا سکتا ہے، جو مساوات کو حل کرنے کی کوشش کرے گا۔ `Path 1` کے لیے، سولور کہے گا کہ پاتھ کو <span dir="ltr">`a = 65`</span> کے ساتھ دریافت کیا جا سکتا ہے۔ `Path 2` کے لیے، سولور `a` کو 65 کے علاوہ کوئی بھی ویلیو دے سکتا ہے، مثال کے طور پر <span dir="ltr">`a = 0`</span>۔
...
- پاتھ 1: <span dir="ltr">`c = a + b`</span>

Manticore کا استعمال کرتے ہوئے، آپ اوور فلو کی جانچ کر سکتے ہیں، اور path predicate میں پابندیاں شامل کر سکتے ہیں:

- <span dir="ltr">`c = a + b AND (c < a OR c < b)`</span>

اگر `a` اور `b` کی ایسی ویلیو تلاش کرنا ممکن ہے جس کے لیے مندرجہ بالا path predicate قابل عمل ہو، تو اس کا مطلب ہے کہ آپ کو ایک اوور فلو مل گیا ہے۔ مثال کے طور پر سولور ان پٹ <span dir="ltr">`a = 10 , b = MAXUINT256`</span> بنا سکتا ہے۔
...
- <span dir="ltr">`c = a + b AND (c >= a) AND (c=>b) AND (c < a OR c < b)`</span>
...
...
...
manticore.core.smtlib.solver - INFO - Generated testcase No. 0 - STOP
manticore.core.smtlib.solver - INFO - Generated testcase No. 1 - REVERT
manticore.core.smtlib.solver - INFO - Generated testcase No. 2 - RETURN
manticore.core.smtlib.solver - INFO - Generated testcase No. 3 - REVERT
manticore.core.smtlib.solver - INFO - Generated testcase No. 4 - STOP
manticore.core.smtlib.solver - INFO - Generated testcase No. 5 - REVERT
manticore.core.smtlib.solver - INFO - Generated testcase No. 6 - REVERT
...
```
...
# # چیک کریں کہ کیا ایگزیکیوشن REVERT یا INVALID پر ختم ہوتی ہے
...
ہم دیکھیں گے کہ ایکسپلوریشن کو کیسے محدود کیا جائے۔ ہم یہ فرض کریں گے کہ `f()` کی دستاویزات میں کہا گیا ہے کہ فنکشن کو کبھی بھی <span dir="ltr">`a == 65`</span> کے ساتھ کال نہیں کیا جاتا، لہذا <span dir="ltr">`a == 65`</span> کے ساتھ کوئی بھی بگ حقیقی بگ نہیں ہے۔ ہدف اب بھی درج ذیل اسمارٹ کانٹریکٹ [`example.sol`](https://github.com/crytic/building-secure-contracts/blob/master/program-analysis/manticore/examples/example.sol) ہے:
...
for state in m.terminated_states:
    last_tx = state.platform.transactions[-1]
    if last_tx.result in ['REVERT', 'INVALID']:
        # ہم اس راستے پر غور نہیں کرتے جہاں a == 65 ہو
