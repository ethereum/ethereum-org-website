...
[آپ کلائنٹ کے لیے JavaScript کوڈ یہاں دیکھ سکتے ہیں](https://github.com/qbzzt/20220915-all-you-can-cache/blob/main/javascript/index.js)۔ نمونہ ریپوزٹری آپٹیمزم گورلی کے لیے لکھی گئی تھی، اس لیے اسے چلانے سے پہلے، اپنے ہدف نیٹ ورک کے لیے `javascript/.env.example` اور `javascript/index.js` میں RPC اینڈ پوائنٹ اور ایکسپلورر URLs کو اپ ڈیٹ کریں۔ اسے استعمال کرنے کے لیے:

1. git ریپوزٹری کو کلون کریں:
...
4. اپنی کنفیگریشن کے لیے `.env` میں ترمیم کریں:
   ...
   | پیرامیٹر | قدر | 
   | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | MNEMONIC | ایک ایسے اکاؤنٹ کا میمونک (mnemonic) جس میں ٹرانزیکشن کی ادائیگی کے لیے کافی ETH موجود ہو۔ [آپٹیمزم کی فوسٹ دستاویزات](https://docs.optimism.io/app-developers/tools/faucets) موجودہ آزمائشی نیٹ ورک فوسٹس کی فہرست فراہم کرتی ہیں۔ |
   | OPTIMISM_GOERLI_URL | اس نیٹ ورک کے لیے RPC URL جہاں آپ WORM کو دوبارہ تعینات کرتے ہیں۔ OP Sepolia کے لیے، ایک OP Sepolia RPC اینڈ پوائنٹ استعمال کریں جیسے کہ `https://sepolia.optimism.io`، یا اپنے فراہم کنندہ سے کوئی اور اینڈ پوائنٹ۔ |
...
5. `index.js` چلائیں۔
   ...
   یہ نمونہ ایپلی کیشن پہلے WORM میں ایک اندراج لکھتی ہے، جس میں کال ڈیٹا اور بلاک ایکسپلورر پر ٹرانزیکشن کا لنک دکھایا جاتا ہے۔ پھر یہ اس اندراج کو واپس پڑھتی ہے، اور اس کی استعمال کردہ کلید اور اندراج میں موجود اقدار (قدر، بلاک نمبر، اور مصنف) کو دکھاتی ہے۔

کلائنٹ کا زیادہ تر حصہ عام غیر مرکزی ایپلی کیشن (dapp) JavaScript ہے۔ اس لیے ہم ایک بار پھر صرف دلچسپ حصوں کا جائزہ لیں گے۔
...
کسی دیے گئے سلاٹ میں صرف ایک بار لکھا جا سکتا ہے، اس لیے ہم ٹائم اسٹیمپ کا استعمال کرتے ہیں تاکہ یہ یقینی بنایا جا سکے کہ ہم سلاٹس کو دوبارہ استعمال نہیں کر رہے ہیں۔
...
Ethers کو توقع ہوتی ہے کہ کال ڈیٹا ایک ہیکس (hex) اسٹرنگ ہوگا، یعنی `0x` کے بعد ہیکسا ڈیسیمل ہندسوں کی جفت تعداد۔ چونکہ `key` اور `val` دونوں `0x` سے شروع ہوتے ہیں، اس لیے ہمیں ان ہیڈرز کو ہٹانے کی ضرورت ہے۔
