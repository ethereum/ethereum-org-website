...
머지 이전까지는 실행 클라이언트(고 이더리움 (geth), 에리곤, 베수 또는 네더마인드 등)만으로도 네트워크에서 가십되는 블록을 수신하고, 적절히 검증하며, 전파하기에 충분했습니다. _머지 이후_에는 실행 페이로드에 포함된 트랜잭션의 유효성이 해당 트랜잭션이 포함된 "합의 블록"의 유효성에도 의존하게 되었습니다.
