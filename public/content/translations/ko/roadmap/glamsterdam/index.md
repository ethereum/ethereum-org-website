...
<AlertContent>
<AlertTitle>
글램스테르담은 2026년 4분기로 예정된 다가오는 이더리움 업그레이드입니다.
</AlertTitle>
<AlertDescription>
...
<AlertContent>
<AlertDescription>
참고: 이 문서는 글램스테르담에 포함될 예정인 일부 EIP를 강조합니다. 데브넷에서 테스트 중인 추가 예정 제안으로는 EIP-7610, EIP-7688, EIP-7778, EIP-7843, EIP-7976, EIP-7981, EIP-8024, EIP-8246 및 EIP-8282가 있습니다. 최신 상태 업데이트는 [Forkcast의 글램스테르담 업그레이드](https://forkcast.org/upgrade/glamsterdam)에서 확인하세요.

글램스테르담을 위해 고려 중이지만 아직 이 페이지에 추가되지 않은 EIP를 추가하려면 [여기에서 ethereum.org에 기여하는 방법을 알아보세요](/contributing/).
</AlertDescription>
</AlertContent>
</Alert>
...
- 처리 속도 향상(병렬화): 네트워크가 데이터 종속성을 기록하는 방식을 재구성하여, 하나씩 처리하는 느린 순차적 방식 대신 많은 트랜잭션을 동시에 안전하게 처리할 수 있도록 합니다.
- 용량 확장: 블록을 생성하고 검증하는 무거운 작업을 분할하여, 네트워크가 속도 저하 없이 더 많은 양의 데이터를 전파할 수 있는 더 많은 시간을 제공합니다.
- 데이터베이스 비대화 방지(지속 가능성): 새로운 데이터를 저장하는 데 드는 장기적인 하드웨어 비용을 정확하게 반영하도록 네트워크 수수료를 조정하여, 하드웨어 성능 저하를 방지하면서 향후 가스 한도 증가를 가능하게 합니다.
...
검증자 의무 및 종료 프로세스의 개선은 대규모 슬래싱 이벤트 동안 네트워크 안정성을 보장하고, 스테이커가 자신의 스테이크를 이동시킬 수 있는 속도를 높입니다. 이러한 개선 사항은 네트워크를 더 안정적으로 만들고 크고 작은 모든 참여자가 공정하게 대우받도록 보장합니다.
...
- 고정된 비율로 제한되는 대신 종료 용량이 스테이킹된 총 ETH 양에 비례하여 확장되도록 하여 스테이킹 인출 시간을 크게 줄입니다.
- 검증자 통합에 전용 대기열 용량을 부여하여 더 크고 효율적인 검증자로의 전환 속도를 높입니다.
- 신중하게 분석된 안전 매개변수를 통해 네트워크 보안을 유지합니다.

이더리움의 변동 한도는 네트워크의 보안이 결코 불안정해지지 않도록 검증자가 스테이킹된 ETH를 진입, 종료 또는 병합(통합)할 수 있는 속도에 대한 안전 한도입니다. 오늘날 종료와 활성화는 단일 제한 한도를 공유하므로, 수요가 많은 기간 동안 스테이커는 ETH를 인출하기 위해 오랜 기다림에 직면할 수 있습니다. 검증자가 최대 2,048 ETH를 가진 더 큰 검증자로 병합되는 통합([펙트라 업그레이드](/roadmap/pectra)를 통해 활성화됨) 또한 이 제한된 용량을 두고 경쟁하므로, 현재 속도로는 전체 검증자 세트를 통합하는 데 수년이 걸릴 수 있습니다.

<strong>종료 및 통합 변동 증가(또는 EIP-8061)</strong>는 이러한 한도를 별도의 레인으로 재구성합니다.

- 검증자 활성화는 기존의 제한된 한도를 변경 없이 유지합니다.
- 종료는 더 이상 제한되지 않으며 대신 스테이킹된 총 ETH 양에 비례하여 확장됩니다.
- 통합은 활성화-종료 결합 한도의 약 절반 크기에 해당하는 자체 전용 용량을 얻습니다.

현재 스테이킹 수준에서 이는 종료 용량을 약 4배, 통합 용량을 약 2배 증가시킵니다. 즉, 스테이커는 수요가 많은 기간 동안 ETH를 훨씬 더 빠르게 인출할 수 있으며, 네트워크는 더 작고 효율적인 검증자 세트로 더 빠르게 전환됩니다.
...
- 최대 컨트랙트 크기 증가(또는 EIP-7954)는 개발자가 더 큰 애플리케이션을 배포할 수 있도록 허용하여, 최대 컨트랙트 크기 한도를 약 24KiB에서 64KiB로 높입니다.
...
- 상태 생성 가스 비용 증가(또는 EIP-8037) 및 상태 액세스 가스 비용 업데이트(또는 EIP-8038)는 새로운 지속 가능성 모델을 도입하여 특정 컨트랙트 배포 비용을 변경합니다. 새로운 계정이나 영구 스토리지를 생성할 때 생성된 데이터의 크기에 따라 새로운 표준화된 고정 수수료가 부과되기 때문입니다.
