...
## 레이어 1 (l1) 확장 및 병렬 처리
의미 있는 레이어 1 (l1) 확장을 위해서는 프로토콜 외부의 신뢰 가정과 순차적 실행 제약에서 벗어나야 합니다. 글램스테르담은 특정 블록 생성 임무의 분리를 내재화하고 네트워크가 병렬 처리를 준비할 수 있도록 하는 새로운 데이터 구조를 도입하여 이 문제를 해결합니다.
...
## 네트워크 지속 가능성
이더리움 네트워크가 더 빠르게 성장함에 따라, 사용 비용이 이더리움을 실행하는 하드웨어의 마모(부하)와 일치하도록 보장하는 것이 중요합니다. 네트워크가 안전하게 확장하고 더 많은 트랜잭션을 처리하려면 전반적인 용량 한도를 늘려야 합니다.
...
## 네트워크 복원력
검증자 임무 및 종료 프로세스의 개선은 대규모 슬래싱 이벤트 중 네트워크 안정성을 보장하고 유동성을 민주화합니다. 이러한 개선 사항은 네트워크를 더 안정적으로 만들고 크고 작은 모든 참여자가 공정하게 대우받도록 보장합니다.
...
## 사용자 및 개발자 경험 향상
이더리움의 글램스테르담 업그레이드는 사용자 경험을 개선하고, 데이터 검색 가능성을 높이며, 동기화 실패를 방지하기 위해 증가하는 메시지 크기를 처리하는 것을 목표로 합니다. 이를 통해 온체인에서 일어나는 일을 더 쉽게 추적할 수 있으며, 네트워크가 확장됨에 따라 발생하는 기술적 문제를 방지할 수 있습니다.
...
- [The Daily Gwei Refuel 팟캐스트 - 포스트 양자 이더리움, 글램스테르담이 다가옵니다](https://www.youtube.com/watch?v=qx9sd50uQjQ)
