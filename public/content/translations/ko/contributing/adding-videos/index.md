...
- 이더리움 기술, 생태계 및 커뮤니티에 대해 사용자를 **교육**합니다.
- 기술적 콘텐츠의 **정확성을 유지**합니다.
- 이더리움 커뮤니티와의 **관련성을 유지**합니다.
...
비디오를 직접 추가하고 싶다면 다음 단계를 따르세요:

#### 1단계: 비디오 파일 생성 {#step-1}

다음 위치에 새 디렉터리와 `index.md` 파일을 생성합니다:
...
| `lang` | 예 | 현재는 항상 `en` 사용 |
| `youtubeId` | 예 | 유튜브 비디오 ID (`v=` 뒤의 URL 부분) |
...
| `breadcrumb` | 아니요 | 브레드크럼(breadcrumb) 내비게이션을 위한 사용자 지정 짧은 레이블 |
| `customThumbnailUrl` | 아니요 | 사용자 지정 썸네일 URL (기본값은 유튜브 썸네일) |
...
주요 섹션을 표시하려면 타임스탬프와 함께 `###` 제목을 사용하세요. 이렇게 하면 대본을 쉽게 훑어볼 수 있고 SEO가 향상됩니다.

대본이 없는 경우 본문을 비워두면 팀에서 생성해 드립니다.

#### 4단계: 주제 태그 선택 {#step-4}

아래 목록에서 주제 태그를 선택하세요. 각 태그는 비디오 갤러리의 필터 카테고리에 직접 매핑되므로 표시된 태그 이름을 정확히 사용해야 합니다.

비디오에 여러 태그를 지정하여 여러 갤러리 필터에 표시되게 할 수 있습니다:
