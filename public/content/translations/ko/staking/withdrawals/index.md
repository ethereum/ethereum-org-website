...
<strong>스테이킹 인출</strong>은 이더리움의 합의 레이어(비콘 체인)에 있는 검증자 계정에서 거래가 가능한 실행 레이어로 ETH를 전송하는 것을 말합니다.

32 ETH를 초과하는 <strong>초과 잔액에 대한 보상금</strong>은 사용자가 출금 주소를 제공하면 각 검증인에 연결된 출금 주소로 자동적이고 정기적으로 전송됩니다. 사용자는 또한 <strong>스테이킹을 완전히 종료</strong>하여 전체 검증자 잔액을 인출할 수도 있습니다.
...
## 스테이킹 완전 종료 {#exiting-staking-entirely}

검증자 계정 잔액에서 자금을 이체하려면 먼저 출금 주소를 제공해야 합니다.
...
인출 기능은 <strong>2023년 4월 12일</strong>에 진행된 상하이/카펠라 업그레이드의 일부로 활성화되었습니다.
...
<AlertContent>
<AlertDescription>
아날로그 시계를 생각해 보세요. 시계 바늘은 시간을 가리키고, 한 방향으로 진행하며, 어떤 시간도 건너뛰지 않고, 마지막 숫자에 도달하면 결국 다시 처음으로 돌아옵니다.<br/><br/>
이제 시계에 1부터 12까지가 아니라 0부터 N까지 있다고 상상해 보세요<em>(2023년 1월 기준 50만 개가 넘는, 합의 레이어에 등록된 총 검증자 계정 수).</em><br/><br/>
시계 바늘은 인출 자격이 있는지 확인해야 하는 다음 검증자를 가리킵니다. 0에서 시작하여 어떤 계정도 건너뛰지 않고 끝까지 진행합니다. 마지막 검증자에 도달하면 주기는 다시 처음부터 시작됩니다.
</AlertDescription>
</AlertContent>
</Alert>

#### 계정 인출 확인 {#checking-an-account-for-withdrawals}
...
2. **검증자가 스테이킹을 종료했으며 인출이 가능한가요?** 검증자가 완전히 종료되고 계정이 '인출 가능'으로 간주되는 에포크에 도달하면 전체 인출이 처리됩니다. 이 작업은 전체 잔액을 출금 주소로 이체합니다.
3. **유효 잔액이 32로 최대치인가요?** 계정에 인출 자격 증명이 있고, 완전히 종료되지 않았으며, 32를 초과하는 보상이 대기 중인 경우, 32를 초과하는 보상만 사용자의 인출 주소로 이체되는 부분 인출이 처리됩니다.
...
스테이킹 출금에 대한 이러한 접근 방식은 스테이커가 특정 양의 ETH를 회수하도록 요청하는 거래를 수동으로 제출하게 요구하는 작업을 건너뜁니다. 이는 <strong>가스(거래 수수료)가 필요하지 않음</strong>을 의미하며, 인출은 기존 실행 레이어 블록 공간을 놓고 경쟁하지도 않습니다.
...
eventName="read more">
실행 레이어 출금 주소를 설정하면 해당 검증자의 출금 자격 증명이 영구적으로 변경됩니다. 즉, 이전의 자격 증명은 더 이상 작동하지 않으며, 새로운 자격 증명이 실행 계층 계정으로 직접 전달됩니다.
...
<ExpandableCard
title="보상 지급(부분 인출)은 자동으로 이루어지나요?"
eventCategory="FAQ"
...
eventCategory="FAQ"
eventAction="I operate a validator. 인출 활성화에 대한 자세한 정보는 어디서 찾을 수 있나요?"
eventName="read more">
...
<ExpandableCard
title="더 많은 ETH를 예치하여 종료 후 검증자를 다시 활성화할 수 있나요?"
eventCategory="FAQ"
...
- [EIP-4895: 비콘 체인 푸시 인출 연산](https://eips.ethereum.org/EIPS/eip-4895)
- [PEEPanEIP #94: 스테이킹된 ETH 인출(테스팅) with Potuz & Hsiao-Wei Wang](https://www.youtube.com/watch?v=G8UstwmGtyE)
- [PEEPanEIP#68: EIP-4895: Alex Stokes와 함께하는 비콘 체인 푸시 인출 연산](https://www.youtube.com/watch?v=CcL9RJBljUs)
- [검증자 유효 잔액 이해하기](https://www.attestant.io/posts/understanding-validator-effective-balance/)
