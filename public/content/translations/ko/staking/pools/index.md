...
<Grid>
  <Card title="Low barrier to entry" icon={<Fish />} description="고래가 아니신가요? 문제없습니다. 32 ETH가 필요한 솔로 스테이킹과 달리, 대부분의 스테이킹 풀에서는 다른 스테이커들과 힘을 합쳐 사실상 원하는 금액의 ETH를 스테이킹할 수 있습니다." />
  <Card title="Stake today" icon={<Clock />} description="풀을 통한 스테이킹은 토큰 스왑만큼 쉽습니다. 하드웨어 설정 및 노드 유지 관리에 대해 걱정할 필요가 없습니다. 풀에 ETH를 예치하면 노드 운영자가 검증자를 실행할 수 있습니다. 그런 다음 노드 운영 수수료를 제외한 보상이 기여자에게 분배됩니다." />
  <Card title="Liquid staking tokens" icon={<Droplets />} description="많은 스테이킹 풀은 스테이킹된 ETH와 그로 인해 발생하는 보상에 대한 청구를 나타내는 토큰을 제공합니다. 이를 통해 스테이킹된 ETH를 탈중앙화 금융(DeFi) 애플리케이션에서 담보로 사용하는 등 다양하게 활용할 수 있습니다." />
</Grid>
...
- **스마트 컨트랙 위험** - 사용자의 ETH는 버그가 있거나 악용될 수 있는 컨트랙트에 보관됩니다. 오픈 소스이고 감사를 받았으며 실전 테스트를 거친 코드를 사용하는 프로토콜을 선호하세요.
