...
**부분 인출**은 검증자가 누적된 보상, 즉 최대 유효 잔액인 32 ETH를 초과하는 추가 잔액을 인출할 때 발생합니다. 부분 인출은 "보상 지급" 또는 "초과 잔액 지급"이라고도 합니다.

**전체 인출**은 검증자가 종료 프로세스를 완료하고 전체 잔액이 인출될 때 발생합니다. 이는 검증자가 자발적으로 시스템을 종료하거나 "슬래싱"이라는 프로세스를 통해 강제로 제거될 때만 발생합니다.
