...
> _참고: 이 결과는 ETH가 아닌 wei 단위입니다. Wei는 이더의 가장 작은 단위로 사용됩니다._

휴! 가짜 돈이 모두 들어왔습니다.

### 6단계: 프로젝트 초기화하기 {#step-6-initialize-our-project}

먼저 프로젝트를 위한 폴더를 만들어야 합니다. 명령줄(command line)로 이동하여 다음을 입력하세요.
...
이제 프로젝트 폴더 안에 있으므로, `npm init`를 사용하여 프로젝트를 초기화하겠습니다.

> 아직 npm이 설치되어 있지 않다면, [이 지침에 따라 Node.js와 npm을 설치](https://docs.alchemyapi.io/alchemy/guides/alchemy-for-macs#1-install-nodejs-and-npm)하세요.

이 튜토리얼의 목적상 초기화 질문에 어떻게 대답하든 상관없습니다. 참고를 위해 저희가 진행한 방법은 다음과 같습니다:
...
Ethers.js는 [표준 JSON-RPC 메서드](https://docs.alchemyapi.io/alchemy/documentation/alchemy-api-reference/json-rpc)를 더 사용자 친화적인 메서드로 래핑하여 이더리움과 상호작용하고 요청을 보내는 것을 더 쉽게 만들어주는 라이브러리입니다.
...
여기서 `.deploy()` 함수를 호출했을 때 Hardhat/Ethers가 내부적으로 생성한 몇 가지 JSON-RPC 메서드를 볼 수 있습니다. 여기서 중요한 두 가지 메서드는 괴를리 체인에 컨트랙트를 기록하라는 요청인 [`eth_sendRawTransaction`](https://docs.alchemyapi.io/alchemy/documentation/alchemy-api-reference/json-rpc#eth_sendrawtransaction)와 해시가 주어졌을 때 트랜잭션에 대한 정보를 읽어오라는 요청인 [`eth_getTransactionByHash`](https://docs.alchemyapi.io/alchemy/documentation/alchemy-api-reference/json-rpc#eth_gettransactionbyhash)입니다. 트랜잭션 전송에 대해 더 알아보려면 [Web3를 사용한 트랜잭션 전송 튜토리얼](/developers/tutorials/sending-transactions-using-web3-and-alchemy/)을 확인하세요.
...
스크립트를 실행하는 동안 새 메시지가 로드되기 전에 `Updating the message...` 단계가 로드되는 데 시간이 꽤 걸리는 것을 알 수 있습니다. 이는 채굴 과정 때문입니다. 채굴되는 동안 트랜잭션을 추적하는 방법이 궁금하다면 [Alchemy 멤풀](https://dashboard.alchemyapi.io/mempool)을 방문하여 트랜잭션 상태를 확인해 보세요. 트랜잭션이 드롭된 경우, [괴를리 Etherscan](https://goerli.etherscan.io)을 확인하고 트랜잭션 해시를 검색해 보는 것도 도움이 됩니다.
...
- [Alchemy Web3](https://docs.alchemy.com/alchemy/documentation/alchemy-web3) API를 사용하여 스마트 컨트랙트에서 데이터 읽기
...
이 튜토리얼의 파트 2에서 [Alchemy Web3 키를 사용하여 스마트 컨트랙트에서 데이터를 읽었던](https://docs.alchemy.com/alchemy/tutorials/hello-world-smart-contract/interacting-with-a-smart-contract#step-1-install-web3-library) 것을 기억하시나요? 체인에서 데이터를 읽으려면 dapp에도 Alchemy Web3 키가 필요합니다.

아직 없다면 먼저 `starter-files`의 루트 디렉토리로 이동하여 터미널에서 다음을 실행하여 [Alchemy Web3](https://github.com/alchemyplatform/alchemy-web3)를 설치하세요:

```text
npm install @alch/alchemy-web3
```

[Alchemy Web3](https://github.com/alchemyplatform/alchemy-web3)는 [Web3.js](https://docs.web3js.org/)의 래퍼(wrapper)로, 향상된 API 메서드와 기타 중요한 이점을 제공하여 Web3 개발자의 작업을 더 쉽게 만들어 줍니다. 최소한의 구성만 필요하도록 설계되어 앱에서 즉시 사용할 수 있습니다!

그런 다음 프로젝트 디렉토리에 [dotenv](https://www.npmjs.com/package/dotenv) 패키지를 설치하여 API 키를 가져온 후 안전하게 저장할 수 있는 공간을 마련합니다.

```text
npm install dotenv --save
```

우리 dapp에서는 HTTP API 키 대신 **Websockets API 키를 사용할 것입니다**. 이를 통해 스마트 컨트랙트에 저장된 메시지가 변경될 때 이를 감지하는 리스너를 설정할 수 있기 때문입니다.

API 키가 준비되면 루트 디렉토리에 `.env` 파일을 만들고 Alchemy Websockets URL을 추가합니다. 그 후 `.env` 파일은 다음과 같아야 합니다:

```javascript
REACT_APP_ALCHEMY_KEY = wss://eth-goerli.ws.alchemyapi.io/v2/<키>
```

이제 dapp에 Alchemy Web3 엔드포인트를 설정할 준비가 되었습니다! `util` 폴더 안에 중첩된 `interact.js`로 돌아가서 파일 상단에 다음 코드를 추가해 보겠습니다:
...
위에서 먼저 `.env` 파일에서 Alchemy 키를 가져온 다음, `alchemyKey`를 `createAlchemyWeb3`에 전달하여 Alchemy Web3 엔드포인트를 설정했습니다.
...
[이 튜토리얼 시리즈의 파트 1](https://docs.alchemy.com/alchemy/tutorials/hello-world-smart-contract#step-10-write-our-contract)에서 작성한 `HelloWorld.sol` 파일을 떠올려 보면, 스마트 컨트랙트의 `update` 함수가 호출된 후 발생하는 `UpdatedMessages`라는 스마트 컨트랙트 이벤트가 있다는 것을 기억하실 것입니다(9번째 및 27번째 줄 참조):
...
잔액이 있는지 다시 확인하기 위해 [Alchemy의 컴포저 도구](https://composer.alchemyapi.io/?composer_state=%7B%22network%22%3A0%2C%22methodName%22%3A%22eth_getBalance%22%2C%22paramValues%22%3A%5B%22%22%2C%22latest%22%5D%7D)를 사용하여 [eth_getBalance](https://docs.alchemyapi.io/alchemy/documentation/alchemy-api-reference/json-rpc#eth_getbalance) 요청을 해보겠습니다. 그러면 지갑에 있는 ETH의 양이 반환됩니다. 메타마스크 계정 주소를 입력하고 "Send Request(요청 보내기)"를 클릭하면 다음과 같은 응답이 표시될 것입니다:

```text
{"jsonrpc": "2.0", "id": 0, "result": "0xde0b6b3a7640000"}
```

**참고:** 이 결과는 ETH가 아니라 wei 단위입니다. Wei는 이더의 가장 작은 단위로 사용됩니다. wei에서 ETH로의 변환은 1 ETH = 10¹⁸ wei입니다. 따라서 0xde0b6b3a7640000을 10진수로 변환하면 1\*10¹⁸이 되며 이는 1 ETH와 같습니다.
