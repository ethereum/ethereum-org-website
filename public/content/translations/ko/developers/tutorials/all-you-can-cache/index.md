...
[여기서 클라이언트용 JavaScript 코드를 확인할 수 있습니다](https://github.com/qbzzt/20220915-all-you-can-cache/blob/main/javascript/index.js). 샘플 리포지토리는 옵티미즘 괴를리용으로 작성되었으므로, 실행하기 전에 대상 네트워크에 맞게 `javascript/.env.example` 및 `javascript/index.js`의 RPC 엔드포인트와 탐색기 URL을 업데이트하세요. 사용 방법은 다음과 같습니다:
...
5. `index.js`를 실행합니다.
   ...
   이 샘플 애플리케이션은 먼저 WORM에 항목을 쓰고, 콜 데이터와 블록 탐색기의 트랜잭션 링크를 표시합니다. 그런 다음 해당 항목을 다시 읽고, 사용하는 키와 항목의 값(값, 블록 번호, 작성자)을 표시합니다.

클라이언트의 대부분은 일반적인 탈중앙화 애플리케이션 (dapp) JavaScript입니다. 따라서 여기서도 흥미로운 부분만 살펴보겠습니다.
...
주어진 슬롯은 한 번만 쓸 수 있으므로, 슬롯을 재사용하지 않도록 타임스탬프를 사용합니다.
...
Ethers는 콜 데이터가 `0x` 뒤에 짝수 개의 16진수가 오는 16진수 문자열일 것으로 예상합니다. `key`와 `val` 모두 `0x`로 시작하므로 해당 헤더를 제거해야 합니다.
...
Solidity 테스트 코드와 마찬가지로 캐시된 함수를 정상적으로 호출할 수 없습니다. 대신 저수준 메커니즘을 사용해야 합니다.
