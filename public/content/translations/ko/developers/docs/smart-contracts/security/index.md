...
스마트 컨트랙트 함수의 무단 사용을 방지하려면 안전한 접근 제어를 구현해야 합니다. 접근 제어 메커니즘은 스마트 컨트랙트의 특정 함수를 사용할 수 있는 권한을 컨트랙트 관리를 담당하는 계정과 같이 승인된 주체로 제한합니다. **Ownable 패턴**과 **역할 기반 제어(role-based control)**는 스마트 컨트랙트에서 접근 제어를 구현하는 데 유용한 두 가지 패턴입니다.
...
##### 다중 서명 지갑 사용 {#use-require-assert-revert}
...
### 2. require(), assert(), revert() 문을 사용하여 컨트랙트 작업 보호 {#test-smart-contracts-and-verify-code-correctness}
...
### 3. 스마트 컨트랙트 테스트 및 코드 정확성 검증 {#get-independent-code-reviews}
...
#### 버그 바운티 {#follow-smart-contract-development-best-practices}
...
### 5. 스마트 컨트랙트 개발 중 모범 사례 준수 {#implement-disaster-recovery-plans}
...
### 6. 강력한 재해 복구 계획 구현 {#contract-upgrades}
...
#### 비상 정지(Emergency stops) {#event-monitoring}
...
#### 이벤트 모니터링 {#design-secure-governance-systems}
...
### 7. 안전한 거버넌스 시스템 설계 {#reduce-code-complexity}
...
### 8. 코드의 복잡성을 최소화 {#mitigate-common-smart-contract-vulnerabilities}
...
### 9. 일반적인 스마트 컨트랙트 취약점 방어 {#reentrancy}

#### 재진입(Reentrancy) {#integer-underflows-and-overflows}
...
`Victim` 컨트랙트의 `withdraw()` 함수는 "확인-상호작용-효과(checks-interactions-effects)" 패턴을 따릅니다. 실행에 필요한 조건이 충족되는지 _확인(checks)_하고(즉, 사용자의 ETH 잔액이 양수인지), 트랜잭션의 _효과(effects)_를 적용하기 전에(즉, 사용자의 잔액 감소) 호출자의 주소로 ETH를 전송하여 _상호작용(interaction)_을 수행합니다.
...
##### 재진입 공격을 방지하는 방법 {#oracle-manipulation}
...
이 컨트랙트는 사용자의 잔액을 _확인(check)_하고, (사용자의 잔액을 0으로 재설정하여) `withdraw()` 함수의 _효과(effects)_를 적용한 다음, _상호작용(interaction)_ (사용자의 주소로 ETH 전송)을 수행합니다. 이렇게 하면 외부 호출 전에 컨트랙트가 스토리지를 업데이트하도록 보장하여 첫 번째 공격을 가능하게 했던 재진입 조건을 제거합니다. `Attacker` 컨트랙트는 여전히 `NoLongerAVictim`를 다시 호출할 수 있지만, `balances[msg.sender]`가 0으로 설정되었으므로 추가 인출 시 오류가 발생합니다.
...
#### 정수 언더플로 및 오버플로 {#smart-contract-security-resources-for-developers}
...
##### 정수 언더플로 및 오버플로를 방지하는 방법 {#code-analysis-tools}
...
#### 오라클 조작 {#smart-contract-monitoring-tools}
...
##### 오라클 조작을 방지하는 방법 {#smart-contract-administration-tools}
...
## 개발자를 위한 스마트 컨트랙트 보안 리소스 {#smart-contract-auditing-services}

### 스마트 컨트랙트 분석 및 코드 정확성 검증 도구 {#bug-bounty-platforms}
...
### 스마트 컨트랙트 모니터링 도구 {#common-smart-contract-vulnerabilities-and-exploits}

- **[Tenderly 실시간 알림](https://tenderly.co/monitoring)** - _스마트 컨트랙트나 지갑에서 비정상적이거나 예상치 못한 이벤트가 발생할 때 실시간 알림을 받을 수 있는 도구입니다._

### 스마트 컨트랙트의 안전한 관리를 위한 도구 {#challenges-for-learning-smart-contract-security}
...
### 스마트 컨트랙트 감사 서비스 {#smart-contract-security-best-practices}
...
### 버그 바운티 플랫폼 {#tutorials-on-smart-contract-security}
