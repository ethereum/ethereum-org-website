...
이더리움 스마트 컨트랙트를 테스트하는 방법은 크게 **자동화 테스트**와 **수동 테스트** 두 가지 범주로 나뉩니다. 자동화 테스트와 수동 테스트는 고유한 이점과 장단점을 제공하지만, 두 가지를 결합하여 컨트랙트를 분석하기 위한 강력한 계획을 세울 수 있습니다.
...
##### 1. 컨트랙트의 비즈니스 로직 및 워크플로 이해 {#integration-testing-for-smart-contracts}
...
##### 2. 컨트랙트 실행과 관련된 모든 가정 평가 {#property-based-testing-for-smart-contracts}
...
##### 4. 잘 개발된 테스트 프레임워크 사용 {#dynamic-analysis}
...
### 통합 테스트 {#running-property-based-tests}
...
### 속성 기반 테스트 {#manual-testing-for-smart-contracts}
...
**정적 분석**과 **동적 분석**은 속성 기반 테스트를 실행하는 두 가지 일반적인 기법이며, 둘 다 프로그램(이 경우 스마트 컨트랙트)의 코드가 미리 정의된 특정 속성을 충족하는지 검증할 수 있습니다. 일부 속성 기반 테스트 도구는 예상되는 컨트랙트 속성에 대해 미리 정의된 규칙과 함께 제공되어 해당 규칙에 따라 코드를 확인하는 반면, 다른 도구는 스마트 컨트랙트에 대한 사용자 지정 속성을 생성할 수 있도록 합니다.

#### 정적 분석 {#testing-on-local-blockchain}
...
#### 동적 분석 {#testing-contracts-on-testnets}
...
### 스마트 컨트랙트의 속성 기반 테스트 실행 지침 {#testing-vs-formal-verification}
...
## 스마트 컨트랙트를 위한 수동 테스트 {#testing-vs-audits-bug-bounties}
...
### 로컬 블록체인에서 컨트랙 테스트 {#testing-tools-and-libraries}
...
### 테스트넷에서 컨트랙트 테스트 {#unit-testing-tools}
...
## 테스트 대 정형 검증 {#property-based-testing-tools}
...
## 테스트 대 감사 및 버그 바운티 {#static-analysis-tools}
...
## 테스트 도구 및 라이브러리 {#dynamic-analysis-tools}

### 단위 테스트 도구 {#related-tutorials}
