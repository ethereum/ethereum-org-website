...
모든 검증자는 스테이킹된 ETH와 보상을 어떻게 그리고 어디로 출금할 수 있는지를 결정하는 <strong>출금 증명서</strong>를 가지고 있습니다. 증명서 유형은 첫 번째 바이트 `0x00`, `0x01` 또는 `0x02`로 표시됩니다. 이러한 유형을 이해하는 것은 스테이킹을 관리하는 검증자에게 중요합니다.
...
`0x02` 유형은 펙트라 업그레이드와 함께 도입되었으며, 오늘날 검증자들에게 <strong>권장되는 선택</strong>입니다. `0x02` 증명서를 가진 검증자는 때때로 "복리 검증자"라고 불립니다.
