...
이를 피하기 위해 정수론의 결과가 활용됩니다. [_안전 소수(Safe Prime)_](https://en.wikipedia.org/wiki/Safe_prime)는 `(P-1)/2`도 소수가 되는 소수 `P`로 정의됩니다. [곱셈군(multiplicative group)](https://en.wikipedia.org/wiki/Multiplicative_group_of_integers_modulo_n) `ℤ/nℤ`의 멤버 `x`의 _위수(order)_는 <pre>xᵐ mod P ≡ 1</pre>을 만족하는 최소 `m`로 정의됩니다.
