...
enode는 URL 주소 형식을 사용하여 이더리움 노드를 식별하는 방법입니다. 16진수 노드 ID는 URL의 사용자 이름 부분에 인코딩되며, @ 기호를 사용하여 호스트와 구분됩니다. 사양에서는 호스트 이름을 IP 주소로만 정의하지만, 대부분의 클라이언트(예: Geth 및 Besu)는 여기서 DNS 이름도 허용하며 시작 시 이를 IP 주소로 변환합니다. 이는 표준의 일부라기보다는 클라이언트별 특정 동작입니다. 호스트 이름 섹션의 포트는 TCP 수신 대기 포트입니다. TCP와 UDP(디스커버리) 포트가 다른 경우, UDP 포트는 쿼리 매개변수 "discport"로 지정됩니다.

다음 예에서 노드 URL은 IP 주소가 `10.3.58.6`이고 TCP 포트가 `30303`이며 UDP 디스커버리 포트가 `30301`인 노드를 설명합니다.
