...
P2P 채널은 사전 정의된 일부 참여자가 눈에 띄는 오버헤드 없이 높은 빈도로 거래하고자 하는 상황에 특히 유용합니다. 블록체인 채널은 <strong>지불 채널</strong>과 <strong>상태 채널</strong>의 두 가지 범주로 나뉩니다.
...
- <strong>논스(nonce)</strong>는 트랜잭션의 고유 ID 역할을 하며 재전송 공격을 방지합니다. 또한 상태 업데이트가 발생한 순서를 식별합니다(분쟁 해결에 중요).
...
그러나 한쪽 당사자는 상대방의 승인을 기다리지 않고 스마트 계약의 실행을 종료하고 채널을 확정하기 위해 온체인 요청을 제출할 수 있습니다. 앞서 설명한 합의 파기 상황 중 하나라도 발생하면 어느 쪽이든 온체인 계약을 트리거하여 채널을 닫고 자금을 분배할 수 있습니다. 이것은 <strong>무신뢰성</strong>을 제공하여 정직한 당사자가 다른 당사자의 행동에 관계없이 언제든지 예치금을 인출할 수 있도록 보장합니다.
