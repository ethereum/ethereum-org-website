...
أسهل طريقة لمعرفة الرمز الشرعي هي النظر إلى المنظمة الأصلية، [أربيتروم](https://arbitrum.foundation/). العناوين الشرعية محددة [في وثائقهم](https://docs.arbitrum.foundation/deployment-addresses#token).
...
من المفترض أن تحتوي عقود ERC-20 على [دالة `approve`](/developers/tutorials/erc20-annotated-code/#approve) للبدلات، وبالفعل يحتوي رمزنا الاحتيالي على مثل هذه الدالة، وهي صحيحة حتى. ومع ذلك، نظرًا لأن لغة سوليديتي مشتقة من لغة C، فهي حساسة لحالة الأحرف. السلسلتان النصيتان "Approve" و"approve" مختلفتان.
...
مشكلات جودة الكود هذه لا _تثبت_ أن هذا الكود عملية احتيال، لكنها تجعله يبدو مشبوهًا. الشركات المنظمة مثل أربيتروم لا تصدر عادةً كودًا بهذا السوء.
...
إليك [برنامج يحدد هذا النوع من الأحداث](https://github.com/qbzzt/20230915-scam-token-detection)، باستخدام [viem](https://viem.sh/) و [تايب سكريبت](https://www.typescriptlang.org/docs/)، وهو متغير من جافا سكريبت مع أمان الأنواع. لتشغيله:
...
إنشاء عميل فيم. نحتاج فقط إلى القراءة من البلوك تشين، لذلك لا يحتاج هذا العميل إلى مفتاح خاص.
...
هذه هي طريقة طلب معلومات الأحداث من فيم. عندما نزوده بتوقيع الحدث الدقيق، بما في ذلك أسماء الحقول، فإنه يحلل الحدث لنا.
...
هذه هي أهم دالة، وهي التي تقرر بالفعل ما إذا كان الحدث مشبوهًا أم لا. نوع الإرجاع، `(Event | null)`، يخبر تايب سكريبت أن هذه الدالة يمكن أن تعيد إما `Event` أو `null`. نعيد `null` إذا لم يكن الحدث مشبوهًا.

```typescript
const owner = ev.args._owner
```

يحتوي فيم على أسماء الحقول، لذلك قام بتحليل الحدث لنا. `_owner` هو مالك الرموز التي سيتم إنفاقها.
...
يمكن أن يساعد الكشف الآلي في حالات معينة، مثل أجزاء التمويل اللامركزي (دي فاي)، حيث يوجد العديد من الرموز ويجب التعامل معها تلقائيًا. ولكن كما هو الحال دائمًا [على المشتري أن يحذر](https://www.investopedia.com/terms/c/caveatemptor.asp)، قم ببحثك الخاص، وشجع المستخدمين على فعل الشيء نفسه.
