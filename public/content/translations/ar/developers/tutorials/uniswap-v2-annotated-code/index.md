---
title: "جولة تفصيلية في عقد يوني سواب-v2"
description: "كيف يعمل عقد يوني سواب-v2؟ لماذا كُتب بهذه الطريقة؟"
...
يمكن لـ [يوني سواب v2](https://app.uniswap.org/whitepaper.pdf) إنشاء سوق تداول بين أي رمزين من رموز ERC-20. في هذه المقالة، سنتناول النص البرمجي للعقود التي تنفذ هذا البروتوكول ونرى سبب كتابتها بهذه الطريقة.

### ماذا يفعل يوني سواب؟ {#what-does-uniswap-do}
...
[يوني سواب v3](https://app.uniswap.org/whitepaper-v3.pdf) هو تطوير أكثر تعقيدًا بكثير من الإصدار v2. من الأسهل تعلم الإصدار v2 أولاً ثم الانتقال إلى الإصدار v3.

### العقود الأساسية مقابل العقود الطرفية {#contract-types}

ينقسم يوني سواب v2 إلى مكونين، أساسي وطرفي. يسمح هذا التقسيم بأن تكون العقود الأساسية، التي تحتفظ بالأصول وبالتالي _يجب_ أن تكون آمنة، أبسط وأسهل في التدقيق. يمكن بعد ذلك توفير جميع الوظائف الإضافية التي يطلبها المتداولون من خلال العقود الطرفية.

## تدفقات البيانات والتحكم {#flows}

هذا هو تدفق البيانات والتحكم الذي يحدث عند تنفيذ الإجراءات الرئيسية الثلاثة لـ يوني سواب:
...
ينفذ [هذا العقد](https://github.com/Uniswap/uniswap-v2-core/blob/master/contracts/UniswapV2Pair.sol) المجمع الفعلي الذي يتداول الرموز. إنها الوظيفة الأساسية لـ يوني سواب.
...
الحل الذي وجده يوني سواب هو استخدام قيم 224 بت، مع 112 بت للجزء الصحيح، و112 بت للجزء الكسري. لذا، يتم تمثيل `1.0` كـ `2^112`، ويتم تمثيل `1.5` كـ `2^112 + 2^111`، وهكذا.
...
أحد أكبر نفقات الغاز في عقود إيثريوم هو التخزين، والذي يستمر من استدعاء لآخر للعقد. طول كل خلية تخزين هو 256 بت. لذا، يتم تخصيص ثلاثة متغيرات، `reserve0`، `reserve1`، و `blockTimestampLast`، بطريقة يمكن لقيمة تخزين واحدة أن تتضمن جميعها (<span dir="ltr">112+112+32=256</span>).
...
لنفترض أننا نريد حساب متوسط سعر **Token0** بين الختمين الزمنيين 5,030 و 5,150. الفرق في قيمة `price0Cumulative` هو <span dir="ltr">143.702 - 29.07 = 114.632</span>. هذا هو المتوسط على مدار دقيقتين (120 ثانية). إذًا متوسط السعر هو <span dir="ltr">114.632 / 120 = 0.955</span>.
...
```solidity
لاحظ أن وظائف سوليديتي [يمكن أن تعيد قيمًا متعددة](https://docs.soliditylang.org/en/v0.8.3/contracts.html#returning-multiple-values).
```
...
| الحدث                                                                                                                                                                |                                                                                                                                                  reserve0 |                                                                                                                                                  reserve1 |                                                       reserve0 \* reserve1 |                                                                                                                        قيمة المجمع (reserve0 + reserve1) | 5,100 | 0.826 |                                                                                                                                                                                                                                                                      <span dir="ltr">29.07+70\*0.890 = 91.37</span> |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------: | --------------------------------------------------------------------------------------------------------------------------------------------------------: | -------------------------------------------------------------------------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | ----: | --------------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| الإعداد الأولي                                                                                                                                                       |                                                                                                                                     8.000 |                                                                                                                                     8.000 |                                                                         64 |                                                                                                                                                       1.018 |     8 |                     8 |                                                                                                                                                                                                                                                                                                                        2.000 |
| إيداع أربعة من كل نوع                                                                                                                                                |                                                                                                                                                     5,150 |                                                                                                                                                     5,150 | <span dir="ltr">99.63+40\*1.1018 = 143.702</span> |                                                                               لنفترض أننا نريد حساب متوسط سعر **Token0** بين الأختام الزمنية 5,030 و 5,150. |     4 |                    12 |                                                                                                                                                                                                                                                                                                                        2.000 |
...
```
    ...
    } عندما لا يدعم سوليديتي رمز تشغيل بعد، يمكننا استدعاؤه باستخدام [التجميع المضمّن](https://docs.soliditylang.org/en/v0.8.3/assembly.html).
```
...
```solidity
إنه مشابه [لعقد ERC-20 الخاص بـ أوبن زبلين](/developers/tutorials/erc20-annotated-code)، لذا سأشرح فقط الجزء المختلف، وهو وظيفة `permit`.
```
...
العقود الطرفية {#periphery-contracts}
...
```solidity
في معظم الحالات، ستستخدم يوني سواب من خلال [هذا العقد](https://github.com/Uniswap/uniswap-v2-periphery/blob/master/contracts/UniswapV2Router02.sol).
```
...
تحتوي هذه المكتبة على بعض دوال الرياضيات التي لا تكون ضرورية عادةً في كود سوليديتي، لذلك فهي ليست جزءًا من اللغة.
...
لا تتعامل سوليديتي مع الكسور بشكل أصلي، لذلك لا يمكننا فقط ضرب المبلغ الناتج في 0.997. بدلاً من ذلك، نضرب البسط في 997 والمقام في 1000، لتحقيق نفس التأثير.
...
[هذه المكتبة](https://github.com/Uniswap/uniswap-lib/blob/master/contracts/libraries/TransferHelper.sol) تضيف عمليات تحقق من النجاح حول تحويلات ERC-20 وإيثريوم لمعاملة الإلغاء وإرجاع القيمة `false` بنفس الطريقة.
