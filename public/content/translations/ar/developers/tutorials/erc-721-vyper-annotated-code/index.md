---
title: "إرشادات تفصيلية لعقد ERC-721 الخاص بـ فايبر"
...
يُستخدم معيار [ERC-721](/developers/docs/standards/tokens/erc-721/) للاحتفاظ بملكية الرموز غير القابلة للاستبدال (إن إف تي).
...
هذا العقد مكتوب بلغة [فايبر](https://vyper.readthedocs.io/en/latest/index.html)، وهي لغة عقود شبيهة بلغة بايثون مصممة لجعل كتابة نصوص برمجية غير آمنة أصعب مما هي عليه في لغة سوليديتي.
...
تبدأ التعليقات في لغة فايبر، كما في بايثون، برمز الهاش (`#`) وتستمر حتى نهاية السطر. تُستخدم التعليقات التي تتضمن `@<keyword>` بواسطة [نات سبيك](https://vyper.readthedocs.io/en/latest/natspec.html) لإنتاج توثيق يمكن للبشر قراءته.
...
واجهة ERC-721 مدمجة في لغة فايبر.
[يمكنك رؤية تعريف النص البرمجي هنا](https://github.com/vyperlang/vyper/blob/master/vyper/builtin_interfaces/ERC721.py).
تعريف الواجهة مكتوب بلغة بايثون، وليس فايبر، لأن الواجهات لا تُستخدم فقط داخل البلوكتشين، ولكن أيضًا عند إرسال معاملة إلى البلوكتشين من عميل خارجي، والذي قد يكون مكتوبًا بلغة بايثون.
...
في بايثون، وفي فايبر، يمكنك أيضًا إنشاء تعليق عن طريق تحديد سلسلة نصية متعددة الأسطر (تبدأ وتنتهي بـ `"""`)، وعدم استخدامها بأي طريقة. يمكن أن تتضمن هذه التعليقات أيضًا [نات سبيك](https://vyper.readthedocs.io/en/latest/natspec.html).
...
على عكس بايثون، فإن فايبر هي [لغة ذات أنواع ثابتة](https://wikipedia.org/wiki/Type_system#Static_type_checking).
...
لإصدار حدث في فايبر، استخدم عبارة `log` ([انظر هنا لمزيد من التفاصيل](https://vyper.readthedocs.io/en/latest/event-logging.html#event-logging)).
...
على عكس سوليديتي، لا تملك فايبر ميزة الوراثة. هذا خيار تصميم متعمد لجعل النص البرمجي أوضح وبالتالي أسهل في التأمين. لذلك لإنشاء عقد فايبر ERC-721 خاص بك، خذ [هذا العقد](https://github.com/vyperlang/vyper/blob/master/examples/tokens/ERC721.vy) وقم بتعديله لتنفيذ منطق الأعمال الذي تريده.
