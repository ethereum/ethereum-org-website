...
إذا كنت تريد تخطي المقالة ورؤية النص البرمجي المصدري فقط، [فهو هنا](https://github.com/qbzzt/20220915-all-you-can-cache). مكدس التطوير هو [فاوندري](https://getfoundry.sh/introduction/installation/).
...
هذا النص البرمجي مكتوب بلغة [يول](https://docs.soliditylang.org/en/v0.8.16/yul.html). تقرأ قيمة 32 بايت من بيانات الاستدعاء. يعمل هذا حتى لو توقفت بيانات الاستدعاء قبل `startByte+32` لأن المساحة غير المهيأة في EVM تعتبر صفرًا.
...
تحاول سوليديتي تقليل عدد الأخطاء عن طريق حظر [تحويلات النوع الضمنية](https://docs.soliditylang.org/en/v0.8.16/types.html#implicit-conversions) التي قد تكون خطيرة. يجب أن يكون خفض المستوى، على سبيل المثال من 256 بت إلى 8 بت، صريحًا.
...
إحدى المزايا الكبيرة لـ فاوندري هي أنها تسمح بكتابة الاختبارات بلغة سوليديتي ([انظر اختبار ذاكرة التخزين المؤقت أدناه](#testing-the-cache)). هذا يجعل اختبارات الوحدة أسهل بكثير. هذه دالة تقرأ أربع معلمات وتعيدها حتى يتمكن الاختبار من التحقق من صحتها.
...
[هذا (`vm.expectRevert`)](https://book.getfoundry.sh/cheatcodes/expect-revert#expectrevert) هو كيف نحدد في اختبار فاوندري أن الاستدعاء التالي يجب أن يفشل، والسبب المبلغ عنه للفشل. ينطبق هذا عندما نستخدم بناء الجملة `<contract>.<function name>()` بدلاً من بناء بيانات الاستدعاء واستدعاء العقد باستخدام واجهة المستوى المنخفض (`<contract>.call()`، إلخ).
...
هذه هي الطريقة التي نتحقق بها من أن النص البرمجي [يصدر حدثًا بشكل صحيح](https://getfoundry.sh/reference/cheatcodes/expect-emit/) في فاوندري.

### العميل {#the-client}

شيء واحد لا تحصل عليه مع اختبارات سوليديتي هو نص برمجي جافا سكريبت يمكنك قصه ولصقه في تطبيقك الخاص. لكتابة هذا النص البرمجي، قمت بنشر WORM على [أوبتيميزم جيرلي](https://community.optimism.io/docs/useful-tools/networks/#optimism-goerli)، شبكة الاختبار الجديدة لـ [أوبتيميزم](https://www.optimism.io/). إنه على العنوان [`0xd34335b1d818cee54e3323d3246bd31d94e6a78a`](https://goerli-optimism.etherscan.io/address/0xd34335b1d818cee54e3323d3246bd31d94e6a78a).

[يمكنك رؤية نص جافا سكريبت البرمجي للعميل هنا](https://github.com/qbzzt/20220915-all-you-can-cache/blob/main/javascript/index.js). لاستخدامه:
...
4. حرر `.env` للتهيئة الخاصة بك:
   ...
   | Parameter                                                     | Value                                                                                                                                                                             |
   | ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | MNEMONIC                                                      | الذاكرة المساعدة لحساب به ما يكفي من ETH لدفع ثمن المعاملة. [يمكنك الحصول على ETH مجانًا لشبكة أوبتيميزم جيرلي هنا](https://optimismfaucet.xyz/). |
   | OPTIMISM_GOERLI_URL | عنوان URL لـ أوبتيميزم جيرلي. نقطة النهاية العامة، `https://goerli.optimism.io`، محدودة المعدل ولكنها كافية لما نحتاجه هنا                                        |
