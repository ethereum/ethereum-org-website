---
title: "مقدمة لمبرمجي بايثون حول إيثريوم، الجزء الأول"
description: "مقدمة لتطوير إيثريوم، وهي مفيدة بشكل خاص لمن لديهم معرفة بلغة برمجة بايثون"
author: "مارك غارو"
lang: ar
tags: [ "Python", "web3.py" ]
skill: beginner
...
يطمح هذا المنشور إلى أن يكون في متناول مجموعة واسعة من المبرمجين. سيتم إشراك [أدوات بايثون](/developers/docs/programming-languages/python/)، لكنها مجرد وسيلة للأفكار – لا توجد مشكلة إذا لم تكن مبرمج بايثون. لكنني سأضع بعض الافتراضات حول ما تعرفه بالفعل، حتى نتمكن من الانتقال بسرعة إلى التفاصيل الخاصة بـ إيثريوم.

الافتراضات:

- يمكنك التعامل مع الوحدة الطرفية،
- لقد كتبت بضعة أسطر من نص بايثون البرمجي،
- أن يكون إصدار بايثون 3.6 أو أحدث مثبتًا على جهازك (يُنصح بشدة باستخدام [بيئة افتراضية](https://realpython.com/effective-python-environment/#virtual-environments))، و
- أنك استخدمت `pip`، وهو مثبت حزم بايثون.
...
<FeaturedText>ملاحظة: تستخدم إيثريوم بانتظام <a href="https://wikipedia.org/wiki/Hash_function">دوال التجزئة (الهاش)</a> لإنتاج قيم ثابتة الحجم ("تجزئات" أو "هاشات"). تلعب التجزئات (الهاشات) دورًا مهمًا في إيثريوم، ولكن يمكنك اعتبارها معرفات فريدة في الوقت الحالي.</FeaturedText>
...
من المرجح أن يلجأ مبرمجو بايثون الذين يرغبون في التفاعل مع إيثريوم إلى [ويب3.باي](https://web3py.readthedocs.io/). ويب3.باي هي مكتبة تبسط إلى حد كبير طريقة اتصالك بعقدة إيثريوم، ثم إرسال البيانات واستلامها منها.

<FeaturedText>ملاحظة: يتم استخدام "عقدة إيثريوم" و"عميل إيثريوم" بالتبادل. في كلتا الحالتين، يشير إلى البرنامج الذي يقوم بتشغيله أحد المشاركين في شبكة إيثريوم. يمكن لهذا البرنامج قراءة بيانات الكتلة، وتلقي التحديثات عند إضافة كتل جديدة إلى السلسلة، وبث معاملات جديدة، والمزيد. من الناحية الفنية، العميل هو البرنامج، والعقدة هي الكمبيوتر الذي يشغل البرنامج.</FeaturedText>

يمكن تكوين [عملاء إيثريوم](/developers/docs/nodes-and-clients/) ليكونوا قابلين للوصول عن طريق [IPC](https://wikipedia.org/wiki/Inter-process_communication)، أو HTTP، أو Websockets، لذلك ستحتاج ويب3.باي إلى محاكاة هذا التكوين. تشير ويب3.باي إلى خيارات الاتصال هذه باسم **الموفرين**. ستحتاج إلى اختيار أحد الموفرين الثلاثة لربط مثيل ويب3.باي بعقدتك.

![رسم تخطيطي يوضح كيفية استخدام web3.py لـ IPC لتوصيل تطبيقك بعقدة إيثريوم](./web3py-and-nodes.png)

_قم بتكوين عقدة إيثريوم وويب3.باي للتواصل عبر نفس البروتوكول، على سبيل المثال، IPC في هذا الرسم التخطيطي._
...
أولًا، قم بتثبيت [IPython](https://ipython.org/) للحصول على بيئة سهلة الاستخدام للاستكشاف. يوفر IPython الإكمال التلقائي بعلامة التبويب، من بين ميزات أخرى، مما يسهل كثيرًا رؤية ما هو ممكن داخل ويب3.باي.

```bash
pip install ipython
```

يتم نشر ويب3.باي تحت اسم `web3`. قم بتثبيته على النحو التالي:
...
سيؤدي هذا إلى طباعة بعض المعلومات حول إصدارات بايثون وIPython التي تقوم بتشغيلها، ثم يجب أن ترى مطالبة تنتظر الإدخال:

```python
In [1]:
```

أنت الآن تنظر إلى واجهة بايثون التفاعلية. بشكل أساسي، إنها بيئة اختبارية للعب بها. إذا وصلت إلى هذا الحد، فقد حان الوقت لاستيراد ويب3.باي:

```python
In [1]: from web3 import Web3
```

## تقديم وحدة ويب3 {#introducing-the-web3-module}

إلى جانب كونها بوابة إلى إيثريوم، توفر وحدة [ويب3](https://web3py.readthedocs.io/en/stable/overview.html#base-api) بعض الوظائف المساعدة. دعنا نستكشف بعضها.

في تطبيق إيثريوم، ستحتاج عادةً إلى تحويل فئات العملات. توفر وحدة ويب3 طريقتين مساعدتين لهذا الغرض فقط: [from_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.from_wei) و[to_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.to_wei).
...
- تنزيل عقدة إيثريوم، على سبيل المثال، [غيث](https://geth.ethereum.org/).
- ابدأ تشغيل غيث في نافذة طرفية واحدة وانتظر حتى تتم مزامنة الشبكة. منفذ HTTP الافتراضي هو `8545`، لكنه قابل للتكوين.
- أخبر ويب3.باي بالاتصال بالعقدة عبر HTTP، على `localhost:8545`.
...
![رسم تخطيطي يوضح EthereumTesterProvider وهو يربط تطبيق web3.py الخاص بك بعقدة إيثريوم تمت محاكاتها](./ethereumtesterprovider.png)

_يتصل EthereumTesterProvider بعقدة تمت محاكاتها وهو مفيد لبيئات التطوير السريعة._

تسمى تلك العقدة التي تمت محاكاتها [eth-tester](https://github.com/ethereum/eth-tester) وقمنا بتثبيتها كجزء من أمر `pip install web3[tester]`. تكوين ويب3.باي لاستخدام هذا الموفر التجريبي بسيط مثل:
