...
description: "ماذا يوجد في عقد أوبن زبلين ERC-20 ولماذا هو موجود؟"
...
[تطبيق أوبن زبلين سوليديتي ERC20](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC20/ERC20.sol)، بالإضافة إلى
...
ميتاماسك، أو تطبيقًا لامركزيًا مثل etherscan.io، أو عقدًا مختلفًا مثل مجمع السيولة.

![توضيح لواجهة ERC-20](erc20_interface.png)

إذا كنت مبرمجًا ذا خبرة، فربما تتذكر رؤية بنيات مماثلة في [جافا](https://www.w3schools.com/java/java_interface.asp)
...
من أوبن زبلين. إنها ترجمة [للمعيار القابل للقراءة البشرية](https://eips.ethereum.org/EIPS/eip-20) إلى كود سوليديتي. بالطبع، الواجهة
...
من المفترض أن تتضمن ملفات سوليديتي مُعرِّف ترخيص. [يمكنك رؤية قائمة التراخيص هنا](https://spdx.org/licenses/). إذا كنت بحاجة إلى ترخيص
...
لا تزال لغة سوليديتي تتطور بسرعة، وقد لا تكون الإصدارات الجديدة متوافقة مع الكود القديم
...
إن `@dev` في التعليق هو جزء من [تنسيق نات سبيك](https://docs.soliditylang.org/en/develop/natspec-format.html)، ويُستخدم لإنتاج
...
تُرجع إجمالي المعروض من الرموز في العقد. تُرجع هذه القيمة باستخدام النوع الأكثر شيوعًا في إيثريوم، وهو 256 بت غير مُوقَّعة (256 بت هو
...
سوليديتي لعقدك، فلن يأخذك أحد على محمل الجد ما لم تنشر الكود المصدري وإصدار سوليديتي الذي تم تجميعه به، حتى يمكن
...
كما يوحي الاسم، تُرجع `balanceOf` رصيد حساب ما. يتم تحديد حسابات إيثريوم في سوليديتي باستخدام نوع `address`، الذي يحمل 160 بت.
...
- `GSN/Context.sol` هو التعريفات المطلوبة لاستخدام [أوبن جي إس إن](https://www.opengsn.org/)، وهو نظام يسمح للمستخدمين الذين ليس لديهم إيثر
  باستخدام البلوك تشين. لاحظ أن هذا إصدار قديم، إذا كنت ترغب في التكامل مع أوبن جي إس إن،
...
- [مكتبة SafeMath](https://ethereumdev.io/using-safe-math-library-to-prevent-from-overflows/)، التي تمنع
  تجاوزات/تدفقات حسابية لإصدارات سوليديتي **&lt;0.8.0**. في سوليديتي ≥0.8.0، تعود العمليات الحسابية تلقائيًا
...
يحدد هذا السطر الوراثة، في هذه الحالة من `IERC20` أعلاه و `Context`، لـ أوبن جي إس إن.
...
من ناحية، لا تحتوي إيثريوم على متغيرات ذات فاصلة عائمة أو كسرية. من ناحية أخرى،
...
[أوبن جي إس إن](https://opengsn.org/). إذا أردنا السماح بالمعاملات بدون إيثر باستخدام رمزنا، فنحن
...
و`_approve`. بالإضافة إلى ذلك، يتجاوز تطبيق أوبن زبلين المعيار الأساسي ليشمل بعض الميزات التي تحسن
...
#### إضافات السلامة من أوبن زبلين {#openzeppelin-safety-additions}
...
لا أحد يمتلك فعليًا العنوان الصفري في إيثريوم (أي لا أحد يعرف مفتاحًا خاصًا يتم تحويل المفتاح العام المطابق له إلى العنوان الصفري). عندما يستخدم الناس هذا العنوان، فإنه عادة ما يكون خطأ برمجيًا - لذلك
...
الطريقة الثانية أفضل بكثير لأن كود أوبن زبلين ERC-20 قد تم تدقيقه بالفعل وتبين أنه آمن. عندما تستخدم الوراثة،
...
- القيم من نوع `uint256` تلتف. بمعنى آخر، _<span dir="ltr">0-1=2^256-1</span>_. إذا لم يكن هذا هو السلوك
  المرغوب، فيجب عليك التحقق منه (أو استخدام مكتبة SafeMath التي تقوم بذلك نيابة عنك). لاحظ أن هذا تغير في
  [سوليديتي 0.8.0](https://docs.soliditylang.org/en/breaking/080-breaking-changes.html).
...
الآن بعد أن رأيت كيف تمت كتابة عقد أوبن زبلين ERC-20، وخاصة كيف تم
