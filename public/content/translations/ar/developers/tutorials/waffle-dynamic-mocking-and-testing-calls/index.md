---
title: "وافل: المحاكاة الديناميكية واختبار استدعاءات العقود"
description: "تعليمات استخدام وافل المتقدمة لاستخدام المحاكاة الديناميكية واختبار استدعاءات العقود"
author: "دانيال إيزديبسكي"
tags:
  [
    "waffle",
    "العقود الذكيه ",
...
نحن الآن جاهزون لبناء العقد باستخدام وافل:
...
يستخدم وافل `chai` للاختبار. ومع ذلك، قبل أن نتمكن من استخدامه، علينا إدخال أدوات المطابقة الخاصة بـ وافل في chai نفسه:
...
ثم نحتاج إلى نشر عقد ERC20. هنا الجزء الصعب - لدينا واجهة فقط. هذا هو الجزء الذي يأتي فيه وافل لإنقاذنا. لدى وافل دالة `deployMockContract()` سحرية تنشئ عقدًا باستخدام _abi_ الخاص بالواجهة فقط:
...
ولكن انتظر لحظة. كيف سيعرف عقدنا المحاكى ما هي القيم التي يجب إرجاعها؟ لم ننفذ أي منطق لدالة `balanceOf()`. مرة أخرى، يمكن لـ وافل المساعدة هنا. يحتوي عقدنا المحاكى الآن على بعض الأشياء الجديدة الرائعة:
...
اختبار استدعاءات العقود باستخدام وافل سهل للغاية. وهذا هو الجزء الأفضل. تعمل أدوات المطابقة هذه مع كل من العقود العادية والمحاكاة! ذلك لأن وافل يسجل ويصفي استدعاءات EVM بدلاً من إدخال النص البرمجي، كما هو الحال في مكتبات الاختبار الشائعة للتقنيات الأخرى.
...
- [اختبار العقود الذكية باستخدام وافل](/developers/tutorials/waffle-test-simple-smart-contract/)
