...
يمكن تثبيت Echidna من خلال docker أو باستخدام الملف الثنائي المترجم مسبقًا.

### Echidna من خلال docker {#echidna-through-docker}
...
Echidna هي أداة تشويش قائمة على الخصائص، كما وصفنا في منشورات مدونتنا السابقة ([1](https://blog.trailofbits.com/2018/03/09/echidna-a-smart-fuzzer-for-ethereum/)، [2](https://blog.trailofbits.com/2018/05/03/state-machine-testing-with-echidna/)، [3](https://blog.trailofbits.com/2020/03/30/an-echidna-for-all-seasons/)).
...
تنتمي Echidna إلى عائلة معينة من أدوات التشويش: التشويش القائم على الخصائص المستوحى بشكل كبير من [QuickCheck](https://wikipedia.org/wiki/QuickCheck). على عكس أداة التشويش الكلاسيكية التي ستحاول العثور على أعطال، ستحاول Echidna كسر الثوابت المحددة من قبل المستخدم.

في العقود الذكية، الثوابت هي وظائف Solidity، والتي يمكن أن تمثل أي حالة غير صحيحة أو غير صالحة يمكن أن يصل إليها العقد، بما في ذلك:
...
### اختبار خاصية باستخدام Echidna {#testing-a-property-with-echidna}

سنرى كيفية اختبار عقد ذكي باستخدام Echidna. الهدف هو العقد الذكي التالي [`token.sol`](https://github.com/crytic/building-secure-contracts/blob/master/program-analysis/echidna/example/token.sol):
...
خصائص Echidna هي وظائف Solidity. يجب أن تستوفي الخاصية ما يلي:

- ألا تحتوي على وسيطة
- إرجاع `true` إذا كانت ناجحة
- أن يبدأ اسمها بـ `echidna`

ستقوم Echidna بما يلي:
...
تحتاج Echidna إلى [دالة بناء](/developers/docs/smart-contracts/anatomy/#constructor-functions) بدون وسيطة. إذا كان عقدك بحاجة إلى تهيئة معينة، فأنت بحاجة إلى القيام بذلك في دالة البناء.

هناك بعض العناوين المحددة في Echidna:
...
### تشغيل Echidna {#run-echidna}

يتم تشغيل Echidna باستخدام:
...
وجدت Echidna أن الخاصية تُنتهك إذا تم استدعاء `backdoor`.
...
يجبر هذا المثال الصغير Echidna على العثور على تسلسل معين من المعاملات لتغيير متغير الحالة.
هذا صعب على أداة التشويش (يوصى باستخدام أداة تنفيذ رمزي مثل [Manticore](https://github.com/trailofbits/manticore)).
يمكننا تشغيل Echidna للتحقق من هذا:
...
تواجه Echidna صعوبة في العثور على التسلسل الصحيح لاختبار هذا العقد لأن وظيفتي إعادة الضبط (`reset1` و `reset2`) ستعينان جميع متغيرات الحالة على `false`.
ومع ذلك، يمكننا استخدام ميزة Echidna خاصة إما لوضع وظيفة إعادة الضبط في القائمة السوداء أو لوضع وظائف `f` و `g` و
...
### تشغيل Echidna {#run-echidna-1}

لتشغيل Echidna باستخدام ملف تكوين `blacklist.yaml`:
...
ستجد Echidna تسلسل المعاملات لتكذيب الخاصية على الفور تقريبًا.

### ملخص: تصفية الوظائف {#summary-filtering-functions}

يمكن لـ Echidna إما وضع وظائف في القائمة السوداء أو في القائمة البيضاء لاستدعائها أثناء حملة التشويش باستخدام:
...
تبدأ Echidna حملة تشويش إما بوضع `f1` و `f2` و `f3` في القائمة السوداء أو باستدعاء هذه الوظائف فقط، وفقًا لقيمة `filterBlacklist` المنطقية.

## كيفية اختبار تأكيد Solidity باستخدام Echidna {#how-to-test-soliditys-assert-with-echidna}

في هذا البرنامج التعليمي القصير، سنوضح كيفية استخدام Echidna لاختبار فحص التأكيد في العقود. لنفترض أن لدينا عقدًا مثل هذا:
...
Echidna، لكننا سنحتاج إلى تخزين قيمة `tmp` في مكان ما. بدلاً من ذلك، يمكننا استخدام تأكيد مثل هذا:
...
### تشغيل Echidna {#run-echidna-2}

لتمكين اختبار فشل التأكيد، قم بإنشاء [ملف تكوين Echidna](https://github.com/crytic/echidna/wiki/Config) `config.yaml`:

```yaml
checkAsserts: true
```

عندما نقوم بتشغيل هذا العقد في Echidna، نحصل على النتائج المتوقعة:
...
كما ترى، تبلغ Echidna عن بعض حالات فشل التأكيد في وظيفة `inc`. إضافة أكثر من تأكيد واحد لكل وظيفة أمر ممكن، لكن Echidna لا تستطيع تحديد أي تأكيد فشل.
...
أخيرًا، يرجى **عدم استخدام** `require` بدلاً من `assert`، لأن Echidna لن تتمكن من اكتشافه (لكن العقد سيعود على أي حال).
...
وجدت Echidna أن التأكيد في `inc` يمكن أن يفشل إذا تم استدعاء هذه الوظيفة عدة مرات بوسيطات كبيرة.

## جمع وتعديل مجموعة بيانات Echidna {#collecting-and-modifying-an-echidna-corpus}

سنرى كيفية جمع واستخدام مجموعة بيانات من المعاملات مع Echidna. الهدف هو العقد الذكي التالي [`magic.sol`](https://github.com/crytic/building-secure-contracts/blob/master/program-analysis/echidna/example/magic.sol):
...
يجبر هذا المثال الصغير Echidna على العثور على قيم معينة لتغيير متغير الحالة. هذا صعب على أداة التشويش
(يوصى باستخدام أداة تنفيذ رمزي مثل [Manticore](https://github.com/trailofbits/manticore)).
يمكننا تشغيل Echidna للتحقق من هذا:
...
ومع ذلك، لا يزال بإمكاننا استخدام Echidna لجمع مجموعة بيانات عند تشغيل حملة التشويش هذه.
...
وملف تكوين Echidna [Echidna configuration file](https://github.com/crytic/echidna/wiki/Config) `config.yaml`:
...
لا تزال Echidna غير قادرة على العثور على القيم السحرية الصحيحة، ولكن يمكننا إلقاء نظرة على مجموعة البيانات التي جمعتها.
...
تحتاج Echidna إلى بعض المساعدة للتعامل مع وظيفة `magic`. سنقوم بنسخ وتعديل المدخلات لاستخدام معلمات مناسبة
لها:
...
سنقوم بتعديل `new.txt` لاستدعاء `magic(42,129,333,0)`. الآن، يمكننا إعادة تشغيل Echidna:
...
سنرى كيفية العثور على المعاملات ذات الاستهلاك العالي للغاز باستخدام Echidna. الهدف هو العقد الذكي التالي:
...
حاليًا، تحتاج Echidna دائمًا إلى خاصية لاختبارها: هنا `echidna_test` تُرجع دائمًا `true`.
يمكننا تشغيل Echidna للتحقق من هذا:
...
لتمكين استهلاك الغاز مع Echidna، قم بإنشاء ملف تكوين `config.yaml`:
...
### تشغيل Echidna {#run-echidna-3}

بمجرد إنشاء ملف التكوين، يمكننا تشغيل Echidna على هذا النحو:
...
إذا تمكنت Echidna من استدعاء جميع الوظائف، فلن تجد بسهولة المعاملات ذات تكلفة الغاز المرتفعة:
...
يمكن لـ Echidna العثور على معاملات ذات استهلاك عالٍ للغاز باستخدام خيار التكوين `estimateGas`:
...
ستبلغ Echidna عن تسلسل بأقصى استهلاك للغاز لكل وظيفة، بمجرد انتهاء حملة التشويش.
