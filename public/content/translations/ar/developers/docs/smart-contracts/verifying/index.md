---
title: "عقد ذكي"
description: "نظرة عامة على التحقق من الكود المصدر لعقود إيثريوم الذكية"
lang: ar
---
...
من المهم التمييز بين "التحقق من النص البرمجي المصدر" و"[التحقق الرسمي](/developers/docs/smart-contracts/formal-verification/)". يشير التحقق من النص البرمجي المصدر، الذي سيُشرح بالتفصيل أدناه، إلى التحقق من أن النص البرمجي المصدر المحدد لعقد ذكي بلغة عالية المستوى (مثل سوليديتي) يُجمَّع إلى نفس الـ bytecode الذي سيتم تنفيذه في عنوان العقد. ومع ذلك، فإن التحقق الرسمي يصف التحقق من صحة العقد الذكي، أي أن العقد يتصرف كما هو متوقع. على الرغم من اعتماد التحقق من العقد على السياق، فإنه يشير عادةً إلى التحقق من كود المصدر.

## ما هو التحقق من الكود المصدر؟ {#what-is-source-code-verification}

قبل نشر عقد ذكي في [آلة إيثريوم الافتراضية (EVM)](/developers/docs/evm/)، [يُجمِّع](/developers/docs/smart-contracts/compiling/) المطورون النص البرمجي المصدر للعقد — وهي تعليمات [مكتوبة بلغة سوليديتي](/developers/docs/smart-contracts/languages/) أو لغة برمجة أخرى عالية المستوى — إلى bytecode. نظرًا لأن EVM لا يمكنه تفسير التعليمات عالية المستوى، فإن تجميع الكود المصدر إلى رمز ثنائي (أي تعليمات الآلة منخفضة المستوى) ضروري لتنفيذ منطق العقد في EVM.
...
على الرغم من أنه معروف في الغالب بأنه [مستكشف بلوكتشين إيثريوم](/developers/docs/data-and-analytics/block-explorers/)، يوفر إيثرسكان أيضًا [خدمة التحقق من النص البرمجي المصدر](https://etherscan.io/verifyContract) لمطوري العقود الذكية ومستخدميها.

يتيح لك إيثرسكان إعادة تجميع البايت كود العقد من الحمولة الأصلية للبيانات (كود المصدر، عنوان المكتبة، إعدادات المترجم، عنوان العقد، وما إلى ذلك). إذا كان bytecode المعاد تجميعه مرتبطًا بـ bytecode (ومعلمات المُنشئ) للعقد الموجود على السلسلة، فعندئذٍ [يتم التحقق من العقد](https://info.etherscan.com/types-of-contract-verification/).

بمجرد التحقق، يتلقى كود مصدر العقد الخاص بك علامة "تم التحقق منه" ويتم نشره على إيثرسكان ليتمكن الآخرون من تدقيقه. يُضاف أيضًا إلى قسم [العقود المُتحقق منها](https://etherscan.io/contractsVerified/) — وهو مستودع للعقود الذكية ذات النصوص البرمجية المصدر المُتحقق منها.
...
[المزيد عن التحقق من العقود على إيثرسكان](https://medium.com/etherscan-blog/verifying-contracts-on-etherscan-f995ab772327).

### بلوك سكوت {#blockscout}

[بلوك سكوت](https://blockscout.com/) هو مستكشف بلوكتشين مفتوح المصدر يوفر أيضًا [خدمة التحقق من العقود](https://eth.blockscout.com/contract-verification) لمطوري العقود الذكية ومستخدميها. كبديل مفتوح المصدر، يوفر بلوك سكوت الشفافية في كيفية إجراء التحقق ويمكّن مساهمات المجتمع لتحسين عملية التحقق.

على غرار خدمات التحقق الأخرى، يسمح لك بلوك سكوت بالتحقق من الكود المصدر لعقدك عن طريق إعادة تجميع الكود الثنائي ومقارنته بالعقد المنشور. على غرار خدمات التحقق الأخرى، يسمح لك بلوك سكوت بالتحقق من الكود المصدر لعقدك عن طريق إعادة تجميع الكود الثنائي ومقارنته بالعقد المنشور. العقود المُتحقق منها مدرجة أيضًا في [مستودع العقود المُتحقق منها](https://eth.blockscout.com/verified-contracts) الخاص بـ بلوك سكوت لسهولة التصفح والاستكشاف.

### سورسيفاي {#sourcify}

[سورسيفاي](https://sourcify.dev/#/verifier) هو أداة أخرى للتحقق من العقود وهي مفتوحة المصدر ولامركزية. إنه ليس مستكشف كتل ويتحقق فقط من العقود على [شبكات مختلفة قائمة على EVM](https://docs.sourcify.dev/docs/chains). يعمل كبنية تحتية عامة للأدوات الأخرى للبناء فوقها، ويهدف إلى تمكين تفاعلات العقود الأكثر سهولة للاستخدام البشري باستخدام [واجهة التطبيق الثنائية (ABI)](/developers/docs/smart-contracts/compiling/#web-applications) وتعليقات [نات سبيك](https://docs.soliditylang.org/en/v0.8.15/natspec-format.html) الموجودة في ملف البيانات الوصفية.

على عكس إيثرسكان، يدعم سورسيفاي المطابقات الكاملة مع تجزئة البيانات الوصفية. تُقدَّم العقود المُتحقق منها في [مستودعه العام](https://docs.sourcify.dev/docs/repository/) على HTTP و [آي بي إف إس](https://docs.ipfs.io/concepts/what-is-ipfs/#what-is-ipfs)، وهو تخزين لامركزي [بعنونة المحتوى](https://docs.storacha.network/concepts/content-addressing/). يتيح هذا جلب ملف البيانات الوصفية لعقد عبر آي بي إف إس نظرًا لأن تجزئة البيانات الوصفية المضافة هي تجزئة آي بي إف إس.

بالإضافة إلى ذلك، يمكنك أيضًا استرداد ملفات التعليمات البرمجية المصدرية عبر آي بي إف إس، حيث يتم العثور على تجزئات آي بي إف إس لهذه الملفات أيضًا في البيانات الوصفية. يمكن التحقق من العقد من خلال توفير ملف البيانات الوصفية والملفات المصدر عبر واجهة برمجة التطبيقات (API) الخاصة به أو [واجهة المستخدم (UI)](https://sourcify.dev/#/verifier)، أو باستخدام المكونات الإضافية. تستمع أداة مراقبة سورسيفاي أيضًا إلى عمليات إنشاء العقود على الكتل الجديدة وتحاول التحقق من العقود إذا تم نشر بياناتها الوصفية وملفات المصدر على آي بي إف إس.

[المزيد عن التحقق من العقود على سورسيفاي](https://soliditylang.org/blog/2020/06/25/sourcify-faq/).

### تيندرلي {#tenderly}

تتيح [منصة تيندرلي](https://tenderly.co/) لمطوري ويب3 بناء العقود الذكية واختبارها ومراقبتها وتشغيلها. من خلال الجمع بين أدوات التصحيح مع إمكانية المراقبة وبناء البنية التحتية، يساعد تيندرلي المطورين على تسريع تطوير العقود الذكية. لتمكين ميزات تيندرلي بالكامل، يحتاج المطورون إلى [إجراء التحقق من النص البرمجي المصدر](https://docs.tenderly.co/monitoring/contract-verification) باستخدام عدة طرق.
...
يمكنك التحقق من عقودك باستخدام [لوحة التحكم](https://docs.tenderly.co/contract-verification)، أو [ملحق تيندرلي هارد هات](https://docs.tenderly.co/contract-verification/hardhat)، أو [واجهة سطر الأوامر (CLI)](https://docs.tenderly.co/monitoring/smart-contract-verification/verifying-contracts-using-cli).
