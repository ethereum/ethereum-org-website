...
عقدة الأرشيف هي مثيل لعميل إيثريوم الذي تم تكوينه لإنشاء أرشيف لجميع الحالات التاريخية. إنها أداة مفيدة لحالات استخدام معينة ولكن قد يكون تشغيلها أكثر صعوبة من تشغيل العقدة الكاملة.
...
- رمز العقد والتخزين
- البيانات المتعلقة بالإجماع، على سبيل المثال، عقد إيداع التحصيص.

للتفاعل مع الشبكة والتحقق من الكتل الجديدة وإنتاجها، يتعين على عملاء إيثريوم مواكبة أحدث التغييرات (نهاية السلسلة) وبالتالي الحالة الحالية. يتحقق عميل طبقة التنفيذ الذي تم تكوينه كعقدة كاملة من أحدث حالة للشبكة ويتبعها، ولكنه يخزن مؤقتًا الحالات القليلة الماضية فقط، على سبيل المثال، الحالة المرتبطة بآخر 128 كتلة، حتى يتمكن من التعامل مع عمليات إعادة تنظيم السلسلة وتوفير وصول سريع إلى البيانات الحديثة. الحالة الأخيرة هي ما يحتاجه جميع العملاء للتحقق من المعاملات الواردة واستخدام الشبكة.
...
الاستخدام المنتظم لـ إيثريوم مثل إرسال المعاملات ونشر العقود والتحقق من الإجماع وما إلى ذلك لا يتطلب الوصول إلى الحالات التاريخية. لا يحتاج المستخدمون أبدًا إلى عقدة أرشيف للتفاعل القياسي مع الشبكة.
...
بصرف النظر عن [التوصيات العامة لتشغيل عقدة](/developers/docs/nodes-and-clients/run-a-node/)، قد تكون عقدة الأرشيف أكثر تطلبًا من حيث الأجهزة والصيانة. بالنظر إلى [الميزات الرئيسية](https://github.com/ledgerwatch/erigon#key-features) لـ إريغون، فإن النهج الأكثر عملية هو استخدام تطبيق عميل [إريغون](/developers/docs/nodes-and-clients/#erigon).
...
- [كيفية إعداد إريغون، و RPC الخاص بـ إريغون، و TrueBlocks (الكشط وواجهة برمجة تطبيقات) كخدمات](https://magnushansson.xyz/blog_posts/crypto_defi/2022-01-10-Erigon-Trueblocks) _– Magnus Hansson، تم التحديث في سبتمبر 2022_
