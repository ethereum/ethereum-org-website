...
O Uniswap v2 é dividido em dois componentes, um principal e um periférico. Essa divisão permite que os contratos principais, que guardam os ativos e, portanto, _têm_ que ser seguros, sejam mais simples e fáceis de auditar. Toda a funcionalidade extra exigida pelos traders pode então ser fornecida por contratos periféricos.
...
2. Chamar uma das muitas funções de troca do contrato periférico (qual delas depende se o ETH está envolvido ou não, se o trader especifica a quantidade de tokens a depositar ou a quantidade de tokens a receber de volta, etc).
   Cada função de troca aceita um `path`, um array de trocas pelas quais passar.

#### No contrato periférico (UniswapV2Router02.sol) {#in-the-periphery-contract-uniswapv2router02-sol}

3. Identificar os valores que precisam ser negociados em cada troca ao longo do caminho.
4. Itera sobre o caminho. Para cada troca ao longo do caminho, ele envia o token de entrada e, em seguida, chama a função `swap` da troca.
   Na maioria dos casos, o endereço de destino para os tokens é a próxima troca de par no caminho. Na troca final, é o endereço fornecido pelo trader.

#### No contrato principal (UniswapV2Pair.sol) {#in-the-core-contract-uniswapv2pairsol-2}

5. Verificar se o contrato principal não está sendo enganado e pode manter liquidez suficiente após a troca.
...
9. Executar qualquer limpeza necessária (por exemplo, queimar tokens de ether empacotado (WETH) para receber de volta ETH para enviar ao trader)
...
3. Criar uma nova troca de par, se necessário
4. Se houver uma troca de par existente, calcular a quantidade de tokens a adicionar. Supõe-se que seja um valor idêntico para ambos os tokens, portanto, a mesma proporção de novos tokens para tokens existentes.
...
3. Enviar os tokens de liquidez para a troca de par
...
A solução que o Uniswap encontrou é usar valores de 224 bits, com 112 bits para a parte inteira e 112 bits para a fração. Então `1.0` é representado como `2^112`, `1.5` é representado como `2^112 + 2^111`, etc.
...
Existe uma classe de vulnerabilidades de segurança que se baseiam no [abuso de reentrada](https://medium.com/coinmonks/ethernaut-lvl-10-re-entrancy-walkthrough-how-to-abuse-execution-ordering-and-reproduce-the-dao-7ec88b912c14). O Uniswap precisa transferir tokens ERC-20 arbitrários, o que significa chamar contratos ERC-20 que podem tentar abusar do mercado Uniswap que os chama.
Ao ter uma variável `unlocked` como parte do contrato, podemos evitar que as funções sejam chamadas enquanto estão em execução (dentro da mesma transação).
...
Para evitar ter que importar uma interface para a função de token, criamos "manualmente" a chamada usando uma das [funções ABI](https://docs.soliditylang.org/en/v0.8.3/units-and-global-variables.html#abi-encoding-and-decoding-functions).
...
A variável de estado `kLast` está localizada no armazenamento, portanto, terá um valor entre diferentes chamadas para o contrato.
...
Este complicado cálculo de taxas é explicado no [whitepaper](https://app.uniswap.org/whitepaper.pdf) na página 5. Sabemos que entre o momento em que `kLast` foi calculado e o presente, nenhuma liquidez foi adicionada ou removida (porque executamos esse cálculo toda vez que a liquidez é adicionada ou removida, antes que ela realmente mude), portanto, qualquer alteração em `reserve0 * reserve1` deve vir de taxas de transação (sem elas, manteríamos `reserve0 * reserve1` constante).
...
```solidity
    // força os saldos a corresponderem às reservas
    ...
    // força as reservas a corresponderem aos saldos
...
Essas variáveis de estado são necessárias para implementar a taxa de protocolo (veja o [whitepaper](https://app.uniswap.org/whitepaper.pdf), p. 5).
...
As transações no Ethereum custam ether (ETH), que é equivalente a dinheiro real. Se você tem tokens ERC-20, mas não tem ETH, não pode enviar transações, então não pode fazer nada com eles. Uma solução para evitar esse problema são as [metatransações](https://docs.uniswap.org/contracts/v2/guides/smart-contract-integration/supporting-meta-transactions).
O proprietário dos tokens assina uma transação que permite que outra pessoa saque tokens offchain e a envia usando a Internet para o destinatário. O destinatário, que tem ETH, então envia a permissão em nome do proprietário.
...
Este é o código para recuperar o [identificador da cadeia](https://chainid.network/). Ele usa um dialeto de assembly EVM chamado [Yul](https://docs.soliditylang.org/en/v0.8.4/yul.html). Observe que na versão atual do Yul você deve usar `chainid()`, não `chainid`.
...
A partir do resumo (digest) e da assinatura, podemos obter o endereço que o assinou usando [ecrecover](https://coders-errand.com/ecrecover-signature-verification-ethereum/).
...
library UniswapV2Library {
    ...
    // retorna endereços de token ordenados, usado para lidar com valores de retorno de pares ordenados nesta ordem
