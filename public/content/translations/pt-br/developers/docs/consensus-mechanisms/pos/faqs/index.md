...
Sim. A prova de participação é vulnerável a 51% dos ataques, bem como a prova de trabalho. Em vez de o ataque exigir 51% do poder de hash da rede, o ataque requer 51% do total de ETH em participação (stake). Um invasor que acumula 51% do total em participação (stake) consegue controlar o algoritmo de escolha da bifurcação (fork). Isso permite que o invasor censure certas transações, faça reorganizações de curto alcance e extraia MEV reordenando blocos a seu favor.
