...
Se você é novo no tópico de execução de um nó, ou está procurando um caminho menos técnico, recomendamos primeiro conferir nossa introdução amigável sobre [como executar um nó do Ethereum](/run-a-node).
...
- Nuvem
  ...
  - Obter um servidor dedicado ou virtual pode ser mais confortável do que construir o seu próprio
...
Os requisitos de hardware diferem por cliente, mas geralmente não são tão altos, já que o nó só precisa se manter sincronizado. Não confunda isso com mineração, que exige muito mais poder de computação. No entanto, o tempo de sincronização e o desempenho melhoram com um hardware mais poderoso.
...
- CPU com 2+ núcleos
- 16 GB de RAM (32 GB recomendados para estabilidade)
- SSD NVMe de 2 TB (provavelmente excedido até 2027, leia mais sobre [SSDs ótimos e menos ótimos para nós do Ethereum](https://gist.github.com/yorickdowne/f3a3e79a573bf35767cd002cc977b038))
- Largura de banda de 25+ MBit/s

##### Especificações recomendadas {#spinning-up-node}

A orientação atual de hardware para operadores de nós é identificada na [EIP-7870](https://eips.ethereum.org/EIPS/eip-7870). Para um nó completo, recomenda-se:

- CPU rápida com 4+ núcleos (8+ núcleos se for um validador)
- 32 GB de RAM (64 GB recomendados se for um validador para garantir a estabilidade)
- SSD NVMe de 4 TB (unidades sem DRAM e QLC são desencorajadas)
- Largura de banda de 50 MBit/s de download / 15+ MBit/s de upload (25+ MBit/s de upload se for um validador)
...
| Cliente    | Tamanho em disco (sincronização snap) | Tamanho em disco (arquivo completo) |
| ---------- | ------------------------------------- | ----------------------------------- |
| Besu       | 800GB+                                | 12TB+                               |
| Erigon     | N/A                                   | 2.5TB+                              |
| Geth       | 500GB+                                | 12TB+                               |
...
- Nota: Erigon e Reth não oferecem sincronização snap, mas a Poda Completa (Full Pruning) é possível (~2TB para Erigon, ~1.2TB para Reth)
...
#### Soluções plug-and-play {#automatized-setup}
...
#### Ethereum em um computador de placa única {#manual-setup}
...
## Criando o nó {#getting-the-client}
...
### Configuração guiada {#client-setup}
...
- [Chainstack Self-Hosted](https://docs.chainstack.com/docs/self-hosted/introduction) - Interface de usuário da Web e CLI para implantar clientes de execução e de consenso no Kubernetes. Inicialização de snapshot e monitoramento integrado incluídos. Gratuito. Nenhuma conta da Chainstack é exigida. Construído pela Chainstack.

### Configuração manual de clientes {#starting-the-execution-client}
...
##### Clientes de execução {#starting-the-consensus-client}
...
A [diversidade de clientes](/developers/docs/nodes-and-clients/client-diversity/) é crítica para nós de consenso executando validadores. Se a maioria dos validadores estiver executando uma única implementação de cliente, a segurança da rede estará em risco. Portanto, é recomendado considerar a escolha de um cliente minoritário.
...
##### Verificando o software {#adding-validators}
...
#### Configuração do cliente {#using-the-node}
...
#### Iniciando o cliente de execução {#reaching-rpc}

Antes de iniciar o software do cliente Ethereum, faça uma última verificação de que o seu ambiente está pronto. Por exemplo, certifique-se de que:
...
#### Executando um cliente de execução {#operating-the-node}
...
##### Executando o Besu {#keeping-node-online}
...
##### Executando o Erigon {#creating-client-services}
...
O Erigon, por padrão, realiza uma sincronização completa com um HDD de 8 GB, o que resultará em mais de 2 TB de dados de arquivo. Certifique-se de que `datadir` esteja apontando para um disco com espaço livre suficiente ou analise a flag `--prune` que pode aparar diferentes tipos de dados. Verifique o `--help` do Erigon para saber mais.

##### Executando o Geth {#updating-clients}
...
##### Executando o Nethermind {#running-additional-services}
...
##### Executando o Reth {#monitoring-the-node}
...
Veja [Configurando o Reth](https://reth.rs/run/config.html?highlight=data%20directory#configuring-reth) para aprender mais sobre os diretórios de dados padrão. A [documentação do Reth](https://reth.rs/run/mainnet.html) contém opções adicionais e detalhes de configuração.

#### Iniciando o cliente de consenso {#further-reading}
...
#### Executando um cliente de consenso {#related-topics}
...
O acesso à interface RPC pode ser estendido através do desenvolvimento de APIs de camada de borda ou aplicativos de servidor web, como o Nginx, e conectando-os ao endereço e porta locais do seu cliente. Aproveitar uma camada intermediária também pode permitir aos desenvolvedores a capacidade de configurar um certificado para conexões `https` seguras com a interface RPC.

Configurar um servidor web, um proxy ou uma API Rest voltada para o exterior não é a única maneira de fornecer acesso ao endpoint RPC do seu nó. Outra maneira de preservar a privacidade para configurar um endpoint acessível publicamente é hospedar o nó no seu próprio serviço onion do [Tor](https://www.torproject.org/). Isso permitirá que você acesse o RPC fora da sua rede local sem um endereço IP público estático ou portas abertas. No entanto, o uso dessa configuração pode permitir apenas que o endpoint RPC seja acessível através da rede Tor, o que não é suportado por todos os aplicativos e pode resultar em problemas de conexão.
...
#### Mantendo um nó online

O seu nó não precisa estar online o tempo todo, mas você deve mantê-lo online o máximo possível para mantê-lo sincronizado com a rede. Você pode desligá-lo para reiniciá-lo, mas tenha em mente que:
...
> Antes de atualizações importantes da rede, a EF publica um post no seu [blog](https://blog.ethereum.org). Você pode se [inscrever nesses anúncios](https://blog.ethereum.org/category/protocol#subscribe) para receber uma notificação no seu e-mail quando o seu nó precisar de uma atualização.
...
Cada implementação de cliente tem uma string de versão legível por humanos usada no protocolo ponto a ponto, mas que também é acessível a partir da linha de comando. Essa string de versão permite que os usuários verifiquem se estão executando a versão correta e permite que exploradores de blocos e outras ferramentas analíticas interessadas quantifiquem a distribuição de clientes específicos pela rede. Por favor, consulte a documentação individual do cliente para obter mais informações sobre strings de versão.

#### Executando serviços adicionais
...
- [Analisando os requisitos de hardware para ser um nó validado completo do Ethereum](https://medium.com/coinmonks/analyzing-the-hardware-requirements-to-be-an-ethereum-full-validated-node-dc064f167902) _– Albert Palau, 24 de setembro de 2018_
