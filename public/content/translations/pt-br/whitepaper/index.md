...
6. Seja `TX` a lista de transações do bloco, com `n` transações. Para todo `i` em `0...n-1`, defina `S[i+1] = APPLY(S[i],TX[i])`. Se alguma aplicação retornar um erro, ou se o gas total consumido no bloco até este ponto exceder o `GASLIMIT`, retorne um erro.
