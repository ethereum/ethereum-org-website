...
<InfoBanner emoji="⚠️" isWarning><strong>A cada cuenta de validador solo se le puede asignar una única dirección de retiro, una sola vez.</strong> Una vez que se elige y se envía una dirección a la capa de consenso, esta acción no puede deshacerse ni cambiarse de nuevo. Vuelva a verificar la propiedad y la precisión de la dirección proporcionada antes de enviarla.
</InfoBanner>
...
<InfoBanner emoji="🕛">
Piense en un reloj analógico. La aguja del reloj apunta a la hora, avanza en una dirección, no se salta ninguna hora y, finalmente, vuelve al principio de nuevo después de alcanzar el último número.<br/><br/>
Ahora, en lugar del 1 al 12, imagine que el reloj tiene desde 0 hasta N <em>(el número total de cuentas de validador que se han registrado en la capa de consenso, más de 500 000 a fecha de enero de 2023).</em><br/><br/>
La aguja del reloj apunta al siguiente validador que debe comprobarse para ver si tiene retiros elegibles. Empieza a partir de 0, y avanza todo el camino alrededor sin saltarse ninguna cuenta. Cuando se alcance el último validador, el ciclo continúa volviendo al principio.
</InfoBanner>
