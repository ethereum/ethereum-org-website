...
![Diagrama de flujo para esta porción](flowchart-entry.png)
...
![Los datos de llamada están vacíos](calldata-empty.png)
...
![Diagrama de flujo de la llamada al proxy](flowchart-proxy.png)
...
![Diagrama de flujo de las llamadas de ABI](flowchart-abi.png)
...
`calldataload(4)` es la primera palabra de los datos de llamada _después de_ la firma del método
...
Ya sabemos lo que hace [el código en el desplazamiento 0xDA](#the-da-code), devuelve el valor superior de la pila a la persona que llama. Así que esta función devuelve el valor de la tabla de búsqueda al que llama.
...
Ya sabemos lo que hace [el código en el desplazamiento 0xDA](#the-da-code), devuelve el valor superior de la pila a la persona que llama. Así que esta función devuelve `Value*`.

### Resumen del método {#method-summary}
...
| [splitter()](#splitter)           | Devolver Almacenamiento[3], la dirección del proxy                                                   |
| [currentWindow()](#currentwindow) | Devolver Almacenamiento[1]                                                                           |
| [merkleRoot()](#merkleroot)       | Devolver Almacenamiento[0]                                                                           |
| [0x81e580d3](#0x81e580d3)                            | Devolver el valor de una tabla de búsqueda, siempre que el parámetro sea menor que Almacenamiento[4] |
| [0x1f135823](#0x1f135823)                            | Devolver Almacenamiento[6], es decir Valor\*                                                         |
...
Cuando [miramos un contrato](https://etherscan.io/address/0x2510c039cc3b061d79e564b38836da87e31b342f) también podemos ver la transacción que lo creó.

![Haga clic en la transacción de creación](create-tx.png)

Si hacemos clic en esa transacción y luego en la pestaña **State**, podemos ver los valores iniciales de los parámetros. Específicamente, podemos ver que Almacenamiento[3] contiene [0x2f81e57ff4f4d83b40a9f719fd892d8e806e0761](https://etherscan.io/address/0x2f81e57ff4f4d83b40a9f719fd892d8e806e0761). Ese contrato debe contener la funcionalidad que falta. Podemos entenderlo usando las mismas herramientas que usamos para el contrato que estamos investigando.
...
Podemos ignorar los cuatro últimos métodos porque nunca llegaremos a ellos. Sus firmas son tales que nuestro contrato original se encarga de ellas por sí mismo (puede hacer clic en las firmas para ver los detalles más arriba), por lo que deben ser [métodos que se anulan](https://medium.com/upstate-interactive/solidity-override-vs-virtual-functions-c0a5dfb83aaf).

Uno de los métodos restantes es `claim(<params>)`, y otro es `isClaimed(<params>)`, por lo que parece un contrato de airdrop. En lugar de revisar el resto código de operación por código de operación, podemos [probar el decompilador](https://etherscan.io/bytecode-decompiler?a=0x2f81e57ff4f4d83b40a9f719fd892d8e806e0761), que produce resultados utilizables para tres funciones de este contrato. La ingeniería inversa de los otros se deja como ejercicio para el lector.
...
![Una transacción de reclamación](claim-tx.png)
