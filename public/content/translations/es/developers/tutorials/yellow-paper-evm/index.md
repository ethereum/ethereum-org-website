...
Esta sección (pág. 14-16) incluye la mayor parte de la definición de la EVM.
...
Una [máquina de Turing](https://en.wikipedia.org/wiki/Turing_machine) es un modelo computacional. Esencialmente, es una versión simplificada de una computadora, que se ha demostrado que tiene la misma capacidad para ejecutar cálculos que una computadora normal (todo lo que una computadora puede calcular, una máquina de Turing lo puede calcular y viceversa). Este modelo facilita la demostración de varios teoremas sobre lo que es y lo que no es computable.
...
El costo inherente del código de operación específico. Para obtener este valor, busque el grupo de costo del código de operación en el Apéndice H (pág. 29, debajo de la ecuación (329)) y busque el grupo de costo en la ecuación (326). Esto le da una función de costo, que en la mayoría de los casos utiliza parámetros del Apéndice G (pág. 28).

Por ejemplo, el código de operación [`CALLDATACOPY`](https://www.evm.codes/#37) es un miembro del grupo _W<sub>copy</sub>_. El costo del código de operación para ese grupo es _G<sub>verylow</sub>+G<sub>copy</sub>×⌈μ<sub>s</sub>[2]÷32⌉_. Al observar el Apéndice G, vemos que ambas constantes son 3, lo que nos da _3+3×⌈μ<sub>s</sub>[2]÷32⌉_.

Todavía necesitamos descifrar la expresión _⌈μ<sub>s</sub>[2]÷32⌉_. La parte más externa, _⌈ \<value\> ⌉_ es la función techo, una función que, dado un valor, devuelve el número entero más pequeño que no sea menor que el valor. Por ejemplo, _⌈2.5⌉ = ⌈3⌉ = 3_. La parte interna es _μ<sub>s</sub>[2]÷32_. Al observar la sección 3 (Convenciones) en la pág. 3, _μ_ es el estado de la máquina. El estado de la máquina se define en la sección 9.4.1 en la pág. 15. Según esa sección, uno de los parámetros del estado de la máquina es _s_ para la pila. Juntándolo todo, parece que _μ<sub>s</sub>[2]_ es la ubicación n.º 2 en la pila. Al observar [el código de operación](https://www.evm.codes/#37), la ubicación n.º 2 en la pila es el tamaño de los datos en bytes. Al observar los otros códigos de operación en el grupo W<sub>copy</sub>, [`CODECOPY`](https://www.evm.codes/#39) y [`RETURNDATACOPY`](https://www.evm.codes/#3e), también tienen un tamaño de datos en la misma ubicación. Por lo tanto, _⌈μ<sub>s</sub>[2]÷32⌉_ es el número de palabras de 32 bytes necesarias para almacenar los datos que se están copiando. Juntando todo, el costo inherente de [`CALLDATACOPY`](https://www.evm.codes/#37) es de 3 de gas más 3 por palabra de datos que se copia.
...
En la ecuación 326, este valor se escribe como _C<sub>mem</sub>(μ<sub>i</sub>')-C<sub>mem</sub>(μ<sub>i</sub>)_. Al observar la sección 9.4.1 nuevamente, vemos que _μ<sub>i</sub>_ es el número de palabras en la memoria. Por lo tanto, _μ<sub>i</sub>_ es el número de palabras en la memoria antes del código de operación y _μ<sub>i</sub>'_ es el número de palabras en la memoria después del código de operación.

La función _C<sub>mem</sub>_ se define en la ecuación 328: _C<sub>mem</sub>(a) = G<sub>memory</sub> × a + ⌊a<sup>2</sup> ÷ 512⌋_. _⌊x⌋_ es la función suelo, una función que, dado un valor, devuelve el número entero más grande que no sea mayor que el valor. Por ejemplo, _⌊2.5⌋ = ⌊2⌋ = 2._ Cuando _a < √512_, _a<sup>2</sup> < 512_, y el resultado de la función suelo es cero. Por lo tanto, para las primeras 22 palabras (704 bytes), el costo aumenta linealmente con el número de palabras de memoria requeridas. Más allá de ese punto, _⌊a<sup>2</sup> ÷ 512⌋_ es positivo. Cuando la memoria requerida es lo suficientemente alta, el costo de gas es proporcional al cuadrado de la cantidad de memoria.

**Nota:** estos factores solo influyen en el costo de gas _inherente_; no tienen en cuenta el mercado de tarifas ni las propinas a los validadores que determinan cuánto debe pagar un usuario final. Este es solo el costo bruto de ejecutar una operación particular en la EVM.
...
Se necesitan algunos otros parámetros para comprender el resto de la sección 9:

| Parámetro | Definido en la sección | Significado                                                                                                                                                                                                                                                        |
| --------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| _σ_       | 2 (pág. 2, ecuación 1) | El estado de la cadena de bloques                                                                                                                                                                                                                                  |
| _g_       | 9.3 (pág. 14)          | Gas restante                                                                                                                                                                                                                                                       |
| _A_       | 6.1 (pág. 9)           | Subestado acumulado (cambios programados para cuando finalice la transacción)                                                                                                                                                                                      |
| _o_       | 9.3 (pág. 14)          | Salida: el resultado devuelto en el caso de una transacción interna (cuando un contrato llama a otro) y llamadas a funciones de vista (cuando solo está solicitando información, por lo que no hay necesidad de esperar a que se procese una transacción) |
...
Las ecuaciones 146-151 nos dan las condiciones iniciales para ejecutar la EVM:
...
La ecuación 152 nos dice que hay cuatro condiciones posibles en cada momento durante la ejecución, y qué hacer con ellas:
...
Esta sección define la función _Z_, que especifica cuándo tenemos una terminación anormal. Esta es una función [booleana](https://en.wikipedia.org/wiki/Boolean_data_type), por lo que usa [_∨_ para un o lógico](https://en.wikipedia.org/wiki/Logical_disjunction) y [_∧_ para un y lógico](https://en.wikipedia.org/wiki/Logical_conjunction).
...
- **_|| μ<sub>s</sub> || < δ<sub>w</sub>_**
  Desbordamiento negativo de la pila (underflow), no hay suficientes elementos en la pila para el código de operación actual.
...
- **_¬I<sub>w</sub> ∧ W(w,μ)_**
  ...
  La función _W(w,μ)_ se define más adelante en la ecuación 159. _W(w,μ)_ es verdadera si una de estas condiciones es verdadera:
...
Antes de pasar a la subsección final de la EVM, 9.5, veamos las instrucciones en sí. Se definen en el Apéndice H.2 que comienza en la pág. 30. Se espera que todo lo que no se especifique que cambia con ese código de operación específico permanezca igual. Las variables que sí cambian se especifican como \<algo\>′.
...
_α_ es el número de valores que volvemos a insertar. En este caso uno, la suma.
...
Este es el primer código de operación que accede a la memoria (en este caso, de solo lectura). Sin embargo, podría expandirse más allá de los límites actuales de la memoria, por lo que necesitamos actualizar _μ<sub>i</sub>._ Hacemos esto usando la función _M_ definida en la ecuación 330 en la pág. 30.

| Valor | Mnemónico | δ   | α   | Descripción                                |
| ----: | --------- | --- | --- | ------------------------------------------ |
|  0x31 | BALANCE   | 1   | 1   | Obtener el saldo de la cuenta especificada. |
|       |           |     |     | ...                                        |

La dirección cuyo saldo necesitamos encontrar es _μ<sub>s</sub>[0] mod 2<sup>160</sup>_. La parte superior de la pila es la dirección, pero debido a que las direcciones tienen solo 160 bits, calculamos el valor [módulo](https://en.wikipedia.org/wiki/Modulo_operation) 2<sup>160</sup>.
...
La ecuación (164) dice que dado el estado:
...
Las ecuaciones (165)-(167) definen la pila y el cambio en ella debido a un código de operación (_μ<sub>s</sub>_). La ecuación (168) es el cambio en el gas (_μ<sub>g</sub>_). La ecuación (169) es el cambio en el contador de programa (_μ<sub>pc</sub>_). Finalmente, las ecuaciones (170)-(173) especifican que los demás parámetros permanecen iguales, a menos que el código de operación los cambie explícitamente.
