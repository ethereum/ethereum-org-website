...
Una característica tan poderosa de Ethereum debe ser manejada con un estándar sólido, ¿verdad? ¡Ahí es exactamente
donde el ERC desempeña su papel! Este estándar permite a los desarrolladores construir aplicaciones de token que son interoperables con otros productos y servicios. El estándar ERC-20 también se utiliza para proporcionar una funcionalidad adicional al [ether](/glossary/#ether).

**¿Qué es el ERC-20?**

El ERC-20 introduce un estándar para los tokens fungibles, es decir, tienen una propiedad que hace que cada token sea exactamente igual (en tipo y valor) que otro token. Por ejemplo, un token ERC-20 actúa igual que ETH, es decir, 1 token
es y siempre será igual a todos los demás tokens.
...
El ERC-20 (Ethereum Request for Comments 20), propuesto por Fabian Vogelsteller en Noviembre 2015, es un estándar de token que
implementa una API para tokens dentro de contratos inteligentes.
...
Vamos a ver cómo un estándar es tan importante para que las cosas sean sencillas para que inspeccionemos cualquier contrato de token de ERC-20 en Ethereum.
...
#  DAI
print("===== %s =====" % symbol)
print("Suministro total:", totalSupply)
print("Saldo de la dirección:", addr_balance)
...
**A fecha de 20/06/2024, se han perdido al menos 83.656.418 dólares en tokens ERC-20 debido a este problema. Tenga en cuenta que una implementación pura de ERC-20 es propensa a este problema, a menos que implemente un conjunto de restricciones adicionales sobre el estándar, como se indica a continuación.**
...
A raíz de este problema han surgido algunos estándares alternativos, como [ERC-223](/developers/docs/standards/tokens/erc-223) o [ERC-1363](/developers/docs/standards/tokens/erc-1363).
...
- [Alchemy - Guía de los tokens ERC-20 de Solidity](https://www.alchemy.com/overviews/erc20-solidity)
