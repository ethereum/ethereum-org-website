...
Para ayudarle a comprender mejor esta página, le recomendamos que primero lea nuestra [introducción a Ethereum](/developers/docs/intro-to-ethereum/).

## Tipos de cuentas {#types-of-account}

Ethereum tiene dos tipos de cuentas:

- Cuenta de propiedad externa (EOA): controlada por cualquier persona que tenga las claves privadas
- Cuenta de contrato: un contrato inteligente implementado en la red, controlado por código. Más información sobre los [contratos inteligentes](/developers/docs/smart-contracts/)
...
- Crear un contrato tiene un costo porque se utiliza el almacenamiento de la red
...
- `nonce`: un contador que indica el número de transacciones enviadas desde una cuenta de propiedad externa o el número de contratos creados por una cuenta de contrato. Solo se puede ejecutar una transacción con un nonce determinado para cada cuenta, lo que protege contra ataques de repetición en los que las transacciones firmadas se transmiten y vuelven a ejecutar repetidamente.
- `balance`: la cantidad de Wei que posee esta dirección. Wei es una denominación de ETH y hay 1e+18 Wei por ETH.
...
Una cuenta está compuesta por un par de claves criptográficas: pública y privada. Ayudan a demostrar que una transacción fue realmente firmada por el remitente y evitan falsificaciones. Su clave privada es lo que utiliza para firmar transacciones, por lo que le otorga la custodia de los fondos asociados con su cuenta. En realidad, nunca posee criptomonedas, posee claves privadas: los fondos siempre están en el libro mayor de Ethereum.

Esto evita que actores malintencionados transmitan transacciones falsas porque siempre se puede verificar al remitente de una transacción.

Si Alice quiere enviar ether desde su propia cuenta a la cuenta de Bob, Alice necesita crear una solicitud de transacción y enviarla a la red para su verificación. El uso de criptografía de clave pública por parte de Ethereum garantiza que Alice pueda demostrar que ella inició originalmente la solicitud de transacción. Sin mecanismos criptográficos, un adversario malintencionado, Eve, podría simplemente transmitir públicamente una solicitud que se parezca a «enviar 5 ETH de la cuenta de Alice a la cuenta de Eve», y nadie podría verificar que no provino de Alice.

## Creación de cuentas {#account-creation}

Cuando desea crear una cuenta, la mayoría de las bibliotecas le generarán una clave privada aleatoria.
...
Necesita una clave privada para firmar mensajes y transacciones que generan una firma. Otros pueden tomar la firma para derivar su clave pública, demostrando así la autoría del mensaje. En su aplicación, puede utilizar una biblioteca de JavaScript para enviar transacciones a la red.
...
La dirección del contrato generalmente se proporciona cuando se implementa un contrato en la cadena de bloques de Ethereum. La dirección proviene de la dirección del creador y del número de transacciones enviadas desde esa dirección (el «nonce»). Así es como la operación `CREATE` deriva una dirección.

Los contratos también se pueden implementar con [`CREATE2`](https://eips.ethereum.org/EIPS/eip-1014), que deriva la dirección a partir de la dirección del creador, un valor que el creador elige (la «sal» o «salt») y un hash del código de creación del contrato. No hay ningún nonce involucrado, por lo que la dirección se puede calcular antes de que exista el contrato y se mantiene igual sin importar cuántas otras transacciones envíe el creador mientras tanto. Esto hace posible hacer referencia a un contrato que aún no se ha implementado.
...
Una cuenta no es una billetera. Una billetera es una interfaz o aplicación que le permite interactuar con su cuenta de Ethereum, ya sea una cuenta de propiedad externa o una cuenta de contrato.

## Una demostración visual {#a-visual-demo}

Vea a Austin guiarle a través de las funciones hash y los pares de claves.
