...
Una extensión de esto es la habilitación de dispositivos del **internet de las cosas (IoT)**. Un cliente ligero podría utilizarse para demostrar rápidamente la propiedad del saldo de algunos tókenes o NFT, con toda la seguridad garantizada proporcionada por los comités de sincronización, propiciando alguna acción en una red IoT. ¡Imagine un [servicio de alquiler de bicicletas](https://youtu.be/ZHNrAXf3RDE?t=929) que utiliza una aplicación con un cliente ligero integrado para verificar rápidamente que usted posee el NFT del servicio de alquiler y, si es así, desbloquea una bicicleta para que se la lleve!
...
- [Lodestar](https://github.com/ChainSafe/lodestar/tree/unstable/packages/light-client): cliente ligero de consenso en TypeScript
...
- [Nimbus](https://nimbus.guide/el-light-client.html): cliente ligero de consenso en Nim

A nuestro entender, ninguna de estas implementaciones se considera lista para entrar aún en la fase de producción.

También se ha realizado un inmenso esfuerzo por mejorar la manera en que los clientes ligeros acceden a los datos en Ethereum. Actualmente, los clientes ligeros dependen de las solicitudes RPC a nodos completos que utilizan un modelo cliente/servidor, pero en el futuro los datos podrían solicitarse de una forma más descentralizada utilizando una red dedicada como la [Portal Network](https://www.ethportal.net/), que podría servir los datos a los clientes ligeros mediante un protocolo de difusión entre pares.

Otros elementos de la [hoja de ruta](/roadmap/), como los [árboles Verkle](/roadmap/verkle-trees/) y la [ausencia de estado](/roadmap/statelessness/), acabarán por equiparar las garantías de seguridad de los clientes ligeros a las de los clientes completos.
...
- [Piper Merriam: El sinuoso camino hacia los clientes ligeros funcionales](https://snakecharmers.ethereum.org/the-winding-road-to-functional-light-clients/)
