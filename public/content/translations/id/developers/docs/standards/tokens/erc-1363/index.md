...
Hal ini menimbulkan kompleksitas dalam pengembangan antarmuka pengguna (UI) dan hambatan pada adopsi karena pengguna harus menunggu transaksi pertama dieksekusi dan kemudian mengirimkan transaksi kedua.
Mereka juga harus membayar gas dua kali.

ERC-1363 membuat token sepadan (fungible) mampu melakukan tindakan dengan lebih mudah dan bekerja tanpa menggunakan pendengar (listener) offchain apa pun.
...
- **Crowdsale**: token yang dikirim memicu alokasi imbalan instan.
...
- **Staking**: token yang ditransfer memicu penguncian otomatis dalam kontrak staking.
- **Pemungutan suara**: token yang diterima mendaftarkan suara dalam sistem tata kelola.
- **Penukaran**: persetujuan token mengaktifkan logika tukar dalam satu langkah.

Token ERC-1363 dapat digunakan untuk utilitas spesifik dalam semua kasus yang mewajibkan panggilan balik untuk dieksekusi setelah transfer atau persetujuan diterima.
...
Kontrak pintar yang mengimplementasikan standar ERC-1363 **WAJIB** mengimplementasikan semua fungsi dalam antarmuka `ERC1363`, serta antarmuka `ERC20` dan `ERC165`.
...
interface ERC1363 is ERC20, ERC165 {
  ...
  /**
   * @dev Menetapkan sejumlah `value` token sebagai jatah (allowance) `spender` atas token pemanggil
...
Kontrak pintar yang ingin menerima token ERC-1363 melalui `transferAndCall` atau `transferFromAndCall` **WAJIB** mengimplementasikan antarmuka `ERC1363Receiver`:

```solidity
/**
 * @title ERC1363Receiver
 * @dev Antarmuka untuk setiap kontrak yang ingin mendukung `transferAndCall` atau `transferFromAndCall` dari kontrak token ERC-1363.
 */
...
Kontrak pintar yang ingin menerima token ERC-1363 melalui `approveAndCall` **WAJIB** mengimplementasikan antarmuka `ERC1363Spender`:

```solidity
/**
 * @title ERC1363Spender
 * @dev Antarmuka untuk setiap kontrak yang ingin mendukung `approveAndCall` dari kontrak token ERC-1363.
 */
