---
title: Ethereum untuk Pengembang Python
description: Belajar cara menjadi developer di Ethereum dengan proyek dan perangkat berbasis Python
lang: id
incomplete: true
---

<FeaturedText>Pelajari cara mengembangkan di Ethereum dengan menggunakan proyek dan perangkat berbasis Python</FeaturedText>

Gunakan Ethereum untuk membuat aplikasi terdesentralisasi (atau "dapp") yang menggunakan keuntungan teknologi mata uang kripto dan blockchain. Dapp ini dapat dipercaya, yang berarti setelah diterapkan ke Ethereum, dapp akan selalu berjalan seperti yang telah diprogramkan. Dapp dapat mengontrol aset digital untuk membuat aplikasi keuangan jenis baru. Dapp juga terdesentralisasi, yang berarti tidak ada entitas atau orang yang mengontrolnya dan hampir mustahil untuk disensor.

## Memulai dengan smart contract dan bahasa Solidity{#getting-started-with-smart-contracts-and-solidity}

**Ambillah langkah awal untuk mengintegrasikan Phyton dengan Ethereum**

Perlu penjelasan yang lebih mendasar? Lihat [ethereum.org/learn](/learn/) atau [ethereum.org/developers](/developers/).

- [Blockchain Explained](https://kauri.io/article/d55684513211466da7f8cc03987607d5/blockchain-explained)
- [memahami Smart Contracts](https://kauri.io/article/e4f66c6079e74a4a9b532148d3158188/ethereum-101-part-5-the-smart-contract)
- [Tulis Smart Contract pertamamu](https://kauri.io/article/124b7db1d0cf4f47b414f8b13c9d66e2/remix-ide-your-first-smart-contract)
- [Pelajar cara Complie dan Deploy Solidity](https://kauri.io/article/973c5f54c4434bb1b0160cff8c695369/understanding-smart-contract-compilation-and-deployment)
- [Laporan kondisi Python di rantai blok 2023](https://tradingstrategy.ai/blog/the-state-of-python-in-blockchain-in-2023)

## Artikel untuk pemula {#beginner-articles}

- [Ikhtisar web3.py](https://web3py.readthedocs.io/en/latest/overview.html)
- [Tur Ekosistem Python Ethereum](https://snakecharmers.ethereum.org/python-ecosystem/)
- [Panduan Pengembang (Python) untuk Ethereum](https://snakecharmers.ethereum.org/a-developers-guide-to-ethereum-pt-1/)
- [Layak Hadiah: Panduan Hackathon Python Ethereum](https://snakecharmers.ethereum.org/prize-worthy/)
- [Pengenalan Kontrak Pintar dengan Vyper](https://kauri.io/#collections/Getting%20Started/an-introduction-to-smart-contracts-with-vyper/)
- [Bagaimana cara mengembangkan kontrak Ethereum menggunakan Python Flask?](https://medium.com/coinmonks/how-to-develop-ethereum-contract-using-python-flask-9758fe65976e)
- [Pengenalan Web3.py · Ethereum untuk Pengembang Python](https://www.dappuniversity.com/articles/web3-py-intro)
- [Cara memanggil fungsi Kontrak Pintar menggunakan Python dan web3.py](https://stackoverflow.com/questions/57580702/how-to-call-a-smart-contract-function-using-python-and-web3-py)

## Artikel menengah {#intermediate-articles}

- [Teman-teman web3.py: Pengenalan Ape](https://snakecharmers.ethereum.org/intro-to-ape/)
- [Pengembangan Dapp untuk Pemrogram Python](https://levelup.gitconnected.com/dapps-development-for-python-developers-f52b32b54f28)
- [Membuat Antarmuka Ethereum Python: Bagian 1](https://hackernoon.com/creating-a-python-ethereum-interface-part-1-4d2e47ea0f4d)
- [Kontrak Pintar Ethereum dalam Python: panduan komprehensif (kurang lebih)](https://hackernoon.com/ethereum-smart-contracts-in-python-a-comprehensive-ish-guide-771b03990988)

## Pola penggunaan tingkat lanjut {#advanced-use-patterns}

- [Pola web3.py: Langganan Aksi Waktu Nyata](https://snakecharmers.ethereum.org/subscriptions/)
- [Pola web3.py: WebSocketProvider](https://snakecharmers.ethereum.org/websocketprovider/)
- [Mengompilasi, menyebarkan, dan memanggil kontrak pintar Ethereum menggunakan Python](https://yohanes.gultom.id/2018/11/28/compiling-deploying-and-calling-ethereum-smartcontract-using-python/)
- [Analisis Kontrak Pintar Solidity dengan Slither](https://kauri.io/#collections/DevOps/analyze-solidity-smart-contracts-with-slither/#analyze-solidity-smart-contracts-with-slither)
- [Tutorial Fintech Rantai Blok: Pinjam Meminjam Dengan Python](https://blog.chain.link/blockchain-fintech-defi-tutorial-lending-borrowing-python/)

## Artikel yang di arsip

- [Sebarkan Token ERC20 Anda sendiri dengan Python dan Brownie](https://betterprogramming.pub/python-blockchain-token-deployment-tutorial-create-an-erc20-77a5fd2e1a58)
- [Menggunakan Brownie dan Python untuk menyebarkan Kontrak Pintar](https://dev.to/patrickalphac/using-brownie-for-to-deploy-smart-contracts-1kkp)
- [Membuat NFT di OpenSea dengan Brownie](https://www.freecodecamp.org/news/how-to-make-an-nft-and-render-on-opensea-marketplace/)

## Proyek dan perangkat Python {#python-projects-and-tools}

### Aktif: {#active}

- [Web3.py](https://github.com/ethereum/web3.py) - _Pustaka Python untuk berinteraksi dengan Ethereum_
- [Vyper](https://github.com/ethereum/vyper/) - _Bahasa Kontrak Pintar Pythonic untuk EVM_
- [Ape](https://github.com/ApeWorX/ape) - _Alat pengembangan kontrak pintar untuk para Pythonista, Ilmuwan Data, dan Profesional Keamanan_
- [py-evm](https://github.com/ethereum/py-evm) - _implementasi Mesin Virtual Ethereum_
- [eth-tester](https://github.com/ethereum/eth-tester) - _perangkat untuk menguji aplikasi berbasis Ethereum_
- [eth-utils](https://github.com/ethereum/eth-utils/) - _fungsi utilitas untuk bekerja dengan basis kode terkait Ethereum_
- [py-solc-x](https://pypi.org/project/py-solc-x/) - _Pembungkus Python di sekitar kompiler solc solidity dengan dukungan 0.5.x_
- [pymaker](https://github.com/makerdao/pymaker) - _API Python untuk kontrak Maker_
- [siwe](https://github.com/signinwithethereum/siwe-py) - _Masuk dengan Ethereum (siwe) untuk Python_
- [Web3 DeFi for Ethereum integrations](https://github.com/tradingstrategy-ai/web3-ethereum-defi) - _Paket Python dengan integrasi siap pakai untuk ERC-20, Uniswap, dan proyek populer lainnya_
- [Wake](https://getwake.io) - _Kerangka kerja Python lengkap untuk pengujian kontrak, fuzzing, penyebaran, pemindaian kerentanan, dan navigasi kode (server bahasa - [Perangkat untuk Solidity](https://marketplace.visualstudio.com/items?itemName=AckeeBlockchain.tools-for-solidity))_

### Diarsipkan / Tidak lagi dipelihara: {#archived--no-longer-maintained}

- [Trinity](https://github.com/ethereum/trinity) - _Klien Python Ethereum_
- [Mamba](https://github.com/arjunaskykok/mamba) - _kerangka kerja untuk menulis, mengompilasi, dan menyebarkan kontrak pintar yang ditulis dalam bahasa Vyper_
- [Brownie](https://github.com/eth-brownie/brownie) - _Kerangka kerja Python untuk menyebarkan, menguji, dan berinteraksi dengan kontrak pintar Ethereum_
- [pydevp2p](https://github.com/ethereum/pydevp2p) - _implementasi tumpukan P2P Ethereum_
- [py-wasm](https://github.com/ethereum/py-wasm) - _implementasi Python untuk penerjemah perakitan web_

Ingin mencari informasi tambahan? Kunjungi [ethereum.org/developers](/developers/).

## Proyek yang menggunakan perangkat Python {#projects-using-python-tooling}

Proyek berbasis Ethereum berikut ini menggunakan peralatan yang disebutkan di halaman ini. Repositori sumber terbuka terkait yang berfungsi sebagai referensi yang bagus untuk kode percontohan dan praktik terbaik.

- [Yearn Finance](https://yearn.finance/) dan [repositori Kontrak Vault Yearn](https://github.com/yearn/yearn-vaults)
- [Curve](https://www.curve.finance/) dan [repositori kontrak pintar Curve](https://github.com/curvefi/curve-contract)
- [BadgerDAO](https://badger.com/) dan [kontrak pintar menggunakan toolchain Brownie](https://github.com/Badger-Finance/badger-system)
- [Sushi](https://sushi.com/) menggunakan [Python dalam mengelola dan menyebarkan kontrak vesting mereka](https://github.com/sushiswap/sushi-vesting-protocols)
- [Alpha Finance](https://alphafinance.io/), yang terkenal dengan Alpha Homora, menggunakan [Brownie untuk menguji dan menyebarkan kontrak pintar](https://github.com/AlphaFinanceLab/alpha-staking-contract)

## Diskusi Komunitas Python {#python-community-contributors}

- [Discord Komunitas Python Ethereum](https://discord.gg/9zk7snTfWe) untuk diskusi Web3.py dan kerangka kerja Python lainnya
- [Discord Vyper](https://discord.gg/SdvKC79cJk) untuk diskusi pemrograman kontrak pintar Vyper

## Daftar agregat lainnya {#other-aggregated-lists}

Wiki Vyper memiliki [daftar sumber daya yang luar biasa untuk Vyper](https://github.com/vyperlang/vyper/wiki/Vyper-tools-and-resources)