...
description: Dalam tutorial ini, kami membedah token penipuan untuk melihat beberapa trik yang dimainkan oleh penipu, bagaimana mereka mengimplementasikannya, dan bagaimana kita dapat mendeteksinya.
author: Ori Pomerantz
tags: ["penipuan", "solidity", "erc-20", "javascript", "typescript"]
skill: intermediate
breadcrumb: Trik token penipuan
...
Jika pemilik kontrak mengirim token, mengapa peristiwa `Transfer` menunjukkan bahwa token tersebut berasal dari `deployer`?

Namun, ada masalah yang lebih penting. Siapa yang memanggil fungsi `_transfer` ini? Ia tidak dapat dipanggil dari luar, ia ditandai `internal`. Dan kode yang kita miliki tidak menyertakan panggilan apa pun ke `_transfer`. Jelas, ia ada di sini sebagai umpan.
...
### Fungsi peristiwa palsu `dropNewTokens` {#the-fake-events-function-dropnewtokens}

Sekarang kita sampai pada sesuatu yang terlihat seperti penipuan yang sebenarnya. Saya mengedit fungsinya sedikit agar mudah dibaca, tetapi secara fungsional setara.
...
Pembatasan ini sangat masuk akal, karena kita tidak ingin akun acak mendistribusikan token. Namun, sisa fungsinya mencurigakan.
...
Kontrak ERC-20 seharusnya memiliki [fungsi `approve`](/developers/tutorials/erc20-annotated-code/#approve) untuk jatah, dan memang token penipuan kita memiliki fungsi seperti itu, dan bahkan benar. Namun, karena Solidity diturunkan dari C, ia peka terhadap huruf besar-kecil. "Approve" dan "approve" adalah string yang berbeda.
...
Fungsi ini dipanggil dengan larik alamat untuk pemegang token.
...
Melihat `require`, kita melihat bahwa hanya pemilik kontrak yang diizinkan untuk mencetak. Itu sah. Tetapi pesan kesalahannya seharusnya _only owner is allowed to mint_ atau semacamnya. Sebaliknya, itu adalah _ERC20: mint to the zero address_ yang tidak relevan. Pengujian yang benar untuk pencetakan ke alamat nol adalah `require(account != address(0), "<error message>")`, yang tidak pernah diperiksa oleh kontrak.
...
Ada dua fakta mencurigakan lagi, yang terkait langsung dengan pencetakan:

- Ada parameter `account`, yang mungkin merupakan akun yang seharusnya menerima jumlah yang dicetak. Tetapi saldo yang meningkat sebenarnya adalah milik `contract_owner`.

- Meskipun saldo yang meningkat adalah milik `contract_owner`, peristiwa yang dipancarkan menunjukkan transfer ke `account`.

### Mengapa ada `auth` dan `approver`? Mengapa `mod` yang tidak melakukan apa-apa? {#why-both-autho-and-approver-why-the-mod-that-does-nothing}
...
`_mod_` mengambil tiga parameter dan tidak melakukan apa pun dengannya. Mengapa memilikinya?
...
`auth` dan `approver` lebih masuk akal, karena mereka memeriksa bahwa kontrak dipanggil oleh `contract_owner`. Kita mengharapkan tindakan istimewa tertentu, seperti pencetakan, dibatasi pada akun tersebut. Namun, apa gunanya memiliki dua fungsi terpisah yang melakukan _hal yang persis sama_?
...
Buat klien Viem. Kita hanya perlu membaca dari rantai blok, jadi klien ini tidak memerlukan kunci privat.
...
Alamat kontrak ERC-20 yang mencurigakan, dan blok di mana kita akan mencari peristiwa. Penyedia node biasanya membatasi kemampuan kita untuk membaca peristiwa karena bandwidth bisa menjadi mahal. Untungnya `wARB` tidak digunakan selama periode delapan belas jam, jadi kita dapat mencari semua peristiwa (hanya ada 13 secara total).
...
Ini adalah cara untuk meminta informasi peristiwa kepada Viem. Ketika kita memberikannya tanda tangan peristiwa yang tepat, termasuk nama bidang, ia mengurai peristiwa tersebut untuk kita.
...
Algoritma kita hanya berlaku untuk akun yang dimiliki secara eksternal. Jika ada kode bita yang dikembalikan oleh `client.getBytecode`, itu berarti ini adalah kontrak dan kita harus melewatinya saja.
...
Fungsi ini mendapatkan tanda terima transaksi dari sebuah peristiwa. Kita memerlukan tanda terima untuk memastikan kita tahu apa tujuan transaksinya.

```typescript
const suspiciousApprovalEvent = async (ev : Event) : (Event | null) => {
```

Ini adalah fungsi yang paling penting, yang benar-benar memutuskan apakah suatu peristiwa mencurigakan atau tidak. Tipe kembalian, `(Event | null)`, memberi tahu TypeScript bahwa fungsi ini dapat mengembalikan `Event` atau `null`. Kita mengembalikan `null` jika peristiwa tersebut tidak mencurigakan.

```typescript
const owner = ev.args._owner
```

Viem memiliki nama bidang, jadi ia mengurai peristiwa tersebut untuk kita. `_owner` adalah pemilik token yang akan dihabiskan.
...
```typescript
// Persetujuan tersebut mencurigakan jika berasal dari pemilik EOA yang bukan `from` transaksi
...
```typescript
// Hal ini juga mencurigakan jika tujuan transaksi bukan kontrak ERC-20 yang sedang kita
// selidiki
if (txn.to.toLowerCase() != testedAddress) return ev
```

Demikian pula, jika alamat `to` transaksi, kontrak pertama yang dipanggil, bukanlah kontrak ERC-20 yang sedang diselidiki maka itu mencurigakan.
...
Jika tidak ada kondisi yang benar maka peristiwa `Approval` tidak mencurigakan.
...
[Fungsi `async`](https://www.w3schools.com/js/js_async.asp) mengembalikan objek `Promise`. Dengan sintaks umum, `await x()`, kita menunggu `Promise` tersebut dipenuhi sebelum kita melanjutkan pemrosesan. Ini mudah diprogram dan diikuti, tetapi juga tidak efisien. Sementara kita menunggu `Promise` untuk peristiwa tertentu dipenuhi, kita sudah dapat mulai mengerjakan peristiwa berikutnya.

Di sini kita menggunakan [`map`](https://www.w3schools.com/jsref/jsref_map.asp) untuk membuat larik objek `Promise`. Kemudian kita menggunakan [`Promise.all`](https://www.javascripttutorial.net/es6/javascript-promise-all/) untuk menunggu semua janji tersebut diselesaikan. Kita kemudian [`filter`](https://www.w3schools.com/jsref/jsref_filter.asp) hasil tersebut untuk menghapus peristiwa yang tidak mencurigakan.

### Peristiwa `Transfer` yang mencurigakan {#suspicious-transfer-events}
...
Deteksi otomatis dapat membantu dalam kasus tertentu, seperti bagian DeFi, di mana ada banyak token dan mereka perlu ditangani secara otomatis. Tetapi seperti biasa [caveat emptor](https://www.investopedia.com/terms/c/caveatemptor.asp), lakukan riset Anda sendiri, dan dorong pengguna Anda untuk melakukan hal yang sama.
