...
Semuanya mencetak NFT mereka menggunakan API Alchemy yang canggih. Dalam tutorial ini, kami akan mengajarkan Anda cara melakukan hal yang sama dalam waktu \<10 menit.

"Mencetak NFT" adalah tindakan menerbitkan instans unik dari token ERC-721 Anda di rantai blok. Menggunakan kontrak pintar kami dari [Bagian 1 dari seri tutorial NFT ini](/developers/tutorials/how-to-write-and-deploy-an-nft/), mari kita asah keterampilan Web3 kita dan mencetak sebuah NFT. Di akhir tutorial ini, Anda akan dapat mencetak NFT sebanyak yang diinginkan hati (dan dompet) Anda!
...
Di dalam direktori scripts Anda, buat file `mint-nft.js` dan tambahkan baris kode berikut:
...
## Langkah 3: Dapatkan ABI kontrak Anda
...
![How to get your nft metadata hashcode on Pinata](./metadataPinata.gif)_Cara mendapatkan kode hash metadata nft Anda di Pinata_

> Periksa kembali apakah kode hash yang Anda salin menautkan ke **metadata.json** Anda dengan memuat `https://gateway.pinata.cloud/ipfs/<metadata-hash-code>` ke jendela terpisah. Halaman tersebut akan terlihat mirip dengan tangkapan layar di bawah ini:

![Your page should display the json metadata](./metadataJSON.png)_Halaman Anda akan menampilkan metadata json_
...
Sekarang, jalankan `node scripts/mint-nft.js` untuk menyebarkan NFT Anda. Setelah beberapa detik, Anda akan melihat respons seperti ini di terminal Anda:
