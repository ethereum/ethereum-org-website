...
- Solidity
- JavaScript
- React dan WAGMI. Jika Anda belum familier dengan alat antarmuka pengguna ini, [kami memiliki tutorial untuk itu](/developers/tutorials/creating-a-wagmi-ui-for-your-contract/).
...
Hook React [`useCallback`](https://react.dev/reference/react/useCallback) memungkinkan kita meningkatkan performa dengan menggunakan kembali fungsi yang sama saat komponen digambar ulang.
...
- `verifyingContract` adalah alamat kontrak yang akan memverifikasi tanda tangan ini. Kita tidak ingin tanda tangan yang sama berlaku untuk beberapa kontrak, seandainya ada beberapa kontrak `Greeter` dan kita ingin semuanya memiliki sapaan yang berbeda.
...
Keluarkan responsnya. Pada sistem produksi, kita juga akan menampilkan respons tersebut kepada pengguna.

### Server {#server}

Saya suka menggunakan [Vite](https://vite.dev/) sebagai front-end saya. Vite secara otomatis menyajikan pustaka React dan memperbarui peramban saat kode front-end berubah. Namun, Vite tidak menyertakan perkakas backend.
...
```js
  ...
  // Biarkan Vite menangani semua hal lainnya
...
Ini hanyalah panggilan rantai blok [viem](https://viem.sh/) standar.
...
```solidity
        // Terapkan sapaan seolah-olah penandatangan memanggilnya
...
Ini _bukanlah_ kode tingkat produksi. Kode ini rentan terhadap serangan yang signifikan dan tidak memiliki fitur-fitur utama. Berikut adalah beberapa di antaranya, beserta cara menyelesaikannya.
...
Solusi lainnya adalah melacak alamat dan hanya mengizinkan tanda tangan dari pelanggan yang valid.

### Tanda tangan sapaan yang salah {#wrong-greeting-sigs}

Saat Anda mengeklik **Signature for wrong greeting**, Anda mengirimkan tanda tangan yang valid untuk alamat (`0xaA92c5d426430D4769c9E878C1333BDe3d689b3e`) dan sapaan (`Hello`) tertentu. Namun, ia mengirimkannya dengan sapaan yang berbeda. Hal ini membingungkan `ecrecover`, yang mengubah sapaan tetapi memiliki alamat yang salah.
...
Solusi lainnya adalah menambahkan stempel waktu ke data yang ditandatangani dan menerima tanda tangan sebagai valid hanya selama beberapa detik setelah stempel waktu tersebut. Ini lebih sederhana dan lebih murah, tetapi kita berisiko terkena serangan pemutaran ulang dalam jendela waktu tersebut, dan kegagalan transaksi yang sah jika jendela waktu terlampaui.
...
Bagaimanapun juga, ini harus menjadi keputusan desain yang disadari, bukan sekadar hasil dari tidak memikirkan masalah tersebut.

### Penanganan kesalahan {#error-handling}

Seorang pengguna mengirimkan sapaan. Mungkin sapaan itu diperbarui di blok berikutnya. Mungkin juga tidak. Kesalahan tidak terlihat. Pada sistem produksi, pengguna harus dapat membedakan antara kasus-kasus ini:
