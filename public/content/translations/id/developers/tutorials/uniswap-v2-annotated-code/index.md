...
[Uniswap v2](https://app.uniswap.org/whitepaper.pdf) dapat membuat pasar pertukaran antara dua token ERC-20 mana pun. Dalam artikel ini kita akan membahas kode sumber untuk kontrak yang mengimplementasikan protokol ini dan melihat mengapa mereka ditulis dengan cara seperti ini.
...
_Pedagang_ mengirimkan satu jenis token ke kolam dan menerima jenis lainnya (misalnya, mengirim **Token0** dan menerima **Token1**) dari kolam yang disediakan oleh penyedia likuiditas. Nilai tukar ditentukan oleh jumlah relatif **Token0** dan **Token1** yang dimiliki kolam tersebut. Selain itu, kolam mengambil persentase kecil sebagai imbalan untuk kolam likuiditas.

Ketika penyedia likuiditas menginginkan aset mereka kembali, mereka dapat membakar token kolam dan menerima kembali token mereka, termasuk bagian imbalan mereka.
...
Uniswap v2 dibagi menjadi dua komponen, inti dan periferal. Pembagian ini memungkinkan kontrak inti, yang menyimpan aset dan oleh karena itu _harus_ aman, menjadi lebih sederhana dan lebih mudah diaudit. Semua fungsionalitas tambahan yang dibutuhkan oleh pedagang kemudian dapat disediakan oleh kontrak periferal.
...
1. Tukar antara token yang berbeda
2. Tambahkan likuiditas ke pasar dan dapatkan imbalan berupa token likuiditas ERC-20 pertukaran pasangan
...
3. Identifikasi jumlah yang perlu diperdagangkan di setiap pertukaran di sepanjang jalur.
4. Lakukan iterasi pada jalur tersebut. Untuk setiap pertukaran di sepanjang jalan, ini mengirimkan token input dan kemudian memanggil fungsi `swap` dari pertukaran tersebut.
   Dalam sebagian besar kasus, alamat tujuan untuk token adalah pertukaran pasangan berikutnya di jalur tersebut. Pada pertukaran terakhir, itu adalah alamat yang diberikan oleh pedagang.
...
1. Berikan akun periphery jatah dalam jumlah yang akan ditambahkan ke kolam likuiditas.
2. Panggil salah satu fungsi `addLiquidity` dari kontrak periphery.

#### Di dalam kontrak periphery (UniswapV2Router02.sol) {#in-the-periphery-contract-uniswapv2router02sol-2}

3. Buat pertukaran pasangan baru jika diperlukan
4. Jika ada pertukaran pasangan yang sudah ada, hitung jumlah token yang akan ditambahkan. Ini seharusnya bernilai identik untuk kedua token, sehingga rasio token baru terhadap token yang ada sama.
5. Periksa apakah jumlahnya dapat diterima (pemanggil dapat menentukan jumlah minimum di mana mereka lebih memilih untuk tidak menambahkan likuiditas jika di bawah jumlah tersebut)
6. Panggil kontrak inti.

#### Di dalam kontrak inti (UniswapV2Pair.sol) {#in-the-core-contract-uniswapv2pairsol-2-2}
...
1. Berikan akun periphery jatah token likuiditas untuk dibakar sebagai ganti token yang mendasarinya.
2. Panggil salah satu fungsi `removeLiquidity` dari kontrak periphery.
...
4. Kirimkan token yang mendasarinya ke alamat tujuan secara proporsional dengan token yang dibakar. Misalnya, jika ada 1000 token A di dalam kolam, 500 token B, dan 90 token likuiditas, dan kita menerima 9 token untuk dibakar, kita membakar 10% dari token likuiditas sehingga kita mengirimkan kembali 100 token A dan 50 token B kepada pengguna.
...
[Kontrak ini](https://github.com/Uniswap/uniswap-v2-core/blob/master/contracts/UniswapV2Pair.sol) mengimplementasikan kolam sebenarnya yang menukar token. Ini adalah fungsionalitas inti Uniswap.
...
[Pustaka SafeMath](https://docs.openzeppelin.com/contracts/2.x/api/math) digunakan untuk menghindari overflow dan underflow. Hal ini penting karena jika tidak, kita mungkin akan berakhir pada situasi di mana sebuah nilai seharusnya `-1`, tetapi malah menjadi `2^256-1`.
...
Untuk menghindari kasus pembagian dengan nol, ada jumlah minimum token likuiditas yang selalu ada (tetapi dimiliki oleh akun nol). Jumlah tersebut adalah **MINIMUM_LIQUIDITY**, yaitu seribu.
...
```solidity
    uint112 private reserve0;           // menggunakan slot penyimpanan tunggal, dapat diakses melalui getReserves
    uint112 private reserve1;           // menggunakan slot penyimpanan tunggal, dapat diakses melalui getReserves
```

Cadangan yang dimiliki kolam untuk setiap jenis token. Kita berasumsi bahwa keduanya mewakili jumlah nilai yang sama, dan oleh karena itu setiap token0 bernilai reserve1/reserve0 token1.

```solidity
    uint32  private blockTimestampLast; // menggunakan slot penyimpanan tunggal, dapat diakses melalui getReserves
```
...
Variabel-variabel ini menyimpan biaya kumulatif untuk setiap token (masing-masing dalam nilai token lainnya). Variabel ini dapat digunakan untuk menghitung nilai tukar rata-rata selama periode waktu tertentu.

```solidity
    uint public kLast; // reserve0 * reserve1, tepat setelah peristiwa likuiditas terbaru
```

Cara pertukaran pasangan memutuskan nilai tukar antara token0 dan token1 adalah dengan menjaga kelipatan dari kedua cadangan tetap konstan selama perdagangan. `kLast` adalah nilai ini. Nilai ini berubah ketika penyedia likuiditas menyetor atau melakukan penarikan token, dan sedikit meningkat karena biaya pasar 0,3%.

Berikut adalah contoh sederhana. Perhatikan bahwa demi kesederhanaan, tabel ini hanya memiliki tiga digit di belakang koma, dan kita mengabaikan biaya perdagangan 0,3% sehingga angkanya tidak akurat.
...
Ada kelas kerentanan keamanan yang didasarkan pada [penyalahgunaan reentransi](https://medium.com/coinmonks/ethernaut-lvl-10-re-entrancy-walkthrough-how-to-abuse-execution-ordering-and-reproduce-the-dao-7ec88b912c14). Uniswap perlu mentransfer token ERC-20 secara sewenang-wenang, yang berarti memanggil kontrak ERC-20 yang mungkin mencoba menyalahgunakan pasar Uniswap yang memanggilnya.
...
Jika `unlocked` sama dengan satu, atur menjadi nol. Jika sudah nol, mengembalikan panggilan, membuatnya gagal.

```solidity
        _;
```

Dalam sebuah pengubah, `_;` adalah panggilan fungsi asli (dengan semua parameternya). Di sini berarti panggilan fungsi hanya terjadi jika `unlocked` bernilai satu saat dipanggil, dan saat berjalan nilai `unlocked` adalah nol.
...
Fungsi ini memberikan state pertukaran saat ini kepada pemanggil. Perhatikan bahwa fungsi Solidity [dapat mengembalikan beberapa nilai](https://docs.soliditylang.org/en/v0.8.3/contracts.html#returning-multiple-values).
...
Fungsi internal ini mentransfer sejumlah token ERC-20 dari pertukaran ke orang lain. `SELECTOR` menentukan bahwa fungsi yang kita panggil adalah `transfer(address,uint)` (lihat definisi di atas).
...
1. Mengembalikan. Jika panggilan ke kontrak eksternal mengembalikan, maka nilai kembalian boolean adalah `false`
2. Berakhir secara normal tetapi melaporkan kegagalan. Dalam hal ini, buffer nilai kembalian memiliki panjang bukan nol, dan ketika didekodekan sebagai nilai boolean, nilainya adalah `false`

Jika salah satu dari kondisi ini terjadi, mengembalikan.
...
Terakhir, `Sync` dipancarkan setiap kali token ditambahkan atau ditarik, apa pun alasannya, untuk memberikan informasi cadangan terbaru (dan karenanya nilai tukar).
...
```solidity
    // dipanggil sekali oleh factory pada saat penerapan
...
Jika waktu yang berlalu bukan nol, itu berarti kita adalah transaksi pertukaran pertama di blok ini. Dalam hal ini, kita perlu memperbarui akumulator biaya.
...
Perhitungan harga ini adalah alasan kita perlu mengetahui ukuran cadangan lama.
...
Untuk mengurangi perhitungan (dan karenanya biaya gas), biaya ini hanya dihitung ketika likuiditas ditambahkan atau ditarik dari kolam, bukan pada setiap transaksi.
...
Variabel state `kLast` terletak di penyimpanan, sehingga akan memiliki nilai di antara panggilan yang berbeda ke kontrak.
...
Penyedia likuiditas mendapatkan bagian mereka hanya dari apresiasi token likuiditas mereka. Namun biaya protokol mewajibkan token likuiditas baru untuk dicetak dan diberikan ke alamat `feeTo`.
...
Gunakan fungsi `UniswapV2ERC20._mint` untuk benar-benar mencetak token likuiditas tambahan dan menetapkannya ke `feeTo`.
...
Fungsi ini dipanggil ketika penyedia likuiditas menambahkan likuiditas ke kolam. Ini mencetak token likuiditas tambahan sebagai imbalan. Ini harus dipanggil dari [kontrak pinggiran](#uniswapv2router02) yang memanggilnya setelah menambahkan likuiditas dalam transaksi yang sama (sehingga tidak ada orang lain yang dapat mengirimkan transaksi yang mengklaim likuiditas baru sebelum pemilik yang sah).

```solidity
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // penghematan gas
```

Ini adalah cara untuk membaca hasil dari fungsi Solidity yang mengembalikan beberapa nilai. Kita membuang nilai kembalian terakhir, yaitu stempel waktu blok, karena kita tidak membutuhkannya.
...
Hitung biaya protokol yang akan dikumpulkan, jika ada, dan cetak token likuiditas yang sesuai. Karena parameter untuk `_mintFee` adalah nilai cadangan lama, biaya dihitung secara akurat hanya berdasarkan perubahan kolam akibat biaya.
...
Kita dapat mempercayai hal ini karena merupakan kepentingan penyetor untuk memberikan nilai yang sama, guna menghindari hilangnya nilai akibat arbitrase.
Katakanlah nilai kedua token tersebut identik, tetapi penyetor kita menyetor **Token1** empat kali lebih banyak daripada **Token0**. Seorang pedagang dapat menggunakan fakta bahwa pertukaran pasangan menganggap **Token0** lebih berharga untuk mengekstrak nilai darinya.
...
Dengan setiap setoran berikutnya, kita sudah mengetahui nilai tukar antara kedua aset tersebut, dan kita mengharapkan penyedia likuiditas untuk memberikan nilai yang sama di keduanya. Jika tidak, kita memberi mereka token likuiditas berdasarkan nilai yang lebih kecil yang mereka berikan sebagai hukuman.

Baik itu setoran awal atau setoran berikutnya, jumlah token likuiditas yang kita berikan sama dengan akar kuadrat dari perubahan `reserve0*reserve1` dan nilai token likuiditas tidak berubah (kecuali kita mendapatkan setoran yang tidak memiliki nilai yang sama dari kedua jenis, dalam hal ini "denda" didistribusikan). Berikut adalah contoh lain dengan dua token yang memiliki nilai yang sama, dengan tiga setoran yang baik dan satu yang buruk (setoran hanya satu jenis token, sehingga tidak menghasilkan token likuiditas apa pun).

| Peristiwa                 | reserve0 | reserve1 | reserve0 \* reserve1 | Nilai kolam (reserve0 + reserve1) | Token likuiditas yang dicetak untuk setoran ini | Total token likuiditas | nilai setiap token likuiditas |
| ------------------------- | -------: | -------: | -------------------: | --------------------------------: | ----------------------------------------------: | ---------------------: | ----------------------------: |
...
Gunakan fungsi `UniswapV2ERC20._mint` untuk benar-benar mencetak token likuiditas tambahan dan memberikannya ke akun yang benar.

```solidity

        _update(balance0, balance1, _reserve0, _reserve1);
        if (feeOn) kLast = uint(reserve0).mul(reserve1); // reserve0 dan reserve1 mutakhir
...
Ini juga harus dipanggil [dari akun pinggiran](#uniswapv2router02).
...
Kontrak pinggiran mentransfer likuiditas yang akan dibakar ke kontrak ini sebelum panggilan. Dengan begitu kita tahu berapa banyak likuiditas yang harus dibakar, dan kita dapat memastikan bahwa itu dibakar.
...
Fungsi ini juga seharusnya dipanggil dari [kontrak pinggiran](#uniswapv2router02).
...
```solidity
        ...
        uint balance0;
        uint balance1;
        { // cakupan untuk _token{0,1}, menghindari kesalahan stack too deep
```
...
Dapatkan saldo saat ini. Kontrak pinggiran mengirimi kita token sebelum memanggil kita untuk pertukaran. Hal ini memudahkan kontrak untuk memeriksa bahwa ia tidak ditipu, sebuah pemeriksaan yang _harus_ terjadi di kontrak inti (karena kita dapat dipanggil oleh entitas lain selain kontrak pinggiran kita).

```solidity
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'UniswapV2: INSUFFICIENT_INPUT_AMOUNT');
        { // cakupan untuk reserve{0,1}Adjusted, menghindari kesalahan stack too deep
            uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3));
            uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3));
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'UniswapV2: K');
```

Ini adalah pemeriksaan kewarasan (sanity check) untuk memastikan kita tidak rugi dari pertukaran. Tidak ada keadaan di mana pertukaran harus mengurangi `reserve0*reserve1`. Di sinilah kita juga memastikan biaya 0,3% dikirim pada pertukaran; sebelum memeriksa kewarasan nilai K, kita mengalikan kedua saldo dengan 1000 dikurangi jumlah yang dikalikan dengan 3, ini berarti 0,3% (3/1000 = 0,003 = 0,3%) dikurangkan dari saldo sebelum membandingkan nilai K-nya dengan nilai K cadangan saat ini.
...
- `skim`, tarik jumlah ekstra. Perhatikan bahwa akun mana pun diizinkan untuk memanggil `skim` karena kita tidak tahu siapa yang menyetor token. Informasi ini dipancarkan dalam sebuah peristiwa, tetapi peristiwa tidak dapat diakses dari rantai blok.
...
Variabel state ini diperlukan untuk mengimplementasikan biaya protokol (lihat [buku putih](https://app.uniswap.org/whitepaper.pdf), hal. 5).
...
Satu-satunya hal yang dilakukan konstruktor adalah menentukan `feeToSetter`. Pabrik dimulai tanpa biaya, dan hanya `feeSetter` yang dapat mengubahnya.
...
Kita ingin alamat pertukaran baru bersifat deterministik, sehingga dapat dihitung sebelumnya secara offchain (ini dapat berguna untuk [transaksi lapisan 2 (l2)](/developers/docs/scaling/)).
...
Kolam likuiditas yang besar lebih baik daripada yang kecil, karena harganya lebih stabil. Kita tidak ingin memiliki lebih dari satu kolam likuiditas per pasangan token. Jika sudah ada pertukaran, tidak perlu membuat yang lain untuk pasangan yang sama.
...
```solidity
        ...
        getPair[token1][token0] = pair; // isi pemetaan ke arah sebaliknya
        allPairs.push(pair);
        emit PairCreated(token0, token1, pair, allPairs.length);
    }
```

Simpan informasi pasangan baru dalam variabel state dan pancarkan peristiwa untuk memberi tahu dunia tentang pertukaran pasangan baru.
...
Tidak mungkin bagi penerima untuk memalsukan tanda tangan digital. Namun, sangat mudah untuk mengirim transaksi yang sama dua kali (ini adalah bentuk [serangan pemutaran ulang](https://wikipedia.org/wiki/Replay_attack)). Untuk mencegah hal ini, kita menggunakan [nonce](https://wikipedia.org/wiki/Cryptographic_nonce). Jika nonce dari `Permit` baru tidak lebih dari satu dari yang terakhir digunakan, kita menganggapnya tidak valid.
...
Modifier ini memastikan bahwa transaksi dengan batas waktu ("lakukan X sebelum waktu Y jika Anda bisa") tidak terjadi setelah batas waktunya habis.
...
Konstruktor hanya mengatur variabel state yang tidak dapat diubah.
...
Ini adalah jumlah minimum yang dapat diterima untuk disetorkan. Jika transaksi tidak dapat berlangsung dengan jumlah ini atau lebih, kembalikan transaksi tersebut. Jika Anda tidak menginginkan fitur ini, cukup tentukan nol.

Penyedia likuiditas biasanya menentukan nilai minimum, karena mereka ingin membatasi transaksi pada nilai tukar yang mendekati nilai tukar saat ini. Jika nilai tukar berfluktuasi terlalu banyak, itu mungkin berarti ada berita yang mengubah nilai yang mendasarinya, dan mereka ingin memutuskan secara manual apa yang harus dilakukan.
...
Selama nilai tukar tetap berada di antara 0,9 dan 1,25, transaksi akan berlangsung. Jika nilai tukar keluar dari kisaran tersebut, transaksi akan dibatalkan.

Alasan dari tindakan pencegahan ini adalah karena transaksi tidak terjadi secara instan, Anda mengirimkannya dan pada akhirnya seorang validator akan memasukkannya ke dalam sebuah blok (kecuali jika harga gas Anda sangat rendah, dalam hal ini Anda perlu mengirimkan transaksi lain dengan nonce yang sama dan harga gas yang lebih tinggi untuk menimpanya). Anda tidak dapat mengontrol apa yang terjadi selama interval antara pengiriman dan penyertaan.
...
Jika cadangan saat ini kosong, maka ini adalah pertukaran pasangan yang baru. Jumlah yang akan disetorkan harus sama persis dengan jumlah yang ingin diberikan oleh penyedia likuiditas.
...
Anda dapat menyetorkan likuiditas secara langsung ke dalam kontrak inti (menggunakan [UniswapV2Pair::mint](https://github.com/Uniswap/uniswap-v2-core/blob/master/contracts/UniswapV2Pair.sol#L110)), tetapi kontrak inti hanya memeriksa bahwa ia tidak dicurangi, sehingga Anda berisiko kehilangan nilai jika nilai tukar berubah antara waktu Anda mengirimkan transaksi dan waktu transaksi tersebut dieksekusi. Jika Anda menggunakan kontrak periphery, kontrak tersebut akan menghitung jumlah yang harus Anda setorkan dan segera menyetorkannya, sehingga nilai tukar tidak berubah dan Anda tidak kehilangan apa pun.
...
Fungsi ini dapat dipanggil oleh sebuah transaksi untuk menyetorkan likuiditas. Sebagian besar parameter sama seperti pada `_addLiquidity` di atas, dengan dua pengecualian:

. `to` adalah alamat yang mendapatkan token likuiditas baru yang dicetak untuk menunjukkan porsi penyedia likuiditas di dalam kolam
...
Sebagai imbalannya, berikan alamat `to` token likuiditas untuk kepemilikan sebagian dari kolam tersebut. Fungsi `mint` dari kontrak inti melihat berapa banyak token ekstra yang dimilikinya (dibandingkan dengan apa yang dimilikinya saat terakhir kali likuiditas berubah) dan mencetak likuiditas yang sesuai.
...
Tidak masalah untuk melakukan transfer terlebih dahulu dan kemudian memverifikasi bahwa itu sah, karena jika tidak, kita akan mengembalikan semua perubahan state.
...
Fungsi ini dapat digunakan untuk token yang memiliki biaya transfer atau penyimpanan. Ketika sebuah token memiliki biaya semacam itu, kita tidak dapat mengandalkan fungsi `removeLiquidity` untuk memberi tahu kita berapa banyak token yang kita dapatkan kembali, jadi kita perlu menariknya terlebih dahulu dan kemudian mendapatkan saldonya.
...
```solidity
    // **** TUKAR ****
    // mengharuskan jumlah awal sudah dikirim ke pasangan pertama
...
Dapatkan pasangan yang sedang kita tangani, urutkan (untuk digunakan dengan pasangan tersebut) dan dapatkan jumlah keluaran yang diharapkan.

```solidity
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
```

Dapatkan jumlah keluaran yang diharapkan, diurutkan sesuai dengan yang diharapkan oleh pertukaran pasangan.
...
Benar-benar memanggil pertukaran pasangan untuk menukar token. Kita tidak memerlukan callback untuk diberi tahu tentang pertukaran tersebut, jadi kita tidak mengirimkan bita apa pun di bidang itu.
...
Parameter ini berisi alamat-alamat dari kontrak ERC-20. Seperti yang dijelaskan di atas, ini adalah sebuah array karena Anda mungkin perlu melalui beberapa pertukaran pasangan untuk mendapatkan aset yang Anda inginkan dari aset yang Anda miliki.

Parameter fungsi dalam Solidity dapat disimpan baik di `memory` maupun di `calldata`. Jika fungsi tersebut adalah titik masuk ke kontrak, dipanggil langsung dari pengguna (menggunakan transaksi) atau dari kontrak yang berbeda, maka nilai parameter dapat diambil langsung dari data panggilan. Jika fungsi dipanggil secara internal, seperti `_swap` di atas, maka parameter harus disimpan di `memory`. Dari perspektif kontrak yang dipanggil, `calldata` bersifat hanya baca.
...
Hitung jumlah yang akan dibeli dalam setiap pertukaran. Jika hasilnya kurang dari minimum yang bersedia diterima oleh pedagang, kembalikan transaksi tersebut.
...
Fungsi sebelumnya, `swapTokensForTokens`, memungkinkan pedagang untuk menentukan jumlah pasti token masukan yang bersedia ia berikan dan jumlah minimum token keluaran yang bersedia ia terima sebagai imbalannya. Fungsi ini melakukan pertukaran sebaliknya, ini memungkinkan pedagang menentukan jumlah token keluaran yang ia inginkan, dan jumlah maksimum token masukan yang bersedia ia bayarkan untuk itu.

Dalam kedua kasus tersebut, pedagang harus terlebih dahulu memberikan jatah kepada kontrak periphery ini untuk memungkinkannya mentransfer token tersebut.
...
```solidity
        for (uint i; i < path.length - 1; i++) {
            ...
            uint amountInput;
            uint amountOutput;
            { // cakupan untuk menghindari kesalahan stack too deep
...
Catatan: Secara teori kita bisa saja menggunakan fungsi ini alih-alih `_swap`, tetapi dalam kasus tertentu (misalnya, jika transfer akhirnya dikembalikan karena tidak ada cukup dana pada akhirnya untuk memenuhi minimum yang diwajibkan) hal itu akan berakhir dengan menghabiskan lebih banyak gas. Token dengan biaya transfer cukup langka, jadi meskipun kita perlu mengakomodasinya, tidak perlu bagi semua pertukaran untuk berasumsi bahwa mereka melalui setidaknya satu dari token tersebut.
...
// pustaka untuk melakukan berbagai operasi matematika
...
Dapatkan perkiraan yang lebih dekat, yaitu rata-rata dari perkiraan sebelumnya dan angka yang akar kuadratnya sedang kita cari dibagi dengan perkiraan sebelumnya. Ulangi hingga perkiraan baru tidak lebih rendah dari yang sudah ada. Untuk detail lebih lanjut, [lihat di sini](https://wikipedia.org/wiki/Methods_of_computing_square_roots#Babylonian_method).
...
Kita seharusnya tidak pernah membutuhkan akar kuadrat dari nol. Akar kuadrat dari satu, dua, dan tiga kira-kira adalah satu (kita menggunakan bilangan bulat, jadi kita mengabaikan pecahannya).
...
// pustaka untuk menangani angka titik tetap biner (https://wikipedia.org/wiki/Q_(number_format))
...
```solidity
    // enkode uint112 sebagai UQ112x112
    function encode(uint112 y) internal pure returns (uint224 z) {
        z = uint224(y) * Q112; // tidak pernah overflow
    }
```

Karena y adalah `uint112`, nilai maksimumnya adalah 2^112-1. Angka tersebut masih dapat disandikan sebagai `UQ112x112`.

```solidity
    // bagi UQ112x112 dengan uint112, mengembalikan UQ112x112
...
library UniswapV2Library {
    ...
    // mengembalikan alamat token yang diurutkan, digunakan untuk menangani nilai kembalian dari pasangan yang diurutkan dalam urutan ini
...
Urutkan kedua token berdasarkan alamat, sehingga kita akan bisa mendapatkan alamat pertukaran pasangan untuk keduanya. Hal ini diperlukan karena jika tidak, kita akan memiliki dua kemungkinan, satu untuk parameter A,B dan satu lagi untuk parameter B,A, yang mengarah pada dua pertukaran alih-alih satu.

```solidity
    // menghitung alamat CREATE2 untuk pasangan tanpa melakukan panggilan eksternal apa pun
...
```solidity
    // diberikan sejumlah aset dan cadangan pasangan, mengembalikan jumlah yang setara dari aset lainnya
...
```solidity
    // diberikan jumlah input aset dan cadangan pasangan, mengembalikan jumlah output maksimum dari aset lainnya
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
```

Fungsi `quote` di atas berfungsi dengan baik jika tidak ada biaya untuk menggunakan pertukaran pasangan. Namun, jika ada biaya pertukaran sebesar 0,3%, jumlah yang sebenarnya Anda dapatkan akan lebih rendah. Fungsi ini menghitung jumlah setelah biaya pertukaran.
...
Solidity tidak menangani pecahan secara bawaan, jadi kita tidak bisa begitu saja mengalikan jumlahnya dengan 0,997. Sebagai gantinya, kita mengalikan pembilang dengan 997 dan penyebut dengan 1000, untuk mencapai efek yang sama.

```solidity
    // diberikan jumlah output aset dan cadangan pasangan, mengembalikan jumlah input yang diperlukan dari aset lainnya
...
Fungsi ini melakukan hal yang kurang lebih sama, tetapi fungsi ini mendapatkan jumlah keluaran dan menyediakan masukannya.
...
- Menggunakan [antarmuka biner aplikasi (ABI)](https://docs.soliditylang.org/en/v0.8.3/abi-spec.html) secara "manual" untuk membuat panggilan. Inilah yang diputuskan oleh penulis kode untuk dilakukan.
...
Demi kompatibilitas mundur dengan token yang dibuat sebelum standar ERC-20, panggilan ERC-20 dapat gagal baik karena mengembalikan (dalam hal ini `success` bernilai `false`) atau karena berhasil dan mengembalikan nilai `false` (dalam hal ini terdapat data keluaran, dan jika Anda mendekodenya sebagai boolean, Anda mendapatkan `false`).
...
Sekarang buatlah sesuatu yang berguna dan buat kami takjub.
