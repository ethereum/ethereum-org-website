...
**Ethereum on Arm adalah citra (image) Linux kustom yang dapat mengubah Raspberry Pi menjadi node Ethereum.**
...
**Citra (image) menangani semua langkah yang diperlukan**, mulai dari menyiapkan lingkungan dan memformat disk SSD hingga menginstal dan menjalankan perangkat lunak Ethereum serta memulai sinkronisasi rantai blok.

## Catatan tentang klien eksekusi dan konsensus {#note-on-execution-and-consensus-clients}

Citra Ethereum on Arm mencakup klien eksekusi dan klien konsensus yang sudah dibangun sebelumnya sebagai layanan. Sebuah node Ethereum memerlukan kedua klien tersebut untuk disinkronisasi dan dijalankan. Anda hanya perlu mengunduh dan mem-flash citra tersebut lalu memulai layanannya. Citra ini sudah dimuat sebelumnya dengan klien eksekusi berikut:
...
Citra Ethereum Raspberry Pi 4 adalah citra "plug and play" yang secara otomatis menginstal dan menyiapkan klien eksekusi dan klien konsensus, mengonfigurasinya untuk saling berkomunikasi dan terhubung ke jaringan Ethereum. Pengguna hanya perlu memulai prosesnya menggunakan perintah sederhana.
...
Geth telah dikonfigurasi sebelumnya untuk melaporkan metrik ke dasbor Grafana yang dapat dilihat di peramban. Pengguna tingkat lanjut mungkin ingin menggunakan fitur ini untuk memantau kesehatan node mereka dengan menavigasi ke `ipaddress:3000`, melewati `user: admin` dan `passwd: ethereum`.

## Validator {#validators}

Sebuah validator juga dapat ditambahkan secara opsional ke klien konsensus. Perangkat lunak validator memungkinkan node Anda untuk berpartisipasi aktif dalam konsensus dan memberikan keamanan kriptoekonomi pada jaringan. Anda mendapatkan imbalan atas pekerjaan ini dalam bentuk ETH. Untuk menjalankan validator, Anda harus memiliki 32 ETH terlebih dahulu, yang harus disetorkan ke dalam kontrak deposit. Deposit dapat dilakukan dengan mengikuti panduan langkah demi langkah di [Launchpad](https://launchpad.ethereum.org/). Lakukan ini di desktop/laptop, tetapi jangan buat kunci — ini dapat dilakukan langsung di Raspberry Pi.
...
Halaman ini memberikan gambaran umum tentang cara menyiapkan node Geth-Lighthouse dan validator menggunakan Raspberry Pi. Instruksi yang lebih mendetail tersedia di [situs web Ethereum-on-Arm](https://ethereum-on-arm-documentation.readthedocs.io/en/latest/).

## Umpan balik sangat dihargai {#feedback-appreciated}

Kami tahu Raspberry Pi memiliki basis pengguna yang sangat besar yang dapat memberikan dampak yang sangat positif pada kesehatan jaringan Ethereum.
