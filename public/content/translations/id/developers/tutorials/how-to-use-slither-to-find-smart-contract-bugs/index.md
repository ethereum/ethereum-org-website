...
Tujuan dari tutorial ini adalah untuk menunjukkan cara menggunakan Slither untuk menemukan bug secara otomatis dalam kontrak pintar.
...
**Baris perintah versus skrip yang ditentukan pengguna.** Slither dilengkapi dengan serangkaian detektor bawaan yang menemukan banyak bug umum. Memanggil Slither dari baris perintah akan menjalankan semua detektor, tidak diperlukan pengetahuan mendetail tentang analisis statis:

```bash
slither project_paths
```

Selain detektor, Slither memiliki kemampuan tinjauan kode melalui [printer](https://github.com/crytic/slither#printers) dan [alat](https://github.com/crytic/slither#tools)-nya.
...
Analisis statis hadir dalam berbagai bentuk. Anda kemungkinan besar menyadari bahwa kompiler seperti [clang](https://clang-analyzer.llvm.org/) dan [gcc](https://lwn.net/Articles/806099/) bergantung pada teknik penelitian ini, tetapi ini juga mendasari ([Infer](https://fbinfer.com/), [CodeClimate](https://codeclimate.com/), [FindBugs](https://findbugs.sourceforge.net/) dan alat yang didasarkan pada metode formal seperti [Frama-C](https://frama-c.com/) dan [Polyspace](https://www.mathworks.com/products/polyspace.html).

Kita tidak akan mengulas teknik analisis statis dan penelitian secara mendalam di sini. Sebaliknya, kita akan berfokus pada apa yang diperlukan untuk memahami cara kerja Slither sehingga Anda dapat menggunakannya secara lebih efektif untuk menemukan bug dan memahami kode.
...
- [State variable shadowing](https://github.com/crytic/slither/wiki/Detector-Documentation#state-variable-shadowing): mengulangi semua variabel state dan memeriksa apakah ada yang membayangi (shadow) variabel dari kontrak yang diwariskan ([state.py#L51-L62](https://github.com/crytic/slither/blob/0441338e055ab7151b30ca69258561a5a793f8ba/slither/detectors/shadowing/state.py#L51-L62))

- [Antarmuka ERC-20 yang salah](https://github.com/crytic/slither/wiki/Detector-Documentation#incorrect-erc20-interface): mencari tanda tangan fungsi ERC-20 yang salah ([incorrect_erc20_interface.py#L34-L55](https://github.com/crytic/slither/blob/0441338e055ab7151b30ca69258561a5a793f8ba/slither/detectors/erc/incorrect_erc20_interface.py#L34-L55))

### Analisis semantik {#semantic-analysis}

Berbeda dengan analisis sintaksis, analisis semantik akan menggali lebih dalam dan menganalisis "makna" dari kode tersebut. Keluarga ini mencakup beberapa jenis analisis yang luas. Mereka mengarah pada hasil yang lebih kuat dan berguna, tetapi juga lebih kompleks untuk ditulis.
...
#### Komputasi titik tetap (Fixed-point) {#fixed-point-computation-2}
...
Analisis Anda perlu mengetahui kapan harus berhenti. Ada dua strategi utama di sini: (1) mengulangi setiap node dalam jumlah yang terbatas, (2) menghitung apa yang disebut _fixpoint_. Fixpoint pada dasarnya berarti bahwa menganalisis node ini tidak memberikan informasi yang berarti.

Contoh penggunaan fixpoint dapat ditemukan di detektor reentransi: Slither mengeksplorasi node, dan mencari panggilan eksternal, menulis dan membaca ke penyimpanan. Setelah mencapai fixpoint ([reentrancy.py#L125-L131](https://github.com/crytic/slither/blob/master/slither/detectors/reentrancy/reentrancy.py#L125-L131)), ia menghentikan eksplorasi, dan menganalisis hasil untuk melihat apakah ada reentransi, melalui pola reentransi yang berbeda ([reentrancy_benign.py](https://github.com/crytic/slither/blob/b275bcc824b1b932310cf03b6bfb1a1fef0ebae1/slither/detectors/reentrancy/reentrancy_benign.py), [reentrancy_read_before_write.py](https://github.com/crytic/slither/blob/b275bcc824b1b932310cf03b6bfb1a1fef0ebae1/slither/detectors/reentrancy/reentrancy_read_before_write.py), [reentrancy_eth.py](https://github.com/crytic/slither/blob/b275bcc824b1b932310cf03b6bfb1a1fef0ebae1/slither/detectors/reentrancy/reentrancy_eth.py)).
...
Representasi perantara (IR) adalah bahasa yang dimaksudkan agar lebih sesuai untuk analisis statis daripada bahasa aslinya. Slither menerjemahkan Solidity ke IR-nya sendiri: [SlithIR](https://github.com/crytic/slither/wiki/SlithIR).
...
Slither memiliki API yang memungkinkan Anda menjelajahi atribut dasar dari kontrak dan fungsi-fungsinya.
...
- `get_modifier_from_signature (str)`: Mengembalikan Pengubah (Modifier) dari tanda tangannya
...
- `nodes (list(Node))`: Daftar node yang menyusun CFG dari fungsi/pengubah
...
- `state_variables_read (list(StateVariable))`: Daftar variabel state yang dibaca (subset dari variables`read)
- `state_variables_written (list(StateVariable))`: Daftar variabel state yang ditulis (subset dari variables`written)