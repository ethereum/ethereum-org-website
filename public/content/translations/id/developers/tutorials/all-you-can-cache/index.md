...
tags: ["layer 2", "caching", "penyimpanan", "penskalaan"]
skill: intermediate
breadcrumb: Caching untuk rollup
...
3. Untuk nilai lainnya, ambil empat bit teratas sebagai jumlah byte tambahan, dan empat bit terbawah sebagai bit paling signifikan dari kunci cache. Berikut adalah beberapa contoh:
...
```solidity
    ...
    // nol sebagai "tidak ada di dalam cache".
    uint[] public key2val;
```
...
```solidity
    ...
    // Hanya publik untuk memungkinkan pengujian berfungsi
    function cacheWrite(uint _value) public returns (uint) {
        // Jika nilai sudah ada di dalam cache, kembalikan kunci saat ini
        if (val2key[_value] != 0) {
...
```solidity
        // Karena 0xFE adalah kasus khusus, kunci terbesar yang dapat
        // ditampung cache adalah 0x0D diikuti oleh 15 0xFF. Jika panjang cache sudah sebesar itu,
        // gagal.
...
Fungsi ini bersifat internal, jadi jika sisa kode ditulis dengan benar, pengujian ini tidak diperlukan. Namun, biayanya tidak mahal jadi sebaiknya kita menyertakannya.
...
Kode ini ada di [Yul](https://docs.soliditylang.org/en/v0.8.16/yul.html). Kode ini membaca nilai 32 byte dari calldata. Ini berfungsi bahkan jika calldata berhenti sebelum `startByte+32` karena ruang yang tidak diinisialisasi di EVM dianggap nol.
...
```solidity
    ...
    // Baca parameter tunggal dari data panggilan, dimulai dari _fromByte
    function _readParam(uint _fromByte) internal
        returns (uint _nextByte, uint _parameterValue)
    {
```

Membaca parameter tunggal dari calldata. Perhatikan bahwa kita perlu mengembalikan bukan hanya nilai yang kita baca, tetapi juga lokasi byte berikutnya karena panjang parameter dapat berkisar dari 1 byte hingga 33 byte.

```solidity
        // Byte pertama memberi tahu kita cara menafsirkan sisanya
        uint8 _firstByte;

        _firstByte = uint8(_calldataVal(_fromByte, 1));
```

Solidity mencoba mengurangi jumlah bug dengan melarang [konversi tipe implisit](https://docs.soliditylang.org/en/v0.8.16/types.html#implicit-conversions) yang berpotensi berbahaya. Penurunan versi, misalnya dari 256 bit ke 8 bit, harus dilakukan secara eksplisit.

```solidity

        // Baca nilai, tetapi jangan tulis ke cache
        if (_firstByte == uint8(DONT_CACHE))
            return(_fromByte+33, _calldataVal(_fromByte+1, 32));

        // Baca nilai, dan tulis ke cache
        ...
        // Jumlah byte tambahan untuk dibaca
        uint8 _extraBytes = _firstByte / 16;
```

Ambil [nibble](https://en.wikipedia.org/wiki/Nibble) bawah dan gabungkan dengan byte lainnya untuk membaca nilai dari cache.
...
```solidity
        ...
        // Parameter dimulai pada byte 4, sebelumnya adalah tanda tangan fungsi
        uint _atByte = 4;
...
Baca parameter hingga Anda mendapatkan jumlah yang Anda butuhkan. Jika kita melewati akhir calldata, `_readParams` akan mengembalikan panggilan.
...
```solidity
    ...
    // Untuk menguji _readParams, uji membaca empat parameter
...
```solidity
    // Dapatkan nilai, kembalikan byte yang akan menyandikannya (menggunakan cache jika memungkinkan)
    function encodeVal(uint _val) public view returns(bytes memory) {
```

`encodeVal` adalah fungsi yang dipanggil oleh kode offchain untuk membantu membuat calldata yang menggunakan cache. Fungsi ini menerima nilai tunggal dan mengembalikan byte yang menyandikannya. Fungsi ini adalah `view`, sehingga tidak memerlukan transaksi dan saat dipanggil secara eksternal tidak memerlukan biaya gas apa pun.

```solidity
        uint _key = val2key[_val];

        // Nilai belum ada di dalam cache, tambahkan
        if (_key == 0)
            return bytes.concat(INTO_CACHE, bytes32(_val));
```

Di [EVM](/developers/docs/evm/) semua penyimpanan yang tidak diinisialisasi diasumsikan bernilai nol. Jadi jika kita mencari kunci untuk nilai yang tidak ada, kita mendapatkan nol. Dalam hal ini byte yang menyandikannya adalah `INTO_CACHE` (sehingga akan di-cache di lain waktu), diikuti oleh nilai sebenarnya.

```solidity
        // Jika kunci <0x10, kembalikan sebagai byte tunggal
        if (_key < 0x10)
            return bytes.concat(bytes1(uint8(_key)));
```

Byte tunggal adalah yang paling mudah. Kita cukup menggunakan [`bytes.concat`](https://docs.soliditylang.org/en/v0.8.16/types.html#the-functions-bytes-concat-and-string-concat) untuk mengubah tipe `bytes<n>` menjadi array byte yang panjangnya bisa berapa saja. Terlepas dari namanya, ini berfungsi dengan baik saat diberikan hanya satu argumen.

```solidity
        // Nilai dua byte, disandikan sebagai 0x1vvv
        if (_key < 0x1000)
            return bytes.concat(bytes2(uint16(_key) | 0x1000));
```

Saat kita memiliki kunci yang kurang dari 16<sup>3</sup>, kita dapat menyatakannya dalam dua byte. Pertama-tama kita mengonversi `_key`, yang merupakan nilai 256 bit, menjadi nilai 16 bit dan menggunakan logika or untuk menambahkan jumlah byte ekstra ke byte pertama. Kemudian kita cukup memasukkannya ke dalam nilai `bytes2`, yang dapat dikonversi menjadi `bytes`.

```solidity
        // Mungkin ada cara cerdas untuk melakukan baris berikut sebagai loop,
        // tetapi ini adalah fungsi view jadi saya mengoptimalkan waktu pemrogram dan
        // kesederhanaan.
...
Salah satu keuntungan Foundry adalah [memungkinkan Anda menulis pengujian dalam Solidity](https://getfoundry.sh/forge/tests/overview/), yang membuatnya lebih mudah untuk menulis pengujian unit. Pengujian untuk kelas `Cache` ada [di sini](https://github.com/qbzzt/20220915-all-you-can-cache/blob/main/test/Cache.t.sol). Karena kode pengujian bersifat repetitif, seperti halnya pengujian pada umumnya, artikel ini hanya menjelaskan bagian-bagian yang menarik.
...
Secara teori mungkin ada bug yang tidak memengaruhi penulisan cache yang berurutan. Jadi di sini kita melakukan beberapa penulisan yang tidak berurutan dan melihat nilainya masih belum ditulis ulang.
...
```solidity
    ...
    // Tanda tangan fungsi untuk fourParams(), atas kebaikan
...
Kita mengharapkan panggilan tersebut berhasil.
...
Satu-satunya pengujian tambahan di `testEncodeVal()` adalah memverifikasi bahwa panjang `_callInput` sudah benar. Untuk panggilan pertama adalah 4+33\*4. Untuk yang kedua, di mana setiap nilai sudah ada di cache, adalah 4+1\*4.

```solidity
    // Uji encodeVal saat kunci lebih dari satu byte tunggal
    // Maksimum tiga byte karena mengisi cache hingga empat byte membutuhkan waktu
    // terlalu lama.
...
```solidity
        ...
        _callInput = bytes.concat(
            FOUR_PARAMS,
            cache.encodeVal(0x000F),   // Satu byte        0x0F
            cache.encodeVal(0x0010),   // Dua byte     0x1010
            cache.encodeVal(0x0100),   // Dua byte     0x1100
            cache.encodeVal(0x1000)    // Tiga byte 0x201000
        );
```
...
```solidity
    ...
    // Uji bahwa dengan buffer yang terlalu kecil kita mendapatkan mengembalikan
...
Karena ini mengembalikan, hasil yang seharusnya kita dapatkan adalah `false`.
...
```solidity
        ...
        .
        .
    // Uji bahwa dengan buffer yang terlalu panjang semuanya berfungsi dengan baik
    function testLongCalldata() public {
        ...
        _callInput = bytes.concat(
            ...
            // Dan nilai lain untuk "semoga berhasil"
            bytes4(0x31112233)
        );
```

Fungsi ini mengirimkan lima nilai. Kita tahu bahwa nilai kelima diabaikan karena bukan entri cache yang valid, yang akan menyebabkan pengembalian seandainya tidak disertakan.
...
Menulis pengujian dalam Solidity memang bagus, tetapi pada akhirnya sebuah aplikasi terdesentralisasi (dapp) harus dapat memproses permintaan dari luar rantai agar berguna. Artikel ini mendemonstrasikan cara menggunakan caching dalam dapp dengan `WORM`, yang merupakan singkatan dari "Write Once, Read Many". Jika kunci belum ditulis, Anda dapat menulis nilai ke dalamnya. Jika kunci sudah ditulis, Anda akan mendapatkan pengembalian.

### Kontrak {#the-contract}

[Ini adalah kontraknya](https://github.com/qbzzt/20220915-all-you-can-cache/blob/main/src/WORM.sol). Sebagian besar mengulangi apa yang telah kita lakukan dengan `Cache` dan `CacheTest`, jadi kita hanya membahas bagian-bagian yang menarik.
...
Fungsi ini mirip dengan `fourParam` di `CacheTest` atas. Karena kita tidak mengikuti spesifikasi ABI, sebaiknya jangan mendeklarasikan parameter apa pun ke dalam fungsi.

```solidity
    // Buat lebih mudah untuk memanggil kami
    // Tanda tangan fungsi untuk writeEntryCached(), atas kebaikan
...
Perhatikan bahwa meskipun kita mendefinisikan `WRITE_ENTRY_CACHED` sebagai variabel state, untuk membacanya secara eksternal perlu menggunakan fungsi getter untuknya, `worm.WRITE_ENTRY_CACHED()`.
...
Fungsi baca adalah `view`, sehingga tidak memerlukan transaksi dan tidak memerlukan biaya gas. Akibatnya, tidak ada manfaat menggunakan cache untuk parameter tersebut. Dengan fungsi view, sebaiknya gunakan mekanisme standar yang lebih sederhana.
...
Di sini kita menggunakan fakta bahwa `cacheWrite` mengembalikan kunci cache. Ini bukanlah sesuatu yang kita harapkan untuk digunakan dalam produksi, karena `cacheWrite` mengubah state, dan oleh karena itu hanya dapat dipanggil selama transaksi. Transaksi tidak memiliki nilai kembalian, jika memiliki hasil, hasil tersebut seharusnya dipancarkan sebagai peristiwa. Jadi nilai kembalian `cacheWrite` hanya dapat diakses dari kode onchain, dan kode onchain tidak memerlukan caching parameter.
...
Karena kita menggunakan fungsi `<address>.call()` tingkat rendah, kita tidak dapat menggunakan `vm.expectRevert()` dan harus melihat nilai keberhasilan boolean yang kita dapatkan dari panggilan tersebut.
...
- Saat ini, kunci cache menggunakan algoritma "datang pertama, kunci terkecil". Enam belas nilai pertama dapat dikirim sebagai satu byte. 4080 nilai berikutnya dapat dikirim sebagai dua byte. Sekitar satu juta nilai berikutnya adalah tiga byte, dll. Sistem produksi harus menyimpan penghitung penggunaan pada entri cache dan mengatur ulangnya sehingga enam belas nilai _paling umum_ adalah satu byte, 4080 nilai paling umum berikutnya adalah dua byte, dll.
  ...
  3. Noam Naive mengirimkan tokennya ke 0x06. Token tersebut masuk ke alamat `0x0000000000000000000000000de0b6b3a7640000`, dan karena tidak ada yang mengetahui kunci privat untuk alamat tersebut, token tersebut hanya tertahan di sana. Noam _tidak senang_.

  Ada beberapa cara untuk memecahkan masalah ini, dan masalah terkait transaksi yang ada di mempool selama penataan ulang cache, tetapi Anda harus menyadarinya.
