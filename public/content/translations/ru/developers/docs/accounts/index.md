...
Адрес контракта обычно назначается при развертывании контракта в блокчейне Эфириума. Адрес формируется из адреса создателя и количества транзакций, отправленных с этого адреса («нонса»). Именно так операция `CREATE` вычисляет адрес.

Контракты также могут быть развернуты с помощью [`CREATE2`](https://eips.ethereum.org/EIPS/eip-1014) — эта операция вычисляет адрес на основе адреса создателя, выбранного им значения («соли») и хеша кода создания контракта. Нонс не используется, поэтому адрес можно вычислить до создания контракта, и он останется неизменным независимо от того, сколько других транзакций создатель отправит за это время. Это позволяет ссылаться на контракт, который еще не был развернут.
