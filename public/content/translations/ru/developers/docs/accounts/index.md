---
title: "Аккаунты Эфириума"
description: "Объяснение аккаунтов Эфириума — их структуры данных и связи с криптографией пар ключей."
lang: ru
---

[Аккаунт Эфириума](/) — это сущность с балансом эфира (ETH), которая может отправлять сообщения в Эфириуме. Аккаунты могут управляться пользователями или быть развернуты как смарт-контракты.
...
- Контрактные аккаунты не имеют приватных ключей. Вместо этого они контролируются логикой кода смарт-контракта

## Изучение аккаунта {#an-account-examined}

Аккаунты Эфириума имеют четыре поля:

- `nonce` — счетчик, который указывает количество транзакций, отправленных с внешнего аккаунта, или количество контрактов, созданных контрактным аккаунтом. Для каждого аккаунта может быть выполнена только одна транзакция с заданным нонсом, что защищает от атак повторного воспроизведения, при которых подписанные транзакции многократно транслируются и выполняются повторно.
- `balance` — количество Wei, принадлежащих этому адресу. Wei — это номинал ETH, и в 1 ETH содержится 1e+18 Wei.
- `codeHash` — этот хеш ссылается на _код_ аккаунта в виртуальной машине Ethereum (EVM). Контрактные аккаунты имеют запрограммированные фрагменты кода, которые могут выполнять различные операции. Этот код EVM выполняется, если аккаунт получает вызов сообщения. Его нельзя изменить, в отличие от других полей аккаунта. Все такие фрагменты кода содержатся в базе данных состояния под соответствующими хешами для последующего извлечения. Это значение хеша известно как codeHash. Для внешних аккаунтов поле codeHash представляет собой хеш пустой строки.
- `storageRoot` — иногда называется хешем хранилища. 256-битный хеш корневого узла [префиксного дерева Меркла-Патриции](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/), которое кодирует содержимое хранилища аккаунта (сопоставление между 256-битными целочисленными значениями), закодированное в дереве как сопоставление 256-битного хеша Keccak-256 256-битных целочисленных ключей с 256-битными целочисленными значениями, закодированными с помощью префикса рекурсивной длины (RLP). Это дерево кодирует хеш содержимого хранилища этого аккаунта и по умолчанию пусто.
...
Можно получить новые открытые ключи из вашего приватного ключа, но вы не можете получить приватный ключ из открытых ключей. Жизненно важно хранить ваши приватные ключи в безопасности и, как следует из названия, ПРИВАТНЫМИ.

Вам нужен приватный ключ для подписания сообщений и транзакций, в результате чего создается подпись. Затем другие могут использовать эту подпись для получения вашего открытого ключа, доказывая авторство сообщения. В своем приложении вы можете использовать библиотеку JavaScript для отправки транзакций в сеть.

## Контрактные аккаунты {#contract-accounts}

Контрактные аккаунты также имеют шестнадцатеричный адрес из 42 символов:

Пример:

`0x06012c8cf97bead5deae237070f9587f8e7a266d`

Адрес контракта обычно выдается при развертывании контракта в блокчейне Эфириума. Адрес формируется из адреса создателя и количества транзакций, отправленных с этого адреса («нонс»). Именно так операция `CREATE` вычисляет адрес.

Контракты также могут быть развернуты с помощью [`CREATE2`](https://eips.ethereum.org/EIPS/eip-1014), который вычисляет адрес на основе адреса создателя, значения, выбранного создателем («соль»), и хеша кода создания контракта. Нонс не используется, поэтому адрес можно вычислить до того, как контракт будет существовать, и он останется неизменным независимо от того, сколько других транзакций создатель отправит за это время. Это позволяет ссылаться на контракт, который еще не был развернут.

## Ключи валидатора {#validators-keys}

В Эфириуме также существует другой тип ключей, представленный, когда Эфириум перешел от консенсуса на основе доказательства выполнения работы (PoW) к доказательству доли владения. Это ключи BLS, и они используются для идентификации валидаторов. Эти ключи можно эффективно агрегировать, чтобы уменьшить пропускную способность, необходимую сети для достижения консенсуса. Без этой агрегации ключей минимальный стейк для валидатора был бы намного выше.

[Подробнее о ключах валидатора](/developers/docs/consensus-mechanisms/pos/keys/).

## Заметка о кошельках {#a-note-on-wallets}

Аккаунт — это не кошелек. Кошелек — это интерфейс или приложение, которое позволяет вам взаимодействовать с вашим аккаунтом Эфириума, будь то внешний аккаунт или контрактный аккаунт.
...
- [Понимание аккаунтов Эфириума](https://info.etherscan.com/understanding-ethereum-accounts/) — Etherscan
