...
Remix IDE предоставляет комплексную интегрированную среду разработки (IDE) для создания и тестирования контрактов как на Solidity, так и на Vyper. [Попробуйте браузерную Remix IDE](https://remix.ethereum.org), чтобы начать программировать.

Более опытные разработчики также могут захотеть использовать Yul, промежуточный язык для [Виртуальной машины Ethereum (EVM)](/developers/docs/evm/), или Yul+, расширение для Yul.

Если вам любопытно и вы хотите помочь в тестировании новых языков, которые все еще находятся в стадии активной разработки, вы можете поэкспериментировать с Fe — новым языком смарт-контрактов, который в настоящее время все еще находится на ранней стадии развития.
...
- Поддерживает:
  ...
  - Библиотеки (вы можете создавать повторно используемый код, который можно вызывать из разных контрактов — как статические функции в статическом классе в других объектно-ориентированных языках программирования).
...
Начиная с версии 0.4.0, Vyper поддерживает [систему модулей](https://docs.vyperlang.org/en/stable/using-modules.html). Повторное использование кода достигается за счет композиции, а не наследования классов.
...
# Отслеживает возвращенные ставки, чтобы мы могли следовать шаблону вывода средств
...
- Хорошая цель для этапов высокоуровневой оптимизации, которая может принести равную пользу как платформам EVM, так и Ewasm.
...
- Если вы новичок, существует множество руководств и инструментов для обучения. Подробнее об этом читайте в разделе [Обучение через программирование](/developers/learning-tools/).
