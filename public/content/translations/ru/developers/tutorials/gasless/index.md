...
## Заключение
На данном этапе вы должны уметь создавать возможность работы без газа для пользователей вашего децентрализованного приложения (dapp) ценой некоторой централизации.

Однако это работает только со смарт-контрактами, поддерживающими ERC-712. Например, чтобы перевести токен ERC-20, необходимо, чтобы владелец подписал транзакцию, а не просто сообщение. Самое простое решение — сделать так, чтобы активы принадлежали не адресу EOA, а отдельному контракту (простая форма [абстракции учетной записи](/roadmap/account-abstraction/)). Вы можете прочитать об этом подробнее [в следующем руководстве](/developers/tutorials/gasless-token).
