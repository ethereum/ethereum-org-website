...
<AlertContent>
<AlertTitle>
Гламстердам — это предстоящее обновление Эфириума, запланированное на 4 квартал 2026 года
</AlertTitle>
<AlertDescription>
...
## Улучшения в Гламстердаме {#improvements-in-glamsterdam}

<Alert variant="info">
<AlertContent>
<AlertDescription>
Примечание: В этой статье освещаются некоторые EIP, запланированные для включения в Гламстердам. Дополнительные запланированные предложения, тестируемые в сетях разработчиков (devnets), включают EIP-7610, EIP-7688, EIP-7778, EIP-7843, EIP-7976, EIP-7981, EIP-8024, EIP-8246 и EIP-8282. Для получения последних обновлений статуса посмотрите [обновление Гламстердам на Forkcast](https://forkcast.org/upgrade/glamsterdam).
...
- Расширение пропускной способности: Разделение тяжелой работы по созданию и валидации блоков, что дает сети больше времени для распространения больших объемов данных без замедления.
- Предотвращение раздувания базы данных (устойчивость): Корректировка сетевых комиссий для точного отражения долгосрочных затрат оборудования на хранение новых данных, снимая препятствия для будущего увеличения лимита газа и предотвращая при этом снижение производительности оборудования.

Вкратце, Гламстердам внедрит структурные изменения, чтобы гарантировать, что по мере увеличения пропускной способности сети она будет оставаться устойчивой, а производительность — высокой.
...
Усовершенствования обязанностей валидаторов и процессов выхода обеспечивают стабильность сети во время массовых событий слэшинга и ускоряют то, как быстро стейкеры могут перемещать свой стейк. Эти улучшения делают сеть более стабильной и гарантируют справедливое отношение ко всем участникам, как крупным, так и мелким.
...
### Увеличение текучести выходов и консолидаций {#increase-exit-and-consolidation-churn}

- Значительно сокращает время вывода средств из стейкинга, позволяя пропускной способности выходов масштабироваться в зависимости от общего количества застейканных ETH, а не ограничиваться фиксированным значением
- Предоставляет консолидациям валидаторов собственную выделенную пропускную способность очереди, ускоряя переход к более крупным и эффективным валидаторам
- Поддерживает безопасность сети с помощью тщательно проанализированных параметров безопасности

Лимит текучести Эфириума — это предел безопасности для скорости, с которой валидаторы могут входить, выходить или объединять (консолидировать) свои застейканные ETH, чтобы гарантировать, что безопасность сети никогда не будет дестабилизирована. Сегодня выходы и активации имеют общий фиксированный лимит, поэтому в периоды высокого спроса стейкеры могут столкнуться с долгим ожиданием вывода своих ETH. Консолидации, при которых валидаторы объединяются в более крупные с балансом до 2048 ETH (включенные обновлением [Пектра](/roadmap/pectra)), также конкурируют за эту ограниченную пропускную способность, что означает, что консолидация всего набора валидаторов заняла бы годы при текущей скорости.

**Увеличение текучести выходов и консолидаций (или EIP-8061)** реорганизует эти лимиты в отдельные направления:

- Активации валидаторов сохраняют свой существующий фиксированный лимит без изменений
- Выходы больше не ограничены фиксированным значением и вместо этого масштабируются в зависимости от общего количества застейканных ETH
- Консолидации получают собственную выделенную пропускную способность, примерно вдвое меньшую, чем комбинированный лимит активаций и выходов

При текущих уровнях стейкинга это увеличивает пропускную способность выходов примерно в 4 раза, а пропускную способность консолидаций — примерно в 2 раза, что означает, что стейкеры могут выводить свои ETH намного быстрее в периоды высокого спроса, а сеть быстрее переходит к меньшему, более эффективному набору валидаторов.

Поскольку стейк может входить в сеть и выходить из нее быстрее, это изменение примерно вдвое сокращает время, в течение которого узел может оставаться в автономном режиме, прежде чем ему потребуется недавняя доверенная контрольная точка для безопасного повторного подключения к сети (период слабой субъективности, примерно с 15,7 до 7 дней). Этот компромисс был тщательно проанализирован, чтобы гарантировать сохранение безопасности сети.

**Ресурсы**: [Техническая спецификация EIP-8061](https://eips.ethereum.org/EIPS/eip-8061)
...
- Увеличение максимального размера контракта (или EIP-7954) позволяет разработчикам развертывать более крупные приложения, повышая лимит максимального размера контракта примерно с 24 КиБ до 64 КиБ.
- Детерминированное предварительное развертывание фабрики (или EIP-7997) вводит универсальный встроенный контракт фабрики. Это позволяет разработчикам развертывать свои приложения и кошельки смарт-контрактов по одному и тому же адресу во всех участвующих EVM-цепях.
- Если ваше приложение полагается на сложную трассировку для поиска переводов ETH, Переводы и сжигания ETH генерируют лог (или EIP-7708) позволит вам переключиться на использование логов для более простого и надежного учета.
- Увеличение стоимости газа для создания состояния (или EIP-8037) и Обновление стоимости газа для доступа к состоянию (или EIP-8038) вводят новые модели устойчивости, которые изменят определенные затраты на развертывание контрактов, поскольку создание новых аккаунтов или постоянного хранилища потребует новой стандартизированной фиксированной комиссии, основанной на размере созданных данных.
