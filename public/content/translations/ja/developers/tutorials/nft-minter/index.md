...
このチュートリアルで呼び出すERC-721 NFTスマートコントラクトへの[リンクはこちら](https://www.alchemy.com/docs/how-to-create-an-nft)です。 その作成方法を学びたい場合は、もう一つのチュートリアル["NFTの作成方法"](https://www.alchemy.com/docs/how-to-create-an-nft)をご覧になることを強くお勧めします。
...
非代替性トークン(NFT)をミントする(または、イーサリアムのブロックチェーンのトランザクションに署名する)には、偽のETHが必要です。 Ethを取得するには、[Ropstenフォーセット](https://www.alchemy.com/faucets/ethereum-sepolia)にアクセスしてRopstenのアカウントアドレスを入力し、「Send Ropsten Eth」をクリックします。 MetamaskアカウントにETHが表示されるはずです。
...
前述したように、このチュートリアルでは[こちらの既存のNFTスマートコントラクト](https://www.alchemy.com/docs/how-to-create-an-nft)を使用します。しかし、その作成方法を学びたい、もしくは自分で作成したい場合は、もう一つのチュートリアル["NFTの作成方法"](https://www.alchemy.com/docs/how-to-create-an-nft)をご覧になることを強くお勧めします。
...
try {
  ...
  return {
    success: true,
    status:
      "✅ Etherscanでトランザクションを確認: https://sepolia.etherscan.io/tx/" +
      txHash,
  }
} catch (error) {
  return {
