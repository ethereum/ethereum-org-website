...
現時点では、ある程度の中央集権化と引き換えに、分散型アプリケーション (dapp) のユーザーにガスレスな体験を提供できるようになっているはずです。

ただし、これはERC-712をサポートするスマート・コントラクトでのみ機能します。たとえば、ERC-20トークンを送金するには、単なるメッセージではなく、所有者によってトランザクションが署名される必要があります。最も簡単な解決策は、資産をEOAアドレスではなく、別のコントラクト（[アカウント抽象化](/roadmap/account-abstraction/)の単純な形式）に所有させることです。詳細については、[続編のチュートリアル](/developers/tutorials/gasless-token)で読むことができます。
