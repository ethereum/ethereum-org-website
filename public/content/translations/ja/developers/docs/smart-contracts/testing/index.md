...
##### 1. コントラクトのビジネスロジックとワークフローを理解する {#integration-testing-for-smart-contracts}
...
##### 2. コントラクトの実行に関連するすべての前提条件を評価する {#property-based-testing-for-smart-contracts}
...
##### 3. コードカバレッジを測定する {#static-analysis}
...
##### 4. 十分に開発されたテストフレームワークを使用する {#dynamic-analysis}
...
### 統合テスト {#running-property-based-tests}
...
### プロパティベーステスト {#manual-testing-for-smart-contracts}
...
**静的分析**と**動的分析**は、プロパティベーステストを実行するための2つの一般的な手法であり、どちらもプログラム(この場合はスマート・コントラクト)のコードが事前定義されたプロパティを満たしていることを検証できます。一部のプロパティベーステストツールには、期待されるコントラクトのプロパティに関する事前定義されたルールが付属しており、それらのルールに照らしてコードをチェックします。一方、スマート・コントラクトのカスタムプロパティを作成できるツールもあります。

#### 静的分析 {#testing-on-local-blockchain}
...
#### 動的分析 {#testing-contracts-on-testnets}
...
### スマート・コントラクトのプロパティベーステストを実行するためのガイドライン {#testing-vs-formal-verification}
...
## スマート・コントラクトの手動テスト {#testing-vs-audits-bug-bounties}
...
### ローカルブロックチェーンでのコントラクトのテスト {#testing-tools-and-libraries}
...
### テストネットでのコントラクトのテスト {#unit-testing-tools}
...
## テストと形式的検証の比較 {#property-based-testing-tools}
...
## テストと監査およびバグバウンティの比較 {#static-analysis-tools}
...
## テストツールとライブラリ {#dynamic-analysis-tools}

### 単体テストツール {#related-tutorials}
...
### プロパティベーステストツール {#further-reading}
...
## チュートリアル: イーサリアムでのスマート・コントラクトのテスト
