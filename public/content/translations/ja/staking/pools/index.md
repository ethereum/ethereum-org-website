...
<CardGrid>
  <Card title="参加しやすい" emoji="🐟" description="大口でなくても大丈夫。32ETHが必要なソロステーキングとは違い、ほとんどのステーキングプールでは他のステーカーと協力して、少額からETHをステークできます。" />
  <Card title="今日からステーク" emoji=":stopwatch:" description="プールでのステーキングはトークンスワップのように簡単です。ハードウェアのセットアップやノードのメンテナンスは不要。プールにETHを預ければ、ノードオペレーターがバリデーターを運用でき、ノード運用の手数料を差し引いた報酬が貢献者に分配されます。" />
  <Card title="ステーキングトークン" emoji=":droplet:" description="多くのステーキングプールでは、ステークしたETHとその報酬を表すトークンが提供されます。このトークンを使えば、ステークしたETHをDeFiアプリケーションの担保にするなど、有効活用できます。" />
</CardGrid>
...
<ExpandableCard title="取引所でのステーキングとは違いますか？">
