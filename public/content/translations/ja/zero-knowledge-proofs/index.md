...
レイヤー2のスイッチングを超えて、ゼロ知識証明はEthereumのL1ブロック実行自体も検証できます。L1検証用のzkEVMを使用すると、バリデータはすべてのトランザクションを再実行するのではなく、証明をチェックすることでブロックを検証できるようになります -- これにより、バリデータのハードウェア要件を上げることなく、より高いガスリミットが可能になります。
