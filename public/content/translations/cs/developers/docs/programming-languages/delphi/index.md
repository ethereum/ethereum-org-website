---
title: Ethereum pro vývojáře v Delphi
description: Naučte se vyvíjet pro Ethereum pomocí programovacího jazyka Delphi
lang: cs
incomplete: true
---

<FeaturedText>

Naučte se vyvíjet pro Ethereum pomocí programovacího jazyka Delphi

</FeaturedText>

Využijte Ethereum k vytváření decentralizovaných aplikací (dapps), které využívají výhod kryptoměn a technologie blockchain. Tyto dapps mohou být důvěryhodné, což znamená, že jakmile jsou nasazeny na Ethereu, budou vždy běžet přesně tak, jak byly naprogramovány. Mohou ovládat digitální aktiva za účelem vytváření nových druhů finančních aplikací. Mohou být decentralizované, což znamená, že je neovládá žádný jediný subjekt ani osoba a je téměř nemožné je cenzurovat.

Vytvářejte decentralizované aplikace na Ethereu a komunikujte s chytrými kontrakty pomocí programovacího jazyka Delphi!

## Začínáme s chytrými kontrakty a jazykem Solidity {#getting-started-with-smart-contracts-and-the-solidity-language}

**Udělejte první kroky k integraci Delphi s Ethereem**

Potřebujete nejprve základnější úvod? Podívejte se na [ethereum.org/learn](/learn/) nebo [ethereum.org/developers](/developers/).

- [Vysvětlení blockchainu](https://kauri.io/article/d55684513211466da7f8cc03987607d5/blockchain-explained)
- [Porozumění chytrým kontraktům](https://kauri.io/article/e4f66c6079e74a4a9b532148d3158188/ethereum-101-part-5-the-smart-contract)
- [Napište svůj první chytrý kontrakt](https://kauri.io/article/124b7db1d0cf4f47b414f8b13c9d66e2/remix-ide-your-first-smart-contract)
- [Naučte se kompilovat a nasadit Solidity](https://kauri.io/article/973c5f54c4434bb1b0160cff8c695369/understanding-smart-contract-compilation-and-deployment)

## Odkazy a materiály pro začátečníky {#beginner-references-and-links}

**Představení knihovny Delphereum**

- [Co je Delphereum?](https://github.com/svanas/delphereum/blob/master/README.md)
- [Připojení Delphi k lokálnímu (in-memory) blockchainu](https://medium.com/@svanas/connecting-delphi-to-a-local-in-memory-blockchain-9a1512d6c5b0)
- [Připojení Delphi k síti Ethereum Mainnet](https://medium.com/@svanas/connecting-delphi-to-the-ethereum-main-net-5faf1feffd83)
- [Připojení Delphi k chytrým kontraktům](https://medium.com/@svanas/connecting-delphi-to-smart-contracts-3146b12803a1)

**Chcete prozatím přeskočit nastavení a přejít rovnou k ukázkám?**

- [Tříminutový chytrý kontrakt a Delphi – Část 1](https://medium.com/@svanas/a-3-minute-smart-contract-and-delphi-61d998571d)
- [Tříminutový chytrý kontrakt a Delphi – Část 2](https://medium.com/@svanas/a-3-minute-smart-contract-and-delphi-part-2-446925faa47b)

## Články pro středně pokročilé {#intermediate-articles}

- [Generování podpisu zprávy podepsané Ethereem v Delphi](https://medium.com/@svanas/generating-an-ethereum-signed-message-signature-in-delphi-75661ce5031b)
- [Převod etheru pomocí Delphi](https://medium.com/@svanas/transferring-ether-with-delphi-b5f24b1a98a4)
- [Převod tokenů ERC-20 pomocí Delphi](https://medium.com/@svanas/transferring-erc-20-tokens-with-delphi-bb44c05b295d)

## Pokročilé vzory použití {#advanced-use-patterns}

- [Delphi a Ethereum Name Service (ENS)](https://medium.com/@svanas/delphi-and-ethereum-name-service-ens-4443cd278af7)
- [QuikNode, Ethereum a Delphi](https://medium.com/@svanas/quiknode-ethereum-and-delphi-f7bfc9671c23)
- [Delphi a Ethereum Dark Forest](https://svanas.medium.com/delphi-and-the-ethereum-dark-forest-5b430da3ad93)
- [Swap jednoho tokenu za jiný v Delphi](https://svanas.medium.com/swap-one-token-for-another-in-delphi-bcb999c47f7)

Hledáte další zdroje? Podívejte se na [ethereum.org/developers](/developers/).