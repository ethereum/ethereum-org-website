...
![Діаграма, що показує, як один ключ валідатора розділяється на частини ключа та розподіляється на кілька вузлів із різними компонентами.](./dvt-cluster.png)
...
1. **Децентралізація** консенсусу proof-of-stake в Ethereum
...
- **[Схема розподілу секрету Шаміра](https://medium.com/@keylesstech/a-beginners-guide-to-shamir-s-secret-sharing-e864efbf3648)** — валідатори використовують [BLS-ключі](https://en.wikipedia.org/wiki/BLS_digital_signature). Окремі «частини ключа» BLS («частини ключа») можуть бути об’єднані в один агрегований ключ (підпис). У DVT приватним ключем валідатора є комбінований BLS-підпис кожного оператора в кластері.
- **[Схема порогового підпису](https://medium.com/nethermind-eth/threshold-signature-schemes-36f40bc42aca)** — визначає кількість окремих частин ключа, які потрібні для виконання обов’язків із підписання, наприклад, 3 з 4.
- **[Генерація розподіленого ключа (DKG)](https://medium.com/toruslabs/what-distributed-key-generation-is-866adc79620)** — криптографічний процес, що генерує частини ключа та використовується для розподілу частин наявного або нового ключа валідатора між вузлами в кластері.
- **[Багатосторонні обчислення (MPC)](https://messari.io/report/applying-multiparty-computation-to-the-world-of-blockchains)** — повний ключ валідатора генерується таємно з використанням багатосторонніх обчислень. Жоден оператор ніколи не знає повного ключа — він знає лише частину свого ключа (свою «частину»).
- **Протокол консенсусу** — протокол консенсусу обирає один вузол, який буде пропонувачем блоків. Вони ділять блок з іншими вузлами кластера, які додають свої частини ключа в загальний підпис. Коли зібрано достатню кількість частин ключа, блок пропонується на Ethereum.
...
## Сценарії використання DVT {#dvt-use-cases}
...
- **Додатковий компонент** — додавання вузла DVT створює ще один елемент, який може бути несправним або вразливим. Щоб знизити ризики, потрібно прагнути до кількох реалізацій DVT-вузла, тобто до кількох DVT-клієнтів (аналогічно тому, як існує кілька клієнтів для рівнів консенсусу й виконання).
...
- **Потенційно збільшена затримка** — оскільки DVT використовує протокол консенсусу для досягнення консенсусу між кількома вузлами, що обслуговують валідатор, це потенційно може призвести до збільшення затримки.

## Додаткові матеріали {#further-reading}

- [Специфікації розподіленого валідатора Ethereum (високорівневі)](https://github.com/ethereum/distributed-validator-specs)
- [Технічні специфікації розподіленого валідатора Ethereum](https://github.com/ethereum/distributed-validator-specs/tree/dev/src/dvspec)
- [Демонстраційний застосунок схеми розподілу секрету Шаміра](https://iancoleman.io/shamir/)
