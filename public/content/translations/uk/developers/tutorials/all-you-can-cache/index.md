...
4. Відредагуйте `.env` для вашої конфігурації:
   ...
   | Параметр           | Значення                                                                                                                                                               |
   | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | MNEMONIC            | Мнемонічна фраза для акаунта, який має достатньо ETH для оплати транзакції. [Ви можете отримати безкоштовні ETH для мережі Optimism Ґерлі тут](https://optimismfaucet.xyz/). |
   | OPTIMISM_GOERLI_URL | URL-адреса до Optimism Ґерлі. Публічна кінцева точка, `https://goerli.optimism.io`, має обмеження швидкості, але її достатньо для наших потреб                                      |
...
5. Запустіть `index.js`.
   ...
   Цей приклад застосунку спочатку записує запис у WORM, відображаючи дані виклику та посилання на транзакцію в Etherscan. Потім він зчитує цей запис і відображає ключ, який він використовує, та значення в записі (значення, номер блоку та автор).

Більша частина клієнта — це звичайний JavaScript для Dapp. Тому ми знову розглянемо лише цікаві частини.
...
У даний слот можна записати лише один раз, тому ми використовуємо часову мітку, щоб переконатися, що ми не використовуємо слоти повторно.
...
Як і у випадку з кодом тестування Solidity, ми не можемо викликати кешовану функцію звичайним способом. Натомість нам потрібно використовувати механізм нижчого рівня.

```javascript
    .
    .
    .
    // Зчитати щойно записаний елемент
