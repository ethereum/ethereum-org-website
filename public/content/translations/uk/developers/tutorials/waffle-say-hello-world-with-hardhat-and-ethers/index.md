...
### Тепер поговоримо про деякі з цих файлів: {#now-lets-talk}
...
console.log("Розгортання Greeter з привітанням:", _greeting);
greeting = _greeting;
}
...
console.log("Зміна привітання з '%s' на '%s'", greeting, _greeting);
greeting = _greeting;
}
}
```
...
describe("Greeter", function () {
  it("Має повертати нове привітання після його зміни", async function () {
...
### Наступний крок — компіляція нашого контракту та запуск тестів: {#compiling-and-testing}
...
### Поки що все чудово, додамо трохи складності до нашого проєкту <Emoji text=":slightly_smiling_face:" size={1}/> {#adding-complexity}
...
```js
it("Має скасовувати операцію в разі передавання порожнього рядка", async () => {
  const Greeter = await ethers.getContractFactory("Greeter")
  const greeter = await Greeter.deploy("Hello, world!")

  await greeter.deployed()
  await expect(greeter.setGreeting("")).to.be.revertedWith(
    "Привітання не повинно бути порожнім"
  )
})
```
...
```solidity
require(bytes(_greeting).length > 0, "Привітання не повинно бути порожнім");
```
...
Щоб знайти більше чудових зіставлень Chai для тестування ваших смарт-контрактів, перегляньте [офіційну документацію Waffle](https://ethereum-waffle.readthedocs.io/en/latest/matchers.html).
