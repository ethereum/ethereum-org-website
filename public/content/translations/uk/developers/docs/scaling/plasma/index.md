...
Вихід з ланцюга Плазми є складнішим, ніж вхід до нього, з кількох причин. Найголовніша з них полягає в тому, що хоча Етеріум має інформацію про стан ланцюга Плазми, він не може перевірити, чи є ця інформація правдивою. Зловмисний користувач може зробити неправильне твердження («У мене є 1000 ETH») і залишитися безкарним, надавши підроблені докази на підтвердження свого затребування.

Щоб запобігти зловмисним виведенням, вводиться «період оскарження». Протягом періоду оскарження (зазвичай це тиждень) будь-хто може оскаржити запит на виведення, використовуючи доказ шахрайства. Якщо оскарження успішне, запит на виведення відхиляється.

Однак зазвичай користувачі є чесними і роблять правильні затребування щодо коштів, якими вони володіють. У цьому сценарії Аліса ініціює запит на виведення в кореневому ланцюзі (Етеріум), надіславши транзакцію до контракту Плазми.

Вона також повинна надати доказ Меркла, який підтверджує, що транзакція, яка створює її кошти в ланцюзі Плазми, була включена в блок. Це необхідно для ітерацій Плазми, таких як Plasma MVP, які використовують модель [невитрачених виходів транзакцій (UTXO)](https://en.wikipedia.org/wiki/Unspent_transaction_output).

Інші, такі як Plasma Cash, представляють кошти як [невзаємозамінні токени](/developers/docs/standards/tokens/erc-721/) замість UTXO. Виведення в цьому випадку вимагає доказу права власності на токени в ланцюзі Плазми. Це робиться шляхом подання двох останніх транзакцій, пов'язаних з токеном, і надання доказу Меркла, що підтверджує включення цих транзакцій у блок.

Користувач також повинен додати заставу до запиту на виведення як гарантію чесної поведінки. Якщо оскаржувач доведе, що запит Аліси на виведення є недійсним, її застава піддається слешингу, і частина її переходить до оскаржувача як винагорода.

Якщо період оскарження минає, і ніхто не надає доказ шахрайства, запит Аліси на виведення вважається дійсним, що дозволяє їй повернути депозити з контракту Плазми в Етеріумі.
...
- [Сайдчейни проти Плазми та шардингу](https://vitalik.eth.limo/general/2019/06/12/plasma_vs_sharding.html)
