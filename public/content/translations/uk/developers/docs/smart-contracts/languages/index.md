...
Remix надає комплексне інтегроване середовище розробки (IDE) для створення та тестування контрактів як на Solidity, так і на Vyper. [Спробуйте браузерне середовище Remix IDE](https://remix.ethereum.org), щоб почати писати код.

Більш досвідчені розробники також можуть використовувати Yul, проміжну мову для [Віртуальної машини Ethereum (EVM)](/developers/docs/evm/), або Yul+, розширення для Yul.

Якщо вам цікаво і ви хочете допомогти в тестуванні нових мов, які все ще перебувають на стадії активної розробки, ви можете поекспериментувати з Fe — новою мовою смарт-контрактів, яка наразі знаходиться на початковому етапі розвитку.
...
- Мова з фігурними дужками, на яку найбільше вплинула C++.
- Статично типізована (тип змінної відомий під час компіляції).
- Підтримує:
  - Успадкування (ви можете розширювати інші контракти).
  - Бібліотеки (ви можете створювати код для багаторазового використання, який можна викликати з різних контрактів — як статичні функції у статичному класі в інших об'єктно-орієнтованих мовах програмування).
...
- Навмисно має менше функцій, ніж Solidity, з метою зробити контракти більш безпечними та простішими для аудиту. Vyper не підтримує:
  - Модифікатори
  - Успадкування
...
Починаючи з версії 0.4.0, Vyper підтримує [систему модулів](https://docs.vyperlang.org/en/stable/using-modules.html). Повторне використання коду досягається за допомогою композиції, а не успадкування класів.
...
# Створює простий аукціон з часом торгів `_bidding_time`
# секунд від імені
...
@payable
def bid():
    # Перевірка, чи закінчився період торгів.
    assert block.timestamp < self.auctionEnd
    # Перевірка, чи достатньо висока ставка
    assert msg.value > self.highestBid
    # Відстеження відшкодування для попереднього учасника з найвищою ставкою
    self.pendingReturns[self.highestBidder] += self.highestBid
    # Відстеження нової найвищої ставки
    self.highestBidder = msg.sender
    self.highestBid = msg.value

# Зняти раніше відшкодовану ставку. Патерн зняття коштів
...
def endAuction():
    ...
    # 1. Умови
    # Перевірка, чи настав час завершення аукціону
    assert block.timestamp >= self.auctionEnd
    # Перевірка, чи ця функція вже була викликана
...
Якщо ви новачок в Етеріумі і ще не писали код мовами смарт-контрактів, ми рекомендуємо почати з Solidity або Vyper. Звертайтеся до Yul або Yul+ лише тоді, коли ви ознайомитеся з найкращими практиками безпеки смарт-контрактів та специфікою роботи з EVM.
...
- Хороша ціль для етапів високорівневої оптимізації, яка може принести однакову користь платформам EVM та Ewasm.
...
Якщо ви вже маєте значний досвід роботи зі смарт-контрактами, повну реалізацію ERC-20 на Yul можна знайти [тут](https://solidity.readthedocs.io/en/latest/yul.html#complete-erc20-example).

## Fe {#fe}

- Статично типізована мова для Віртуальної машини Ethereum (EVM).
- Натхненна Python та Rust.
- Має на меті бути простою у вивченні — навіть для розробників, які є новачками в екосистемі Етеріуму.
- Розробка Fe все ще перебуває на ранніх стадіях, альфа-версія мови вийшла в січні 2021 року.
