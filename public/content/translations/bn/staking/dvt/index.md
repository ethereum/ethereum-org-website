...
এটি একটি ভ্যালিডেটরকে সুরক্ষিত করতে ব্যবহৃত **প্রাইভেট কী**-কে একটি "ক্লাস্টার" হিসাবে সংগঠিত **একাধিক কম্পিউটারে বিভক্ত** করার মাধ্যমে এটি করে। এর সুবিধা হল এতে আক্রমণকারীদের জন্য কী টিতে অ্যাক্সেস অর্জন করা খুব কঠিন হয়ে যায়, কারণ এটি কোনও একক মেশিনে সম্পূর্ণরূপে সংরক্ষণ করা হয় না। এটি কিছু নোডকে অফলাইনে যেতে দেয়, কারণ প্রয়োজনীয় স্বাক্ষর প্রতিটি ক্লাস্টারে মেশিনের একটি উপসেট দ্বারা করা যেতে পারে। এটি নেটওয়ার্ক থেকে অসফলের সিঙ্গেল পয়েন্ট হ্রাস করে এবং পুরো ভ্যালিডেটরকে আরও শক্তিশালী করে তোলে।

![একটি ডায়াগ্রাম যা দেখাচ্ছে কীভাবে একটি একক ভ্যালিডেটর কী, কী শেয়ারে বিভক্ত হয় এবং বিভিন্ন উপাদানসহ একাধিক নোডে বিতরণ করা হয়।](./dvt-cluster.png)
...
### ব্যর্থতার কোনো একক পয়েন্ট নেই {#no-single-point-of-failure}
...
**DVT, Ethereum-কে নিম্নলিখিত সুবিধাগুলি প্রদান করে:**

1. **Ethereum-এর প্রুফ-অফ-স্টেক কনসেন্সাসের বিকেন্দ্রীকরণ**
2. নেটওয়ার্কের **লাইভনেস** নিশ্চিত করে
3. ভ্যালিডেটরের **ফল্ট টলারেন্স** তৈরি করে
4. **ট্রাস্ট মিনিমাইজড** ভ্যালিডেটর অপারেশন
5. **ন্যূনতম স্ল্যাশিং** এবং ডাউনটাইমের ঝুঁকি
6. **বৈচিত্র্যের উন্নতি করে** (ক্লায়েন্ট, ডেটা সেন্টার, অবস্থান, প্রবিধান, ইত্যাদি)
7. ভ্যালিডেটর কী ব্যবস্থাপনার **উন্নত নিরাপত্তা**
...
- **[থ্রেশহোল্ড সিগনেচার স্কিম](https://medium.com/nethermind-eth/threshold-signature-schemes-36f40bc42aca)** - স্বাক্ষরের দায়িত্ব পালনের জন্য প্রয়োজনীয় স্বতন্ত্র কী শেয়ারের সংখ্যা নির্ধারণ করে, যেমন, ৪টির মধ্যে ৩টি।
- **[ডিস্ট্রিবিউটেড কী জেনারেশন (DKG)](https://medium.com/toruslabs/what-distributed-key-generation-is-866adc79620)** - ক্রিপ্টোগ্রাফিক প্রক্রিয়া যা কী শেয়ার তৈরি করে এবং একটি ক্লাস্টারের নোডগুলিতে একটি বিদ্যমান বা নতুন ভ্যালিডেটর কী-এর শেয়ার বিতরণ করতে ব্যবহৃত হয়।
- **[মাল্টিপার্টি কম্পিউটেশন (MPC)](https://messari.io/report/applying-multiparty-computation-to-the-world-of-blockchains)** - সম্পূর্ণ ভ্যালিডেটর কী মাল্টিপার্টি কম্পিউটেশন ব্যবহার করে গোপনে তৈরি করা হয়। সম্পূর্ণ কী টি কোন একজন অপারেটরের জানার কথা নয় — তারা শুধুমাত্র তাদের নিজস্ব অংশটি জানবে (তাদের "শেয়ার")।
- **কনসেন্সাস প্রোটোকল** - কনসেন্সাস প্রোটোকল ব্লক প্রস্তাবক হতে একটি নোড নির্বাচন করে। তারা ক্লাস্টারের অন্যান্য নোডগুলির সাথে ব্লকটি ভাগ করে, যারা তাদের মূল শেয়ারগুলি সামগ্রিক স্বাক্ষরে যুক্ত করে। যখন পর্যাপ্ত মূল শেয়ারগুলি একত্রিত করা হয়, তখন ব্লকটি ইথেরিয়ামে প্রস্তাবিত হয়।
...
### পরিষেবা হিসাবে স্টেকিং (SaaS) {#saas}
...
- **পরিচালন ব্যয়** - যেহেতু DVT একাধিক পক্ষের মধ্যে ভ্যালিডেটরকে বণ্টন করে, তাই একটি মাত্র নোডের পরিবর্তে অপারেশনের জন্য আরও নোডের প্রয়োজন হয়, যা পরিচালন ব্যয় বাড়িয়ে দেয়।
- **সম্ভাব্য বর্ধিত লেটেন্সি** - যেহেতু DVT একটি ভ্যালিডেটর পরিচালনাকারী একাধিক নোডের মধ্যে কনসেন্সাস অর্জনের জন্য একটি কনসেন্সাস প্রোটোকল ব্যবহার করে, এটি সম্ভাব্যভাবে লেটেন্সি বাড়াতে পারে।

## আরও পড়ুন {#further-reading}

- [Ethereum ডিস্ট্রিবিউটেড ভ্যালিডেটর স্পেসিফিকেশন (উচ্চ-স্তরের)](https://github.com/ethereum/distributed-validator-specs)
- [Ethereum ডিস্ট্রিবিউটেড ভ্যালিডেটর টেকনিক্যাল স্পেসিফিকেশন](https://github.com/ethereum/distributed-validator-specs/tree/dev/src/dvspec)
