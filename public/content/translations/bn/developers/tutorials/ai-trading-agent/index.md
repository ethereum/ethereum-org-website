---
title: "ইথেরিয়ামে আপনার নিজস্ব এআই ট্রেডিং এজেন্ট তৈরি করুন"
description: "এই টিউটোরিয়ালে আপনি শিখবেন কীভাবে একটি সাধারণ এআই ট্রেডিং এজেন্ট তৈরি করতে হয়। এই এজেন্ট ব্লকচেইন থেকে তথ্য পড়ে, সেই তথ্যের ওপর ভিত্তি করে একটি LLM-এর কাছে সুপারিশ চায়, LLM-এর সুপারিশ অনুযায়ী ট্রেড করে এবং তারপর অপেক্ষা করে ও পুনরাবৃত্তি করে।"
...
স্বয়ংক্রিয় ট্রেডিং এজেন্টগুলো ডেভেলপারদের একটি ট্রেডিং কৌশল নির্বাচন এবং কার্যকর করার সুযোগ দেয়। [এআই এজেন্টগুলো](/ai-agents) আরও জটিল এবং ডায়নামিক ট্রেডিং কৌশলের সুযোগ দেয়, যা সম্ভবত এমন তথ্য এবং অ্যালগরিদম ব্যবহার করে যা ডেভেলপার হয়তো ব্যবহার করার কথা ভাবেনওনি।

## টুলস {#tools}

এই টিউটোরিয়ালে কোট (quote) এবং ট্রেডিংয়ের জন্য [Python](https://www.python.org/), [Web3 লাইব্রেরি](https://web3py.readthedocs.io/en/stable/) এবং [ইউনিসোয়াপ v3](https://github.com/Uniswap/v3-periphery) ব্যবহার করা হয়েছে।

### Python কেন? {#python}

এআই-এর জন্য সবচেয়ে বেশি ব্যবহৃত ভাষা হলো [Python](https://www.python.org/), তাই আমরা এখানে এটি ব্যবহার করছি। আপনি যদি Python না জানেন তবে চিন্তা করবেন না। ভাষাটি খুব স্পষ্ট, এবং এটি ঠিক কী করে তা আমি ব্যাখ্যা করব।

[Web3 লাইব্রেরি](https://web3py.readthedocs.io/en/stable/) হলো সবচেয়ে সাধারণ Python ইথেরিয়াম API। এটি ব্যবহার করা বেশ সহজ।

### ব্লকচেইনে ট্রেডিং {#trading-on-blockchain}

ইথেরিয়ামে টোকেন ট্রেড করার জন্য [অনেক ডিস্ট্রিবিউটেড এক্সচেঞ্জ (DEX)](/apps/categories/defi/) রয়েছে। তবে, [আর্বিট্রেজ (arbitrage)](/developers/docs/smart-contracts/composability/#better-user-experience)-এর কারণে সেগুলোর এক্সচেঞ্জ রেট প্রায় একই রকম থাকে।

[ইউনিসোয়াপ](https://app.uniswap.org/) হলো একটি বহুল ব্যবহৃত DEX যা আমরা কোট (টোকেনের আপেক্ষিক মান দেখতে) এবং ট্রেড উভয়ের জন্যই ব্যবহার করতে পারি।

### OpenAI {#openai}

একটি লার্জ ল্যাঙ্গুয়েজ মডেলের জন্য, আমি [OpenAI](https://openai.com/) দিয়ে শুরু করার সিদ্ধান্ত নিয়েছি। এই টিউটোরিয়ালের অ্যাপ্লিকেশনটি চালানোর জন্য আপনাকে API অ্যাক্সেসের জন্য অর্থ প্রদান করতে হবে। ন্যূনতম $5 পেমেন্ট যথেষ্টর চেয়েও বেশি।

## ধাপে ধাপে ডেভেলপমেন্ট {#step-by-step}
...
এগুলো হলো সেই দুটি কন্ট্রাক্টের [ABI](https://docs.soliditylang.org/en/latest/abi-spec.html) যার সাথে আমাদের যোগাযোগ করতে হবে। কোডটি সংক্ষিপ্ত রাখার জন্য, আমরা কেবল সেই ফাংশনগুলো অন্তর্ভুক্ত করি যেগুলো আমাদের কল করতে হবে।

```python
w3 = Web3(Web3.HTTPProvider(MAINNET_URL))
```

[`Web3`](https://web3py.readthedocs.io/en/stable/quickstart.html#remote-providers) লাইব্রেরি ইনিশিয়েট করুন এবং একটি ইথেরিয়াম নোড-এর সাথে কানেক্ট করুন।
...
Python-এ ডেটা ক্লাস তৈরি করার এটি একটি উপায়। কন্ট্রাক্টের সাথে কানেক্ট করার জন্য [`Contract`](https://web3py.readthedocs.io/en/stable/web3.contract.html) ডেটা টাইপ ব্যবহার করা হয়। `(frozen=True)` লক্ষ্য করুন। Python-এ [বুলিয়ান (booleans)](https://en.wikipedia.org/wiki/Boolean_data_type) `True` বা `False` হিসেবে সংজ্ঞায়িত করা হয়, যার প্রথম অক্ষর বড় হাতের হয়। এই ডেটা ক্লাসটি হলো `frozen`, যার মানে ফিল্ডগুলো পরিবর্তন করা যাবে না।

ইনডেন্টেশন লক্ষ্য করুন। [C-ভিত্তিক ভাষাগুলোর](https://en.wikipedia.org/wiki/List_of_C-family_programming_languages) বিপরীতে, Python ব্লক বোঝাতে ইনডেন্টেশন ব্যবহার করে। Python ইন্টারপ্রেটার জানে যে নিচের সংজ্ঞাটি এই ডেটা ক্লাসের অংশ নয় কারণ এটি ডেটা ক্লাসের ফিল্ডগুলোর মতো একই ইনডেন্টেশনে শুরু হয়নি।
...
Web3 থেকে EVM-এ একটি ফাংশন কল করার সিনট্যাক্স হলো এটি: `<contract object>.functions.<function name>().call(<parameters>)`। প্যারামিটারগুলো হতে পারে EVM ফাংশনের প্যারামিটার (যদি থাকে; এখানে নেই) অথবা ব্লকচেইনের আচরণ পরিবর্তন করার জন্য [নেমড প্যারামিটার](https://en.wikipedia.org/wiki/Named_parameter)। এখানে আমরা একটি ব্যবহার করছি, `block_identifier`, যাতে আমরা যে [ব্লক নম্বরে](/developers/docs/apis/json-rpc/#default-block) রান করতে চাই তা নির্দিষ্ট করতে পারি।

ফলাফল হলো [এই স্ট্রাক্ট, অ্যারে ফর্মে](https://github.com/Uniswap/v3-core/blob/main/contracts/UniswapV3Pool.sol#L56-L72)। প্রথম মানটি হলো দুটি টোকেনের মধ্যে এক্সচেঞ্জ রেটের একটি ফাংশন।

```python
        raw_price = (sqrt_price_x96 / Decimal(2**96)) ** 2
```

অনচেইন ক্যালকুলেশন কমানোর জন্য, ইউনিসোয়াপ v3 আসল এক্সচেঞ্জ ফ্যাক্টর সংরক্ষণ করে না বরং এর স্কয়ার রুট সংরক্ষণ করে। যেহেতু EVM ফ্লোটিং পয়েন্ট ম্যাথ বা ভগ্নাংশ সমর্থন করে না, তাই আসল মানের পরিবর্তে, রেসপন্স হলো <math><msqrt><mi>price</mi></msqrt><mo>&#x22C5;</mo><msup><mn>2</mn><mn>96</mn></msup></math>
...
আমরা যে র (raw) দাম পাই তা হলো প্রতিটি `token1`-এর জন্য আমরা যে পরিমাণ `token0` পাই। আমাদের পুলে `token0` হলো USDC (স্টেবলকয়েন যার মান একটি ইউএস ডলারের সমান) এবং `token1` হলো [WETH](https://opensea.io/learn/blockchain/what-is-weth)। আমরা আসলে যে মানটি চাই তা হলো প্রতি WETH-এর জন্য ডলারের পরিমাণ, এর বিপরীতটি নয়।

ডেসিমাল ফ্যাক্টর হলো দুটি টোকেনের [ডেসিমাল ফ্যাক্টরগুলোর](https://docs.openzeppelin.com/contracts/4.x/erc20#a-note-on-decimals) অনুপাত।
...
এটিকে মানুষ এবং লার্জ ল্যাঙ্গুয়েজ মডেল (LLM)-এর পড়ার উপযোগী একটি ফরম্যাটে রূপান্তর করতে [`datetime` লাইব্রেরি](https://docs.python.org/3/library/datetime.html) ব্যবহার করুন। মানটিকে দুই দশমিক স্থান পর্যন্ত রাউন্ড করতে [`Decimal.quantize`](https://docs.python.org/3/library/decimal.html#decimal.Decimal.quantize) ব্যবহার করুন।

```python
def get_quotes(pool: PoolInfo, start_block: int, end_block: int, step: int) -> list[Quote]:
```

Python-এ আপনি `list[<type>]` ব্যবহার করে এমন একটি [তালিকা (list)](https://docs.python.org/3/library/stdtypes.html#typesseq-list) সংজ্ঞায়িত করতে পারেন যা কেবল একটি নির্দিষ্ট টাইপ ধারণ করতে পারে।
...
Python-এ একটি [`for` লুপ](https://docs.python.org/3/tutorial/controlflow.html#for-statements) সাধারণত একটি তালিকার ওপর ইটারেট করে। কোট খোঁজার জন্য ব্লক নম্বরগুলোর তালিকা [`range`](https://docs.python.org/3/library/stdtypes.html#range) থেকে আসে।
...
এটি হলো স্ক্রিপ্টের মূল কোড। পুলের তথ্য পড়ুন, 12টি কোট পান এবং সেগুলোকে [`pprint`](https://docs.python.org/3/library/pprint.html#pprint.pprint) করুন।
...
1. তথ্য। LLM-গুলোর কাছে তাদের ট্রেনিং থেকে অনেক তথ্য থাকে, তবে সাধারণত তাদের কাছে সর্বশেষ তথ্য থাকে না। এই কারণেই আমাদের এখানে সর্বশেষ কোটগুলো পুনরুদ্ধার করতে হবে। একটি প্রম্পটে তথ্য যোগ করাকে [রিট্রিভাল অগমেন্টেড জেনারেশন (RAG)](https://en.wikipedia.org/wiki/Retrieval-augmented_generation) বলা হয়।
...
আমরা যে সময়ের জন্য একটি অনুমান চাই তা আমাদের LLM-কে প্রদান করতে হবে। ভবিষ্যতে "n মিনিট/ঘণ্টা/দিন" সময় পেতে, আমরা [`timedelta` ক্লাস](https://docs.python.org/3/library/datetime.html#datetime.timedelta) ব্যবহার করি।
...
এখানে, আমরা `format_quotes` দিয়ে প্রতিটি কোট তালিকার জন্য একটি স্ট্রিং তৈরি করতে [MapReduce](https://en.wikipedia.org/wiki/MapReduce) প্যাটার্ন ব্যবহার করি, তারপর প্রম্পটে ব্যবহারের জন্য সেগুলোকে একটি একক স্ট্রিংয়ে রিডিউস করি।
...
2. [অ্যাকাউন্টে ফান্ড যোগ করুন](https://platform.openai.com/settings/organization/billing/overview)—লেখার সময় ন্যূনতম পরিমাণ হলো $5
...
```
2026-01-05T19:50-এর জন্য প্রেডিকশন: প্রেডিক্টেড 3138.93 USD, আসল 3218.92 USD, এরর 79.99 USD
2026-01-06T19:56-এর জন্য প্রেডিকশন: প্রেডিক্টেড 3243.39 USD, আসল 3221.08 USD, এরর 22.31 USD
2026-01-07T20:02-এর জন্য প্রেডিকশন: প্রেডিক্টেড 3223.24 USD, আসল 3146.89 USD, এরর 76.35 USD
2026-01-08T20:11-এর জন্য প্রেডিকশন: প্রেডিক্টেড 3150.47 USD, আসল 3092.04 USD, এরর 58.43 USD
.
.
.
2026-01-31T22:33-এর জন্য প্রেডিকশন: প্রেডিক্টেড 2637.73 USD, আসল 2417.77 USD, এরর 219.96 USD
2026-02-01T22:41-এর জন্য প্রেডিকশন: প্রেডিক্টেড 2381.70 USD, আসল 2318.84 USD, এরর 62.86 USD
2026-02-02T22:49-এর জন্য প্রেডিকশন: প্রেডিক্টেড 2234.91 USD, আসল 2349.28 USD, এরর 114.37 USD
29টি প্রেডিকশনের ওপর গড় প্রেডিকশন এরর: 83.87103448275862068965517241 USD
...
এজেন্ট যে সংখ্যক নমুনা ব্যবহার করে ঠিক সেই সংখ্যক নমুনা পেতে [স্লাইস (slices)](https://www.w3schools.com/python/ref_func_slice.asp) ব্যবহার করুন। এখান থেকে পরবর্তী সেগমেন্টের মধ্যবর্তী কোডটি এজেন্টে থাকা একই প্রেডিকশন-পাওয়ার কোড।
...
লাভজনক দিনের সংখ্যা এবং লোকসানের দিনের সংখ্যা গণনা করতে [`filter`](https://www.w3schools.com/python/ref_func_filter.asp) ব্যবহার করুন। ফলাফলটি হলো একটি ফিল্টার অবজেক্ট, যার দৈর্ঘ্য পেতে আমাদের সেটিকে একটি তালিকায় রূপান্তর করতে হবে।

### ট্রানজ্যাকশন জমা দেওয়া {#submit-txn}
...
4. এগুলো হলো সেই কন্ট্রাক্ট যা আমাদের ব্যবহার করতে হবে। [`SwapRouter`](https://github.com/Uniswap/v3-periphery/blob/main/contracts/SwapRouter.sol) হলো ইউনিসোয়াপ v3 কন্ট্রাক্ট যা আমরা আসলে ট্রেড করার জন্য ব্যবহার করি। আমরা সরাসরি পুলের মাধ্যমে ট্রেড করতে পারতাম, তবে এটি অনেক সহজ।
...
বর্তমান দাম: 1843.16
2026-02-06T23:07-এ, প্রত্যাশিত দাম: 1724.41 USD
ট্রেডের আগে অ্যাকাউন্ট ব্যালেন্স:
USDC ব্যালেন্স: 927301.578272
WETH ব্যালেন্স: 500
বিক্রি করুন, আমি আশা করি দাম 118.75 USD কমে যাবে
অনুমোদন ট্রানজ্যাকশন পাঠানো হয়েছে: 74e367ddbb407c1aaf567d87aa5863049991b1d2aa092b6b85195d925e2bd41f
অনুমোদন ট্রানজ্যাকশন মাইন করা হয়েছে।
বিক্রি ট্রানজ্যাকশন পাঠানো হয়েছে: fad1bcf938585c9e90364b26ac7a80eea9efd34c37e5db81e58d7655bcae28bf
বিক্রি ট্রানজ্যাকশন মাইন করা হয়েছে।
ট্রেডের পরে অ্যাকাউন্ট ব্যালেন্স:
...
ট্রানজ্যাকশন প্যারামিটার। আমাদের এখানে একটি ফাংশন প্রয়োজন কারণ [নন্স](https://en.wikipedia.org/wiki/Cryptographic_nonce) প্রতিবার পরিবর্তন হতে হবে।
...
এই এজেন্টটি বর্তমানে কেবল একবার কাজ করে। তবে, আপনি এটিকে [`crontab`](https://man7.org/linux/man-pages/man1/crontab.1.html) থেকে রান করে অথবা 368-400 লাইনগুলোকে একটি লুপে র‍্যাপ করে এবং পরবর্তী সাইকেলের সময় হওয়া পর্যন্ত অপেক্ষা করতে [`time.sleep`](https://docs.python.org/3/library/time.html#time.sleep) ব্যবহার করে ক্রমাগত কাজ করার জন্য পরিবর্তন করতে পারেন।
...
- _প্রত্যাশিত পরিবর্তনের মাত্রা_। দাম কমার প্রত্যাশা থাকলে এজেন্ট একটি নির্দিষ্ট পরিমাণ `WETH` বিক্রি করে, কমার মাত্রা যাই হোক না কেন।
  যুক্তিযুক্তভাবে, ছোটখাটো পরিবর্তনগুলো উপেক্ষা করা এবং দাম কতটা কমবে বলে আমরা আশা করি তার ওপর ভিত্তি করে বিক্রি করা ভালো হবে।
- _বর্তমান পোর্টফোলিও_। যদি আপনার পোর্টফোলিওর 10% WETH-এ থাকে এবং আপনি মনে করেন দাম বাড়বে, তবে সম্ভবত আরও কেনা যৌক্তিক। কিন্তু যদি আপনার পোর্টফোলিওর 90% WETH-এ থাকে, তবে আপনি হয়তো যথেষ্ট এক্সপোজড, এবং আরও কেনার কোনো প্রয়োজন নেই। আপনি যদি দাম কমার প্রত্যাশা করেন তবে এর বিপরীতটি সত্য।
...
### এআই বট থেকে এআই এজেন্ট {#bot-to-agent}

আপনি একটি ভালো যুক্তি দিতে পারেন যে এটি [একটি এআই বট, কোনো এআই এজেন্ট নয়](/ai-agents/#ai-agents-vs-ai-bots)। এটি একটি অপেক্ষাকৃত সহজ কৌশল বাস্তবায়ন করে যা পূর্বনির্ধারিত তথ্যের ওপর নির্ভর করে। আমরা সেলফ-ইমপ্রুভমেন্ট সক্ষম করতে পারি, উদাহরণস্বরূপ, ইউনিসোয়াপ v3 পুলগুলোর একটি তালিকা এবং সেগুলোর সর্বশেষ মান প্রদান করে এবং জিজ্ঞাসা করে যে কোন কম্বিনেশনের সেরা প্রেডিক্টিভ মান রয়েছে।

### স্লিপেজ সুরক্ষা {#slippage-protection}

বর্তমানে কোনো [স্লিপেজ সুরক্ষা](https://uniswapv3book.com/milestone_3/slippage-protection.html) নেই। যদি বর্তমান কোট $2000 হয় এবং প্রত্যাশিত দাম $2100 হয়, তবে এজেন্ট কিনবে। তবে, এজেন্ট কেনার আগে যদি খরচ বেড়ে $2200 হয়ে যায়, তবে আর কেনার কোনো মানে হয় না।

স্লিপেজ সুরক্ষা বাস্তবায়ন করতে, [`agent.py`](https://github.com/qbzzt/260215-ai-agent/blob/05-trade/agent.py#L325)-এর 325 এবং 334 নম্বর লাইনে একটি `amountOutMinimum` মান নির্দিষ্ট করুন।
