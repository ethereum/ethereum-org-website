...
Fusaka 主要專注於基礎架構的改進，而 Glamsterdam 則透過確立不同網路參與者之間的職責分離，並引入更有效率的資料處理方式，為高吞吐量的平行化做好準備，進而推進「Scale L1」和「Scale Blobs」的<GlossaryTooltip termKey="state">狀態</GlossaryTooltip>。 

這些改進確保以太坊在處理更多交易時，仍然保持快速、經濟實惠和去中心化，同時也讓在家運行<GlossaryTooltip termKey="node">節點的</GlossaryTooltip>人，其硬體需求保持在可管理的範圍內。
...
區塊提交者和建構者之間這種非協定式的關係，在區塊驗證期間也會產生一個「熱路徑」，迫使<GlossaryTooltip termKey="validator">驗證者</GlossaryTooltip>在短短兩秒的視窗內快速完成交易廣播和執行，這限制了網路可以處理的資料量。

**確立的提議者-建構者分離（ePBS，或稱 EIP-7732）**正式將提議者（選擇區塊者）和建構者（組裝交易者）的工作分離，將這個過程「確立」到以太坊協議中，以消除協議外的信任。它還引入了有效負載及時性委員會（PTC）和雙重截止日期邏輯，驗證者分別證明及時性和資料可用性，以最大化吞吐量。 

<YouTube id="u8XvkTrjITs" />

在協議層級將區塊提交者和建構者的角色分開，可以將資料傳播到整個網路所需的時間從2秒延長到大約9秒。 

ePBS減少了對額外第三方軟體的依賴，讓以太坊能夠安全地處理更多資料（例如<GlossaryTooltip termKey="layer-2">第二層</GlossaryTooltip>的更多資料塊），而不會對網路造成壓力。
...
新增帳戶、代幣和<GlossaryTooltip termKey="smart-contract">智能合約</GlossaryTooltip>會產生永久性資料（稱為「狀態」），而每個運行網路的電腦都必須無限期地儲存這些資料。目前新增或讀取這些資料的費用不一致，而且不一定能反映它們對網路硬體造成的實際長期儲存負擔。
...
### 州政府燃料成本更新 {#state-access-gas-cost-update}
