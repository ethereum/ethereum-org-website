...
考慮客戶端的[要求](#requirements)，決定是在你自己的[硬體還是雲端](#local-vs-cloud)上執行軟體。

準備好環境後，可以使用[適合初學者的介面](#automatized-setup)，或使用終端機透過進階選項[手動](#manual-setup)安裝所選的客戶端。
...
- 雲端
  - 供應商提供高伺服器正常運行時間和靜態公共 IP 地址
  - 取得專用或虛擬伺服器可能比自己組裝更方便
  - 缺點是需要信任第三方（伺服器供應商）
  - 由於全節點所需的儲存空間較大，租用伺服器的價格可能會很高
- 自有硬體
  - 更無須信任且具主權的方法
  - 一次性投資
  - 可選擇購買預先設定好的機器
...
這兩種選項都有上述總結的不同優勢。如果你正在尋找雲端解決方案，除了許多傳統的雲端運算供應商之外，還有專注於部署節點的服務。請查看[節點即服務 (nodes as a service)](/developers/docs/nodes-and-clients/nodes-as-a-service/)，以了解更多關於託管節點的選項。
...
硬體要求因客戶端而異，但通常不會太高，因為節點只需要保持同步。請不要將其與挖礦混淆，挖礦需要多得多的運算能力。不過，更強大的硬體確實能改善同步時間和效能。
...
- 4 TB NVMe SSD（不建議使用無 DRAM 和 QLC 硬碟）
- 50 MBit/s 下載 / 15+ MBit/s 上傳頻寬（如果進行驗證則需要 25+ MBit/s 上傳）
...
你可以簡單地下載適合你作業系統和架構的執行檔應用程式或安裝套件。請務必驗證下載套件的簽章和校驗和。某些客戶端還提供儲存庫或 Docker 映像檔，以便更輕鬆地安裝和更新。所有客戶端都是開源的，因此你也可以從原始碼編譯它們。這是一種更進階的方法，但在某些情況下可能是必需的。

每個客戶端的安裝說明都在上方客戶端清單中連結的文件裡提供。
...
另一種驗證形式是確保你下載的軟體的雜湊（一種獨特的密碼學指紋）與開發人員提供的雜湊相符。這比使用 PGP 更簡單，而且某些客戶端僅提供此選項。只需對下載的軟體執行雜湊函數，並將其與發布頁面上的雜湊進行比較。例如：
...
讓我們先從會顯著影響客戶端效能和資料使用量的選項開始。[同步模式](/developers/docs/nodes-and-clients/#sync-modes)代表下載和驗證區塊鏈資料的不同方法。在啟動節點之前，你應該決定要使用哪個網路和同步模式。最需要考慮的是客戶端所需的磁碟空間和同步時間。請注意客戶端的文件，以確定哪種同步模式是預設的。如果這不適合你，請根據安全等級、可用資料和成本選擇另一種模式。除了同步演算法之外，你還可以設定修剪不同種類的舊資料。修剪可以刪除過時的資料，即移除從最近區塊無法到達的狀態樹節點。

其他基本設定選項包括：選擇網路（主網或測試網）、啟用 RPC 或 WebSockets 的 HTTP 端點等。你可以在客戶端的文件中找到所有功能和選項。可以透過在 CLI 或設定檔中直接使用相應的標記執行客戶端來設定各種客戶端配置。每個客戶端都略有不同；請務必參閱其官方文件或說明頁面，以了解設定選項的詳細資訊。

出於測試目的，你可能更傾向於在其中一個測試網網路上執行客戶端。[查看支援網路的概覽](/developers/docs/nodes-and-clients/#execution-clients)。
...
- 指定要連接的網路，在我們的範例中為主網
  - 你可以改為選擇[其中一個測試網](/developers/docs/networks/)，對你的設定進行初步測試
...
寧布斯 (Nimbus) 附帶共識和執行客戶端。它可以在各種裝置上執行，即使運算能力非常有限。
在[安裝相依套件和寧布斯 (Nimbus) 本身](https://nimbus.guide/quick-start.html)之後，你可以執行其共識客戶端：
...
普萊斯姆 (Prysm) 附帶允許輕鬆自動安裝的腳本。詳細資訊可以在 [Prysm 文件](https://prysm.offchainlabs.com/docs/install-prysm/install-with-script/)中找到。
...
設定網頁伺服器、代理伺服器或面向外部的 Rest API 並不是提供存取節點 RPC 端點的唯一方法。另一種保護隱私的設定公開可存取端點的方法是，將節點託管在你自己的 [Tor](https://www.torproject.org/) 洋蔥服務上。這將讓你無需靜態公共 IP 地址或開啟連接埠，即可在區域網路外部存取 RPC。然而，使用此設定可能只允許透過 Tor 網路存取 RPC 端點，這並非所有應用程式都支援，並且可能會導致連線問題。

為此，你必須建立自己的[洋蔥服務](https://community.torproject.org/onion-services/)。查看關於洋蔥服務設定的[文件](https://community.torproject.org/onion-services/setup/)以託管你自己的服務。你可以將其指向帶有 RPC 連接埠代理的網頁伺服器，或者直接指向 RPC。
...
- 強制關機可能會損壞資料庫，需要你重新同步整個節點。
- 你的客戶端將與網路失去同步，並在重新啟動時需要重新同步。雖然節點可以從上次關閉的地方開始同步，但該過程可能需要一些時間，具體取決於它離線了多久。
