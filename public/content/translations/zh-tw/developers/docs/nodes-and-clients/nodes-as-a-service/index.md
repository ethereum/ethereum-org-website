...
- [**Alchemy**](https://alchemy.com/)
  ...
  - 功能
    - 最大的免費方案，每月提供 3 億個運算單元（約 3000 萬次 getLatestBlock 請求）
    - 支援 Polygon、Starknet、Optimism、Arbitrum 等多鏈
    - 為約 70% 的大型以太坊去中心化應用程式 (dapp) 和去中心化金融 (DeFi) 交易量提供支援
    - 透過 Alchemy Notify 提供即時 webhook 警報
    - 一流的支援與可靠性/穩定性
    - Alchemy 的 NFT API
    - 包含請求瀏覽器、記憶體池觀察器和 Composer 的儀表板
    - 整合的測試網水龍頭存取權限
...
- [**Allnodes**](https://www.allnodes.com/)
  ...
  - 功能
    - 使用在 Allnodes 投資組合頁面上建立的 PublicNode 代幣，無速率限制。
    - 在 [PublicNode](https://www.publicnode.com) 上提供注重隱私的免費 RPC 端點（超過 100 條區塊鏈）
    - 為超過 90 條區塊鏈提供無速率限制的專屬節點
    - 為超過 30 條區塊鏈提供專屬歸檔節點
    - 在 3 個地區（美國、歐盟、亞洲）可用
    - 在 [PublicNode](https://www.publicnode.com/snapshots) 上提供超過 100 條區塊鏈的快照
    - 24/7 全天候技術支援，提供 99.90%-99.98% 的正常運行時間 SLA（取決於方案）。
    - 按小時付費定價
    - 使用信用卡、PayPal 或加密貨幣付款
...
- [**Ankr**](https://www.ankr.com/)
  ...
  - 功能
    - Ankr 協定 - 開放存取超過 8 條鏈的公共 RPC API 端點
    - 負載平衡和節點健康監控，提供快速可靠的閘道器以連接最近的可用節點
    - 進階方案啟用 WSS 端點和無上限的速率限制
    - 一鍵部署超過 40 條鏈的全節點和驗證者節點
    - 隨需擴展
    - 分析工具
    - 儀表板
    - RPC、HTTPS 和 WSS 端點
...
- [**dRPC**](https://drpc.org/)
  ...
  - NodeCloud：隨插即用的 RPC 基礎設施，起價 10 美元——全速且無限制
  - NodeCloud 功能：
    - 支援 185 個網路的 API
    - 超過 40 個提供商的分散式資源池
    - 全球覆蓋，擁有九 (9) 個地理叢集
    - AI 驅動的負載平衡系統
    - 隨需付費的統一定價——不漲價、不過期、無綁約
    - 無限制的金鑰、精細的金鑰調整、團隊角色、前端保護
    - 方法統一費率為每個方法 20 個運算單元 (CU)
    - [公共端點鏈列表](https://drpc.org/chainlist)
    - [定價計算機](https://drpc.org/pricing#calculator)
  - NodeCore：為希望擁有完全控制權的組織提供的開源堆疊
...
- [**Kaleido**](https://kaleido.io/)
  ...
  - 功能
    ...
    - 用於事件傳遞的對外串流（由 Apache Kafka 支援）
    - 豐富的「鏈下」和輔助服務集合（例如，雙邊加密訊息傳輸）
    - 透過治理和基於角色的存取控制，實現簡單明瞭的網路入門引導
    - 為管理員和終端使用者提供完善的使用者管理
    - 高度可擴展、具備彈性的企業級基礎設施
    - 雲端 HSM 私鑰管理
    - 以太坊主網綁定
...
- [**NodeFlare**](https://nodeflare.app/)
  ...
  - 功能
    - 23 條 EVM 鏈，包含以太坊、Base、Arbitrum One 和 Nova、Optimism、Linea 以及 Unichain
    - 5 個地區（歐洲、英國、亞洲、美國東部、美國西部），具備自動容錯移轉至最近健康節點的功能
    - 免費公共端點（無需 API 金鑰）+ 每月 300 萬個運算單元的免費方案
    - 運算單元計費——只為你使用的資源付費，較繁重的呼叫成本較高
    - 付費方案無節流限制
...
- [**Pocket Network**](https://www.pokt.network/)
  ...
  - 功能
    ...
    - 支援超過 15 條區塊鏈
    - 超過 6400 個節點透過為應用程式提供服務來賺取 POKT
    - 支援歸檔節點、具備 Trace 功能的歸檔節點和測試網節點
    - 以太坊主網節點客戶端多樣性
    - 無單點故障
    - 零停機時間
    - 具成本效益的近零代幣經濟學（質押一次 POKT 即可獲得網路頻寬）
...
- [**QuickNode**](https://www.quicknode.com)
  ...
  - 功能
    ...
    - 地理平衡、多雲/裸機、低延遲網路
    - 多鏈支援（Optimism、Arbitrum、Polygon 及其他 11 條鏈）
    - 用於提升速度和穩定性的中介層（呼叫路由、快取、索引）
    - 透過 Webhook 監控智能合約
    - 直覺的儀表板、分析套件、RPC composer
    - 進階安全功能（JWT、遮罩、白名單）
    - NFT 資料和分析 API
...
- [**Tokenview**](https://services.tokenview.io/)
  ...
  - 功能
    ...
    - 多鏈支援（比特幣、以太坊、Tron、BNB 智能鏈、以太坊經典）
    - RPC 和 WSS 端點皆開放使用
    - 無限制存取歸檔資料 API
    - 包含請求瀏覽器和記憶體池觀察器的儀表板
...
- [**Zeeve**](https://www.zeeve.io/)
  ...
  - 功能
    ...
    - 支援超過 30 種協定和整合，並持續增加中
    - 為現實世界用例提供附加價值的 Web3 基礎設施服務，如去中心化儲存、去中心化身分 (DID) 和區塊鏈帳本資料 API
    - 24/7 全天候支援和主動監控，確保節點始終保持健康。
    - RPC 端點提供經過驗證的 API 存取權限，並透過直覺的儀表板和分析實現無憂管理。
    - 提供託管雲端和自帶雲端選項供選擇，並支援所有主要雲端提供商，如 AWS、Azure、Google Cloud、Digital Ocean 以及本地部署。
