...
ERC-4626 是一個用於最佳化和統一收益型金庫技術參數的標準。它為代表單一基礎 ERC-20 代幣份額的代幣化收益型金庫提供了一個標準 API。ERC-4626 還概述了使用 ERC-20 的代幣化金庫的選用擴充功能，提供存款、提款代幣和讀取餘額的基本功能。

**ERC-4626 在收益型金庫中的角色**

借貸市場、聚合器和本質上生息的代幣透過執行不同的策略，幫助使用者在他們的加密貨幣代幣上找到最佳收益。這些策略的執行方式略有不同，這可能容易出錯或浪費開發資源。
...
ERC-4626 針對達到上限前的原子化存款和贖回進行了最佳化。如果達到上限，則無法提交新的存款或贖回。這種限制對於任何將非同步操作或延遲作為與金庫互動先決條件的智能合約系統（例如：現實世界資產協定、抵押不足借貸協定、跨鏈借貸協定、流動性質押代幣或保險安全模組）來說，運作效果並不理想。

ERC-7540 擴展了 ERC-4626 金庫在非同步使用案例中的實用性。現有的金庫介面（`deposit`/`withdraw`/`mint`/`redeem`）被充分利用來申領非同步請求。

了解更多關於 [ERC-7540 非同步代幣化金庫](/developers/docs/standards/tokens/erc-7540/)的資訊。

**多資產金庫擴充功能 (ERC-7575)**

ERC-4626 不支援的一個缺失使用案例是具有多種資產或入口點（例如流動性提供者 (LP) 代幣）的金庫。由於 ERC-4626 要求其本身必須是 ERC-20，這些金庫通常難以管理或不符合規範。
