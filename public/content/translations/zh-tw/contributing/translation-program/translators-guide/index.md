...
![側邊欄中的文章標題](./article-titles-in-sidebar.png)
![編輯器中的文章標題](./article-titles-in-editor.png)
...
![Crowdin 警告範例](./crowdin-warning-example.png)
...
1. 開啟設定
   ![如何在編輯器中開啟設定](./editor-settings.png)

2. 向下捲動至「HTML tags displaying」區段

3. 選取「隱藏」
   ![請選取「隱藏」](./hide-tags.png)
...
![link.png](./example-of-link.png 的例子)
...
![tags.png](./example-of-links-inside-tags.png 內的連結的範例)
...
範例：`<strong x-id="1">`去中心化`</strong>`

`<strong x-id="1">` - _讓文字變粗體的開頭標籤_

去中心化 - _可翻譯的文字_

`</strong>` - _結束標籤_

![「strong」標籤範例](./example-of-strong-tags.png)
...
![程式碼 snippets.png](./example-of-code-snippets.png 的例子)

原文還包含縮短的標籤，這類標籤只含有數字，這意味著標籤的功能不明顯。 你可以將滑鼠停留在這些標籤上，查看標籤的確切功能。

在下方的範例中，您可以看到將滑鼠懸停在 `<0>` 標籤上會顯示它代表 `<code>` 並包含程式碼片段，因此不應翻譯這些標籤內的內容。

![意義含糊的 tags.png](./example-of-ambiguous-tags.png 例子)
