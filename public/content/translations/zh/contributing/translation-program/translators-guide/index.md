...
不同的语言有不同的语法规则、习惯用法和语序。在翻译时，请注意目标语言的句子结构，避免对英文原文进行直译，因为这会导致句子结构糟糕且可读性差。

建议你通读整个句子，并根据目标语言的习惯进行调整，而不是逐字翻译原文。

## 正式与非正式 {#formal-vs-informal}

我们使用正式的称呼，这始终是礼貌的，并且适合所有访问者。

使用正式称呼可以避免显得不够官方或具有冒犯性，并且无论访问者的年龄和性别如何都适用。

大多数印欧语系和亚非语系语言使用区分男女的特定性别第二人称代词。在称呼用户或使用物主代词时，我们可以避免假设访问者的性别，因为正式称呼通常适用且一致，无论他们的性别认同如何。

## 简单明了的词汇和含义 {#simple-vocabulary}

我们的目标是让尽可能多的人能够理解网站上的内容。

在大多数情况下，通过使用易于理解的简短词汇就可以轻松实现这一点。如果某个词在你的语言中有多个含义相同的可能翻译，最好的选择通常是能清晰反映含义的最短词汇。
...
在将新页面上传到 Crowdin 时，我们会隐藏译者绝对不应翻译的内容，这意味着在 Crowdin 中对译者可见的所有元数据都应被翻译。

在翻译源文本为“en”的任何字符串时，请务必特别注意。这代表页面可用的语言，应翻译为[你所用语言的 ISO 语言代码](https://www.andiamo.co.uk/resources/iso-language-codes/)。这些字符串应始终使用拉丁字符进行翻译，而不是目标语言的本地书写文字。
...
某些字符串包含外部文章的标题。我们的大多数开发者文档页面都包含指向外部文章的链接，以供进一步阅读。包含文章标题的字符串需要被翻译，无论文章使用的是何种语言，以确保使用母语浏览页面的访问者获得更一致的用户体验。

你可以在下面找到这些字符串对译者来说是什么样子的，以及如何识别它们的一些示例（文章链接通常可以在这些页面的底部“延伸阅读”部分找到）：
...
**切勿忽略这些警告，因为它们通常意味着出现了错误，或者翻译中遗漏了原文的关键部分。**

当你忘记在翻译中添加标签时，Crowdin 警告的示例：
![Example of a Crowdin warning](./crowdin-warning-example.png)

## 处理标签和代码片段 {#dealing-with-tags}

许多源内容包含标签和变量，它们在 Crowdin 编辑器中以黄色突出显示。它们具有不同的功能，应正确处理。

**Crowdin 设置**

为了更轻松地管理标签并直接从原文中复制它们，我们建议在 Crowdin 编辑器中更改你的设置。
...
处理链接的最佳方法是直接从原文中复制它们，可以通过点击它们或使用“Copy Source”（复制原文）按钮（`Alt+C`）来实现。

![Example of link.png](./example-of-link.png)

链接也会以标签的形式出现在源文本中（即 `<0>` `</0>`）。如果你将鼠标悬停在标签上，编辑器将显示其完整内容——有时这些标签代表链接。

从原文中复制链接并且不改变它们的顺序是非常重要的。
...
源文本包含许多不同类型的标签，应始终从原文中复制且切勿更改。与上述类似，翻译中这些标签的顺序也应与原文保持一致。
...
源文本还包含缩短的标签，它们仅包含数字，这意味着它们的功能并不一目了然。你可以将鼠标悬停在这些标签上，以确切查看它们的作用。
...
## 缩写与全称 {#short-vs-full-forms}

网站上使用了许多缩写，例如 dapps、NFT、DAO、DeFi 等。这些缩写在英语中很常用，大多数网站访问者都熟悉它们。
...
- 去中心化应用 (dapps) → _翻译后的全称（括号内为英文缩写）_

## 没有既定翻译的术语 {#terms-without-established-translations}

某些术语在其他语言中可能没有既定的翻译，而是以原始英文术语广为人知。此类术语主要包括较新的概念，如工作量证明 (PoW)、权益证明 (PoS)、信标链、质押等。

虽然翻译这些术语听起来可能不太自然，因为英文版本在其他语言中也很常用，但我们强烈建议对它们进行翻译。
...
另一种形式的包容性是努力为全球受众进行翻译，而不针对任何特定国家、种族或地区。

最后，语言应适合所有受众和年龄段。

## 特定语言的翻译 {#language-specific-translations}

在翻译时，重要的是遵循你所用语言的语法规则、习惯用法和格式，而不是照搬原文。源文本遵循英语的语法规则和习惯用法，这并不适用于许多其他语言。
...
- 列表项的标点符号也是如此。列表中的句末标点符号可以是句号（**.**）、逗号（**,**）或分号（**;**），具体取决于语言。

**引号**

- 各种语言使用许多不同的引号。简单地从原文中复制英文引号通常是不正确的。
- 一些最常见的引号类型包括：
  - „example text“
  - ‚example text’
  - »example text«
...
- 在翻译日期时，根据语言的不同，有许多需要考虑的因素和差异。这些包括日期格式、分隔符、大写和前导零。全称日期和数字日期之间也存在差异。
  - 不同日期格式的一些示例：
...
- 除了计量单位的本地化之外，还需要注意不同语言处理这些单位的差异。主要区别在于数字和单位之间的空格，这可能因语言而异。这方面的例子包括 100kB 与 100 kB，或 50ºF 与 50 ºF。
