...
有意义的一层网络 (l1) 扩容需要摆脱协议外信任假设和串行执行限制。格拉姆斯特丹通过将某些区块构建职责的分离写入协议，并引入允许网络为并行处理做准备的新数据结构来解决这个问题。
...
随着以太坊网络发展得越来越快，确保使用它的成本与运行以太坊的硬件的损耗相匹配非常重要。网络需要提高其整体容量限制，以便安全地扩容并处理更多交易。
...
以太坊的格拉姆斯特丹升级旨在改善用户体验，增强数据可发现性，并处理不断增加的消息大小以防止同步失败。这使得跟踪链上发生的事情变得更加容易，同时防止在网络扩展时出现技术故障。
