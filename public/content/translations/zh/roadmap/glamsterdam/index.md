...
简而言之，格拉姆斯特丹将引入结构性变化，以确保随着网络容量的增加，它保持可持续性并且性能保持在较高水平。
...
对验证者职责和退出流程的改进确保了在大规模罚没事件期间的网络稳定性，并加快了质押者转移其质押的速度。这些改进使网络更加稳定，并确保所有参与者（无论大小）都受到公平对待。
...
- 通过让退出容量随质押的 ETH 总量扩展，而不是限制在固定速率，显著减少质押提款时间
- 为验证者合并提供其专用的队列容量，加速向更大、更高效的验证者过渡
- 通过仔细分析的安全参数维护网络安全

以太坊的流转限制是对验证者进入、退出或合并其质押的 ETH 速率的安全限制，以确保网络的安全性永远不会被破坏。如今，退出和激活共享一个单一的上限，因此在需求旺盛的时期，质押者可能面临漫长的等待才能提取他们的 ETH。合并，即验证者合并成拥有多达 2,048 个 ETH 的更大验证者（由[佩克特拉升级](/roadmap/pectra)启用），也竞争这种有限的容量，这意味着以目前的速度合并整个验证者集将需要数年时间。

**增加退出和合并流转限制（或 EIP-8061）** 将这些限制重组为独立的通道：

- 验证者激活保持其现有的上限，保持不变
- 退出不再受限，而是随质押的 ETH 总量扩展
- 合并获得其专用的容量，大约是激活和退出组合限制大小的一半

在当前的质押水平下，这使退出容量增加了约 4 倍，合并容量增加了约 2 倍，这意味着质押者可以在高需求时期更快地提取他们的 ETH，并且网络更快地过渡到更小、更高效的验证者集。
...
### 在格拉姆斯特丹升级后，我现有的智能合约会有任何变化吗？ {#will-my-smart-contracts-change}

现有的合约在格拉姆斯特丹升级后将继续正常运行。开发人员可能会获得几个新工具，并且应该审查他们的 gas 使用情况：

- 增加最大合约大小（或 EIP-7954）允许开发人员部署更大的应用程序，将最大合约大小限制从大约 24KiB 提高到 64KiB。
- 确定性工厂预部署（或 EIP-7997）引入了一个通用的、内置的工厂合约。它允许开发人员将他们的应用程序和智能合约钱包部署到所有参与的 EVM 链上完全相同的地址。
- 如果你的应用程序依赖于复杂的跟踪来查找 ETH 转账，ETH 转账和销毁发出日志（或 EIP-7708）将允许你切换到使用日志进行更简单、更可靠的记账。
