...
Fusaka 专注于基础改进，而 Glamsterdam 则通过明确不同网络参与者之间的职责分离，并引入更有效的数据处理方式，为高吞吐量并行化做好<GlossaryTooltip termKey="state">状态</GlossaryTooltip>，从而推进“Scale L1”和“Scale Blobs”目标。 

这些改进确保了以太坊在处理更多活动时仍然保持快速、经济和去中心化，同时使在家运行<GlossaryTooltip termKey="node">节点的</GlossaryTooltip>人们能够轻松管理硬件要求。
...
提案者和构建者之间这种协议外的关系，在区块验证期间也创建了一个“热路径”，迫使<GlossaryTooltip termKey="validator">验证者</GlossaryTooltip>在短短 2 秒的窗口内快速完成交易广播和执行，从而限制了网络可以处理的数据量。

**确立的提议者-构建者分离（ePBS，或 EIP-7732）**正式将提议者（选择区块）和构建者（组装交易）的工作分离，将此过程“确立”到以太坊协议中，以消除协议外的信任。它还引入了有效载荷及时性委员会（PTC）和双截止日期逻辑，验证者分别证明及时性和数据可用性，以最大化吞吐量。 

<YouTube id="u8XvkTrjITs" />

在协议层面分离提案者和构建者的角色，将传播窗口（或在网络中传播数据的时间）从2秒扩展到大约9秒。 

ePBS 减少了对额外第三方软件的依赖，并允许以太坊安全地处理更多数据（例如<GlossaryTooltip termKey="layer-2">第 2 层</GlossaryTooltip>中的更多 blob），而不会给网络带来压力。
...
添加新账户、代币和<GlossaryTooltip termKey="smart-contract">智能合约</GlossaryTooltip>会创建永久数据（称为“状态”），运行网络的每台计算机都必须无限期地存储这些数据。目前添加或读取这些数据的费用不一致，并且不一定反映它们对网络硬件造成的实际长期存储负担。
...
- **谨防诈骗！**<Emoji text="⚠️" /> **任何指示您“升级”ETH 的人都是想诈骗您。**您无需为此升级做任何操作。您的资产将完全不受影响。请记住，随时了解最新信息是防范诈骗的最佳方法。
