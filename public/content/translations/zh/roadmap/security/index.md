...
- **区块构建与抗审查性**：ePBS 和区块级访问列表正在格拉姆斯特丹开发网上运行。FOCIL 计划用于 Hegotá，目标是在 2027 年进行。
- **最终性**：Minimmit 和更广泛的精简以太坊共识工作仍处于活跃研究阶段，尚未分配分叉。
- **抗量子性**：每周的后量子互操作开发网正在运行，核心基础设施里程碑目标大约在 2029 年完成。
- **简化**：佩克特拉和弗萨卡已发布；格拉姆斯特丹和 Hegotá 将进行下一轮清理。

这项工作没有任何部分是已经完成的，所有时间表都是可能会发生变化的估计。
