---
title: 以太坊开发入门
description: 这是一份以太坊开发入门的初学者指南。我们将带你从启动 API 端点、发起命令行请求，一直到编写你的第一个 Web3 脚本！无需任何区块链开发经验！
author: 埃兰·哈尔彭
tags:
  - javascript
  - ethers.js
...
这是一份以太坊开发入门的初学者指南。在本教程中，我们将使用 [Alchemy](https://alchemyapi.io/)，这是一家领先的区块链开发者平台，为排名前 70% 的区块链应用程序（包括 Maker、0x、MyEtherWallet、Dharma 和 Kyber）的数百万用户提供支持。Alchemy 将为我们提供以太坊链上的 API 端点访问权限，以便我们可以读取和写入交易。

我们将带你从注册 Alchemy 开始，一直到编写你的第一个 Web3 脚本！无需任何区块链开发经验！

## 1. 注册免费的 Alchemy 账户 {#sign-up-for-a-free-alchemy-account}

注册 Alchemy 账户非常简单，[在此免费注册](https://auth.alchemy.com/)。

## 2. 创建 Alchemy 应用程序 {#create-an-alchemy-app}

为了与以太坊链通信并使用 Alchemy 的产品，你需要一个 API 密钥来验证你的请求。
...
在“Create App”下填写详细信息以获取你的新密钥。你还可以在此处查看你以前创建的应用程序以及你的团队创建的应用程序。通过点击任何应用程序的“View Key”（查看密钥）来提取现有密钥。
...
**如果你已有客户端，**请将你当前的节点提供商 URL 更改为带有你 API 密钥的 Alchemy URL：`“https://eth-mainnet.alchemyapi.io/v2/your-api-key"`

**_注意：_** 以下脚本需要在 **Node 环境**中运行或**保存在文件中**，而不是从命令行运行。如果你尚未安装 Node 或 npm，请查看这份简短的 [Mac 设置指南](https://app.gitbook.com/@alchemyapi/s/alchemy/guides/alchemy-for-macs)。

有许多 [Web3 库](https://docs.alchemyapi.io/guides/getting-started#other-web3-libraries)可以与 Alchemy 集成，但是，我们建议使用 [Alchemy Web3](https://docs.alchemy.com/reference/api-overview)，它是 Web3.js 的直接替代品，经过构建和配置，可与 Alchemy 无缝协作。这提供了多种优势，例如自动重试和强大的 WebSocket 支持。
...
**1. 如果你还没有这样做，请在终端中创建一个新的项目目录并 cd 进入该目录：**
...
对异步（async）内容不熟悉？请查看这篇 [Medium 文章](https://medium.com/better-programming/understanding-async-await-in-javascript-1d81bb079b2c)。
...
不知道接下来该做什么？尝试在我们的 [Hello World 智能合约指南](https://www.alchemy.com/docs/hello-world-smart-contract)中部署你的第一个智能合约并亲自动手进行一些 Solidity 编程，或者通过 [Dashboard Demo App](https://docs.alchemyapi.io/tutorials/demo-app) 测试你的仪表板知识！
