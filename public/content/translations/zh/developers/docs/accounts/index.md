...
- `storageRoot` – 有时被称为存储哈希。 对 [Merkle Patricia Trie](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/)（默克尔帕特里夏树）根节点的 256 位哈希，该树编码了帐户的存储内容（一种 256 位整数值间的映射关系），在该树中，该映射关系被编码为以“256 位整数键的 Keccak 256 位哈希值” 为键，以“经 RLP 编码的 256 位整数值”为值的键值对。 此 Trie 对此帐户存储内容的哈希进行编码，默认情况下为空。
