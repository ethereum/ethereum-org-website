...
[以太坊](/)节点必须使用一些基本信息来标识自己，以便连接到对等节点。为了确保任何潜在的对等节点都能解释这些信息，它会以任何以太坊节点都能理解的三种标准化格式之一进行中继：multiaddr、enode 或以太坊节点记录 (ENR)。ENR 是目前以太坊网络地址的标准。

## 前提条件 {#prerequisites}

需要对以太坊的[网络层](/developers/docs/networking-layer/)有所了解才能理解本页内容。

## Multiaddr {#multiaddr}

最初的以太坊节点地址格式是“multiaddr”（“multi-addresses”的缩写）。Multiaddr 是一种专为点对点网络设计的通用格式。地址表示为键值对，键和值之间用正斜杠分隔。例如，IPv4 地址为 `192.168.22.27` 且监听 TCP 端口 `33000` 的节点的 multiaddr 如下所示：

`/ip4/192.168.22.27/tcp/33000`

对于以太坊节点，multiaddr 包含节点 ID（其公钥的哈希）：
