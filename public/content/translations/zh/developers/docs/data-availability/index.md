...
**数据可用性**是指用户确信验证区块所需的数据确实可供所有网络参与者使用。对于[以太坊](/)一层网络 (l1) 上的全节点来说，这相对简单；全节点会下载每个区块中所有数据的副本——数据_必须_可用才能进行下载。丢失数据的区块将被丢弃，而不是被添加到区块链中。这就是“链上数据可用性”，也是单体区块链的一个特征。全节点不会被欺骗去接受无效交易，因为它们会亲自下载并执行每一笔交易。然而，对于模块化区块链、二层网络 (l2) 汇总和轻客户端来说，数据可用性的情况更为复杂，需要一些更复杂的验证程序。
