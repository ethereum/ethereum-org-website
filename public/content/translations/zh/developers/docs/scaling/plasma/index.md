...
她还必须提供一个默克尔证明，以验证在 Plasma 链上创建其资金的交易已包含在区块中。这对于使用 [未花费的交易输出 (UTXO)](https://en.wikipedia.org/wiki/Unspent_transaction_output) 模型的 Plasma 迭代（例如 [Plasma MVP](https://www.learnplasma.org/en/learn/mvp.html)）是必要的。

其他迭代（如 [Plasma Cash](https://www.learnplasma.org/en/learn/cash.html)）将资金表示为[非同质化代币](/developers/docs/standards/tokens/erc-721/)，而不是 UTXO。在这种情况下，提款需要提供 Plasma 链上代币的所有权证明。这是通过提交涉及该代币的最近两笔交易，并提供验证这些交易包含在区块中的默克尔证明来完成的。
...
- [理解 Plasma，第 1 部分：基础知识](https://www.theblockcrypto.com/amp/post/10793/understanding-plasma-part-1-the-basics)
