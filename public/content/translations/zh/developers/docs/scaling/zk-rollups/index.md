...
零知识卷叠 (ZK-rollup) 是二层网络 (l2) [扩容解决方案](/developers/docs/scaling/)，通过将计算和状态存储转移到链下，来提高[以太坊](/)主网的吞吐量。ZK-rollup 可以批量处理数千笔交易，然后仅将一些最少的摘要数据发布到主网。该摘要数据定义了应对以太坊状态进行的更改，以及证明这些更改正确无误的一些密码学证明。
...
## ZK-rollup 和 EVM 兼容性 {#zk-rollups-and-evm-compatibility}

与乐观 Rollup不同，ZK-rollup 并不容易与[以太坊虚拟机 (EVM)](/developers/docs/evm/) 兼容。在电路中证明通用 EVM 计算比证明简单计算（如前面描述的代币转账）更困难且更耗费资源。
...
## ZK-rollup 如何扩展以太坊？ {#scaling-ethereum-with-zk-rollups}
...
ZK-rollup 可以比乐观 Rollup更好地压缩交易数据，因为它们不必发布验证每笔交易所需的所有数据。它们只需发布重建 Rollup 上账户和余额最新状态所需的最少数据。
...
### ZK-rollup 的优缺点 {#zk-rollups-pros-and-cons}
...
| 依靠无须信任的密码学机制来保证安全，而不是像[乐观 Rollup](/developers/docs/scaling/optimistic-rollups/#optimistic-pros-and-cons)那样依赖受激励参与者的诚实。 | 生成有效性证明需要专门的硬件，这可能会助长少数几方对链的中心化控制。                                                                    |
...
## 关于 ZK-rollup 的进一步阅读 {#further-reading-on-zk-rollups}
