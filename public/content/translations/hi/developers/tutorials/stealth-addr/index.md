...
यह लेख एक गोपनीय-पता एप्लिकेशन की व्याख्या करता है जो [गिटहब पर उपलब्ध है](https://github.com/qbzzt/251022-stealth-addresses.git)।

### उपकरण {#tools}

एक [टाइपस्क्रिप्ट गोपनीय पता लाइब्रेरी](https://github.com/ScopeLift/stealth-address-sdk) है जिसका हम उपयोग कर सकते हैं। हालाँकि, क्रिप्टोग्राफ़िक संचालन CPU-गहन हो सकते हैं। मैं उन्हें [रस्ट](https://rust-lang.org/) जैसी संकलित भाषा में लागू करना पसंद करता हूँ, और ब्राउज़र में कोड चलाने के लिए [WASM](https://webassembly.org/) का उपयोग करता हूँ।

हम [वीट](https://vite.dev/) और [रिएक्ट](https://react.dev/) का उपयोग करने जा रहे हैं। ये उद्योग-मानक उपकरण हैं; यदि आप इनसे परिचित नहीं हैं, तो आप [इस ट्यूटोरियल](/developers/tutorials/creating-a-wagmi-ui-for-your-contract/) का उपयोग कर सकते हैं। वीट का उपयोग करने के लिए, हमें Node की आवश्यकता है।

### गोपनीय पतों को क्रिया में देखें {#in-action}

1. आवश्यक उपकरण स्थापित करें: [रस्ट](https://rust-lang.org/tools/install/) और [Node](https://nodejs.org/en/download)।
...
WASM में संकलित होने वाला स्रोत कोड [रस्ट](https://rust-lang.org/) में लिखा गया है। आप इसे [`src/rust_wasm/src/lib.rs`](https://github.com/qbzzt/251022-stealth-addresses/blob/main/src/rust-wasm/src/lib.rs) में देख सकते हैं। यह कोड मुख्य रूप से जावास्क्रिप्ट कोड और [`eth-stealth-addresses` लाइब्रेरी](https://github.com/kassandraoftroy/eth-stealth-addresses) के बीच एक इंटरफ़ेस है।

**`Cargo.toml`**

रस्ट में [`Cargo.toml`](https://doc.rust-lang.org/cargo/reference/manifest.html) जावास्क्रिप्ट में [`package.json`](https://docs.npmjs.com/cli/v9/configuring-npm/package-json) के अनुरूप है। इसमें पैकेज जानकारी, निर्भरता घोषणाएँ आदि शामिल हैं।
...
रस्ट से WASM पैकेज बनाने के लिए परिभाषाएँ। वे [यहां](https://wasm-bindgen.github.io/wasm-bindgen/reference/attributes/index.html) प्रलेखित हैं।
...
रस्ट आमतौर पर मानों के लिए बाइट [ऐरे](https://doc.rust-lang.org/std/primitive.array.html) (`[u8; <size>]`) का उपयोग करता है। लेकिन जावास्क्रिप्ट में, हम आमतौर पर हेक्साडेसिमल स्ट्रिंग्स का उपयोग करते हैं। [`hex` लाइब्रेरी](https://docs.rs/hex/latest/hex/) हमारे लिए एक प्रतिनिधित्व से दूसरे में अनुवाद करती है।
...
वापसी मान `Option<[u8; N]>` है, जिसका अर्थ है कि लौटाया गया ऐरे [वैकल्पिक](https://doc.rust-lang.org/std/option/) है। यह रस्ट में उन फ़ंक्शन के लिए एक विशिष्ट पैटर्न है जो विफल हो सकते हैं।
...
रस्ट को किसी फ़ंक्शन के अंत में मान लौटाते समय [`return`](https://doc.rust-lang.org/std/keyword.return.html) कीवर्ड का उपयोग करने की आवश्यकता नहीं होती है।
...
हमें दो वीट प्लगइन्स की आवश्यकता है: [react](https://www.npmjs.com/package/@vitejs/plugin-react) और [wasm](https://github.com/Menci/vite-plugin-wasm#readme)।
...
जब हम [`wasm-pack`](https://rustwasm.github.io/docs/wasm-pack/) का उपयोग करते हैं, तो यह दो फाइलें बनाता है जिनका हम यहां उपयोग करते हैं: वास्तविक कोड वाली एक wasm फ़ाइल (यहाँ, `src/rust-wasm/pkg/rust_wasm_bg.wasm`) और इसका उपयोग करने के लिए परिभाषाओं वाली एक जावास्क्रिप्ट फ़ाइल (यहाँ, `src/rust-wasm/pkg/rust_wasm.js`)। उस जावास्क्रिप्ट फ़ाइल का डिफ़ॉल्ट एक्सपोर्ट वह कोड है जिसे WASM शुरू करने के लिए चलाने की आवश्यकता होती है।
