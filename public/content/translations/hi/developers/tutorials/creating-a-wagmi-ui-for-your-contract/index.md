...
आपको एथेरियम इकोसिस्टम में एक ऐसी सुविधा मिली जिसकी हमें आवश्यकता है। आपने इसे लागू करने के लिए स्मार्ट अनुबंध लिखे, और शायद कुछ संबंधित कोड भी जो ऑफ-चेन चलता है। यह बहुत बढ़िया है! दुर्भाग्य से, यूज़र इंटरफ़ेस के बिना आपके पास कोई यूज़र नहीं होगा, और पिछली बार जब आपने कोई वेबसाइट लिखी थी तो लोग डायल-अप मोडेम का उपयोग करते थे और जावास्क्रिप्ट नया था।

यह लेख आपके लिए है। मैं मानता हूँ कि आप प्रोग्रामिंग जानते हैं, और शायद थोड़ी जावास्क्रिप्ट और HTML भी, लेकिन आपके यूज़र इंटरफ़ेस कौशल पुराने और आउट ऑफ डेट हैं। हम साथ मिलकर एक सरल आधुनिक एप्लिकेशन पर काम करेंगे ताकि आप देख सकें कि इन दिनों यह कैसे किया जाता है।
...
6. आप अनुबंध स्रोत कोड, हार्डहैट के Greeter का थोड़ा संशोधित संस्करण, [एक ब्लॉकचेन एक्सप्लोरर पर](https://eth-holesky.blockscout.com/address/0x432d810484AdD7454ddb3b5311f0Ac2E95CeceA8?tab=contract) देख सकते हैं।
...
फ़ाइल एक्सटेंशन हमें बताता है कि यह फ़ाइल [टाइपस्क्रिप्ट](https://www.typescriptlang.org/) में लिखा गया एक [रिएक्ट घटक](https://www.w3schools.com/react/react_components.asp) है, जो जावास्क्रिप्ट का एक एक्सटेंशन है जो [टाइप चेकिंग](https://en.wikipedia.org/wiki/Type_system#Type_checking) का समर्थन करता है। टाइपस्क्रिप्ट को जावास्क्रिप्ट में संकलित किया जाता है, इसलिए हम इसे क्लाइंट-साइड निष्पादन के लिए उपयोग कर सकते हैं।
...
उस रिएक्ट घटक को आयात करें जो एप्लिकेशन को लागू करता है (नीचे देखें)।

```tsx
ReactDOM.createRoot(document.getElementById('root')!).render(
```

रूट रिएक्ट घटक बनाएँ। `render` का पैरामीटर [JSX](https://www.w3schools.com/react/react_jsx.asp) है, जो एक एक्सटेंशन भाषा है जो HTML और जावास्क्रिप्ट/टाइपस्क्रिप्ट दोनों का उपयोग करती है। यहाँ विस्मयादिबोधक बिंदु टाइपस्क्रिप्ट घटक को बताता है: \"आप नहीं जानते कि `document.getElementById('root')` `ReactDOM.createRoot` के लिए एक वैध पैरामीटर होगा, लेकिन चिंता न करें - मैं डिवेलपर हूँ और मैं आपको बता रहा हूँ कि यह होगा\"।

```tsx
  <React.StrictMode>
```

एप्लिकेशन [एक `React.StrictMode` घटक](https://react.dev/reference/react/StrictMode) के अंदर जा रहा है। यह घटक रिएक्ट लाइब्रेरी को अतिरिक्त डिबगिंग जाँचें डालने के लिए कहता है, जो विकास के दौरान उपयोगी है।

```tsx
    <WagmiConfig config={config}>
```

एप्लिकेशन [एक `WagmiConfig` घटक](https://wagmi.sh/react/api/WagmiProvider) के अंदर भी है। [wagmi (we are going to make it) लाइब्रेरी](https://wagmi.sh/) रिएक्ट UI परिभाषाओं को एथेरियम विकेंद्रीकृत अनुप्रयोग लिखने के लिए [viem लाइब्रेरी](https://viem.sh/) से जोड़ती है।
...
अब हमारे पास एप्लिकेशन के लिए घटक हो सकता है, जो वास्तव में UI को लागू करता है। घटक के अंत में `/>` रिएक्ट को बताता है कि इस घटक के अंदर कोई परिभाषा नहीं है, जैसा कि XML मानक के अनुसार है।
...
यह एक रिएक्ट घटक बनाने का मानक तरीका है - एक फ़ंक्शन को परिभाषित करें जिसे हर बार रेंडर करने की आवश्यकता होने पर कॉल किया जाता है। इस फ़ंक्शन में आम तौर पर शीर्ष पर कुछ टाइपस्क्रिप्ट या जावास्क्रिप्ट कोड होता है, जिसके बाद एक `return` कथन होता है जो JSX कोड लौटाता है।
...
परंपरा के अनुसार, रिएक्ट में `use...` नामक फ़ंक्शन [हुक](https://www.w3schools.com/react/react_hooks.asp) होते हैं जो किसी प्रकार का डेटा लौटाते हैं। जब आप ऐसे हुक का उपयोग करते हैं, तो न केवल आपके घटक को डेटा मिलता है, बल्कि जब वह डेटा बदलता है तो घटक को अद्यतन जानकारी के साथ फिर से प्रस्तुत किया जाता है।
...
एक रिएक्ट घटक के JSX को एक घटक लौटाना _होता_ है। जब हमारे पास कई घटक होते हैं और हमारे पास कुछ भी नहीं होता है जो \"स्वाभाविक रूप से\" लपेटता है, तो हम एक खाली घटक (`<> ...` और `</>`) का उपयोग करते हैं उन्हें एक ही घटक में बनाने के लिए।
...
[`abitype` लाइब्रेरी](https://abitype.dev/) हमें विभिन्न एथेरियम डेटा प्रकारों के लिए टाइपस्क्रिप्ट परिभाषाएँ प्रदान करती है, जैसे कि [`AddressType`](https://abitype.dev/config#addresstype)।
...
यदि आप एक ही समय में अनुबंध और UI विकसित कर रहे हैं, तो आप सामान्य रूप से उन्हें एक ही रिपॉजिटरी में रखेंगे और अपने एप्लिकेशन में एक फ़ाइल के रूप में सॉलिडिटी कंपाइलर द्वारा उत्पन्न ABI का उपयोग करेंगे। हालाँकि, यहाँ यह आवश्यक नहीं है क्योंकि अनुबंध पहले ही विकसित हो चुका है और बदलने वाला नहीं है।
...
टाइपस्क्रिप्ट दृढ़ता से टाइप किया गया है। हम इस परिभाषा का उपयोग उस पते को निर्दिष्ट करने के लिए करते हैं जिसमें `Greeter` अनुबंध विभिन्न चेनों पर तैनात है। की एक संख्या (chainId) है, और मान एक `AddressType` (एक पता) है।
...
रिएक्ट का [`useState` हुक](https://www.w3schools.com/react/react_usestate.asp) हमें एक स्टेट वेरिएबल निर्दिष्ट करने देता है, जिसका मान घटक के एक रेंडरिंग से दूसरे में बना रहता है। प्रारंभिक मान पैरामीटर है, इस मामले में खाली स्ट्रिंग।
...
JSX के भीतर, `{` घुंघराले कोष्ठक `}` के अंदर के कोड को जावास्क्रिप्ट के रूप में व्याख्या किया जाता है। फिर, `(` नियमित कोष्ठक `)` के अंदर के कोड की फिर से JSX के रूप में व्याख्या की जाती है।
...
रिएक्ट को [DOM ट्री](https://www.w3schools.com/js/js_htmldom.asp) में टैग के लिए अलग पहचानकर्ता की आवश्यकता होती है। इसका मतलब है कि एक ही टैग के बच्चों (इस मामले में, [अनऑर्डर्ड सूची](https://www.w3schools.com/tags/tag_ul.asp)), को अलग-अलग `key` विशेषताओं की आवश्यकता होती है।
...
[वॉलेटकनेक्ट](https://walletconnect.com/) का उपयोग करने में सक्षम होने के लिए आपको अपने एप्लिकेशन के लिए एक प्रोजेक्ट ID की आवश्यकता है। आप इसे [cloud.walletconnect.com पर](https://cloud.walletconnect.com/sign-in) प्राप्त कर सकते हैं।
