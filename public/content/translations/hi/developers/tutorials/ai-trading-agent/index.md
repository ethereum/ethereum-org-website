...
यह एजेंट प्रदर्शित करता है कि जानकारी कैसे पढ़ें, इसे एक ऐसी क्वेरी में अनुवाद करें जो एक उपयोगी उत्तर देती है, और उस उत्तर का उपयोग करें। ये सभी एक AI एजेंट के लिए आवश्यक कदम हैं। यह एजेंट पायथन में लागू किया गया है क्योंकि यह AI में इस्तेमाल होने वाली सबसे आम भाषा है।
...
यह ट्यूटोरियल कोट्स और ट्रेडिंग के लिए [पायथन](https://www.python.org/), [वेब3 लाइब्रेरी](https://web3py.readthedocs.io/en/stable/), और [यूनिस्वैप v3](https://github.com/Uniswap/v3-periphery) का उपयोग करता है।

### पायथन क्यों? {#python}

AI के लिए सबसे व्यापक रूप से इस्तेमाल की जाने वाली भाषा [पायथन](https://www.python.org/) है, इसलिए हम इसे यहाँ इस्तेमाल करते हैं। अगर आप पायथन नहीं जानते हैं तो चिंता न करें। भाषा बहुत स्पष्ट है, और मैं ठीक से समझाऊंगा कि यह क्या करती है।

[वेब3 लाइब्रेरी](https://web3py.readthedocs.io/en/stable/) सबसे आम पायथन एथेरियम API है। इसका उपयोग करना बहुत आसान है।
...
[यूनिस्वैप](https://app.uniswap.org/) एक व्यापक रूप से उपयोग किया जाने वाला DEX है जिसका उपयोग हम कोट्स (टोकन के सापेक्ष मान देखने के लिए) और ट्रेड्स दोनों के लिए कर सकते हैं।
...
विकास को सरल बनाने के लिए, हम चरणों में आगे बढ़ते हैं। प्रत्येक चरण गिटहब में एक शाखा है।

### शुरुआत करना {#getting-started}

UNIX या लिनक्स (जिसमें [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) शामिल है) के तहत आरंभ करने के लिए कुछ चरण हैं

1. यदि आपके पास यह पहले से नहीं है, तो [पायथन](https://www.python.org/downloads/) डाउनलोड और इंस्टॉल करें।
...
6. यह सत्यापित करने के लिए कि पायथन और वेब3 सही ढंग से काम कर रहे हैं, `python3` चलाएं और इसे यह प्रोग्राम प्रदान करें। आप इसे `>>>` प्रॉम्प्ट पर दर्ज कर सकते हैं; फ़ाइल बनाने की कोई आवश्यकता नहीं है।

   ```python
   from web3 import वेब3
   MAINNET_URL = "https://eth.drpc.org"
   w3 = वेब3(वेब3.HTTPProvider(MAINNET_URL))
   w3.eth.block_number
   quit()
   ```
...
पायथन के `print` को एक ऐसे संस्करण से बदल देता है जो हमेशा आउटपुट को तुरंत फ्लश करता है। यह एक लंबे समय तक चलने वाली स्क्रिप्ट में उपयोगी है क्योंकि हम स्थिति अपडेट या डिबगिंग आउटपुट के लिए इंतजार नहीं करना चाहते हैं।
...
कोट मूल्य यूनिस्वैप 3 USDC/WETH पूल से पते [`0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640`](https://eth.blockscout.com/address/0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640?tab=read_write_contract) पर लिए गए हैं। यह पता पहले से ही चेकसम प्रारूप में है, लेकिन कोड को पुन: प्रयोज्य बनाने के लिए [`Web3.to_checksum_address`](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.to_checksum_address) का उपयोग करना बेहतर है।
...
यह पायथन में डेटा क्लास बनाने का एक तरीका है। [`Contract`](https://web3py.readthedocs.io/en/stable/web3.contract.html) डेटा प्रकार का उपयोग कॉन्ट्रैक्ट से कनेक्ट करने के लिए किया जाता है। ` (frozen=True)` पर ध्यान दें। पायथन में [बूलियन](https://en.wikipedia.org/wiki/Boolean_data_type) को `True` या `False`, कैपिटलाइज़्ड के रूप में परिभाषित किया गया है। यह डेटा क्लास `frozen` है, जिसका अर्थ है कि फ़ील्ड को संशोधित नहीं किया जा सकता है।

इंडेंटेशन पर ध्यान दें। [C-व्युत्पन्न भाषाओं](https://en.wikipedia.org/wiki/List_of_C-family_programming_languages) के विपरीत, पायथन ब्लॉक को दर्शाने के लिए इंडेंटेशन का उपयोग करता है। पायथन इंटरप्रेटर जानता है कि निम्नलिखित परिभाषा इस डेटा क्लास का हिस्सा नहीं है क्योंकि यह डेटा क्लास फ़ील्ड के समान इंडेंटेशन पर शुरू नहीं होती है।
...
यह पायथन में फ़ंक्शन को परिभाषित करने का तरीका है। परिभाषा यह दिखाने के लिए इंडेंट की गई है कि यह अभी भी `PoolInfo` का हिस्सा है।
...
ऑन-चेन गणनाओं को कम करने के लिए, यूनिस्वैप v3 वास्तविक विनिमय कारक को संग्रहीत नहीं करता है, बल्कि इसके वर्गमूल को संग्रहीत करता है। क्योंकि EVM फ्लोटिंग पॉइंट गणित या भिन्नों का समर्थन नहीं करता है, वास्तविक मान के बजाय, प्रतिक्रिया <math><msqrt><mi>price</mi></msqrt><mo>&#x22C5</mo><msup><mn>2</mn><mn>96</mn></msup></math> है
...
यह फ़ंक्शन एक पता लेता है और उस पते पर टोकन अनुबंध के बारे में जानकारी देता है। एक नया [वेब3 `Contract`](https://web3py.readthedocs.io/en/stable/web3.contract.html) बनाने के लिए, हम `w3.eth.contract` को पता और ABI प्रदान करते हैं।
...
पायथन में आप एक [सूची](https://docs.python.org/3/library/stdtypes.html#typesseq-list) परिभाषित करते हैं जिसमें `list[<type>]` का उपयोग करके केवल एक विशिष्ट प्रकार हो सकता है।
...
पायथन में एक [`for` लूप](https://docs.python.org/3/tutorial/controlflow.html#for-statements) आमतौर पर एक सूची पर पुनरावृति करता है। उद्धरणों को खोजने के लिए ब्लॉक नंबरों की सूची [`रेंज`](https://docs.python.org/3/library/stdtypes.html#range) से आती है।
...
पायथन में [मल्टी-लाइन स्ट्रिंग लिटरल](https://www.w3schools.com/python/gloss_python_multi_line_strings.asp) `"""` के रूप में लिखे जाते हैं .... `"""`.
...
4. ये वे अनुबंध हैं जिनका हमें उपयोग करने की आवश्यकता है। [`SwapRouter`](https://github.com/Uniswap/v3-periphery/blob/main/contracts/SwapRouter.sol) यूनिस्वैप v3 कॉन्ट्रैक्ट है जिसका उपयोग हम वास्तव में व्यापार करने के लिए करते हैं। हम सीधे पूल के माध्यम से व्यापार कर सकते हैं, लेकिन यह बहुत आसान है।

   दो नीचे के चर WETH और USDC के बीच स्वैप करने के लिए आवश्यक यूनिस्वैप v3 पथ हैं।
...
[`खाता`](https://web3py.readthedocs.io/en/stable/web3.eth.account.html) और `SwapRouter` अनुबंध के लिए वेब3 परिभाषाएँ।
...
यह है कि हम वेब3 में एक लेनदेन कैसे भेजते हैं। सबसे पहले हम लेन-देन बनाने के लिए [`कॉन्ट्रैक्ट` ऑब्जेक्ट](https://web3py.readthedocs.io/en/stable/web3.contract.html) का उपयोग करते हैं। फिर हम लेन-देन पर हस्ताक्षर करने के लिए [`web3.eth.account.sign_transaction`](https://web3py.readthedocs.io/en/stable/web3.eth.account.html#sign-a-contract-transaction) का उपयोग करते हैं, `PRIVATE_KEY` का उपयोग करके। अंत में, हम लेन-देन भेजने के लिए [`w3.eth.send_raw_transaction`](https://web3py.readthedocs.io/en/stable/transactions.html#chapter-2-w3-eth-send-raw-transaction) का उपयोग करते हैं।
