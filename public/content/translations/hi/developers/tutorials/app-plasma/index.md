...
क्लाइंट-साइड कोड के लिए, हम [वीट](https://vite.dev/), [रिएक्ट](https://react.dev/), [वीएम](https://viem.sh/), और [वैग्मी](https://wagmi.sh/) का उपयोग करने जा रहे हैं। ये उद्योग-मानक उपकरण हैं; यदि आप इनसे परिचित नहीं हैं, तो आप [इस ट्यूटोरियल](/developers/tutorials/creating-a-wagmi-ui-for-your-contract/) का उपयोग कर सकते हैं।

सर्वर का अधिकांश भाग [Node](https://nodejs.org/en) का उपयोग करके जावास्क्रिप्ट में लिखा गया है। ज़ीरो-नॉलेज भाग [नुआर](https://noir-lang.org/) में लिखा गया है। हमें संस्करण `1.0.0-beta.10` की आवश्यकता है, इसलिए [निर्देशानुसार नुआर इंस्टॉल करने](https://noir-lang.org/docs/getting_started/quick_start) के बाद, चलाएँ:

```
noirup -v 1.0.0-beta.10
```

हम जिस ब्लॉकचेन का उपयोग करते हैं वह `anvil` है, जो एक स्थानीय परीक्षण ब्लॉकचेन है जो [फाउंड्री](https://getfoundry.sh/introduction/installation) का हिस्सा है।
...
1. सुनिश्चित करें कि आपके पास [Node](https://nodejs.org/en/download) और [नुआर](https://noir-lang.org/install) इंस्टॉल हैं। अधिमानतः, उन्हें मैकओएस, लिनक्स, या [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) जैसे UNIX सिस्टम पर इंस्टॉल करें।
...
2. चरण 1 कोड डाउनलोड करें और क्लाइंट कोड की सेवा के लिए वेब सर्वर शुरू करें।
   ...
   यहां आपको एक वेब सर्वर की आवश्यकता का कारण यह है कि, कुछ प्रकार की धोखाधड़ी को रोकने के लिए, कई वॉलेट (जैसे मेटामास्क) सीधे डिस्क से परोसी गई फ़ाइलों को स्वीकार नहीं करते हैं
...
[यह फ़ाइल](https://github.com/qbzzt/250911-zk-bank/blob/01-manual-zk/server/noir/Prover.toml) नुआर द्वारा अपेक्षित सूचना प्रारूप दिखाती है।

```toml
message="send 0x70997970C51812dc3A010C7d01b50e0d17dc79C8 500 finney (milliEth) 0                             "
```

संदेश टेक्स्ट प्रारूप में है, जो उपयोगकर्ता के लिए समझना आसान बनाता है (जो हस्ताक्षर करते समय आवश्यक है) और नुआर कोड को पार्स करने के लिए। एक ओर भिन्नात्मक स्थानांतरण को सक्षम करने के लिए, और दूसरी ओर आसानी से पठनीय होने के लिए, राशि को फिनी में उद्धृत किया गया है। अंतिम संख्या [नॉन्स](https://en.wikipedia.org/wiki/Cryptographic_nonce) है।
...
यह फ़ंक्शन `Transfer` रिएक्ट घटक बनाता है, जिसे अन्य फ़ाइलें आयात कर सकती हैं।
...
ये [वैग्मी हुक](https://wagmi.sh/react/api/hooks) हमें [viem](https://viem.sh/) लाइब्रेरी और वॉलेट तक पहुंचने देते हैं।
...
संदेश हैश प्राप्त करें। उपयोगकर्ता को डिबगिंग (नुआर कोड का) के लिए इसे प्रदान करना सहायक होता है।
...
[सार्वजनिक कुंजी प्राप्त करें](https://viem.sh/docs/utilities/recoverPublicKey)। यह [नुआर `ecrecover`](https://github.com/colinnielsen/ecrecover-noir) फ़ंक्शन के लिए आवश्यक है।
...
वीएम हमें 65-बाइट हेक्साडेसिमल स्ट्रिंग के रूप में सार्वजनिक कुंजी प्रदान करता है। पहला बाइट `0x04` है, जो एक संस्करण मार्कर है। इसके बाद सार्वजनिक कुंजी के `x` के लिए 32 बाइट और फिर सार्वजनिक कुंजी के `y` के लिए 32 बाइट्स हैं।
...
हस्ताक्षर भी 65-बाइट हेक्साडेसिमल स्ट्रिंग के रूप में प्रदान किया जाता है। हालाँकि, सार्वजनिक कुंजी को पुनर्प्राप्त करने के लिए अंतिम बाइट केवल आवश्यक है। चूंकि सार्वजनिक कुंजी पहले से ही नुआर कोड को प्रदान की जाएगी, इसलिए हमें हस्ताक्षर को सत्यापित करने के लिए इसकी आवश्यकता नहीं है, और नुआर कोड को इसकी आवश्यकता नहीं है।
...
नुआर [रस्ट](https://www.rust-lang.org/) से प्रेरित है। वेरिएबल, डिफ़ॉल्ट रूप से, स्थिरांक होते हैं। इस तरह हम वैश्विक कॉन्फ़िगरेशन स्थिरांक को परिभाषित करते हैं। विशेष रूप से, `ACCOUNT_NUMBER` हमारे द्वारा संग्रहीत खातों की संख्या है।
...
नुआर में, फ़ंक्शन केवल अंत में एक मान लौटा सकते हैं (कोई जल्दी वापसी नहीं है)। रिटर्न मान निर्दिष्ट करने के लिए, आप इसे फ़ंक्शन के समापन ब्रैकेट से ठीक पहले मूल्यांकन करते हैं।
...
यह एक परिवर्तनीय चर निर्दिष्ट करने का तरीका है, अर्थात, _नहीं_ एक स्थिरांक। नुआर में चरों का हमेशा एक मान होना चाहिए, इसलिए हम इस चर को सभी शून्यों पर आरम्भ करते हैं।

```
    for i in 0..ACCOUNT_NUMBER {
```

यह एक `for` लूप है। ध्यान दें कि सीमाएँ स्थिरांक हैं। नुआर लूपों की सीमाएँ संकलन समय पर ज्ञात होनी चाहिए। इसका कारण यह है कि अंकगणितीय परिपथ प्रवाह नियंत्रण का समर्थन नहीं करते हैं। `for` लूप को संसाधित करते समय, कंपाइलर बस इसके अंदर के कोड को कई बार रखता है, प्रत्येक पुनरावृत्ति के लिए एक।
...
हम नुआर में एक प्रारूप स्ट्रिंग के अंदर संरचना तत्वों तक नहीं पहुंच सकते हैं, इसलिए हम एक प्रयोग करने योग्य प्रतिलिपि बनाते हैं।
...
[टपल](https://noir-lang.org/docs/noir/concepts/data_types/tuples) लौटाना एक फ़ंक्शन से कई मान लौटाने का नुआर तरीका है।
...
यह [सॉलिडिटी के `ecrecover`](https://docs.soliditylang.org/en/v0.8.30/cheatsheet.html#mathematical-and-cryptographic-functions) के समान है, जिसमें दो महत्वपूर्ण अंतर हैं:
...
1. वीट को रोकें यदि यह चल रहा है।
...
2. सर्वर वाली शाखा डाउनलोड करें और सुनिश्चित करें कि आपके पास सभी आवश्यक मॉड्यूल हैं।
   ...
   नुआर कोड को संकलित करने की कोई आवश्यकता नहीं है, यह वही कोड है जिसका उपयोग आपने चरण 1 के लिए किया था।
...
4. एक अलग कमांड-लाइन विंडो में, ब्राउज़र कोड की सेवा के लिए वीट चलाएँ।
...
[यह फ़ाइल](https://github.com/qbzzt/250911-zk-bank/blob/02-add-server/server/index.mjs) में सर्वर प्रक्रिया शामिल है, और [`main.nr`](https://github.com/qbzzt/250911-zk-bank/blob/02-add-server/server/noir/src/main.nr) पर नुआर कोड के साथ इंटरैक्ट करती है। यहाँ दिलचस्प भागों की व्याख्या है।

```js
import { Noir } from '@noir-lang/noir_js'
```

[noir.js](https://www.npmjs.com/package/@noir-lang/noir_js) लाइब्रेरी जावास्क्रिप्ट कोड और नुआर कोड के बीच इंटरफ़ेस करती है।
...
अरिथमैटिक सर्किट लोड करें—संकलित नुआर प्रोग्राम जिसे हमने पिछले चरण में बनाया था—और इसे निष्पादित करने की तैयारी करें।
...
अब जब हम सर्वर पर जावास्क्रिप्ट चलाते हैं, तो हम क्लाइंट के बजाय वहां सार्वजनिक कुंजी पुनर्प्राप्त कर सकते हैं।
...
लेनदेन लागू करें। हमने इसे पहले ही नुआर कोड में कर लिया है, लेकिन परिणाम को वहां से निकालने के बजाय इसे यहां फिर से करना आसान है।
...
हमें वास्तविक प्रमाण बनाने के लिए [बैरटेनबर्ग पैकेज](https://github.com/AztecProtocol/aztec-packages/tree/next/barretenberg) का उपयोग करने की आवश्यकता है जिसे ऑन-चेन भेजना है। हम इस पैकेज का उपयोग या तो कमांड-लाइन इंटरफ़ेस (`bb`) चलाकर या [जावास्क्रिप्ट लाइब्रेरी, `bb.js`](https://www.npmjs.com/package/@aztec/bb.js) का उपयोग करके कर सकते हैं। जावास्क्रिप्ट लाइब्रेरी मूल रूप से कोड चलाने की तुलना में बहुत धीमी है, इसलिए हम कमांड-लाइन का उपयोग करने के लिए यहां [`exec`](https://nodejs.org/api/child_process.html#child_processexeccommand-options-callback) का उपयोग करते हैं।
...
प्रमाण `Field` मानों का एक JSON सरणी है, जिनमें से प्रत्येक को एक हेक्साडेसिमल मान के रूप में दर्शाया गया है। हालाँकि, हमें इसे लेनदेन में एकल `bytes` मान के रूप में भेजने की आवश्यकता है, जिसे वीएम एक बड़ी हेक्साडेसिमल स्ट्रिंग द्वारा दर्शाता है। यहां हम सभी मानों को जोड़कर, सभी `0x` को हटाकर और फिर अंत में एक जोड़कर प्रारूप बदलते हैं।
...
सार्वजनिक फ़ील्ड को 32-बाइट मानों की एक सरणी होनी चाहिए। हालाँकि, चूँकि हमें लेन-देन हैश को दो `Field` मानों के बीच विभाजित करने की आवश्यकता थी, यह 16-बाइट मान के रूप में दिखाई देता है। यहां हम शून्य जोड़ते हैं ताकि वीएम समझ सके कि यह वास्तव में 32 बाइट है।
