...
description: "ऑन-चेन गेम सीमित हैं क्योंकि वे कोई भी छिपी हुई जानकारी नहीं रख सकते हैं। इस ट्यूटोरियल को पढ़ने के बाद, एक पाठक एक गुप्त स्टेट, ऑफ़-चेन, कंपोनेंट के साथ सत्यापन योग्य गेम बनाने के लिए ज़ीरो-नॉलेज प्रमाण और सर्वर कंपोनेंट को संयोजित करने में सक्षम होगा। ऐसा करने की तकनीक को एक माइनस्वीपर गेम बनाकर प्रदर्शित किया जाएगा।"
...
सबसे सरल समाधान गुप्त स्टेट को रखने के लिए एक [सर्वर कंपोनेंट](/developers/tutorials/server-components/) का उपयोग करना है। हालांकि, हम ब्लॉकचेन का उपयोग गेम डेवलपर द्वारा धोखाधड़ी को रोकने के लिए करते हैं। हमें सर्वर कंपोनेंट की ईमानदारी सुनिश्चित करने की आवश्यकता है। सर्वर स्टेट का हैश प्रदान कर सकता है, और यह साबित करने के लिए [ज़ीरो-नॉलेज प्रमाण](/zero-knowledge-proofs/#why-zero-knowledge-proofs-are-important) का उपयोग कर सकता है कि एक चाल के परिणाम की गणना के लिए उपयोग किया गया स्टेट सही है।
...
| --------------------------------------------- | ----------------------------------------------- | --------------------------------------: |
| [ज़ोक्रेट्स](https://zokrates.github.io/)       | ज़ीरो-नॉलेज प्रमाण और उनका सत्यापन              |   1.1.9 |
...
| [वीएम](https://viem.sh/)                      | ब्लॉकचेन के साथ संचार                           |  2.9.20 |
| [MUD](https://mud.dev/)                       | ऑन-चेन डेटा प्रबंधन                             |  2.0.12 |
| [रिएक्ट](https://react.dev/)                   | क्लाइंट यूज़र इंटरफ़ेस                          |  18.2.0 |
| [वीट](https://vitejs.dev/)                   | क्लाइंट कोड सर्व करना                           |   4.2.1 |
...
1. [`mprocs`](https://github.com/pvolok/mprocs) चार कंपोनेंट चलाता है:
   ...
   - [Client](https://github.com/qbzzt/20240901-secret-state/tree/main/packages/client), जो वेब ब्राउज़रों को UI और क्लाइंट कोड सर्व करने के लिए [वीट](https://vitejs.dev/) चलाता है।
...
5. `createGame` सबसे पहले [उचित संख्या में माइन के साथ एक यादृच्छिक मैप बनाता है](https://github.com/qbzzt/20240901-secret-state/blob/main/packages/server/src/app.ts#L120-L135)। फिर, यह ज़ोक्रेट्स के लिए आवश्यक खाली बॉर्डर के साथ एक मैप बनाने के लिए [`makeMapBorders`](https://github.com/qbzzt/20240901-secret-state/blob/main/packages/server/src/app.ts#L147-L166) को कॉल करता है। अंत में, `createGame` मैप का हैश प्राप्त करने के लिए [`calculateMapHash`](#calculateMapHash) को कॉल करता है, जिसका उपयोग गेम आईडी के रूप में किया जाता है।
...
## ज़ोक्रेट्स का उपयोग करना {#using-zokrates}
...
ये दो लाइनें [ज़ोक्रेट्स स्टैंडर्ड लाइब्रेरी](https://zokrates.github.io/toolbox/stdlib.html) से दो फ़ंक्शन आयात करती हैं। [पहला फ़ंक्शन](https://github.com/Zokrates/ZoKrates/blob/latest/zokrates_stdlib/stdlib/hashes/poseidon/poseidon.zok) एक [Poseidon हैश](https://www.poseidon-hash.info/) है। यह [`field` तत्वों](https://zokrates.github.io/language/types.html#field) की एक सारणी लेता है और एक `field` लौटाता है।
...
हम `${width+2}` और `${height+2}` का उपयोग कर सकते हैं क्योंकि ज़ोक्रेट्स प्रोग्राम इस एप्लिकेशन में [टेम्पलेट स्ट्रिंग्स](https://www.w3schools.com/js/js_string_templates.asp) के रूप में संग्रहीत हैं। `${` और `}` के बीच के कोड का मूल्यांकन जावास्क्रिप्ट द्वारा किया जाता है, और इस तरह प्रोग्राम का उपयोग विभिन्न मैप आकारों के लिए किया जा सकता है। मैप पैरामीटर में इसके चारों ओर एक स्थान चौड़ा बॉर्डर होता है जिसमें कोई बम नहीं होता है, यही कारण है कि हमें चौड़ाई और ऊंचाई में दो जोड़ना पड़ता है।
...
हमें सरणी को आरंभ करने की आवश्यकता है क्योंकि ज़ोक्रेट्स में `undefined` नहीं है। `[false; 512]` अभिव्यक्ति का अर्थ है [512 `false` मानों की एक सरणी](https://zokrates.github.io/language/types.html#declaration-and-initialization)।
...
यह है कि आप ज़ोक्रेट्स में एक [`for` लूप](https://zokrates.github.io/language/control_flow.html#for-loops) कैसे घोषित करते हैं। एक ज़ोक्रेट्स `for` लूप में निश्चित सीमाएँ होनी चाहिए, क्योंकि जबकि यह एक लूप प्रतीत होता है, कंपाइलर वास्तव में इसे "अनरोल" करता है। अभिव्यक्ति `${width+2}` एक कंपाइल समय स्थिरांक है क्योंकि `width` टाइपस्क्रिप्ट कोड द्वारा कंपाइलर को कॉल करने से पहले सेट किया जाता है।
...
ज़ीरो-नॉलेज प्रमाण [अरिथमैटिक सर्किट](https://medium.com/web3studio/simple-explanations-of-arithmetic-circuits-and-zero-knowledge-proofs-806e59a79785) का उपयोग करते हैं, जिनका `if` स्टेटमेंट का कोई आसान समकक्ष नहीं होता है। इसके बजाय, वे [सशर्त ऑपरेटर](https://en.wikipedia.org/wiki/Ternary_conditional_operator) के समकक्ष का उपयोग करते हैं। यदि `a` शून्य या एक हो सकता है, तो आप `if a { b } else { c }` की गणना `ab+(1-a)c` के रूप में कर सकते हैं।
...
डिफ़ॉल्ट रूप से ज़ोक्रेट्स प्रमाणों में उनके इनपुट शामिल होते हैं। यह जानने का कोई फायदा नहीं है कि किसी स्थान के आसपास पाँच माइन हैं जब तक कि आप वास्तव में यह नहीं जानते कि वह कौन सा स्थान है (और आप इसे केवल अपने अनुरोध से मेल नहीं खा सकते, क्योंकि तब प्रोवर अलग-अलग मानों का उपयोग कर सकता है और आपको इसके बारे में नहीं बता सकता है)। हालांकि, हमें ज़ोक्रेट्स को प्रदान करते हुए मैप को गुप्त रखने की आवश्यकता है। समाधान एक `private` पैरामीटर का उपयोग करना है, जो प्रमाण द्वारा प्रकट _नहीं_ किया जाता है।
...
### टाइपस्क्रिप्ट से ज़ोक्रेट्स का उपयोग करना {#using-zokrates-from-typescript}

ज़ोक्रेट्स का एक कमांड लाइन इंटरफ़ेस है, लेकिन इस प्रोग्राम में हम इसका उपयोग [टाइपस्क्रिप्ट कोड](https://zokrates.github.io/toolbox/zokrates_js.html) में करते हैं।

ज़ोक्रेट्स परिभाषाओं वाली लाइब्रेरी को [`zero-knowledge.ts`](https://github.com/qbzzt/20240901-secret-state/blob/main/packages/server/src/zero-knowledge.ts) कहा जाता है।

```typescript
import { initialize as zokratesInitialize } from "zokrates-js"
```

[ज़ोक्रेट्स जावास्क्रिप्ट बाइंडिंग](https://zokrates.github.io/toolbox/zokrates_js.html) आयात करें। हमें केवल [`initialize`](https://zokrates.github.io/toolbox/zokrates_js.html#initialize) फ़ंक्शन की आवश्यकता है क्योंकि यह एक वादा लौटाता है जो सभी ज़ोक्रेट्स परिभाषाओं का समाधान करता है।

```typescript
export const zkFunctions = async (width: number, height: number) : Promise<any> => {
```

ज़ोक्रेट्स की तरह ही, हम भी केवल एक ही फ़ंक्शन निर्यात करते हैं, जो [एसिंक्रोनस](https://www.w3schools.com/js/js_async.asp) भी है। जब यह अंततः लौटता है, तो यह कई फ़ंक्शन प्रदान करता है जैसा कि हम नीचे देखेंगे।
...
[`computeWitness`](https://zokrates.github.io/toolbox/zokrates_js.html#computewitnessartifacts-args-options) फ़ंक्शन वास्तव में ज़ोक्रेट्स प्रोग्राम चलाता है। यह दो फ़ील्ड के साथ एक संरचना लौटाता है: `output`, जो प्रोग्राम का आउटपुट एक JSON स्ट्रिंग के रूप में है, और `witness`, जो परिणाम का एक ज़ीरो-नॉलेज प्रमाण बनाने के लिए आवश्यक जानकारी है। यहाँ हमें केवल आउटपुट की आवश्यकता है।
...
सबसे सरल संभावना ज़ीरो-नॉलेज प्रमाण में गलत उत्तर प्रदान करना है। ऐसा करने के लिए, हम `zkDig` के अंदर जाते हैं और [लाइन 91 को संशोधित करते हैं](https://github.com/qbzzt/20240901-secret-state/blob/main/packages/server/src/zero-knowledge.ts#L91):
...
1. [ज़ोक्रेट्स इंस्टॉल करें](https://zokrates.github.io/gettingstarted.html)।
...
3. ज़ोक्रेट्स कोड संकलित करें और सत्यापन कुंजी बनाएँ। सत्यापन कुंजी को उसी एन्ट्रापी के साथ बनाया जाना चाहिए जिसका उपयोग मूल सर्वर में किया गया था, [इस मामले में एक खाली स्ट्रिंग](https://github.com/qbzzt/20240901-secret-state/blob/main/packages/server/src/zero-knowledge.ts#L67)।
...
### ज़ोक्रेट्स क्यों? {#why-zokrates}

[ज़ोक्रेट्स](https://zokrates.github.io/) एकमात्र उपलब्ध ज़ीरो-नॉलेज लाइब्रेरी नहीं है, लेकिन यह एक सामान्य, [इंपरेटिव](https://en.wikipedia.org/wiki/Imperative_programming) प्रोग्रामिंग भाषा के समान है और बूलियन वेरिएबल्स का समर्थन करती है।
...
### ज़ोक्रेट्स को कब संकलित करें {#when-compile-zokrates}

इस प्रोग्राम में हम ज़ोक्रेट्स प्रोग्राम को [हर बार जब सर्वर शुरू होता है](https://github.com/qbzzt/20240901-secret-state/blob/main/packages/server/src/zero-knowledge.ts#L60-L61) संकलित करते हैं। यह स्पष्ट रूप से संसाधनों की बर्बादी है, लेकिन यह एक ट्यूटोरियल है, जिसे सरलता के लिए अनुकूलित किया गया है।
...
इसके अतिरिक्त, हम [एक सेटअप समारोह](https://zokrates.github.io/toolbox/trusted_setup.html#initializing-a-phase-2-ceremony) का उपयोग कर सकते हैं। सेटअप समारोह का लाभ यह है कि ज़ीरो-नॉलेज प्रमाण पर धोखा देने के लिए आपको प्रत्येक भागीदार से या तो एन्ट्रॉपी या कुछ मध्यवर्ती परिणाम की आवश्यकता होती है। यदि कम से कम एक समारोह भागीदार ईमानदार है और उस जानकारी को हटा देता है, तो ज़ीरो-नॉलेज प्रमाण कुछ हमलों से सुरक्षित हैं। हालांकि, यह सत्यापित करने के लिए _कोई तंत्र नहीं_ है कि जानकारी हर जगह से हटा दी गई है। यदि ज़ीरो-नॉलेज प्रमाण गंभीर रूप से महत्वपूर्ण हैं, तो आप सेटअप समारोह में भाग लेना चाहेंगे।
...
हम ज़ीरो-नॉलेज प्रमाण को या तो ऑन-चेन (जिसमें गैस लगती है) या क्लाइंट में ([`verify`](https://zokrates.github.io/toolbox/zokrates_js.html#verifyverificationkey-proof) का उपयोग करके) सत्यापित कर सकते हैं। मैंने पहला चुना, क्योंकि यह आपको [वेरिफायर को सत्यापित करने](#user-verify-zero-trust) देता है और फिर भरोसा करता है कि जब तक इसके लिए अनुबंध का पता समान रहता है तब तक यह नहीं बदलता है। यदि सत्यापन क्लाइंट पर किया जाता, तो आपको हर बार क्लाइंट डाउनलोड करने पर प्राप्त कोड को सत्यापित करना पड़ता।
...
### टाइपस्क्रिप्ट या ज़ोक्रेट्स में मैप को समतल करें? {#where-flatten}
...
हालांकि, हम अभी भी [ज़ोक्रेट्स में मैप को समतल करते हैं](https://github.com/qbzzt/20240901-secret-state/blob/main/packages/server/src/zero-knowledge.ts#L15-L20), जबकि हम इसे टाइपस्क्रिप्ट में कर सकते थे। कारण यह है कि अन्य विकल्प, मेरी राय में, बदतर हैं।

- ज़ोक्रेट्स कोड को बूलियन की एक-आयामी सरणी प्रदान करें, और द्वि-आयामी मानचित्र प्राप्त करने के लिए `x*(height+2)
...
- _कुछ केंद्रीकरण स्वीकार्य_: ज़ीरो-नॉलेज प्रमाण अखंडता को सत्यापित कर सकते हैं, कि कोई इकाई परिणामों में हेरफेर नहीं कर रही है। वे यह सुनिश्चित नहीं कर सकते कि इकाई अभी भी उपलब्ध होगी और संदेशों का जवाब देगी। उन स्थितियों में जहां उपलब्धता को भी विकेंद्रीकृत करने की आवश्यकता होती है, ज़ीरो-नॉलेज प्रमाण एक पर्याप्त समाधान नहीं हैं, और आपको [मल्टी-पार्टी कंप्यूटेशन](https://en.wikipedia.org/wiki/Secure_multi-party_computation) की आवश्यकता है।
