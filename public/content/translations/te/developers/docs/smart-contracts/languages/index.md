...
[ఎథీరియం](/) గురించిన ఒక గొప్ప విషయం ఏమిటంటే, స్మార్ట్ కాంట్రాక్ట్‌లను డెవలపర్‌లకు అనుకూలమైన భాషలను ఉపయోగించి ప్రోగ్రామ్ చేయవచ్చు. మీకు Python లేదా ఏదైనా [కర్లీ-బ్రాకెట్ భాష](https://wikipedia.org/wiki/List_of_programming_languages_by_type#Curly-bracket_languages)తో అనుభవం ఉంటే, మీకు తెలిసిన సింటాక్స్‌తో కూడిన భాషను మీరు కనుగొనవచ్చు.
...
Remix IDE అనేది Solidity మరియు Vyper రెండింటిలోనూ కాంట్రాక్ట్‌లను సృష్టించడానికి మరియు పరీక్షించడానికి ఒక సమగ్ర అభివృద్ధి పర్యావరణాన్ని అందిస్తుంది. కోడింగ్ ప్రారంభించడానికి [ఇన్-బ్రౌజర్ Remix IDEని ప్రయత్నించండి](https://remix.ethereum.org).

మరింత అనుభవజ్ఞులైన డెవలపర్‌లు [ఈథీరియం వర్చువల్ మెషీన్ (EVM)](/developers/docs/evm/) కోసం ఇంటర్మీడియట్ భాష అయిన Yul లేదా Yulకి పొడిగింపు అయిన Yul+ని ఉపయోగించాలనుకోవచ్చు.
...
- స్టాటికల్లీ టైప్ చేయబడింది (వేరియబుల్ రకం కంపైల్ సమయంలో తెలుస్తుంది).
...
- [Solidity Gitter చాట్‌రూమ్](https://gitter.im/ethereum/solidity) నుండి [Solidity Matrix చాట్‌రూమ్](https://matrix.to/#/#ethereum_solidity:gitter.im)కి బ్రిడ్జ్ చేయబడింది
...
contract Coin {
    // "public" కీవర్డ్ వేరియబుల్స్‌ను
    // ఇతర కాంట్రాక్ట్‌ల నుండి యాక్సెస్ చేయడానికి వీలుగా చేస్తుంది
    address public minter;
    mapping (address => uint) public balances;

    // ఈవెంట్‌లు క్లయింట్‌లను మీరు ప్రకటించే నిర్దిష్ట
    // కాంట్రాక్ట్ మార్పులకు ప్రతిస్పందించేలా చేస్తాయి
    ...
    // ఇప్పటికే ఉన్న నాణేల మొత్తాన్ని
    // ఏదైనా కాలర్ నుండి ఒక చిరునామాకు పంపుతుంది
...
ఈ ఉదాహరణ మీకు Solidity కాంట్రాక్ట్ సింటాక్స్ ఎలా ఉంటుందో ఒక అవగాహనను ఇస్తుంది. ఫంక్షన్‌లు మరియు వేరియబుల్స్ యొక్క మరింత వివరణాత్మక వివరణ కోసం, [డాక్స్‌ని చూడండి](https://docs.soliditylang.org/en/latest/contracts.html).
...
- కాంట్రాక్ట్‌లను మరింత సురక్షితంగా మరియు ఆడిట్ చేయడానికి సులభంగా చేయాలనే లక్ష్యంతో ఉద్దేశపూర్వకంగా Solidity కంటే తక్కువ ఫీచర్‌లను కలిగి ఉంది. Vyper వీటికి మద్దతు ఇవ్వదు:
  - మాడిఫైయర్‌లు
  - ఇన్‌హెరిటెన్స్
  - ఇన్‌లైన్ అసెంబ్లీ
...
v0.4.0 నుండి, Vyper [మాడ్యూల్ సిస్టమ్](https://docs.vyperlang.org/en/stable/using-modules.html)కి మద్దతు ఇస్తుంది. క్లాస్ ఇన్‌హెరిటెన్స్ ద్వారా కాకుండా కంపోజిషన్ ద్వారా కోడ్ పునర్వినియోగం సాధించబడుతుంది.
...
# వేలం పారామితులు
# అత్యధిక బిడ్డర్ నుండి లబ్ధిదారుడు డబ్బును అందుకుంటాడు
...
# చివరిలో trueకి సెట్ చేయబడుతుంది, ఎలాంటి మార్పును అనుమతించదు
ended: public(bool)

# మేము ఉపసంహరణ (withdraw) పద్ధతిని అనుసరించడానికి వాపసు చేయబడిన బిడ్‌లను ట్రాక్ చేయండి
pendingReturns: public(HashMap[address, uint256])

# `_bidding_time` తో ఒక సాధారణ వేలాన్ని సృష్టించండి
# సెకన్ల బిడ్డింగ్ సమయం, దీని తరపున
# లబ్ధిదారుని చిరునామా `_beneficiary`.
@deploy
...
# ఈ లావాదేవీతో పాటు.
# ఈ క్రింది సందర్భంలో మాత్రమే విలువ వాపసు చేయబడుతుంది
# వేలంలో గెలవకపోతే.
@external
@payable
def bid():
    ...
    # మునుపటి అత్యధిక బిడ్డర్ కోసం వాపసును ట్రాక్ చేయండి
    self.pendingReturns[self.highestBidder] += self.highestBid
    # కొత్త అత్యధిక బిడ్‌ను ట్రాక్ చేయండి
    self.highestBidder = msg.sender
    self.highestBid = msg.value

# మునుపు వాపసు చేసిన బిడ్‌ను ఉపసంహరించుకోండి. ఉపసంహరణ (withdraw) పద్ధతి
# భద్రతా సమస్యను నివారించడానికి ఇక్కడ ఉపయోగించబడింది. ఒకవేళ వాపసులు నేరుగా
# bid() లో భాగంగా పంపబడితే, ఒక హానికరమైన బిడ్డింగ్ కాంట్రాక్ట్
# ఆ వాపసులను మరియు తద్వారా కొత్త అత్యధిక బిడ్‌లు రాకుండా నిరోధించగలదు.
@external
def withdraw():
...
# వేలాన్ని ముగించండి మరియు అత్యధిక బిడ్‌ను
# లబ్ధిదారునికి పంపండి.
@external
def endAuction():
    # పరస్పర చర్య చేసే ఫంక్షన్‌లను నిర్మించడం మంచి మార్గదర్శకం
    # ఇతర కాంట్రాక్ట్‌లతో (అంటే, అవి ఫంక్షన్‌లను కాల్ చేయడం లేదా ఈథర్‌ను పంపడం)
    # మూడు దశలుగా:
    # 1. షరతులను తనిఖీ చేయడం
    # 2. చర్యలను నిర్వహించడం (బహుశా షరతులను మార్చడం)
    # 3. ఇతర కాంట్రాక్ట్‌లతో పరస్పర చర్య చేయడం
    # ఈ దశలు కలిసిపోతే, ఇతర కాంట్రాక్ట్
    # ప్రస్తుత కాంట్రాక్ట్‌లోకి తిరిగి కాల్ చేయవచ్చు మరియు స్థితిని సవరించవచ్చు లేదా
    # ప్రభావాలు (ఈథర్ చెల్లింపు) బహుళ సార్లు జరిగేలా చేయవచ్చు.
    # అంతర్గతంగా కాల్ చేయబడిన ఫంక్షన్‌లు బాహ్య
    # కాంట్రాక్ట్‌లతో పరస్పర చర్యను కలిగి ఉంటే, వాటిని కూడా బాహ్య
    # కాంట్రాక్ట్‌లతో పరస్పర చర్యగా పరిగణించాలి.
...
ఈ ఉదాహరణ మీకు Vyper కాంట్రాక్ట్ సింటాక్స్ ఎలా ఉంటుందో ఒక అవగాహనను ఇస్తుంది. ఫంక్షన్‌లు మరియు వేరియబుల్స్ యొక్క మరింత వివరణాత్మక వివరణ కోసం, [డాక్స్‌ని చూడండి](https://vyper.readthedocs.io/en/latest/vyper-by-example.html#simple-open-auction).

## Yul మరియు Yul+ {#yul}

మీరు ఎథీరియంకి కొత్త అయితే మరియు ఇంకా స్మార్ట్ కాంట్రాక్ట్ భాషలతో ఎలాంటి కోడింగ్ చేయకపోతే, Solidity లేదా Vyperతో ప్రారంభించాలని మేము సిఫార్సు చేస్తున్నాము. మీరు స్మార్ట్ కాంట్రాక్ట్ భద్రతా ఉత్తమ పద్ధతులు మరియు EVMతో పనిచేయడంలోని ప్రత్యేకతలతో పరిచయం పెంచుకున్న తర్వాత మాత్రమే Yul లేదా Yul+ని పరిశీలించండి.
...
- EVM మరియు Ewasm ప్లాట్‌ఫారమ్‌లు రెండింటికీ సమానంగా ప్రయోజనం చేకూర్చే హై-లెవల్ ఆప్టిమైజేషన్ దశలకు మంచి లక్ష్యం.
...
మీరు ఇప్పటికే స్మార్ట్ కాంట్రాక్ట్‌లతో మంచి అనుభవం కలిగి ఉంటే, Yulలో పూర్తి ERC-20 అమలును [ఇక్కడ](https://solidity.readthedocs.io/en/latest/yul.html#complete-erc20-example) కనుగొనవచ్చు.

## Fe {#fe}

- ఈథీరియం వర్చువల్ మెషీన్ (EVM) కోసం స్టాటికల్లీ టైప్ చేయబడిన భాష.
...
ఏదైనా ఇతర ప్రోగ్రామింగ్ భాష వలె, ఇది ఎక్కువగా సరైన పని కోసం సరైన సాధనాన్ని ఎంచుకోవడంతో పాటు వ్యక్తిగత ప్రాధాన్యతలకు సంబంధించినది.
...
- Vyper తక్కువ సంఖ్యలో ఫీచర్‌లను కలిగి ఉంది, ఇది ఆలోచనల యొక్క శీఘ్ర నమూనా కోసం గొప్పగా చేస్తుంది.
...
- సరళమైన మరియు క్రియాత్మకమైన తక్కువ-స్థాయి భాష.
...
ప్రాథమిక సింటాక్స్, కాంట్రాక్ట్ జీవితచక్రం, ఇంటర్‌ఫేస్‌లు, ఆపరేటర్‌లు, డేటా స్ట్రక్చర్‌లు, ఫంక్షన్‌లు, కంట్రోల్ ఫ్లో మరియు మరిన్నింటి పోలికల కోసం [Auditless వారి ఈ చీట్‌షీట్‌ని](https://reference.auditless.com/cheatsheet/) చూడండి
