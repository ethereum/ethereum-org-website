...
ఒక ఖాతా ఒకటి కంటే ఎక్కువ ఆపరేటర్‌లను కలిగి ఉండవచ్చు. వాటిని ట్రాక్ చేయడానికి సాధారణ `HashMap` సరిపోదు, ఎందుకంటే ప్రతి కీ ఒకే విలువకు దారి తీస్తుంది. బదులుగా, మీరు `HashMap[address, bool]`ని విలువగా ఉపయోగించవచ్చు. అప్రమేయంగా ప్రతి చిరునామాకు విలువ `False`గా ఉంటుంది, అంటే అది ఆపరేటర్ కాదు. మీరు అవసరమైన విధంగా విలువలను `True`కి సెట్ చేయవచ్చు.
...
Pythonలో మరియు Vyperలో, మీరు బహుళ-లైన్ స్ట్రింగ్‌ను (`"""`తో ప్రారంభమై ముగిసేది) పేర్కొనడం ద్వారా మరియు దానిని ఏ విధంగానూ ఉపయోగించకుండా వ్యాఖ్యను కూడా సృష్టించవచ్చు. ఈ వ్యాఖ్యలు [NatSpec](https://vyper.readthedocs.io/en/latest/natspec.html)ని కూడా కలిగి ఉండవచ్చు.

```python
    self.minter = msg.sender
```

స్థితి వేరియబుల్స్‌ను యాక్సెస్ చేయడానికి మీరు `self.<variable name>`ని ఉపయోగిస్తారు (మళ్ళీ, Pythonలో వలె). కన్స్ట్రక్టర్ కాంట్రాక్ట్‌ను డిప్లాయ్ చేసిన ఖాతాను `minter`గా రికార్డ్ చేస్తుంది.
...
ఫంక్షన్ నిర్వచనానికి ముందు ఎట్ గుర్తు (`@`)తో ప్రారంభమయ్యే ఈ కీవర్డ్‌లను _డెకరేషన్స్_ అని పిలుస్తారు. ఫంక్షన్‌ను ఏ పరిస్థితులలో కాల్ చేయవచ్చో అవి నిర్దేశిస్తాయి.
...
Pythonకి విరుద్ధంగా, Vyper అనేది [స్టాటిక్ టైప్డ్ లాంగ్వేజ్](https://wikipedia.org/wiki/Type_system#Static_type_checking). [డేటా రకాన్ని](https://vyper.readthedocs.io/en/latest/types.html) గుర్తించకుండా మీరు వేరియబుల్ లేదా ఫంక్షన్ పారామితిని ప్రకటించలేరు. ఈ సందర్భంలో ఇన్‌పుట్ పారామితి `bytes4`, ఇది నాలుగు-బైట్ల విలువ, మరియు అవుట్‌పుట్ బూలియన్ విలువ.
...
`SUPPORTED_INTERFACES` జాబితాలోని ఇంటర్‌ఫేస్ IDలలో `interface_id` ఒకటి అయితే `True`ని తిరిగి ఇస్తుంది.
...
def balanceOf(_owner: address) -> uint256:
    """
    @dev `_owner` కలిగి ఉన్న NFTల సంఖ్యను తిరిగి ఇస్తుంది.
         `_owner` శూన్య చిరునామా అయితే త్రో చేస్తుంది. శూన్య చిరునామాకు కేటాయించబడిన NFTలు చెల్లనివిగా పరిగణించబడతాయి.
    @param _owner బ్యాలెన్స్‌ను క్వెరీ చేయాల్సిన చిరునామా.
    """
    assert _owner != empty(address)
```

ఈ లైన్ `_owner` శూన్య చిరునామా కాదని [నిర్ధారిస్తుంది](https://vyper.readthedocs.io/en/latest/statements.html#assert), ఇది `empty(address)`గా వ్రాయబడింది. అలా అయితే, లోపం ఏర్పడుతుంది మరియు ఆపరేషన్ రివర్ట్ చేయబడుతుంది.
...
def getApproved(_tokenId: uint256) -> address:
    ...
    @dev ఒకే NFT కోసం ఆమోదించబడిన చిరునామాను పొందండి.
         `_tokenId` చెల్లుబాటు అయ్యే NFT కాకపోతే త్రో చేస్తుంది.
    @param _tokenId ఆమోదాన్ని క్వెరీ చేయాల్సిన NFT యొక్క ID.
    """
    # `_tokenId` చెల్లుబాటు అయ్యే NFT కాకపోతే త్రో చేస్తుంది
...
def isApprovedForAll(_owner: address, _operator: address) -> bool:
    """
    @dev `_owner` కోసం `_operator` ఆమోదించబడిన ఆపరేటర్ అవునా కాదా అని తనిఖీ చేస్తుంది.
...
#### బదిలీ హెల్పర్ ఫంక్షన్‌లు {#transfer-helpers}
...
def _isApprovedOrOwner(_spender: address, _tokenId: uint256) -> bool:
    """
    @dev ఇచ్చిన ఖర్చు చేసేవారు ఇచ్చిన టోకెన్ IDని బదిలీ చేయగలరా లేదా అని తిరిగి ఇస్తుంది
    @param spender క్వెరీ చేయాల్సిన ఖర్చు చేసేవారి చిరునామా
    @param tokenId బదిలీ చేయాల్సిన టోకెన్ యొక్క uint256 ID
    @return bool msg.sender ఇచ్చిన టోకెన్ ID కోసం ఆమోదించబడ్డారా,
        యజమాని యొక్క ఆపరేటర్ అవునా, లేదా టోకెన్ యజమాని అవునా అనేది
    """
    owner: address = self.idToOwner[_tokenId]
...
పై ఫంక్షన్ స్థితిని మార్చదు కాబట్టి ఇది ఒక వ్యూ కావచ్చు. నిర్వహణ ఖర్చులను తగ్గించడానికి, వ్యూ _కాగల_ ఏదైనా ఫంక్షన్ వ్యూ _కావాలి_.
...
def _clearApproval(_owner: address, _tokenId: uint256):
    """
    @dev ఇచ్చిన చిరునామా ఆమోదాన్ని క్లియర్ చేయండి
...
అవసరమైతే మాత్రమే విలువను మార్చండి. స్థితి వేరియబుల్స్ స్టోరేజ్‌లో ఉంటాయి. స్టోరేజ్‌కు వ్రాయడం అనేది EVM (ఎథీరియం వర్చువల్ మెషిన్) చేసే అత్యంత ఖరీదైన ఆపరేషన్‌లలో ఒకటి ([గ్యాస్](/developers/docs/gas/) పరంగా). కాబట్టి, దానిని తగ్గించడం మంచి ఆలోచన, ఇప్పటికే ఉన్న విలువను వ్రాయడం కూడా అధిక ఖర్చుతో కూడుకున్నది.
...
def _transferFrom(_from: address, _to: address, _tokenId: uint256, _sender: address):
    ...
    @dev NFT బదిలీని అమలు చేయండి.
         ...
         చిరునామా కాకపోతే త్రో చేస్తుంది. (గమనిక: ప్రైవేట్ ఫంక్షన్‌లో `msg.sender` అనుమతించబడదు కాబట్టి `_sender`ని పాస్ చేయండి.)
...
Vyperలో ఈవెంట్‌ను విడుదల చేయడానికి మీరు `log` స్టేట్‌మెంట్‌ను ఉపయోగిస్తారు ([మరిన్ని వివరాల కోసం ఇక్కడ చూడండి](https://vyper.readthedocs.io/en/latest/event-logging.html#event-logging)). ఈవెంట్‌లు దిగుమతి చేసుకున్న ఇంటర్‌ఫేస్‌కు చెందినవి కాబట్టి, మనం వాటిని `IERC721.Transfer`గా సూచిస్తాము మరియు వాటి ఫీల్డ్‌లను కీవర్డ్ ద్వారా పాస్ చేస్తాము.
...
def transferFrom(_from: address, _to: address, _tokenId: uint256):
    ...
    @notice `_to` NFTలను స్వీకరించగలదని నిర్ధారించుకోవాల్సిన బాధ్యత కాలర్‌పై ఉంటుంది, లేకపోతే
            అవి శాశ్వతంగా కోల్పోయే అవకాశం ఉంది.
...
`IERC721` ఇంటర్‌ఫేస్ `transferFrom`, `safeTransferFrom` మరియు `approve`లను చెల్లించదగినవిగా (payable) ప్రకటిస్తుంది కాబట్టి ఇక్కడ `@payable` డెకరేషన్ ఉంది, కాబట్టి ఇంటర్‌ఫేస్‌ను అమలు చేసే కాంట్రాక్ట్ ఆ సంతకాలతో సరిపోలాలి.
...
def safeTransferFrom(
    ...
    @dev ఒక NFT యాజమాన్యాన్ని ఒక చిరునామా నుండి మరొక చిరునామాకు బదిలీ చేస్తుంది.
         ...
         `_to` శూన్య చిరునామా అయితే త్రో చేస్తుంది.
         `_tokenId` చెల్లుబాటు అయ్యే NFT కాకపోతే త్రో చేస్తుంది.
         `_to` స్మార్ట్ కాంట్రాక్ట్ అయితే, అది `_to`లో `onERC721Received`ని కాల్ చేస్తుంది మరియు
         తిరిగి ఇచ్చే విలువ `bytes4(keccak256("onERC721Received(address,address,uint256,bytes)"))` కాకపోతే త్రో చేస్తుంది.
    @param _from NFT యొక్క ప్రస్తుత యజమాని.
    @param _to కొత్త యజమాని.
    @param _tokenId బదిలీ చేయాల్సిన NFT.
    @param _data నిర్దిష్ట ఫార్మాట్ లేని అదనపు డేటా, `_to`కి కాల్‌లో పంపబడుతుంది.
    """
    self._transferFrom(_from, _to, _tokenId, msg.sender)
```
...
def approve(_approved: address, _tokenId: uint256):
    """
    @dev NFT కోసం ఆమోదించబడిన చిరునామాను సెట్ చేయండి లేదా పునరుద్ఘాటించండి. శూన్య చిరునామా ఆమోదించబడిన చిరునామా లేదని సూచిస్తుంది.
...
def setApprovalForAll(_operator: address, _approved: bool):
    """
    @dev `msg.sender` యొక్క అన్ని ఆస్తులను నిర్వహించడానికి మూడవ పక్షానికి ("ఆపరేటర్") ఆమోదాన్ని
         ప్రారంభిస్తుంది లేదా నిలిపివేస్తుంది. ఇది ApprovalForAll ఈవెంట్‌ను కూడా విడుదల చేస్తుంది.
         `_operator` `msg.sender` అయితే త్రో చేస్తుంది. (గమనిక: ఇది EIPలో వ్రాయబడలేదు)
    @notice ఆ సమయంలో పంపినవారికి ఎలాంటి టోకెన్‌లు లేకపోయినా ఇది పని చేస్తుంది.
    @param _operator అధీకృత ఆపరేటర్‌ల సెట్‌కు జోడించాల్సిన చిరునామా.
    @param _approved ఆపరేటర్‌లు ఆమోదించబడితే True, ఆమోదాన్ని ఉపసంహరించుకోవడానికి false.
    """
