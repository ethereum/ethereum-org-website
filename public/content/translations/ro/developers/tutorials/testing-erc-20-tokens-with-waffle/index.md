---
title: Testarea tokenurilor ERC-20 cu Waffle
description: Învățați cum să testați contractele inteligente Solidity și cum să folosiți validatorii-matcher ai contractelor inteligente cu Waffle.
...
- cum să scrieți teste pentru contractele inteligente cu Waffle
- cum să utilizați o serie de validatori-matcher populari pentru testarea contractelor inteligente cu Waffle

Ipoteze:

- știți cum să folosiți un terminal;
- știți cum să creați un nou proiect `JavaScript`;
- ați scris câteva linii de cod în `Solidity`;
- ați scris câteva teste în `JavaScript`;
- ați folosit `yarn` sau `npm`, programul de instalare al pachetelor JavaScripts.

Și în acest caz, dacă nu este valabilă vreuna din ipoteze sau dacă nu intenționați să reproduceți codul din acest articol, tot veți putea ține pasul fără probleme.

## Câteva cuvinte despre Waffle {#a-few-words-about-waffle}

[Waffle](https://getwaffle.io) este cea mai avansată bibliotecă pentru scrierea și testarea contractelor inteligente.
...
Să o luăm de la bun început: creați un nou proiect `JavaScript` sau `TypeScript` (eu aș folosi `TS`, dar nu este nicio problemă dacă folosiți `JS`):
...
Așadar, adăugați `OpenZeppelin` instalându-l cu `yarn`:

```bash
 yarn add @openzeppelin/contracts -D
```

Apoi creați `BasicToken.sol` contractul în directorul `src`:
...
De asemenea, adăugați fișierul `waffle.json` în directorul principal al proiectului dvs.

Iată un exemplu de configurație a fișierului `waffle.json`:
...
Veți vedea că Waffle v-a compilat contractul și a plasat rezultatul din JSON în directorul `build`.
...
În `Waffle` testele sunt scrise folosind `Mocha` împreună cu `Chai`. Putem folosi și alte medii de testare, dar validatorii-matcher `Waffle` funcționează doar cu `Chai`.

Deci trebuie să adăugăm `Chai` la dependențele noastre:
...
Deci folosim metoda `deployContract` din `Waffle` pentru a ne implementa tokenul. Ca argumente ar trebui să transmitem `portofelul`, fișierul „json” compilat al contractului nostru și soldul implicit.
...
`Waffle` ne permite să testăm ce evenimente au fost emise.

În acest tutorial voi testa metoda `transfer` a contractului nostru.

În acest test voi face un transfer dintr-un portofel în altul și voi verifica dacă a fost apelat evenimentul `Transfer`.
...
Totodată, un mare avantaj al acestui validator-matcher este că putem verifica prin ce argumente a fost invocat acest eveniment, adăugând `withArgs` la testul nostru.
...
Pentru a verifica aceasta, vom utiliza în testul nostru validatorul-matcher `revertedWith`.

Putem scrie un test în care vom transfera o sumă mai mare decât cea pe care o avem în portofel. Și apoi vom verifica dacă tranzacția s-a inversat cu mesajul potrivit!
...
Putem utiliza validatorul-matcher `changeTokenBalance` pentru a verifica modificarea soldului sau `changeTokenBalances` pentru mai multe conturi.

Validatorul-matcher poate accepta `numbers`, `strings` și `BigNumbers` ca o modificare a soldului, iar adresa trebuie specificată ca portofel sau contract.

Să scriem următorul test:
...
Acesta este un test pentru un singur portofel.

Iar următorul este pentru mai multe portofele:
...
Se preconizează ca tranzacția să fie transmisă ca un callback (trebuie verificat soldul înainte de apel) sau ca răspuns la tranzacție.
