...
Fusaka temel iyileştirmelere odaklanırken, Glamsterdam farklı ağ katılımcıları arasındaki görev ayrımını güvence altına alarak ve yüksek verimli paralelleştirmeye <GlossaryTooltip termKey="state">hazırlanmak</GlossaryTooltip> için verileri işlemenin daha verimli yollarını tanıtarak "Scale L1" ve "Scale Blobs" hedeflerini ilerletiyor. 

Bu iyileştirmeler, Ethereum'un daha fazla aktiviteyi yönetirken hızlı, uygun fiyatlı ve merkeziyetsiz kalmasını sağlarken, evde <GlossaryTooltip termKey="node">düğüm</GlossaryTooltip> çalıştıran kişiler için donanım gereksinimlerini yönetilebilir tutar.
...
Teklif sahipleri ve oluşturucular arasındaki protokol dışı ilişki, blok doğrulama sırasında bir 'kısa yol' da oluşturur; bu da <GlossaryTooltip termKey="validator">doğrulayıcılar</GlossaryTooltip> 2 saniyelik dar bir pencerede işlem yayınlama ve yürütme konusunda acele etmeye zorlayarak ağ işleyebileceği veri miktarını sınırlar.
...
ePBS, ek üçüncü taraf yazılımlara olan bağımlılığı azaltır ve Ethereum'un ağ zorlamadan çok daha büyük miktarda veriyi ( <GlossaryTooltip termKey="layer-2">Katman 2'ler</GlossaryTooltip> için daha fazla blob gibi) güvenli bir şekilde işlemesini sağlar.
...
Yeni hesaplar, token'lar ve <GlossaryTooltip termKey="smart-contract">akıllı sözleşmeler</GlossaryTooltip> eklemek, ağ çalıştıran her bilgisayarın süresiz olarak saklaması gereken kalıcı veriler (durum olarak bilinir) oluşturur. Bu verileri ekleme veya okuma için mevcut ücretler tutarsızdır ve ağın donanımına getirdikleri gerçek, uzun vadeli depolama yükünü yansıtmaz.
...
### Eyalet erişim gaz maliyeti güncellemesi {#state-access-gas-cost-update}
