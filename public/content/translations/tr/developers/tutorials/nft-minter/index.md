...
NFT'lerimizi basmak (veya Ethereum blok zincirindeki herhangi bir işlemi imzalamak) için biraz sahte Eth'e ihtiyacımız olacak. Eth almak için [Ropsten musluğuna](https://www.alchemy.com/faucets/ethereum-sepolia) gidip Ropsten hesap adresinizi girebilir, ardından “Send Ropsten Eth” (Ropsten Eth Gönder) düğmesine tıklayabilirsiniz. Kısa bir süre sonra MetaMask hesabınızda Eth'i görmelisiniz!
...
Daha önce de belirttiğimiz gibi, bu öğreticide [bu mevcut NFT akıllı sözleşmesini](https://www.alchemy.com/docs/how-to-create-an-nft) kullanıyor olacağız; ancak, bunu nasıl yaptığımızı öğrenmek veya kendiniz yapmak isterseniz, diğer öğreticimiz olan ["Bir NFT Nasıl Oluşturulur."](https://www.alchemy.com/docs/how-to-create-an-nft) başlıklı yazımıza göz atmanızı şiddetle tavsiye ederiz.
...
try {
  ...
  return {
    success: true,
    status:
      "✅ İşleminizi Etherscan'de kontrol edin: https://sepolia.etherscan.io/tx/" +
      txHash,
  }
} catch (error) {
  return {
