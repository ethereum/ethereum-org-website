...
Bunu, bir doğrulayıcıyı güvence altına almak için kullanılan **özel anahtarı** "küme" halinde düzenlenmiş **birçok bilgisayara bölerek** yapar. Bunun faydası, tam olarak hiçbir makinede saklanmadığı için saldırganların anahtara erişimini zorlaştırmasıdır. Ayrıca gerekli imzalamalar her kümedeki sistemlerin bir alt kümesi tarafından yapılabildiği için bazı düğümlerin çevrimdışı olmasına da izin verir. Bu sayede ağdaki tek hata noktaları azalırken doğrulayıcı setin tamamı da daha sağlam hale gelmiş olur.

![Tek bir doğrulayıcı anahtarının anahtar paylarına nasıl bölündüğünü ve çeşitli bileşenlere sahip birden çok düğüme nasıl dağıtıldığını gösteren bir şema.](./dvt-cluster.png)
...
### Tek hata noktası yok {#no-single-point-of-failure}
...
4. **Asgari güvene dayalı** doğrulayıcı işlemi
5. **En aza indirilmiş slashing** ve kapalı kalma riskleri
6. **Çeşitliliği artırır** (istemci, veri merkezi, konum, düzenleme vb.)
7. Doğrulayıcı anahtar yönetiminin **geliştirilmiş güvenliği**

## DVT nasıl çalışır? {#how-does-dvt-work}

Bir DVT çözümü aşağıdaki bileşenleri içerir:

- **[Shamir'in Sır Paylaşımı](https://medium.com/@keylesstech/a-beginners-guide-to-shamir-s-secret-sharing-e864efbf3648)** - Doğrulayıcılar [BLS anahtarlarını](https://en.wikipedia.org/wiki/BLS_digital_signature) kullanır. Ayrı BLS "anahtar payları" ("anahtar payları") tek bir kümelenmiş anahtarda (imza) birleştirilebilir. DVT'de, doğrulayıcı için olan özel anahtar her kümedeki operatörün BLS imzasıyla birleştirilmiştir.
- **[Eşik imza şeması](https://medium.com/nethermind-eth/threshold-signature-schemes-36f40bc42aca)** - İmzalama görevleri için gerekli olan bireysel anahtar paylarının sayısını belirler; ör. 4 üzerinden 3.
- **[Dağıtılmış anahtar üretimi (DAÜ)](https://medium.com/toruslabs/what-distributed-key-generation-is-866adc79620)** - Anahtar paylarını oluşturan ve mevcut veya yeni bir doğrulayıcı anahtarının paylarını bir kümedeki düğümlere dağıtmak için kullanılan kriptografik bir süreçtir.
- **[Çok taraflı hesaplama (MPC)](https://messari.io/report/applying-multiparty-computation-to-the-world-of-blockchains)** - Tam doğrulayıcı anahtarı, çok taraflı hesaplama kullanılarak gizlice oluşturulur. Tam anahtar asla hiçbir ayrı operatör tarafından bilinmeyecek - Operatörler sadece kendi bölümlerini (kendilerinin "paylarını") bilecekler.
- **Mutabakat protokolü** - Mutabakat protokolü, blok önericisi olacak bir düğüm seçer. Bloklarını, anahtar paylarını imzaları birleştirmek için ekleyen diğer düğümlerle bir hücrenin içinde paylaşırlar. Anahtar payları yeteri kadar toplandığında blok, Ethereum üzerinde sunulur.
...
## DVT kullanım senaryoları {#dvt-use-cases}
...
### Hizmet Olarak Hisseleme (SaaS) {#saas}
...
### Paydaşlık havuzları {#staking-pools}
...
- **İşletim maliyetleri** - DVT, doğrulayıcıyı birden çok taraf arasında dağıttığı için, işletim için tek bir düğüm yerine daha fazla düğüm gerekir ve bu da artan işletim maliyetlerine yol açar.
- **Potansiyel olarak artan gecikme** - DVT, bir doğrulayıcıyı yöneten birden fazla düğüm arasında mutabakat sağlamak için bir mutabakat protokolü kullandığından, bu durum potansiyel olarak gecikmeyi artırabilir.

## Ek Okumalar {#further-reading}

- [Ethereum dağıtılmış doğrulayıcı teknik özellikleri (yüksek seviye)](https://github.com/ethereum/distributed-validator-specs)
...
- [Shamir'in Sır Paylaşımı demo uygulaması](https://iancoleman.io/shamir/)
