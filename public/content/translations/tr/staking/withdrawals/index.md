...
Bir hesap "çekilebilir" olarak işaretlendikten ve çekim kimlik bilgileri sağlandıktan sonra, kullanıcının beklemek dışında yapması gereken başka bir şey yoktur. Hesaplar, uygun çıkış yapmış fonlar için blok teklif ediciler tarafından otomatik ve sürekli olarak taranır ve hesap bakiyeniz bir sonraki <a href="#validator-sweeping" customEventOptions={{ eventCategory: "Anchor link", eventAction: "Exiting staking entirely (sweep)", eventName: "click" }}>tarama</a> sırasında tam olarak transfer edilir (bu aynı zamanda "tam çekim" olarak da bilinir).
