...
description: "Unter Verwendung moderner Komponenten wie TypeScript, React, Vite und Wagmi werden wir eine moderne, aber minimale Benutzeroberfläche durchgehen und lernen, wie man eine Wallet mit der Benutzeroberfläche verbindet, einen Smart Contract aufruft, um Informationen zu lesen, eine Transaktion an einen Smart Contract sendet und Ereignisse von einem Smart Contract überwacht, um Änderungen zu erkennen."
author: Ori Pomerantz
tags: ["TypeScript", "React", "Vite", "Wagmi", "frontend"]
skill: beginner
breadcrumb: "Benutzeroberfläche mit WAGMI"
published: 2023-11-01
lang: de
sidebarDepth: 3
---

Sie haben eine Funktion gefunden, die wir im Ethereum-Ökosystem benötigen. Sie haben die Smart Contracts geschrieben, um sie zu implementieren, und vielleicht sogar einigen zugehörigen Code, der offchain ausgeführt wird. Das ist großartig! Leider werden Sie ohne eine Benutzeroberfläche keine Nutzer haben, und das letzte Mal, als Sie eine Website geschrieben haben, benutzten die Leute Einwahlmodems und JavaScript war neu.

Dieser Artikel ist für Sie. Ich gehe davon aus, dass Sie programmieren können und vielleicht ein wenig JavaScript und HTML kennen, aber dass Ihre Fähigkeiten im Bereich Benutzeroberflächen eingerostet und veraltet sind. Gemeinsam werden wir eine einfache moderne Anwendung durchgehen, damit Sie sehen, wie das heutzutage gemacht wird.

## Warum ist das wichtig? {#why-important}

Theoretisch könnten Sie die Leute einfach [Etherscan](https://sepolia.etherscan.io/address/0xC87506C66c7896366b9E988FE0aA5B6dDE77CFfA#readContract) oder [Blockscout](https://eth-sepolia.blockscout.com/address/0xC87506C66c7896366b9E988FE0aA5B6dDE77CFfA?tab=read_write_contract) nutzen lassen, um mit Ihren Verträgen zu interagieren. Das ist großartig für erfahrene Ethereans. Aber wir versuchen, [einer weiteren Milliarde Menschen](https://blog.ethereum.org/2021/05/07/ethereum-for-the-next-billion) zu dienen. Das wird ohne eine großartige Benutzererfahrung nicht passieren, und eine freundliche Benutzeroberfläche ist ein großer Teil davon.

## Greeter-Anwendung {#greeter-app}

Es gibt viel Theorie darüber, wie moderne Benutzeroberflächen funktionieren, und [viele gute Websites](https://react.dev/learn/thinking-in-react), [die das erklären](https://wagmi.sh/core/getting-started). Anstatt die hervorragende Arbeit dieser Websites zu wiederholen, gehe ich davon aus, dass Sie lieber durch Ausprobieren lernen und mit einer Anwendung beginnen, mit der Sie herumspielen können. Sie brauchen die Theorie trotzdem, um Dinge zu erledigen, und wir werden dazu kommen – wir gehen einfach Quelldatei für Quelldatei durch und besprechen die Dinge, wenn wir auf sie stoßen.

### Installation {#installation}

1. Die Anwendung verwendet das [Sepolia](https://sepolia.dev/)-Testnetzwerk. Falls erforderlich, [holen Sie sich Sepolia-Test-ETH](/developers/docs/networks/#sepolia) und [fügen Sie Sepolia zu Ihrer Wallet hinzu](https://chainlist.org/chain/11155111).
...
3. Die Anwendung verwendet kostenlose Zugangspunkte, die Leistungseinschränkungen haben. Wenn Sie einen Anbieter für [Knoten als Dienstleistung](/developers/docs/nodes-and-clients/nodes-as-a-service/) (Node as a Service) nutzen möchten, ersetzen Sie die URLs in [`src/wagmi.ts`](#wagmi-ts).
...
5. Rufen Sie die von der Anwendung angezeigte URL auf. In den meisten Fällen ist das [http://localhost:5173/](http://localhost:5173/).

6. Sie können den Quellcode des Vertrags, eine modifizierte Version von Hardhats Greeter, [in einem Blockchain-Explorer ansehen](https://eth-sepolia.blockscout.com/address/0xC87506C66c7896366b9E988FE0aA5B6dDE77CFfA?tab=contract_code).

### Durchgang durch die Dateien {#file-walk-through}
...
Die Dateierweiterung zeigt an, dass dies eine [React-Komponente](https://www.w3schools.com/react/react_components.asp) ist, die in [TypeScript](https://www.typescriptlang.org/) geschrieben wurde, einer Erweiterung von JavaScript, die [Typprüfung](https://en.wikipedia.org/wiki/Type_system#Type_checking) unterstützt. TypeScript wird zu JavaScript kompiliert, sodass wir es auf der Client-Seite verwenden können.
...
Importieren Sie den Code der Bibliothek, den wir benötigen.
...
- Den Zustand für das erneute Abrufen im Hintergrund
...
Die Anwendung befindet sich auch in [einer `WagmiProvider`-Komponente](https://wagmi.sh/react/api/WagmiProvider). [Die Wagmi-Bibliothek (wir werden sie erstellen)](https://wagmi.sh/) verbindet die React-UI-Definitionen mit [der Viem-Bibliothek](https://viem.sh/) zum Schreiben einer dezentralen Anwendung (Dapp) für Ethereum.

```tsx
      <QueryClientProvider client={queryClient}>
```

Und schließlich fügen Sie einen React Query-Provider hinzu, damit jede Anwendungskomponente zwischengespeicherte Abfragen verwenden kann.
...
Verwenden Sie [`useConnection`](https://wagmi.sh/react/api/hooks/useConnection), um Informationen zur aktuellen Verbindung zu erhalten, wie die Adresse und `chainId`.

Konventionsgemäß sind in React Funktionen, die `use...` genannt werden, [Hooks](https://www.w3schools.com/react/react_hooks.asp). Diese Funktionen geben nicht nur Daten an die Komponente zurück; sie stellen auch sicher, dass sie neu gerendert wird (die Komponentenfunktion wird erneut ausgeführt und ihre Ausgabe ersetzt die vorherige im HTML), wenn sich diese Daten ändern.
...
[Dieser Hook](https://wagmi.sh/react/api/hooks/useDisconnect) gibt uns die Funktion, um die Verbindung zur Wallet zu trennen.

```tsx
  const { switchChain } = useSwitchChain()
```

[Dieser Hook](https://wagmi.sh/react/api/hooks/useSwitchChain) lässt uns zwischen Chains wechseln.
...
Führen Sie die Funktion jedes Mal erneut aus, wenn sich entweder der Verbindungsstatus oder die Verbindungs-chainId ändert.
...
Die Syntax `{<condition> && <value>} means "if the condition is `true`, evaluate to the value; if it isn't, evaluate to `false`“.
...
Wenn wir _nicht_ verbunden sind, zeigen Sie die erforderlichen Optionen an, um sich mit der Wallet zu verbinden.
...
In JSX ist es notwendig, dass „Geschwister“-Tags (Tags, die vom selben übergeordneten Element abstammen) unterschiedliche Identifikatoren haben.
...
[Die Bibliothek `abitype`](https://abitype.dev/) stellt uns TypeScript-Definitionen für verschiedene Ethereum-Datentypen zur Verfügung, wie z. B. [`AddressType`](https://abitype.dev/config#addresstype).
...
Die ABI für den Vertrag `Greeter`.
...
Wir verwenden [`as const`](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html#const-assertions), um TypeScript mitzuteilen, dass dies eine _echte_ Konstante ist. Wenn Sie in JavaScript `const x = {"a": 1}` angeben, können Sie normalerweise den Wert in `x` ändern, Sie können ihm nur nichts Neues zuweisen.
...
TypeScript ist stark typisiert. Wir verwenden diese Definition, um die Adresse anzugeben, an der der Vertrag `Greeter` über verschiedene Chains hinweg bereitgestellt wird. Der Schlüssel ist eine Zahl (die chainId), und der Wert ist eine `AddressType` (eine Adresse).
...
Die Komponente `Timer` zeigt die Anzahl der Sekunden seit einem bestimmten Zeitpunkt an. Dies ist für die Benutzerfreundlichkeit wichtig. Wenn Benutzer etwas tun, erwarten sie eine sofortige Reaktion. Bei Blockchains ist dies oft unmöglich, da nichts passiert, bis eine Transaktion in einem Block platziert wird. Eine Lösung besteht darin, anzuzeigen, wie lange es her ist, seit der Benutzer die Aktion ausgeführt hat, damit der Benutzer entscheiden kann, ob die benötigte Zeit angemessen ist.
...
Die Komponente `Timer` nimmt einen Parameter entgegen, `lastUpdate`, der den Zeitpunkt der letzten Aktion darstellt.
...
Die Funktion [`setInterval`](https://www.w3schools.com/jsref/met_win_setinterval.asp) lässt uns eine Funktion so planen, dass sie regelmäßig ausgeführt wird. In diesem Fall jede Sekunde. Die Funktion ruft `setNow` auf, um den Zustand zu aktualisieren, sodass die Komponente `Timer` neu gerendert wird. Wir verpacken dies in [`useEffect`](https://react.dev/reference/react/useEffect) mit einer leeren Abhängigkeitsliste, damit es nur einmal passiert und nicht jedes Mal, wenn die Komponente gerendert wird.
...
Schließlich kommen wir dazu, die Komponente zu definieren.
...
[Der Hook `useReadContract`](https://wagmi.sh/react/api/hooks/useReadContract) ruft die Funktion `greet` [des Vertrags](https://eth-sepolia.blockscout.com/address/0xC87506C66c7896366b9E988FE0aA5B6dDE77CFfA?tab=contract) auf.
...
Der Hook [`useState`](https://www.w3schools.com/react/react_usestate.asp) von React lässt uns eine Zustandsvariable angeben, deren Wert von einem Rendering der Komponente zum nächsten erhalten bleibt. Der Anfangswert ist der Parameter, in diesem Fall die leere Zeichenfolge.

Der Hook `useState` gibt eine Liste mit zwei Werten zurück:
...
Benutzer sehen gerne, dass ihre Aktionen eine sofortige Wirkung haben. Auf einer Blockchain ist dies jedoch nicht der Fall. Diese Zustandsvariablen lassen uns den Benutzern zumindest etwas anzeigen, damit sie wissen, dass ihre Aktion in Bearbeitung ist.
...
Wenn `readResults` oben die Daten ändert und sie nicht auf einen falschen Wert (zum Beispiel `undefined`) gesetzt sind, aktualisieren Sie die aktuelle Begrüßung auf die aus der Blockchain gelesene. Aktualisieren Sie außerdem den Status.
...
`!!<value>` bedeutet, dass, wenn der Wert `false` ist oder ein Wert, der als falsch ausgewertet wird, wie `undefined`, `0` oder eine leere Zeichenfolge, der Ausdruck insgesamt `false` ist. Für jeden anderen Wert ist er `true`. Es ist eine Möglichkeit, Werte in Boolesche Werte umzuwandeln, denn wenn es kein `greeterAddr` gibt, wollen wir nicht auf Ereignisse hören.
...
Wenn wir Protokolle sehen (was passiert, wenn wir ein neues Ereignis sehen), bedeutet das, dass die Begrüßung geändert wurde. In diesem Fall können wir `currentGreeting` und `lastSetterAddress` auf die neuen Werte aktualisieren. Außerdem wollen wir die Statusanzeige aktualisieren.
...
Wenn wir den Status aktualisieren, wollen wir zwei Dinge tun:
...
Dies ist der Event-Handler für Änderungen am Eingabefeld für die neue Begrüßung. Wir könnten den Typ des Parameters `evt` angeben, aber TypeScript ist eine Sprache mit optionaler Typisierung. Da diese Funktion nur einmal in einem HTML-Event-Handler aufgerufen wird, halte ich das nicht für notwendig.

```tsx
  const { writeContractAsync } = useWriteContract()
```

Die Funktion zum Schreiben in einen Vertrag. Sie ist ähnlich wie [`writeContracts`](https://wagmi.sh/core/api/actions/writeContracts#writecontracts), ermöglicht aber bessere Statusaktualisierungen.
...
Dies ist der Prozess zum Einreichen einer Blockchain-Transaktion aus der Client-Perspektive:
...
3. Wenn die Antwort empfangen wird, bitten Sie den Benutzer, die Transaktion über die Wallet zu signieren. Dieser Schritt _muss_ erfolgen, nachdem die Antwort des Knotens empfangen wurde, da dem Benutzer vor dem Signieren die Gaskosten der Transaktion angezeigt werden.
4. Warten Sie auf die Zustimmung des Benutzers.
...
Schritt 2 wird wahrscheinlich eine spürbare Zeit in Anspruch nehmen, in der sich die Benutzer möglicherweise fragen, ob ihr Befehl von der Benutzeroberfläche empfangen wurde und warum sie noch nicht aufgefordert werden, die Transaktion zu signieren. Das führt zu einer schlechten Benutzererfahrung (UX).
...
Wenn wir wissen, wer die Begrüßung zuletzt festgelegt hat, zeigen Sie diese Informationen an. `Greeter` verfolgt diese Informationen nicht, und wir wollen nicht nach `SetGreeting`-Ereignissen in der Vergangenheit suchen, also erhalten wir sie nur, wenn die Begrüßung geändert wird, während wir ausgeführt werden.
...
Dies ist das Eingabetextfeld, in dem der Benutzer eine neue Begrüßung festlegen kann. Jedes Mal, wenn der Benutzer eine Taste drückt, rufen wir `greetingChange` auf, was `setNewGreeting` aufruft. Da `setNewGreeting` von `useState` stammt, führt dies dazu, dass die Komponente `Greeter` neu gerendert wird. Das bedeutet, dass:

- Wir `value` angeben müssen, um den Wert der neuen Begrüßung beizubehalten, da er sich sonst wieder in den Standardwert, die leere Zeichenfolge, verwandeln würde.
- `simulation` ebenfalls jedes Mal aktualisiert wird, wenn sich `newGreeting` ändert, was bedeutet, dass wir eine Simulation mit der korrekten Begrüßung erhalten. Dies könnte relevant sein, da die Gaskosten von der Größe der Aufrufdaten abhängen, die wiederum von der Länge der Zeichenfolge abhängt.
...
Aktualisieren Sie den Status. An diesem Punkt muss der Benutzer in der Wallet bestätigen.
...
`writeContractAsync` kehrt erst zurück, nachdem die Transaktion tatsächlich gesendet wurde. Dadurch können wir dem Benutzer anzeigen, wie lange die Transaktion bereits darauf wartet, in die Blockchain aufgenommen zu werden.
...
[Dieser Konnektor](https://wagmi.sh/core/api/connectors/injected) lässt uns mit einer im Browser installierten Wallet kommunizieren.
...
Heutzutage gibt es viele [L2-Skalierungslösungen](https://ethereum.org/layer-2/), und Sie möchten vielleicht einige unterstützen, die Viem noch nicht unterstützt. Dazu ändern Sie `src/wagmi.ts`. Diese Anleitung erklärt, wie Sie [Optimism Sepolia](https://chainlist.org/chain/11155420) hinzufügen.
...
4.  In Ihrem Browser.
    ...
    A. Navigieren Sie zu [ChainList](https://chainlist.org/chain/11155420?testnets=true) und klicken Sie auf eine der Schaltflächen auf der rechten Seite der Tabelle, um die Chain zu Ihrer Wallet hinzuzufügen.

    B. Klicken Sie in der Anwendung auf **Disconnect** (Trennen) und verbinden Sie sich dann erneut, um die Blockchain zu wechseln. Es gibt elegantere Wege, dies zu handhaben, aber sie würden Änderungen an der Anwendung erfordern.
