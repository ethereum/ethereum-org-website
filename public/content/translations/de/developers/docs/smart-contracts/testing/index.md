...
Automatisiertes Testen verwendet Tools, die den Code eines Smart Contracts automatisch auf Ausführungsfehler prüfen. Der Vorteil des automatisierten Testens liegt in der Verwendung von [Skripten](https://www.techtarget.com/whatis/definition/script?amp=1) zur Steuerung der Bewertung von Vertragsfunktionen. Skriptbasierte Tests können so geplant werden, dass sie wiederholt mit minimalem menschlichen Eingreifen ausgeführt werden, was automatisiertes Testen effizienter macht als manuelle Testansätze.
...
Manuelles Testen wird von Menschen unterstützt und beinhaltet die Ausführung jedes Testfalls in Ihrer Test-Suite nacheinander bei der Analyse der Korrektheit eines Smart Contracts. Dies steht im Gegensatz zum automatisierten Testen, bei dem Sie gleichzeitig mehrere isolierte Tests an einem Vertrag ausführen und einen Bericht erhalten können, der alle fehlgeschlagenen und bestandenen Tests zeigt.
...
Bevor Sie Unit-Tests schreiben, ist es hilfreich zu wissen, welche Funktionalitäten ein Smart Contract bietet und wie Benutzer auf diese Funktionen zugreifen und sie nutzen werden. Dies ist besonders nützlich für die Ausführung von [Happy-Path-Tests](https://en.m.wikipedia.org/wiki/Happy_path), die bestimmen, ob Funktionen in einem Vertrag die korrekte Ausgabe für gültige Benutzereingaben zurückgeben. Wir werden dieses Konzept anhand dieses (gekürzten) Beispiels [eines Auktionsvertrags](https://docs.soliditylang.org/en/v0.8.17/solidity-by-example.html?highlight=Auction%20contract#simple-open-auction) erklären.
...
Dies ist ein einfacher Auktionsvertrag, der darauf ausgelegt ist, Gebote während der Bietfrist entgegenzunehmen. Wenn das `highestBid` steigt, erhält der vorherige Höchstbietende sein Geld zurück; sobald die Bietfrist abgelaufen ist, ruft der `beneficiary` den Vertrag auf, um sein Geld zu erhalten.
...
- **[Ausführen von Unit-Tests mit Moccasin](https://github.com/Cyfrin/moccasin)**
...
Integrationstests sind nützlich, wenn Ihr Vertrag eine modulare Architektur annimmt oder während der Ausführung mit anderen Onchain-Verträgen interagiert. Eine Möglichkeit, Integrationstests durchzuführen, besteht darin, die Blockchain auf einer bestimmten Höhe zu [forken](/glossary/#fork) (mit einem Tool wie [Forge](https://book.getfoundry.sh/forge/fork-testing) oder [Hardhat](https://hardhat.org/hardhat-network/docs/guides/forking-other-networks)) und Interaktionen zwischen Ihrem Vertrag und bereitgestellten Verträgen zu simulieren.

Die geforkte Blockchain verhält sich ähnlich wie das Mainnet und verfügt über Konten mit zugehörigen Zuständen und Salden. Sie fungiert jedoch nur als lokale Sandbox-Entwicklungsumgebung, was bedeutet, dass Sie beispielsweise keine echten ETH für Transaktionen benötigen und Ihre Änderungen das echte Ethereum-Protokoll nicht beeinflussen.
...
### Richtlinien für die Durchführung von eigenschaftsbasiertem Testen für Smart Contracts {#running-property-based-tests}

Die Durchführung von eigenschaftsbasiertem Testen beginnt typischerweise mit der Definition einer Eigenschaft (z. B. das Fehlen von [Ganzzahl-Überläufen](https://github.com/ConsenSysDiligence/mythril/wiki/Integer-Overflow)) oder einer Sammlung von Eigenschaften, die Sie in einem Smart Contract verifizieren möchten. Möglicherweise müssen Sie beim Schreiben von Eigenschaftstests auch einen Wertebereich definieren, innerhalb dessen das Programm Daten für Transaktionseingaben generieren kann.

Sobald das Tool für Eigenschaftstests richtig konfiguriert ist, führt es die Funktionen Ihres Smart Contracts mit zufällig generierten Eingaben aus. Wenn es Verletzungen von Zusicherungen gibt, sollten Sie einen Bericht mit konkreten Eingabedaten erhalten, die die zu bewertende Eigenschaft verletzen. In einigen der folgenden Leitfäden erfahren Sie, wie Sie mit der Durchführung von eigenschaftsbasiertem Testen mit verschiedenen Tools beginnen können:
...
### Testen von Verträgen in Testnetzen {#testing-contracts-on-testnets}
...
Die Bereitstellung in einem Testnetz nach dem Testen auf einer lokalen Blockchain ist ideal, da ersteres dem Verhalten der Virtuellen Ethereum-Maschine (EVM) näher kommt. Daher ist es für viele Ethereum-native Projekte üblich, Dapps in Testnetzen bereitzustellen, um den Betrieb eines Smart Contracts unter realen Bedingungen zu bewerten.
...
- **[ApeWorx](https://docs.apeworx.io/ape/stable/userguides/testing.html)** - _Python-basiertes Entwicklungs- und Test-Framework für Smart Contracts, das auf die Virtuelle Ethereum-Maschine (EVM) abzielt._

- **[Wake](https://ackeeblockchain.com/wake/docs/latest/testing-framework/overview/)** - _Python-basiertes Framework für Unit-Testing und Fuzzing mit starken Debugging-Funktionen und Unterstützung für kettenübergreifendes Testen, das Pytest und Anvil für beste Benutzererfahrung und Leistung nutzt._

### Tools für eigenschaftsbasiertes Testen {#property-based-testing-tools}

#### Tools zur statischen Analyse {#static-analysis-tools}
...
#### Tools zur dynamischen Analyse {#dynamic-analysis-tools}
...
- **[Mythril](https://github.com/ConsenSysDiligence/mythril)** - _EVM-Bytecode-Bewertungstool zur Erkennung von Vertragsschwachstellen mithilfe von Taint-Analyse, konkolische Analyse und Kontrollflussprüfung._
...
- [Wie man eine Dapp in einem lokalen Multi-Client-Testnetz entwickelt und testet](/developers/tutorials/develop-and-test-dapps-with-a-multi-client-local-eth-testnet/) _– Anleitung zur Bereitstellung eines Smart Contracts in einem lokalen Testnetz und zur Durchführung von Tests._
