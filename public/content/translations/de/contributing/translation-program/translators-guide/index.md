...
Wenn Sie Fragen, Vorschläge oder Feedback haben, können Sie uns gerne unter translations@ethereum.org kontaktieren, eine Nachricht an @ethdotorg auf Crowdin senden oder [unserem Discord beitreten](/discord/), wo Sie uns im Kanal #translations eine Nachricht schreiben oder sich an jedes der Teammitglieder wenden können.

## Nutzung von Crowdin {#using-crowdin}

Das Crowdin-Projekt von ethereum.org wird geschlossen und akzeptiert keine neuen Beiträge mehr (siehe [Status des Übersetzungsprogramms](/contributing/translation-program/#program-status)). Die folgenden Hinweise dienen weiterhin als Referenz für Übersetzungs-Communitys, die Crowdin für ihre eigenen Projekte nutzen.

Wenn Sie mehr über Crowdin und die Nutzung einiger seiner erweiterten Funktionen erfahren möchten, bietet die [Crowdin-Wissensdatenbank](https://support.crowdin.com/online-editor/) viele ausführliche Leitfäden und Übersichten über alle Crowdin-Funktionen.
...
Es ist wichtig, dass die Übersetzungen die Essenz der Nachricht erfassen. Dies kann bedeuten, dass bestimmte Phrasen umformuliert oder beschreibende Übersetzungen verwendet werden, anstatt den Inhalt Wort für Wort zu übersetzen.
...
Wir verwenden die formelle Anrede, die immer höflich und für alle Besucher angemessen ist.
...
## Übersetzung von Seiten-Metadaten {#translating-metadata}
...
Einige Beispiele für Sprachcodes der am häufigsten gesprochenen Sprachen:
...
Um die Verwaltung von Tags zu erleichtern und sie direkt aus dem Ausgangstext zu kopieren, empfehlen wir, Ihre Einstellungen im Crowdin-Editor zu ändern.
...
Diese sollten mit dem Ausgangstext identisch sein und nicht geändert oder übersetzt werden. Wenn Sie einen Link übersetzen oder in irgendeiner Weise ändern, selbst wenn Sie nur einen Teil davon entfernen, wie z. B. einen Schrägstrich (/), führt dies zu fehlerhaften und unbrauchbaren Links.

Der beste Weg, mit Links umzugehen, besteht darin, sie direkt aus dem Ausgangstext zu kopieren, entweder durch Anklicken oder über die Schaltfläche 'Copy Source' (Ausgangstext kopieren) (`Alt+C`).
...
Es ist sehr wichtig, die Links aus dem Ausgangstext zu kopieren und ihre Reihenfolge nicht zu ändern.
...
Der Ausgangstext enthält viele verschiedene Arten von Tags, die immer aus dem Ausgangstext kopiert und niemals geändert werden sollten. Ähnlich wie oben sollte auch die Reihenfolge dieser Tags in der Übersetzung mit der des Ausgangstextes übereinstimmen.
...
Beispiel: `<strong x-id="1">`Dezentral`</strong>`

`<strong x-id="1">` - _Öffnendes Tag, das den Text fett macht_

Dezentral - _Übersetzbarer Text_
...
Im folgenden Beispiel können Sie sehen, dass das Überfahren des Tags `<0>` mit der Maus zeigt, dass es `<code>` darstellt und ein Code-Snippet enthält. Daher sollte der Inhalt innerhalb dieser Tags nicht übersetzt werden.

![Example of ambiguous tags.png](./example-of-ambiguous-tags.png)

## Kurz- vs. Vollformen/Abkürzungen {#short-vs-full-forms}

Auf der Website werden viele Abkürzungen verwendet, z. B. Dapps, NFT, DAO, DeFi usw. Diese Abkürzungen werden im Englischen häufig verwendet und die meisten Besucher der Website sind mit ihnen vertraut.

Da es für sie in anderen Sprachen normalerweise keine etablierten Übersetzungen gibt, ist es am besten, bei diesen und ähnlichen Begriffen eine beschreibende Übersetzung der Vollform bereitzustellen und die englische Abkürzung in Klammern hinzuzufügen.

Übersetzen Sie diese Abkürzungen nicht, da die meisten Menschen nicht mit ihnen vertraut wären und die lokalisierten Versionen für die meisten Besucher wenig Sinn ergeben würden.
...
Einige Begriffe haben möglicherweise keine etablierten Übersetzungen in anderen Sprachen und sind unter dem englischen Originalbegriff weithin bekannt. Zu diesen Begriffen gehören meist neuere Konzepte wie Proof-of-Work, Proof-of-Stake, Beacon Chain, Staking usw.
...
Seien Sie bei der Übersetzung gerne kreativ, verwenden Sie beschreibende Übersetzungen oder übersetzen Sie sie einfach wörtlich.
...
Schaltflächentext kann identifiziert werden, indem man sich die Kontext-Screenshots ansieht, die mit den meisten Zeichenfolgen verbunden sind, oder indem man den Kontext im Editor überprüft, der den Begriff „button“ enthält.
...
## Inklusive Übersetzung {#translating-for-inclusivity}

Die Besucher von ethereum.org kommen aus der ganzen Welt und haben unterschiedliche Hintergründe. Die Sprache auf der Website sollte daher neutral, für alle einladend und nicht exklusiv sein.

Ein wichtiger Aspekt dabei ist die Geschlechtsneutralität. Dies lässt sich leicht durch die Verwendung der formellen Anrede und die Vermeidung geschlechtsspezifischer Wörter in den Übersetzungen erreichen.

Eine weitere Form der Inklusivität besteht darin, für ein globales Publikum zu übersetzen, das nicht auf ein bestimmtes Land, eine bestimmte Ethnie oder Region beschränkt ist.
...
Beim Übersetzen ist es wichtig, die Grammatikregeln, Konventionen und Formatierungen Ihrer Sprache zu befolgen, anstatt sie aus dem Ausgangstext zu kopieren. Der Ausgangstext folgt den englischen Grammatikregeln und Konventionen, was auf viele andere Sprachen nicht anwendbar ist.

Sie sollten sich der Regeln für Ihre Sprache bewusst sein und entsprechend übersetzen. Wenn Sie Hilfe benötigen, wenden Sie sich an uns, und wir helfen Ihnen dabei, Ressourcen dazu zu finden, wie diese Elemente in Ihrer Sprache verwendet werden sollten.
...
- Einige häufige Unterschiede bei der Verwendung von Leerzeichen zwischen Englisch und anderen Sprachen:
...
- Sprachen verwenden viele verschiedene Anführungszeichen. Das einfache Kopieren der englischen Anführungszeichen aus dem Ausgangstext ist oft falsch.
...
- Die Übersetzung von Währungen kann aufgrund der unterschiedlichen Formate, Konventionen und Umrechnungen eine Herausforderung sein. Als allgemeine Regel gilt: Bitte behalten Sie die Währungen wie im Ausgangstext bei. Sie können Ihre lokale Währung und die Umrechnung in Klammern hinzufügen, um dem Leser zu helfen.
- Die Hauptunterschiede beim Schreiben von Währungen in verschiedenen Sprachen umfassen die Platzierung von Symbolen, Dezimalkommas vs. Dezimalpunkte, Leerzeichen und Abkürzungen vs. Symbole.
  - Platzierung von Symbolen: $100 oder 100$
  - Dezimalkommas vs. Dezimalpunkte: 100,50$ oder 100.50$
  - Leerzeichen: 100$ oder 100 $
  - Abkürzungen vs. Symbole: 100 $ oder 100 USD

**Maßeinheiten**

- Als allgemeine Regel gilt: Bitte behalten Sie die Maßeinheiten wie im Ausgangstext bei. Wenn Ihr Land ein anderes System verwendet, können Sie die Umrechnung in Klammern angeben.
- Abgesehen von der Lokalisierung von Maßeinheiten ist es auch wichtig, die Unterschiede in der Handhabung dieser Einheiten in den verschiedenen Sprachen zu beachten. Der Hauptunterschied ist das Leerzeichen zwischen der Zahl und der Einheit, das je nach Sprache unterschiedlich sein kann. Beispiele hierfür sind 100kB vs. 100 kB oder 50ºF vs. 50 ºF.
...
Versuchen Sie beim Übersetzen, sich nicht zu hetzen. Lassen Sie es ruhig angehen und haben Sie Spaß!

Vielen Dank, dass Sie sich am Übersetzungsprogramm beteiligen und uns dabei helfen, die Website einem breiteren Publikum zugänglich zu machen. Die Ethereum-Community ist global und wir freuen uns, dass Sie ein Teil davon sind!