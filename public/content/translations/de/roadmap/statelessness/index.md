...
## Reduzierung des Speicherplatzes für Knoten {#reducing-storage-for-nodes}
...
### Historienverfall {#history-expiry}
...
Damit dies geschehen kann, müssen [Verkle-Bäume](/roadmap/verkle-trees/) bereits in Ethereum-Clients implementiert worden sein. Verkle-Bäume sind eine Ersatz-Datenstruktur zur Speicherung von Ethereum-Zustandsdaten, die es ermöglichen, kleine „Zeugen“ fester Größe für die Daten zwischen Peers weiterzugeben und zur Verifizierung von Blöcken zu verwenden, anstatt Blöcke gegen lokale Datenbanken zu verifizieren. Die [Proposer-Builder-Trennung (PBS)](/roadmap/pbs/) ist ebenfalls erforderlich, da dies ermöglicht, dass Block-Builder spezialisierte Knoten mit leistungsfähigerer Hardware sind, und diese sind diejenigen, die Zugriff auf die vollständigen Zustandsdaten benötigen.

<ExpandableCard title="Warum ist es in Ordnung, sich auf weniger Block-Proposer zu verlassen?" eventCategory="/roadmap/statelessness" eventName="clicked why is it OK to rely on fewer block proposers?">
...
Schwache Zustandslosigkeit, Historienverfall und Zustandsablauf befinden sich alle in der Forschungsphase und werden voraussichtlich erst in einigen Jahren veröffentlicht. Es gibt keine Garantie, dass alle diese Vorschläge implementiert werden; wenn beispielsweise der Zustandsablauf zuerst implementiert wird, besteht möglicherweise keine Notwendigkeit, auch den Historienverfall zu implementieren. Es gibt auch andere Roadmap-Punkte, wie [Verkle-Bäume](/roadmap/verkle-trees) und die [Proposer-Builder-Trennung (PBS)](/roadmap/pbs), die zuerst abgeschlossen werden müssen.

## Weiterführende Literatur {#further-reading}

- [Was ist zustandsloses Ethereum?](https://stateless.fyi/)
- [Vitalik AMA zur Zustandslosigkeit](https://www.reddit.com/r/ethereum/comments/o9s15i/impromptu_technical_ama_on_statelessness_and/)
- [Eine Theorie des Managements der Zustandsgröße](https://hackmd.io/@vbuterin/state_size_management)
...
- [Die ursprünglichen Konzeptnotizen für zustandslose Clients](https://ethresear.ch/t/the-stateless-client-concept/172)
