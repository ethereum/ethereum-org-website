...
### Manticore thông qua pip {#manticore-through-pip}
...
## Giới thiệu về thực thi tượng trưng động {#introduction-to-dynamic-symbolic-execution}

### Tóm tắt về Thực thi tượng trưng động {#dynamic-symbolic-execution-in-a-nutshell}
...
Mỗi vị từ đường dẫn là một công thức toán học có thể được đưa cho một cái gọi là [trình giải quyết SMT](https://wikipedia.org/wiki/Satisfiability_modulo_theories), nó sẽ cố gắng giải phương trình. Đối với `Path 1`, trình giải quyết sẽ nói rằng đường dẫn có thể được khám phá với `a = 65`. Đối với `Path 2`, trình giải quyết có thể cung cấp cho `a` bất kỳ giá trị nào khác 65, ví dụ `a = 0`.
...
Chúng ta sẽ xem cách khám phá một hợp đồng thông minh với API Manticore. Mục tiêu là hợp đồng thông minh sau [`example.sol`](https://github.com/crytic/building-secure-contracts/blob/master/program-analysis/manticore/examples/example.sol):
...
### Thao tác với một hợp đồng thông minh thông qua API {#manipulate-a-smart-contract-through-the-api}
...
Nếu dữ liệu là tượng trưng, Manticore sẽ khám phá tất cả các hàm của hợp đồng trong quá trình thực thi giao dịch. Sẽ rất hữu ích khi xem giải thích về Hàm dự phòng trong bài viết [Thực hành với Ethernaut CTF](https://blog.trailofbits.com/2017/11/06/hands-on-the-ethernaut-ctf/) để hiểu cách hoạt động của việc chọn hàm.
...
## Lấy các đường dẫn ném lỗi {#getting-throwing-paths}

Bây giờ chúng ta sẽ tạo các đầu vào cụ thể cho các đường dẫn gây ra ngoại lệ trong `f()`. Mục tiêu vẫn là hợp đồng thông minh sau [`example.sol`](https://github.com/crytic/building-secure-contracts/blob/master/program-analysis/manticore/examples/example.sol):
...
_Lưu ý rằng chúng ta có thể đã tạo một tập lệnh đơn giản hơn nhiều, vì tất cả các trạng thái được trả về bởi terminated_state đều có REVERT hoặc INVALID trong kết quả của chúng: ví dụ này chỉ nhằm mục đích chứng minh cách thao tác với API._

## Thêm các ràng buộc {#adding-constraints}

Chúng ta sẽ xem cách ràng buộc việc khám phá. Chúng ta sẽ giả định rằng tài liệu của `f()` tuyên bố rằng hàm không bao giờ được gọi với `a == 65`, vì vậy bất kỳ lỗi nào với `a == 65` đều không phải là lỗi thực sự. Mục tiêu vẫn là hợp đồng thông minh sau [`example.sol`](https://github.com/crytic/building-secure-contracts/blob/master/program-analysis/manticore/examples/example.sol):
...
Mô-đun [Toán tử](https://github.com/trailofbits/manticore/blob/master/manticore/core/smtlib/operators.py) tạo điều kiện thuận lợi cho việc thao tác với các ràng buộc, trong số những thứ khác, nó cung cấp:
...
### Các ràng buộc {#state-constraint}
...
### Tóm tắt: Thêm các Ràng buộc {#summary-adding-constraints}
