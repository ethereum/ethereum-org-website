...
description: "Trang tóm tắt về việc rút tiền đặt cọc tự động (push withdrawals) là gì, cách thức hoạt động và những gì người đặt cọc cần làm để nhận phần thưởng của họ"
lang: vi
template: staking
image: /images/staking/leslie-withdrawal.png
sidebarDepth: 2
summaryPoints:
  - Người vận hành trình xác thực phải cung cấp một địa chỉ rút tiền để kích hoạt việc rút tiền
  - Các trình xác thực cũ (legacy) có số dư vượt quá 32 ETH sẽ được tự động rút vài ngày một lần
  - Các trình xác thực tích lũy (compounding) kiếm được phần thưởng trên toàn bộ số dư của họ lên đến 2048 ETH
...
> Nếu bạn là thành viên của một [nhóm đặt cọc](/staking/pools/) hoặc nắm giữ các token đặt cọc, bạn nên kiểm tra với nhà cung cấp của mình để biết thêm chi tiết về cách xử lý việc rút tiền đặt cọc, vì mỗi dịch vụ hoạt động khác nhau.
...
- **Trình xác thực tích lũy (Loại 2)**: Phần thưởng được tích lũy vào số dư hiệu dụng của trình xác thực lên đến 2048 ETH, làm tăng trọng số của trình xác thực và kiếm được nhiều phần thưởng hơn. Chỉ số dư vượt quá 2048 ETH mới được tự động quét (swept).

Người dùng cũng có thể **thoát hoàn toàn khỏi việc đặt cọc**, bằng cách gửi một giao dịch để rút tiền, chờ đợi theo thời gian của hàng đợi rút tiền (dựa trên nhu cầu của mạng lưới) và mở khóa toàn bộ số dư trình xác thực của họ.

## Phần thưởng đặt cọc {#staking-rewards}

Cách xử lý phần thưởng phụ thuộc vào loại thông tin xác thực của trình xác thực:

**Các trình xác thực cũ (Loại 1)** có số dư hiệu dụng được giới hạn ở mức 32 ETH. Bất kỳ số dư nào trên 32 ETH nhận được dưới dạng phần thưởng mạng lưới đều không đóng góp vào số dư hiệu dụng hoặc làm tăng trọng số của trình xác thực này trên mạng lưới, và những phần thưởng này được tự động rút về địa chỉ rút tiền chuyên dụng của trình xác thực vài ngày một lần. Ngoài việc cung cấp địa chỉ rút tiền một lần, việc nhận những phần thưởng này không yêu cầu bất kỳ hành động nào từ người vận hành trình xác thực. Tất cả điều này được khởi tạo trên lớp đồng thuận, do đó không yêu cầu Gas (phí giao dịch) ở bất kỳ bước nào.

**Các trình xác thực tích lũy (Loại 2)** có thể có số dư hiệu dụng nằm trong khoảng từ 32 đến 2048 ETH. Phần thưởng mạng lưới mà các trình xác thực này nhận được sẽ tích lũy vào số dư hiệu dụng của chúng, làm tăng trọng số của trình xác thực và tiềm năng nhận được phần thưởng trong tương lai. Việc quét tự động chỉ xảy ra đối với số dư vượt quá 2048 ETH. Để rút phần thưởng dưới ngưỡng 2048 ETH, các trình xác thực tích lũy phải kích hoạt việc rút tiền một phần theo cách thủ công từ lớp thực thi, điều này có yêu cầu Gas.

### Chúng ta đã đạt được điều này như thế nào? {#how-did-we-get-here}

Trong vài năm qua, Ethereum đã trải qua một số bản nâng cấp mạng lưới để chuyển đổi sang một mạng lưới được bảo mật bằng chính ETH, thay vì khai thác tiêu tốn nhiều năng lượng như trước đây. Việc tham gia vào đồng thuận trên Ethereum hiện được gọi là "đặt cọc", vì những người tham gia đã tự nguyện khóa ETH, đặt nó "vào rủi ro" (at stake) để có khả năng tham gia vào mạng lưới. Những người dùng tuân thủ các quy tắc sẽ được nhận phần thưởng, trong khi những nỗ lực gian lận có thể bị phạt.
...
Trước bản nâng cấp Thượng Hải/Capella, bạn không thể sử dụng hoặc truy cập vào ETH đã đặt cọc của mình. Nhưng giờ đây, bạn có thể chọn tự động nhận phần thưởng của mình vào một tài khoản đã chọn và bạn cũng có thể rút ETH đã đặt cọc của mình bất cứ khi nào bạn muốn.
...
Các tài khoản trình xác thực được yêu cầu cung cấp một địa chỉ rút tiền trước khi chúng có thể truy cập và rút các phần thưởng mạng lưới đã tích lũy, hoặc xử lý việc rút tiền toàn bộ khi thoát khỏi việc đặt cọc.
...
Nếu bạn chưa cung cấp địa chỉ rút tiền cho tài khoản trình xác thực của mình, thì **không có mối đe dọa nào đối với tiền của bạn trong thời gian chờ đợi**, giả sử cụm từ gợi nhớ/cụm từ hạt giống của bạn vẫn an toàn ngoại tuyến và không bị xâm phạm theo bất kỳ cách nào. Việc không thêm thông tin xác thực rút tiền sẽ chỉ khiến ETH bị khóa trong tài khoản trình xác thực cho đến khi một địa chỉ rút tiền được cung cấp.

## Trình xác thực tích lũy {#compounding-validators}

Các trình xác thực có thể chọn tham gia **tích lũy** bằng cách chuyển đổi thông tin xác thực rút tiền của họ từ Loại 1 sang Loại 2. Điều này nâng mức số dư hiệu dụng tối đa từ 32 ETH lên **2048 ETH**, cho phép phần thưởng được tích lũy vào số dư hiệu dụng của trình xác thực thay vì được tự động quét.

Khi tính năng tích lũy được bật:
...
- Nhiều trình xác thực có thể được **hợp nhất** thành một trình xác thực tích lũy duy nhất, giúp giảm chi phí vận hành
...
Việc cung cấp một địa chỉ rút tiền là bắt buộc trước khi _bất kỳ_ khoản tiền nào có thể được chuyển ra khỏi số dư tài khoản trình xác thực.

Người dùng muốn thoát hoàn toàn khỏi việc đặt cọc và rút lại toàn bộ số dư của họ phải khởi tạo một "lệnh thoát tự nguyện". Điều này có thể được thực hiện theo hai cách:
...
Khi một trình xác thực được lên lịch để đề xuất khối tiếp theo, nó được yêu cầu xây dựng một hàng đợi rút tiền, với tối đa 16 khoản rút tiền đủ điều kiện. Điều này được thực hiện bằng cách bắt đầu từ chỉ số trình xác thực 0, xác định xem có khoản rút tiền đủ điều kiện nào cho tài khoản này theo các quy tắc của giao thức hay không và thêm nó vào hàng đợi nếu có. Trình xác thực được thiết lập để đề xuất khối tiếp theo sẽ tiếp tục từ nơi trình xác thực trước đó dừng lại, tiến triển theo thứ tự vô thời hạn.

<Alert variant="update">
<AlertIcon size="xl"><Clock /></AlertIcon>
<AlertContent>
<AlertDescription>
Hãy nghĩ về một chiếc đồng hồ kim. Kim trên đồng hồ chỉ vào giờ, di chuyển theo một hướng, không bỏ qua bất kỳ giờ nào và cuối cùng quay trở lại điểm bắt đầu sau khi đạt đến số cuối cùng.
...
Kim trên đồng hồ chỉ vào trình xác thực tiếp theo cần được kiểm tra các khoản rút tiền đủ điều kiện. Nó bắt đầu ở số 0 và di chuyển một vòng mà không bỏ qua bất kỳ tài khoản nào. Khi đạt đến trình xác thực cuối cùng, chu kỳ sẽ tiếp tục quay lại từ đầu.
</AlertDescription>
</AlertContent>
</Alert>
...
Trong khi người đề xuất đang quét qua các trình xác thực để tìm các khoản rút tiền có thể xảy ra, mỗi trình xác thực đang được kiểm tra sẽ được đánh giá dựa trên một loạt câu hỏi ngắn để xác định xem có nên kích hoạt việc rút tiền hay không và nếu có, thì nên rút bao nhiêu ETH.
...
Các đợt quét rút tiền tự động không yêu cầu người đặt cọc phải gửi giao dịch theo cách thủ công. Điều này có nghĩa là **không yêu cầu Gas (phí giao dịch)** cho các đợt quét tự động và chúng không cạnh tranh không gian khối của lớp thực thi hiện có.

Lưu ý rằng [các trình xác thực tích lũy](#compounding-validators) muốn kích hoạt việc rút tiền một phần dưới ngưỡng 2048 ETH phải thực hiện thủ công từ lớp thực thi, điều này có yêu cầu Gas.
...
Mở rộng tính toán này, chúng ta có thể ước tính thời gian cần thiết để xử lý một số lượng khoản rút tiền nhất định:

<TableContainer>

| Số lượng khoản rút tiền | Thời gian hoàn thành |
| :-------------------: | :--------------: |
|        400.000        |     3,5 ngày     |
|        500.000        |     4,3 ngày     |
|        600.000        |     5,2 ngày     |
...
Như bạn thấy, quá trình này sẽ chậm lại khi có nhiều trình xác thực hơn trên mạng lưới. Sự gia tăng các slot bị bỏ lỡ có thể làm chậm quá trình này theo tỷ lệ tương ứng, nhưng điều này thường sẽ đại diện cho khía cạnh chậm hơn của các kết quả có thể xảy ra.

## Các câu hỏi thường gặp {#faq}

<ExpandableCard
title="Sau khi đã cung cấp địa chỉ rút tiền, tôi có thể đổi sang một địa chỉ rút tiền khác không?"
eventCategory="FAQ"
...
<ExpandableCard
title="Tại sao địa chỉ rút tiền của trình xác thực chỉ có thể được thiết lập một lần?"
eventCategory="FAQ"
...
<ExpandableCard
title="Làm cách nào để rút tiền từ việc đặt cọc nếu tôi đặt cọc thông qua một nhà cung cấp, nhóm đặt cọc, hoặc tham gia bằng token staking thanh khoản (LST)?"
eventCategory="FAQ"
...
eventName="read more">
Nếu bạn sử dụng một [nhóm đặt cọc](/staking/pools/) hoặc nắm giữ các token staking thanh khoản (LST), bạn không tương tác trực tiếp với cơ chế rút tiền của giao thức; các hợp đồng thông minh của nhóm và những người vận hành nút kiểm soát các trình xác thực, và thông tin xác thực rút tiền thường trỏ đến các hợp đồng của nhóm, chứ không phải bạn. Thay vào đó, bạn thường đổi token của mình thông qua nhà cung cấp (tùy thuộc vào hàng đợi quy đổi và thanh khoản có sẵn của họ) hoặc bán chúng trên thị trường mở. Hãy liên hệ với nhà cung cấp của bạn để tìm hiểu cách họ xử lý việc rút tiền, vì các quy trình khác nhau tùy theo dịch vụ.

Nhìn chung, khi đặt cọc thông qua một nhà cung cấp hoặc nhóm, bạn sẽ được tự do lấy lại ETH đã đặt cọc cơ sở của mình, hoặc rút tiền và thay đổi nhà cung cấp đặt cọc mà bạn sử dụng. Nếu một nhóm cụ thể đang trở nên quá lớn, ETH đã đặt cọc có thể được thoát, quy đổi và đặt cọc lại với một [nhà cung cấp nhỏ hơn](https://rated.network/). Hoặc, nếu bạn đã tích lũy đủ ETH, bạn có thể [đặt cọc tại nhà](/staking/solo/).

</ExpandableCard>

<ExpandableCard
title="Việc nhận phần thưởng mạng lưới (rút tiền một phần) có diễn ra tự động không?"
eventCategory="FAQ"
eventAction="Does claiming network rewards (partial withdrawals) happen automatically?"
eventName="read more">
Đối với **các trình xác thực cũ (Loại 1)**, có—miễn là trình xác thực của bạn đã cung cấp một địa chỉ rút tiền. Điều này phải được cung cấp một lần để kích hoạt bất kỳ khoản rút tiền nào, sau đó việc phân phối phần thưởng mạng lưới đến địa chỉ rút tiền sẽ được tự động kích hoạt vài ngày một lần với mỗi đợt quét trình xác thực.

Đối với **các trình xác thực tích lũy (Loại 2)**, phần thưởng được tích lũy vào số dư hiệu dụng của trình xác thực (lên đến 2048 ETH) thay vì được quét đến địa chỉ rút tiền. Việc quét tự động chỉ xảy ra đối với số dư vượt quá 2048 ETH. Để rút phần thưởng dưới ngưỡng này, bạn phải kích hoạt thủ công việc rút tiền một phần từ lớp thực thi.
</ExpandableCard>
...
eventName="read more">
Đối với **các trình xác thực cũ (Loại 1)**, bất kỳ phần thưởng mạng lưới ETH nào đã tích lũy vượt quá số dư hiệu dụng 32 ETH của trình xác thực đều được tự động đẩy đến địa chỉ rút tiền. Các trình xác thực Loại 1 đã gửi giao dịch rút tiền toàn bộ và hoàn tất quá trình thoát khỏi việc đặt cọc sẽ được rút toàn bộ số dư ETH của họ về địa chỉ rút tiền. Trình xác thực Loại 1 không thể yêu cầu rút một số lượng ETH cụ thể theo cách thủ công.

**Các trình xác thực tích lũy (Loại 2)** có thể kích hoạt việc rút tiền một phần với một số lượng cụ thể từ lớp thực thi, miễn là số dư còn lại của trình xác thực duy trì ở mức bằng hoặc trên 32 ETH. Điều này yêu cầu gửi một giao dịch rút tiền một phần và tốn Gas.
</ExpandableCard>

<ExpandableCard
title="Tôi vận hành một trình xác thực. Tôi có thể tìm thêm thông tin về việc quản lý quá trình rút tiền ở đâu?"
eventCategory="FAQ"
...
eventName="read more">
Không. Khi một trình xác thực đã thoát và toàn bộ số dư của nó đã được rút, bất kỳ ETH bổ sung nào được gửi vào trình xác thực đó sẽ tự động được chuyển đến địa chỉ rút tiền trong đợt quét trình xác thực tiếp theo. Để bắt đầu đặt cọc lại bằng cách sử dụng ETH đó, bạn phải kích hoạt một trình xác thực mới.
</ExpandableCard>

<ExpandableCard
title="Sự khác biệt giữa trình xác thực truyền thống và trình xác thực tích lũy là gì?"
eventCategory="FAQ"
...
<ExpandableCard
title="Tính năng rút tiền đặt cọc được kích hoạt khi nào?"
eventCategory="FAQ"
eventAction="When were staking withdrawals enabled?"
eventName="read more">
Chức năng rút tiền ban đầu được kích hoạt như một phần của bản nâng cấp Thượng Hải/Capella vào **ngày 12 tháng 4 năm 2023**. [Bản nâng cấp Pectra](/roadmap/pectra/) (tháng 5 năm 2025) sau đó đã giới thiệu các trình xác thực tích lũy với số dư hiệu dụng tối đa cao hơn là 2048 ETH, cũng như các lệnh thoát và rút tiền một phần được kích hoạt bởi lớp thực thi.

Bản nâng cấp Thượng Hải/Capella cho phép ETH đã đặt cọc trước đó được lấy lại vào các tài khoản Ethereum thông thường. Điều này đã khép kín vòng lặp về thanh khoản đặt cọc và đưa Ethereum tiến thêm một bước trên hành trình xây dựng một hệ sinh thái phi tập trung bền vững, có thể mở rộng và an toàn.
