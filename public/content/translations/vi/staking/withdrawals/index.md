...
**Việc rút tiền đặt cọc** là việc chuyển ETH từ tài khoản của nút xác thực trên lớp đồng thuận của Ethereum (Chuỗi Beacon) sang lớp thực thi, nơi ETH có thể được giao dịch.

\*\*Các khoản thanh toán phần thưởng của số dư vượt mức trên 32 ETH sẽ được tự động và thường xuyên gửi đến một địa chỉ rút tiền được liên kết với mỗi nút xác thực, sau khi người dùng cung cấp địa chỉ này. Người dùng cũng có thể **thoát đặt cọc hoàn toàn**, qua đó mở khóa toàn bộ số dư của nút xác thực.
...
### Các thông báo quan trọng {#important-notices}
...
## Thoát hoàn toàn việc đặt cọc {#exiting-staking-entirely}

Việc cung cấp địa chỉ rút tiền là bắt buộc trước khi _bất kỳ_ khoản tiền nào có thể được chuyển ra khỏi số dư tài khoản của nút xác thực.
...
Sau khi một tài khoản được đánh dấu là "có thể rút hoàn toàn" và đã cung cấp thông tin xác thực rút tiền, người dùng không cần thực hiện thêm bất kỳ thao tác nào khác ngoài việc chờ đợi. Các tài khoản sẽ được người đề xuất khối quét tự động và liên tục để tìm các khoản tiền đã thoát đủ điều kiện và số dư tài khoản của bạn sẽ được chuyển toàn bộ (còn được gọi là "rút toàn bộ") trong lần <a href="#validator-sweeping" customEventOptions={{ eventCategory: "Anchor link", eventAction: "Exiting staking entirely (sweep)", eventName: "click" }}>quét</a> tiếp theo.

## Tính năng rút tiền đặt cọc đã được kích hoạt khi nào? {#when}

Chức năng rút tiền đã được kích hoạt như một phần của bản nâng cấp Shanghai/Capella diễn ra vào **ngày 12 tháng 4 năm 2023**.
...
- [Tìm hiểu thêm về lịch sử của Ethereum](/ethereum-forks/)
- [Tìm hiểu thêm về lộ trình của Ethereum](/roadmap/)
...
<AlertContent>
<AlertDescription>
Hãy tưởng tượng một chiếc đồng hồ kim. Kim đồng hồ chỉ giờ, di chuyển theo một hướng, không bỏ qua giờ nào và cuối cùng quay lại từ đầu sau khi đến số cuối cùng.<br/><br/>
Bây giờ thay vì từ 1 đến 12, hãy tưởng tượng đồng hồ có từ 0 đến N <em>(tổng số tài khoản nút xác thực đã từng được đăng ký trên lớp đồng thuận, hơn 500.000 tính đến tháng 1 năm 2023).</em><br/><br/>
Kim đồng hồ chỉ vào nút xác thực tiếp theo cần được kiểm tra để rút tiền đủ điều kiện. Kim bắt đầu từ 0 và di chuyển theo vòng tròn mà không bỏ qua bất kỳ tài khoản nào. Khi đến nút xác thực cuối cùng, chu kỳ sẽ tiếp tục trở lại từ đầu.
</AlertDescription>
</AlertContent>
</Alert>

#### Kiểm tra một tài khoản để rút tiền {#checking-an-account-for-withdrawals}
...
1. **Địa chỉ rút tiền đã được cung cấp chưa?** Nếu chưa cung cấp địa chỉ rút tiền, tài khoản sẽ bị bỏ qua và không có lệnh rút tiền nào được khởi tạo.
2. **Nút xác thực đã thoát và có thể rút tiền chưa?** Nếu nút xác thực đã thoát hoàn toàn và đã đến epoch mà tài khoản của họ được coi là "có thể rút tiền", thì một lệnh rút toàn bộ sẽ được xử lý. Bước này sẽ chuyển toàn bộ số dư còn lại đến địa chỉ rút.
3. **Số dư hiệu dụng có đạt tối đa 32 không?** Nếu tài khoản có thông tin xác thực rút tiền, chưa thoát hoàn toàn và có phần thưởng vượt quá 32 đang chờ, một lệnh rút tiền một phần sẽ được xử lý, chỉ chuyển các phần thưởng trên 32 đến địa chỉ rút tiền của người dùng.
...
Phương pháp rút đặt cọc này tránh yêu cầu người đặt cọc gửi thủ công giao dịch yêu cầu rút một lượng ETH cụ thể. Điều này có nghĩa là **không cần gas (phí giao dịch)** và các lệnh rút tiền cũng không cạnh tranh cho không gian khối của lớp thực thi hiện có.
...
<ExpandableCard
title="Sau khi tôi đã cung cấp địa chỉ rút tiền, tôi có thể đổi sang một địa chỉ rút tiền thay thế không?"
eventCategory="FAQ"
...
eventName="read more">
Bằng việc đặt địa chỉ rút tiền trên lớp thực thi, thông tin xác thực rút tiền cho nút xác thực đó đã được thay đổi vĩnh viễn. Điều này có nghĩa là thông tin xác thực cũ sẽ không còn có hiệu lực và thông tin xác thực mới sẽ chuyển hướng trực tiếp đến tài khoản lớp thực thi.
...
<ExpandableCard
title="Các khoản thanh toán phần thưởng (rút một phần) có diễn ra tự động không?"
eventCategory="FAQ"
...
eventCategory="FAQ"
eventAction="I operate a validator. Tôi có thể tìm thêm thông tin về việc bật tính năng rút tiền ở đâu?"
eventName="read more">
...
<ExpandableCard
title="Tôi có thể kích hoạt lại nút xác thực của mình sau khi thoát bằng cách nạp thêm ETH không?"
eventCategory="FAQ"
...
- [EIP-4895: Các lệnh rút tiền đẩy của Chuỗi Beacon dưới dạng thao tác](https://eips.ethereum.org/EIPS/eip-4895)
...
- [Tìm hiểu về Số dư Hiệu dụng của Nút xác thực](https://www.attestant.io/posts/understanding-validator-effective-balance/)
