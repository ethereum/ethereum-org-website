...
### 2\. Czym jest „signer” (podpisujący)? {#what-is-a-signer}

- Signery podpisują transakcje za Ciebie przy użyciu Twojego klucza prywatnego. W tym samouczku użyjemy [Alchemy Web3](https://docs.alchemyapi.io/alchemy/documentation/alchemy-web3) do podpisania naszej transakcji, ale możesz również użyć dowolnej innej biblioteki Web3.
- Na frontendzie dobrym przykładem signera jest [MetaMask](https://metamask.io/), który podpisze i wyśle transakcje w Twoim imieniu.
...
### 7\. Jak wysyłać bezpieczne, zoptymalizowane pod kątem gazu i prywatne transakcje? {#how-to-send-secure-gas-optimized-and-private-transactions}

- [Alchemy posiada pakiet Transact API](https://docs.alchemy.com/reference/transact-api-quickstart). Możesz ich użyć do wysyłania wzmocnionych transakcji, symulowania transakcji przed ich wykonaniem, wysyłania prywatnych transakcji oraz wysyłania transakcji zoptymalizowanych pod kątem gazu.
- Możesz również użyć [Notify API](https://docs.alchemy.com/docs/alchemy-notify), aby otrzymywać powiadomienia, gdy Twoja transakcja zostanie pobrana z mempoola i dodana do łańcucha.
...
3.  [Postępuj zgodnie z tymi krokami, aby zainstalować Node.js i npm](https://docs.alchemy.com/alchemy/guides/alchemy-for-macs)
...
Przejdź do swojego [pulpitu nawigacyjnego Alchemy](https://dashboard.alchemyapi.io/) i utwórz nową aplikację, wybierając Sepolia (lub dowolną inną sieć testową) jako swoją sieć.
...
Uruchom następujące polecenie w katalogu swojego projektu, aby zainstalować [Alchemy Web3](https://docs.alchemy.com/reference/api-overview):

Uwaga, jeśli wolisz użyć biblioteki Ethers.js, [postępuj zgodnie z instrukcjami tutaj](https://docs.alchemy.com/docs/how-to-send-transactions-on-ethereum).
...
Utwórz plik `sendTx.js`, w którym skonfigurujemy i wyślemy naszą przykładową transakcję, i dodaj do niego następujące wiersze kodu:
...
async function main() {
    ...
    if (!error) {
      console.log("🎉 Hash Twojej transakcji to: ", hash, "\n Sprawdź Mempool Alchemy, aby zobaczyć status swojej transakcji!");
    } else {
      console.log("❗Coś poszło nie tak podczas przesyłania Twojej transakcji:", error)
    }
   });
}
...
- `transaction`: Obiekt transakcji ma kilka aspektów, które musimy określić:
  ...
  - `gas`: Istnieje wiele sposobów na określenie odpowiedniej ilości gazu do dołączenia do transakcji. Alchemy ma nawet [webhook ceny gazu](https://docs.alchemyapi.io/guides/alchemy-notify#address-activity-1), który powiadamia Cię, gdy cena gazu spadnie poniżej określonego progu. W przypadku transakcji w Sieci głównej dobrą praktyką jest sprawdzenie estymatora gazu, takiego jak [ETH Gas Station](https://ethgasstation.info/), aby określić odpowiednią ilość gazu do dołączenia. 21000 to minimalna ilość gazu, jaką zużyje operacja w Ethereum, więc aby upewnić się, że nasza transakcja zostanie wykonana, wpisujemy tutaj 30000.
  - `nonce`: zobacz definicję nonce powyżej. Nonce zaczyna liczyć od zera.
  - [OPCJONALNIE] data: Służy do wysyłania dodatkowych informacji wraz z transferem lub wywoływania inteligentnego kontraktu, nie jest wymagane w przypadku transferów salda, sprawdź poniższą uwagę.
- `signedTx`: Aby podpisać nasz obiekt transakcji, użyjemy metody `signTransaction` z naszym `PRIVATE_KEY`.
- `sendSignedTransaction`: Gdy mamy już podpisaną transakcję, możemy ją wysłać, aby została dołączona do kolejnego bloku, używając `sendSignedTransaction`.
...
- Transakcja inteligentnego kontraktu: Wykonanie kodu inteligentnego kontraktu w łańcuchu. W tym przypadku pole danych powinno zawierać funkcję, którą chcesz wykonać, wraz z wszelkimi parametrami.
  - Praktyczny przykład znajdziesz w Kroku 8 w tym [samouczku Hello World](https://docs.alchemyapi.io/alchemy/tutorials/hello-world-smart-contract#step-8-create-the-transaction).
...
_Pierwotnie opublikowano na [https://docs.alchemyapi.io/tutorials/sending-transactions-using-web3-and-alchemy](https://docs.alchemyapi.io/tutorials/sending-transactions-using-web3-and-alchemy)_