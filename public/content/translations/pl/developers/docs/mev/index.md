...
Builder API jest zmodyfikowaną wersją [Engine API](https://github.com/ethereum/execution-apis/blob/main/src/engine/common.md) używaną przez klientów warstwy konsensusu do żądania ładunków wykonawczych od klientów warstwy wykonawczej. Jak określono w [specyfikacji uczciwego walidatora](https://github.com/ethereum/consensus-specs/blob/master/specs/bellatrix/validator.md), walidatorzy wybrani do obowiązków proponowania bloków żądają pakietu transakcji od podłączonego klienta wykonawczego, który włączają do proponowanego bloku Beacon Chain.
