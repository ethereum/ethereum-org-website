...
To deploy your smart contract to the test network, you'll need some fake ETH. To get ETH on the Sepolia network, go to a Sepolia faucet and enter your Sepolia account address. Note that testnet faucets can be a bit unreliable at times - see the [test networks page](/developers/docs/networks/#sepolia) for a list of options to try:
...
If you skipped Part 3 of this tutorial, use the HelloWorld contract you deployed in [Step 16](#step-16-deploy-our-contract): open its address on [Sepolia Etherscan](https://sepolia.etherscan.io) and, once the source is verified, copy the ABI from the **Contract > Code** tab.
...
To sign a transaction on the Ethereum blockchain, we’ll need some fake Eth. To get Eth, pick a faucet from the [test networks page](/developers/docs/networks/#sepolia) and enter your Sepolia account address to request funds. You should see Eth in your MetaMask account soon after!
