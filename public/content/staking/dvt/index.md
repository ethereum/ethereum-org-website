...
- [Staking services](/staking/saas/) and [staking pools](/staking/pools/) can use DVT to add resilience and harden their staking infrastructure, or to distribute validator operations across many independent operators.
...
<ExpandableCard title="Does a cluster have to be 3 of 4?" eventCategory="DVT" eventName="clicked does a cluster have to be 3 of 4">
No. "3 of 4" is just the smallest common configuration, and it is used as an example throughout this page. Cluster size and the signing threshold are chosen when the cluster is created.

Clusters are usually sized so the threshold is a two-thirds supermajority of the nodes, which is what lets the cluster keep signing while tolerating faulty or offline members. A 4-node cluster signs with 3 and tolerates 1 failure; 7 nodes sign with 5 and tolerate 2; 10 nodes sign with 7 and tolerate 3. Larger clusters buy more fault tolerance at the cost of more machines to run and more coordination between them.

[More on cluster size and resilience](https://docs.obol.org/next/learn/charon/cluster-configuration#cluster-size-and-resilience)
</ExpandableCard>
