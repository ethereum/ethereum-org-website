...
description: Learn about staking pools, liquid staking tokens, and how to tell transparent pools from opaque products
lang: en
template: staking
...
Some pools operate using smart contracts, where funds can be deposited to a contract, which manages and tracks your stake, and issues you a token that represents this value. Other pools may not involve smart contracts and are instead mediated offchain.
...
Every pooled option solves a real access problem: staking with less than 32 ETH, or without running hardware. But each also puts an intermediary between you and the protocol. Only [solo staking](/staking/solo/) gives you a direct, unmediated relationship with Ethereum.
...
<Grid>
  ...
  <Card title="Liquid staking tokens" emoji=":droplet:" description="Many staking pools provide a token that represents a claim on your staked ETH and the rewards it generates. This allows you to make use of your staked ETH, e.g., as collateral in DeFi applications." />
</Grid>
...
Most transparent staking pools issue a **[liquid staking token (LST)](/glossary/#liquid-staking-tokens)**: an ERC-20 token that represents a claim on staked ETH and the rewards it earns. When you deposit ETH, the protocol stakes it with its node operators and mints tokens to your wallet. You hold the token yourself, and can transfer or sell it at any time; the underlying ETH stays staked on the consensus layer. Liquid staking protocols account for around a third of all staked ETH, making LSTs the most common way to stake today.
...
Both designs deliver rewards net of the pool's fee. Neither is inherently better, but they behave differently in wallets and DeFi applications, and are treated differently for tax purposes in some jurisdictions; rebasing tokens often have "wrapped" non-rebasing versions for compatibility.
...
- **Sell on secondary markets** at any time. Because the token trades freely, its market price can deviate from the value of the ETH backing it, particularly during periods of market stress. Selling below the redemption value means realizing that discount.

Since the Pectra upgrade, [EIP-7002](https://eips.ethereum.org/EIPS/eip-7002) allows validator exits to be triggered directly from the execution layer by the withdrawal address holder. Staking pools can use this to ensure their validators can be exited without relying on node operators to cooperate, so redemptions rely less on trusting node operators than they used to.

### Holding an LST is not the same as staking {#holding-an-lst-is-not-the-same-as-staking}

The Ethereum protocol pays rewards to validators; it doesn't know your token exists. When you hold an LST, you are not a staker from the protocol's point of view: you hold a claim on a protocol that stakes on your behalf. This works well in normal conditions, but your position depends on the pool's contracts, governance, and operators working correctly, not just on Ethereum itself.
...
- **Operator-set centralization** - pools concentrate stake with their chosen node operators. Large amounts of staked ETH under the control of a few organizations create conditions for censorship, value extraction, and cartel-like behavior. Prefer pools with permissionless, distributed operator sets. [More on the risks of staked-ETH tokens](https://notes.ethereum.org/@djrtwo/risks-of-lsd).
...
- **Custodial** - the provider holds the keys and the ETH. What you hold is an account balance, an IOU from a company.
...
2. Is the node operator set published, and can anyone join it?
...
<AlertContent>
<AlertDescription>
Some products advertise "enhanced" or "boosted" yield by combining staking with <strong>restaking</strong>, which commits staked ETH to secure additional protocols under additional slashing conditions. Restaking is a separate risk category, not a form of staking. If a yield figure is meaningfully higher than the network staking rate, find out exactly where the extra yield comes from. <a href="/restaking/">What is restaking?</a>
</AlertDescription>
</AlertContent>
</Alert>
...
There is a middle path between holding a token and solo staking: becoming a bonded node operator for a pool. Some staking protocols let individuals run validators using pooled ETH from other users. You post a bond of your own ETH as collateral, run the hardware and keys, and earn a commission on the stake matched to you. For example, Rocket Pool megapool validators require a 4 ETH bond per validator, and Lido's Community Staking Module requires around 2.4 ETH for a first validator key (1.5 ETH for Identified Community Stakers). This offers people with less than 32 ETH a way to run their own hardware and strengthen the network's operator set, while accepting the pool's rules, performance requirements, and penalty conditions. If running a node appeals to you, start with the hardware and node-management skills covered in [solo staking](/staking/solo/).
...
Since the Pectra upgrade, pools can also trigger validator exits directly from the execution layer using the withdrawal address (EIP-7002), without relying on node operators' signing keys, reducing the trust required for redemptions to be honored.
...
- [Staking with Rocket Pool - Liquid Staking Overview](https://docs.rocketpool.net/en/liquid-staking/overview) - _RocketPool docs_
