...
> To deploy the registry, 0.08 ether MUST be sent to this account _first_.
...
_NOTE_: If the result is not cached or the cached value is out-of-date, the cache MUST be updated manually by calling `updateERC165Cache` with the contract address.
...
_NOTE_: The `_addr` MAY be `0`, then `msg.sender` is assumed.
...
_NOTE_: If the last 28 bytes of the `_interfaceHash` are zeroes (`0`), then the first 4 bytes are considered an [ERC165] interface and the registry SHALL forward the call to the contract at `_addr` to see if it implements the [ERC165] interface (the first 4 bytes of `_interfaceHash`).
