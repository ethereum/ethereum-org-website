...
in the state, effectively, only net change in the number of slots is tracked. In the subsequent change, called _Gross contract size accounting_, the total
...
2. Snapshot sync protocols, like _fast sync_, _warp sync_, _firehose_, _red queen_, and perhaps others, will benefit from having the correct size of the
...
- If previous value of the [`location`] is 0, and value is not 0, _increment_ `storagesize` (semantics of _increment_ described below)
- If previous value of the [`location`] is not 0, and value is 0, _decrement_ `storagesize` (semantics of _decrement_ described below)
