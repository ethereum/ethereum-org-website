...
/**
 * Compact, row-based wallet detail shown inside the intercepting modal. Distinct
 * from the standalone `[wallet]` page (`WalletDetail`), which keeps its wider,
 * grouped-checklist layout. Every info dimension is a labelled row: networks as
 * chain glyphs, devices/languages as text, and each feature as a yes/no value.
 */
const WalletModalDetail = async ({
  locale,
  wallet,
}: {
  locale: string
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-row items-center gap-4">
        ...
        />
        <h2 className="mt-0 text-h3">{wallet.name}</h2>
      </div>
...
export default WalletModalDetail
