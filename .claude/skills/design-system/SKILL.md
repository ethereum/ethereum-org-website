...
5. **All text is translatable.** `getTranslations` from `next-intl/server` (server) or the project hook `useTranslation` from `@/hooks/useTranslation` (client, `const { t } = useTranslation("namespace")`). Don't import `useTranslations` from `next-intl` directly in client components. Never hard-code user-facing English.
