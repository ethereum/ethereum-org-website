...
4. Check the anti-patterns table in `SKILL.md` — it's the cross-cutting list that catches what the table misses.
...
| `unstable_cache(` on an MDX-rendered route                       | `edge-caching.md` — triggers ISR + silent 404s on dynamic segments                                                                                    |
...
| New file `app/**/page.tsx`                                       | `edge-caching.md`, `bundle.md` — verify `revalidate` strategy + RSC payload                                                                           |
| New file under `app/api/`                                        | `edge-caching.md` — TTL strategy, internal-only fetch boundary                                                                                        |
| New keys in `src/intl/en/common.json`                            | `bundle.md` — `common` is `pick`ed into the client provider on every route (`app/[locale]/layout.tsx`); page-specific strings belong in a page namespace |
...
If the diff is small or none of the rows apply cleanly, scan against the anti-patterns table in `SKILL.md` alone.
