...
| New keys in `src/intl/en/common.json`                            | `rsc.md` — `common` is `pick`ed into the client provider on every route (`app/[locale]/layout.tsx`); page-specific strings belong in a page namespace |
