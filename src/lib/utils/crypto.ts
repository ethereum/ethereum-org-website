 ...
 */
export const getHashFromBuffer = async (buffer: BufferSource, options?: HashOptions): Promise<string> => {
  const hashBuffer = await crypto.subtle.digest(options?.algorithm || "SHA-256", buffer)
