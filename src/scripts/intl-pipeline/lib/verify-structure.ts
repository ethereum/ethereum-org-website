/**
 * Structural fidelity checks for translated content -- the library half of
 * `verify-structure.ts`. Every check compares a translated file against its
 * English source and asserts an invariant that must hold regardless of target
 * language. Meaning is not checked, only structure.
 *
 * Kept free of `import.meta` and process-level code so the pipeline's
 * pre-commit gates (`lib/gates.ts`) and the unit tests can import it under a
 * CJS build; the CLI wrapper stays in `verify-structure.ts`.
 */
