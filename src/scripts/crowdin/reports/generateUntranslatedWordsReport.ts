...
/**
 * Generates a report of words still untranslated for each content bucket in all languages.
 * Report in CSV format with columns: Language, Bucket Name, Words needing review, Untranslated word count, Translation progress (%)
 * To run:
 ...
 * - Can be run with `yarn crowdin-untranslated`
 * - Results are saved to src/data/crowdin/bucketsUntranslatedWordsReport.csv
...
async function main() {
  ...
  for (const dir of directories) {
    ...
    i18n.forEach(({ crowdinCode }) => {
      ...
      const untranslated = words.total - words.translated
  ...
  csvArray.unshift(
    "Language,Bucket Name,Words needing review,Untranslated word count,Translation progress (%)"
  )
  ...
  const csvPath = path.resolve(
    process.cwd(),
    "src/data/crowdin/bucketsUntranslatedWordsReport.csv"
  )
