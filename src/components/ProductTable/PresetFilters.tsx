      ...
      {presets.map((preset, idx) => {
        return (
          <div key={idx} className="grid-rows-1">
            <div
              className={`flex h-full flex-col items-start border-2 p-2 ${activePresets.includes(idx) ? "border-primary" : "border-transparent"} duration-50 group cursor-pointer rounded bg-background-highlight transition-all hover:border-primary-hover`}
              onClick={() => {
            ...
            >
              <div className="flex items-center gap-2 px-1.5">
                ...
                <label
                  htmlFor={`radio-${idx}`}
                  className={`custom-radio h-5 w-5 ${activePresets.includes(idx) ? "bg-primary" : ""} rounded-full border-4 border-background-highlight outline outline-1 outline-primary`}
                />
