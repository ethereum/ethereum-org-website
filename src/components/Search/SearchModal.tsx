 ...
 * renders. Adding or removing one there is the only edit needed; nothing here enumerates
 * them. Built once -- the data is static.
 */
const EXPLORER_NETWORKS: ExplorerNetwork[] = [
  ethereumNetworkData,
  ...layer2Data,
].map((network) => ({
  name: network.name,
  icon: network.logo.src,
  addressFormat: network.addressFormat,
  explorer: network.searchExplorer,
}))
 ...
 * featured networks. None of that belongs in a bundle a reader pays for before they have
 * opened search.
 */
const SearchModal = ({ onClose, className }: SearchModalProps) => {
  ...
  const transformSearchClient = useCallback(
    (client: MinimalSearchClient): MinimalSearchClient => ({
      ...
      search: async (requests) => {
        ...
        const parsed = parseExplorerQuery(query, EXPLORER_NETWORKS)
