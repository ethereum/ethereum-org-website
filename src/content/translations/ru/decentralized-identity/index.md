...
Инфраструктура открытых ключей (PKI) – это средство защиты информации, которое генерирует <GlossaryTooltip termKey="public-key">открытый ключ</GlossaryTooltip> и <GlossaryTooltip termKey="private-key"> закрытый ключ</GlossaryTooltip> для объекта. Криптография с открытым ключом используется в сетях блокчейн для аутентификации пользователей и подтверждения права собственности на цифровые активы.
