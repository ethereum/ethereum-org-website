...
_Если никто не владеет Ethereum, как принимаются решения о прошлых и будущих изменениях в Ethereum? Под управлением Ethereum понимается процесс, который позволяет принимать такие решения._
...
Никто не владеет протоколом Ethereum и не контролирует его, Однако необходимо принимать решения , чтобы наилучшим образом обеспечить функционирование сети и ее процветание. Такое отсутствие собственности делает традиционное для организаций управление невозможным.

## Управление Ethereum {#ethereum-governance}

Управление Ethereum — это процесс внесения изменений в протокол. Важно отметить, что этот процесс не имеет отношения к тому, как люди и приложения используют протокол: в Ethereum нет разрешений. Любой человек из любой точки мира может принимать участвие в работе сети. Нет установленных правил о том, кто может создавать приложения или отправлять транзакции. Однако есть процесс внесения изменений в основной протокол, на котором эти приложения функционируют. Так как для всех важна стабильность Ethereum, порог действий для основных изменений, включая социальные и технические процессы, очень высок. Это необходимо, чтобы обеспечить безопасность и поддержку сообществом всех изменений в работе Ethereum.
...
Когда Beacon Chain объединится с исполнительным уровнем Ethereum, процесс управления предложениями изменений будет гармонизирован. Этот процесс по реализации слияния [уже идет](https://github.com/ethereum/EIPs/pull/3675).
