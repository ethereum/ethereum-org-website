...
最后，我们可以更新 Player 实体的所有数据。 数组不能直接推送，需要按如下所示进行更新。 我们使用 id 来引用投注。 最后需要调用.save()方法来存储一个实体。
...
使用 Apollo Boost 之类的工具，您可以轻松地将 The Graph 集成到您的 React 去中心化应用（或 Apollo-Vue）中。 特别是当使用 React hooks 和 Apollo 这样的工具时，获取数据与在组件中写入单个 GraphQl 查询一样简单。 一个典型的设置可能如下所示：
...
或者，您可以运行自己的节点：https://github.com/graphprotocol/graph-node#Quick-start。 这样做的一个原因可能是使用托管服务不支持的网络。 当前支持的网络包括主网、Kovan、Rinkeby、Ropsten、Goerli、PoA-Core、xDAI 和 Sokol。
