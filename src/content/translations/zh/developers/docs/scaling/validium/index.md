...
二层网络扩容项目（例如乐观卷叠和零知识卷叠）通过将部分交易数据发布到一层网络，牺牲了纯链下扩容协议（例如 [Plasma](/developers/docs/scaling/plasma/)）的无限可扩展性来换取安全性。 然而，这意味着卷叠的可扩展性属性受到以太坊主网上的数据带宽的限制（为此，[数据分片](/roadmap/danksharding/)提议是要提高以太坊的数据存储容量）。
