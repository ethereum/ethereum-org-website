...
- `nonce` – A counter that indicates the number of transactions sent from the account. This ensures transactions are only processed once and in the order that they were created by the account. In a contract account, this number represents the number of contracts created by the account.
