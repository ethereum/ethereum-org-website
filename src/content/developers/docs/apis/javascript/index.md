...
Please note that since [The Merge](/upgrades/merge) two connected pieces of Ethereum software - an execution client and a consensus client - are required to run a node. This page assumes that both clients are installed and running correctly. For more information please see [Run a Node](/developers/docs/nodes-and-clients/run-a-node).
