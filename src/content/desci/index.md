...
## Decentralized Search and Decentralized Science (DeSearch){#DeSearch}

Decentralized search aims at transforming publicly available data (including web data) into an indexable, searchable and adaptive knowledge system. Existing web search technologies are imperfect. They suffer from centralization and focusing on generating immediate revenue from advertisement. They are developed by small closed communities of engineers within commercial companies. At the same time search engines are a gateway for the many into the world of data and knowledge. Therefore, it is important to improve these technologies. This community aims at using trustless technologies to build search engines that satisfy the following principles.

1. Open source code. Evolving code should be visible to foster competition and faster development.

2. Open and competitive ranking algorithms. Any team that has the competence can contribute a ranking algorithm. Users should have the possibility to choose from multiple algorithms, fine tuned for different use cases.

3. Different machine learning models can be run over the web data. There should be a mechanism for researchers to schedule distributed jobs and for node maintainers to collect the fees for such computations.

4. Users , be that individuals or organizations, should have the possibility to construct their knowledge agents that continuously analyse and accumulate data that is specific to their organization.

5. Users should have a set of tools to record, analyse and trade attention data. This data can come from individual people or from multiple individuals working as a team. This data should be tied to the existing knowledge graph. It should allow building a set of tools to analyse new knowledge generated by this community and incorporation of it into the world knowledge space.

6. This attention data should be tradable by the individuals. It should be made into a merchandise that companies can purchase in order to run advertisement campaings. The advertising should evolve taking into considertion the attention structure and long term motivation of indiviuals.




### Decentralized web data aggregation{#deData}

Despite the fact that web data is publicly available, it is expensive to collect and aggregate this data for analysis. A technical problem here is to develpp a set of tools to aggregate this data and develop an easy mechanism which would make distributed computing with this data intuitive. 


## Decentralized computing over web data{#deComp}

Decentralized computing refers to a set of tools to run distributed jobs in trustless decentralized environments. Blockchain technology features prominently in such architectures.


### Decentralized ranking{#deRank}

Ranking is central to web search. It governs which sites are seen by people and which are not. It is therefore very important to compute rank correctly in a trustless way. Such technologies are available.



### Decentralized advertising and attention data{#deAdv}

When humans interact with web data, they generate attention maps. These attention maps are invlaubale source of numeric information about human thinking process. Unfortunately, this data is being savagely wasted nowadays. It is therefore imperative to develop a set of tools and incentives to collect this information and add it to knowledge system. 



### Decentralized knowledge graph{#DeKGraph}

Text is a projection of high dimensional processes that take place in human brains. It is one of the challenges of artificial intelligence to reconstruct this high dimensional data and integrate it with the logic of the reasoning process. Currently artificial neural networks provide an interesting possibility. Distributed search will help this effort by providing accessible data and execution environment to train AI models that do this high dimensional reconstruction.



### Decentralized Search as a tool for DeSci{#DeSearchTool}

Decentralized Science would benefit from decentralized search. Decentralized search aims at constructing distributed inference systems, that include knowledge graphs, abstract syntax trees, abstract semantic spaces. It aims at incorporating automated theorem provers to represent inductive trees that are at the center of representing proofs of theorems in formal languages. It aims at extending this formalism to English. This is challenging task as English is not a formal language in a traditonal sense, and only fuzzy type assignment is possible for English words. Nonetheless, during recent years an understanding gradually emerged that there are notions of ASTs for English , constructed in a probabilistic way , that extend the well known tools like wordnet. The construction of such tools only makes sense based on large corpora of data, such as Arxiv. It seems plausible that there will be synergy  between distributed search and decentralized science. The synergy will come mainly from the development of tools that would facilitate user thinking about mathmatical concepts. Distributed search aims at modeling human metal space and the thinking activity ( in particular, as related to research) and as such may be useful for Decentralized Science.

### Further reading 


https://rorur.com , https://searchdao.net
