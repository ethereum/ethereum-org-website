...
Bei der Übersetzung des Inhalts sollten Sie sicherstellen, dass die Übersetzungen konsistent sind und keine lateinischen Zeichen enthalten.
...
Wir verbergen den Inhalt, den Übersetzer niemals übersetzen sollten, wenn wir neue Seiten auf Crowdin hochladen. Das bedeutet, dass alle Metadaten, die für Übersetzer in Crowdin sichtbar sind, übersetzt werden sollten.
...
Ein Großteil des Ausgangsinhalts enthält Tags und Variablen, die im Crowdin-Editor gelb hervorgehoben sind. Diese erfüllen unterschiedliche Funktionen und sollten korrekt behandelt werden.
...
2. Scrollen Sie nach unten zum Abschnitt 'HTML-Tags anzeigen'
...
Der beste Weg, mit Links umzugehen, besteht darin, sie direkt aus dem Ausgangstext zu kopieren, entweder indem Sie darauf klicken oder die Schaltfläche 'Ausgangstext kopieren' (`Alt+C`) verwenden.
...
Im folgenden Beispiel können Sie sehen, dass das Bewegen der Maus über das Tag `<0>` anzeigt, dass es `<code>` darstellt und ein Code-Snippet enthält. Daher sollte der Inhalt innerhalb dieser Tags nicht übersetzt werden.
...
Da sie in anderen Sprachen normalerweise keine etablierten Übersetzungen haben, ist der beste Weg, mit diesen und ähnlichen Begriffen umzugehen, eine beschreibende Übersetzung der Vollform bereitzustellen und die englische Abkürzung in Klammern hinzuzufügen.
...
Einige Begriffe haben möglicherweise keine etablierten Übersetzungen in anderen Sprachen und sind unter dem englischen Originalbegriff weithin bekannt. Solche Begriffe umfassen meist neuere Konzepte wie Proof-of-Work, Proof-of-Stake, Beacon Chain, Staking usw.
...
Eine weitere Form der Inklusivität besteht darin, für ein globales Publikum zu übersetzen und sich nicht auf ein bestimmtes Land, eine bestimmte Ethnie oder Region zu beschränken.
...
- Gleiches gilt für die Zeichensetzung von Listenelementen. Das Satzzeichen am Ende von Listen kann je nach Sprache ein Punkt (**.**), ein Komma (**,**) oder ein Semikolon (**;**) sein.
...
- Schließlich können negative Zahlen je nach Sprache unterschiedlich angezeigt werden: -100, 100-, (100) oder [100].
...
- Als allgemeine Regel gilt: Bitte behalten Sie die Maßeinheiten gemäß dem Ausgangstext bei. Wenn Ihr Land ein anderes System verwendet, können Sie die Umrechnung in Klammern angeben.
- Abgesehen von der Lokalisierung von Maßeinheiten ist es auch wichtig, die Unterschiede in der Handhabung dieser Einheiten in den verschiedenen Sprachen zu beachten. Der Hauptunterschied ist der Abstand zwischen der Zahl und der Einheit, der je nach Sprache unterschiedlich sein kann. Beispiele hierfür sind 100kB vs. 100 kB oder 50ºF vs. 50 ºF.
