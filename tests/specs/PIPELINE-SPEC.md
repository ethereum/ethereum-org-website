...
- Insert newly added keys at the position they occupy in english-B, copying inert
  values verbatim
- Remove deleted keys from the locale frontmatter

New translatable frontmatter keys are routed to the LLM and inserted in
english-B key order after translation. Key removals are always deterministic.

**Test assertions:**

- Each inert change from the mutation table is correctly applied
- Added and removed frontmatter keys are propagated
- New translatable frontmatter keys are sent to the LLM
